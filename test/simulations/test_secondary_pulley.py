...
class TestSecondaryPulley(unittest.TestCase):
    ...
    def test_calculate_spring_comp_force(self):
        compression = 0.02
        expected_force = tm.hookes_law_comp(self.pulley.spring_coeff_comp, self.pulley.initial_compression + compression)
        result = self.pulley.calculate_spring_comp_force(compression)
        self.assertAlmostEqual(result, expected_force, places=5)

    def test_calculate_spring_tors_torque(self):
        shift_distance = 0.025
        rotation = self.pulley.initial_rotation + self.pulley.ramp.height(shift_distance) / self.pulley.helix_radius
    ...
    def test_calculate_net_force(self):
        ...
        helix_force = self.pulley.calculate_helix_force(torque, spring_tors_torque, shift_distance)
