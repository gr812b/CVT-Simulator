...
parser.add_argument("--end_time", type=float, default=60)
...
start_real_time = np.max([args.start_time, real_data["Timestamp (ms)"].iloc[0] / 1000])

# filter real data to match start and end time
real_data = real_data[(real_data["Timestamp (ms)"] >= start_real_time * 1000) & (real_data["Timestamp (ms)"] <= args.end_time * 1000)]

# convert real data timestamps to seconds and adjust by start time
real_data["time"] = real_data["Timestamp (ms)"] / 1000 - args.start_time
...
ax2 = ax1.twinx()
ax1.plot(real_data["time"], real_data[args.real_col], label="Real Data", color="blue")
ax1.plot(real_data["time"], interpolated_data, label="Simulation Data", color="green")
