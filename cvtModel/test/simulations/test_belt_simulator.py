...
from cvt_simulator.constants.car_specs import SHEAVE_ANGLE, BELT_CROSS_SECTIONAL_AREA, BELT_HEIGHT


class TestBeltSimulator(unittest.TestCase):

    def setUp(self):
        self.simulator_primary = BeltSimulator(
            μ_static=0.5, μ_kinetic=0.4, primary=True
        )
        self.simulator_secondary = BeltSimulator(
            μ_static=0.5, μ_kinetic=0.4, primary=False
        )
    ...
    def test_calculate_centrifugal_force_primary(self):
        ...
        result = self.simulator_primary.calculate_centrifugal_force(
    ...
    def test_calculate_centrifugal_force_secondary(self):
        ...
        expected_mass = RUBBER_DENSITY * BELT_CROSS_SECTIONAL_AREA * expected_length
        expected_force = tm.centrifugal_force(expected_mass, ω, expected_radius)
        result = self.simulator_secondary.calculate_centrifugal_force(
            ω, shift_distance, wrap_angle
        )
        self.assertAlmostEqual(result, expected_force, places=5)
    ...
    def test_calculate_net_radial_force(self):
        ...
        result = self.simulator_primary.calculate_net_radial_force(
    ...
    def test_calculate_radial_force(self):
        ...
        wrap_angle = np.pi / 2
        clamping_force = 1000
        centrifugal_force = self.simulator_primary.calculate_centrifugal_force(
            ω, shift_distance, wrap_angle
        )
        radial_force = self.simulator_primary.radial_force_from_clamping(clamping_force)
        expected_force = self.simulator_primary.calculate_net_radial_force(
            centrifugal_force, radial_force, wrap_angle
        )
        result = self.simulator_primary.calculate_radial_force(
            ω, shift_distance, wrap_angle, clamping_force
        )
        self.assertAlmostEqual(result, expected_force, places=5)
    ...
    def test_calculate_slack_tension(self):
        ...
        result = self.simulator_primary.calculate_slack_tension(
            radial_force, wrap_angle, μ
        )
    ...
    def test_calculate_max_transferable_torque(self):
        ...
        result = self.simulator_primary.calculate_max_transferable_torque(
            tension, μ, wrap_angle, radius
        )
