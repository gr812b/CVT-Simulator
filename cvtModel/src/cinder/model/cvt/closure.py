...
class ClosureGains:
    ...
    def from_by_unknown(
        cls,
        coefficients: dict[ClosureUnknown, float],
    ) -> "ClosureGains":
        """Build a gain row from explicitly named closure columns.

        This is intentionally useful to mounted components such as a helical
        torque reaction: the force law receives the host shaft channels rather
        than hard-coding "primary" or "secondary" columns.
        """

        values = [0.0] * CLOSURE_UNKNOWN_COUNT
        for unknown, coefficient in coefficients.items():
            if not isinstance(unknown, ClosureUnknown):
                raise TypeError("Closure gain keys must be ClosureUnknown values.")
            _require_finite(coefficient=float(coefficient))
            values[int(unknown)] += float(coefficient)
        return cls(*values)
