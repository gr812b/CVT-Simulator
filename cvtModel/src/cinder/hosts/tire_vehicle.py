...
class TireVehicleHost:
    ...
    def initial_state(
        ...
        *,
        ...
        return np.asarray(
            [secondary_shaft_angle, vehicle_position, vehicle_speed], dtype=float
        )
    ...
    def rhs(
        ...
        *,
        ...
        road_force = _road_force(
            self.tire_boundary, vehicle_speed=speed, grade_angle=grade_angle
        )
        mass = self.tire_boundary.road_load.vehicle.mass
        acceleration = (tire_force + road_force) / mass
        return np.asarray(
            [cvt_state.secondary_angular_speed, speed, acceleration], dtype=float
        )
...
def _road_force(
    boundary: TireCoupledShaftBoundary, *, vehicle_speed: float, grade_angle: float
) -> float:
    ...
    rolling_direction = vehicle_speed / sqrt(
        vehicle_speed**2 + spec.rolling_speed_regularization**2
    )
    rolling_force = (
        -spec.rolling_resistance_coefficient * normal_force * rolling_direction
    )
    aero_force = (
        -0.5
        * spec.air_density
