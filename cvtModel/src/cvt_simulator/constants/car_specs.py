...
class CarSpecs(BaseModel):
    ...
    wheel_radius: float = Field(default=22 / 2 * 0.0254, description="Wheel radius in meters")
    
    # Aerodynamics
    frontal_area: float = Field(default=1.11484, description="Frontal area in m^2")
    drag_coefficient: float = Field(default=0.6, description="Drag coefficient (unitless)")
    
    # Pulley geometry
    # TODO: These are all guesses, need to be gotten
    sheave_angle: float = Field(default=deg_to_rad(11.5 * 2), description="Sheave angle in radians")
    initial_flyweight_radius: float = Field(default=0.05, description="Initial flyweight radius in meters")
    helix_radius: float = Field(default=0.04445, description="Helix radius in meters")
    
    # Belt specifications
    belt_angle: float = Field(default=deg_to_rad(14), description="Belt angle in radians")
    belt_height: float = Field(default=inch_to_meter(0.613), description="Belt height in meters")
    belt_length: float = Field(default=inch_to_meter(37.53), description="Belt length in meters")
    belt_width_top: float = Field(default=inch_to_meter(0.85), description="Belt width at top in meters")
    
    # Pulley radii
    min_prim_radius: float = Field(default=inch_to_meter(0.75), description="Minimum primary pulley radius in meters")
    max_sec_radius: float = Field(default=inch_to_meter(4.0), description="Maximum secondary pulley radius in meters")
    initial_sheave_displacement: float = Field(default=inch_to_meter(0.121), description="Initial sheave displacement in meters")
    ...
    def center_to_center(self) -> float:
        ...
        return (
            ...
            + math.sqrt(
                (np.pi * (self.min_prim_radius + self.max_sec_radius + self.belt_height)) ** 2
                - 2 * np.pi * self.belt_length * (self.min_prim_radius + self.max_sec_radius + self.belt_height)
                + self.belt_length**2
                - 8 * (self.max_sec_radius - self.min_prim_radius - self.belt_height) ** 2
            )
        ) / 4
