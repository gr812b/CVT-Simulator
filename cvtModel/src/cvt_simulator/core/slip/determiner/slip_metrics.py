...
class SlipMetrics:
    ...
    def get_slip_metrics(
        self, state: SystemState, τ_eng: float, τ_load: float
    ) -> SlipMetricsResult:
        ...
        primary_relative_speed, secondary_relative_speed = (
            self._relative_contact_speeds(state)
        )
    ...
    def _relative_contact_speeds(self, state: SystemState) -> tuple[float, float]:
        ...
        primary_relative_speed = (
            self.cvt.primary_effective_radius(s) * state.ω_p - state.v_b
        )
        secondary_relative_speed = (
            state.v_b - self.cvt.secondary_effective_radius(s) * state.ω_s
        )
