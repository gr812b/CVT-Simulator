...
class ShiftInitiationSolver(SolverBase):
            ...
            for i in range(len(force_diff_samples) - 1):
                ...
                if below_threshold and above_threshold:
                    ...
                    def threshold_function(omega):
                        return self._evaluate_force_difference(omega) - self.FORCE_THRESHOLD
    ...
    def _evaluate_force_difference(self, angular_velocity: float) -> float:
        ...
        cvt_breakdown = self.cvt_shift_model.get_breakdown(state, coupling_torque=torque_demand)
...
def main():
    ...
    print(f"  Secondary torsion spring rate: {args.secondary_torsion_spring_rate} Nm/rad")
    print(f"  Secondary compression spring rate: {args.secondary_compression_spring_rate} N/m")
    print(f"  Secondary rotational pretension: {args.secondary_rotational_spring_pretension} deg")
    ...
    if result.success:
        ...
        print(f"✓ CVT begins shifting at {shift_rpm:.1f} RPM ({result.value:.2f} rad/s)")
    else:
    ...
    # Plot the curve
    ax.plot(rpm_array, force_diff_array, 'b-', linewidth=2, 
    ...
    if result.success and result.value is not None:
        ...
        ax.plot(shift_rpm, 0, 'ro', markersize=10, 
