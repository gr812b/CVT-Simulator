...
def analyze_secondary_precompression(
    ...
    precompression_values = np.linspace(
        precompression_range[0], precompression_range[1], num_points
    )
    ...
    print()
    print(
        f"Varying secondary linear pretension from {precompression_range[0]} to {precompression_range[1]} m"
    )
    print()
    ...
    for i, precomp in enumerate(precompression_values):
        ...
        if result.success:
            ...
            print(
                f"  [{i + 1:2d}/{num_points}] Precomp: {precomp:.3f} m → Shift at {shift_rpm:.1f} RPM"
            )
        else:
            shift_rpm_values.append(np.nan)
            print(
                f"  [{i + 1:2d}/{num_points}] Precomp: {precomp:.3f} m → Solver failed"
            )
...
def plot_results(precompression_values, shift_rpm_values, args_base):
    ...
    ax.set_title(
        "Effect of Secondary Precompression on CVT Shift Initiation",
        fontsize=14,
        weight="bold",
    )
