...
def analyze_flyweight_mass(
    ...
    print(f"  Secondary linear pretension: {args_base.secondary_linear_spring_pretension} m")
    print()
    ...
    for i, mass in enumerate(mass_values):
        ...
        if mass < 0.01:
            ...
            print(f"  [{i+1:2d}/{num_points}] Mass: {mass:.3f} kg → Skipped (too small)")
            continue
        ...
        if result.success:
            ...
            print(f"  [{i+1:2d}/{num_points}] Mass: {mass:.3f} kg → Shift at {shift_rpm:.1f} RPM")
        else:
            shift_rpm_values.append(np.nan)
...
def plot_results(mass_values, shift_rpm_values, args_base):
    ...
    ax.plot(mass_values * 1000, shift_rpm_values, 'b-', linewidth=2, marker='o', markersize=4)
    
    # Styling
    ax.set_xlabel('Flyweight Mass (g)', fontsize=12)
    ax.set_ylabel('Shift Initiation RPM', fontsize=12)
    ax.set_title('Effect of Flyweight Mass on CVT Shift Initiation', 
