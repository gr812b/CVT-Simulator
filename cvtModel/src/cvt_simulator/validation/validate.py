...
def plot_engine_rpm_vs_speed(real_df, sim_df):
    ...
    interp_real_rpm = np.interp(
        sim_df["car_velocity"], real_df["GPS SPEED"], real_df["RPM PRIM"]
    )
    ...
    ax1.plot(
        ...
        interp_real_rpm,
        "-",
        label="Real Engine RPM (interpolated)",
        linewidth=2,
    )
...
def plot_velocity_analysis(real_df, sim_df):
    ...
    interp_real_velocity = np.interp(
        sim_df["time"], real_df["time_s"], real_df["GPS SPEED"]
    )
    ...
    ax1.plot(
        sim_df["time"],
        interp_real_velocity,
        "-",
        label="Real Velocity (interpolated)",
        linewidth=2,
    )
...
def plot_position_analysis(real_df, sim_df):
    ...
    ax1.plot(
        sim_df["time"],
        interp_real_position,
        "-",
        label="Real Position (integrated)",
        linewidth=2,
    )
    ax1.set_xlabel("Time (s)")
...
if __name__ == "__main__":
    ...
    real_df["time_s"] = (
        real_df["Timestamp (ms)"] - real_df["Timestamp (ms)"].iloc[0]
    ) / 1000.0 + time_offset
