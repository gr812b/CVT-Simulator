...
class SecondaryMaxTorqueModel:
    ...
    def get_max_torque_sec(self, state: SystemState):
        ...
        spring_forces = sec_breakdown.net * np.tan(SHEAVE_ANGLE / 2)
        ...
        centrifugal_force = radial_from_centrifugal * wrap_angle

        # Capstan portion e^
        mini_cap = np.exp(RUBBER_ALUMINUM_STATIC_FRICTION * wrap_angle)
        capstan_term = wrap_angle / (2 * radius) * (mini_cap + 1) / (mini_cap - 1)

        # Transmission portion
        cvt_ratio = tm.current_cvt_ratio(state.shift_distance)
        ramp_angle = sec_breakdown.helix_force.angle
        transmission_term = cvt_ratio / (self.sec_model.helix_radius * np.tan(ramp_angle)) * np.tan(SHEAVE_ANGLE / 2)
