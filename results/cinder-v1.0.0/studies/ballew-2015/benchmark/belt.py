...
def build_equivalent_belt_mapping() -> BallewEquivalentBeltMapping:
    ...
    # from Ballew's own L, C, and r_s,max (Reconstruction A4).
    primary_zero = brentq(
    ...
    # Ballew begins Chapter 5 already engaged, so s=0 is the compatible
    # low-ratio endpoint and no deadzone width is introduced.
...
def solve_initial_shift_from_published_speeds(geometry: BeltPulleyGeometry) -> float:
    ...
    explicitly tabulated shaft speeds are authoritative (Reconstruction A3).
    """
    ...
    def ratio_residual(shift: float) -> float:
        ...
        geometric_speed_ratio = (
            position.secondary.effective / position.primary.effective
        )
...
def _open_belt_length(
    *, center_distance: float, primary_radius: float, secondary_radius: float
    ...
    return (
        primary_radius * primary_wrap
        + secondary_radius * secondary_wrap
        + 2.0 * span
    )
...
def _validate_mapping(mapping: BallewEquivalentBeltMapping) -> None:
    ...
    if not isclose(
        ...
        raise RuntimeError(
            "Equivalent belt mapping no longer preserves Ballew belt length."
        )
