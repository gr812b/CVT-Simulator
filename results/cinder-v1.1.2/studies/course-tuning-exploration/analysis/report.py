...
def plot_lines(path,series,xkey,ykey,xlabel,ylabel,title,course=None,events=None,levels=()):
    ...
    for label,rows in series:
        ...
        if np.isfinite(y).any(): ax.plot(x,y,label=label,linewidth=1.2);has=True
...
def build_report(run_dir:Path,focus:list[str]|None=None,plots=True,reference='R00'):
    ...
    images=[]
    if focus is None:
        focus=list(dict.fromkeys([reference,'W90','H18','C115','B01','D01','D02']+[r['id'] for r in summaries if r.get('status')=='finished' and not r.get('review_required')][:2]))
    ...
    if plots:
        ...
        sets=[('focus',focus)]+[(family,[r['id'] for r in manifest['competitors'] if r['family'] in (family,'reference') and data.get(r['id'])]) for family in ('flyweight','primary_spring','helix','torsional_preload','secondary_spring','combined','stress_control')]
    ...
    body=f'''<!doctype html><html><head><meta charset="utf-8"><title>Common-course tuning exploration</title><style>body{{font:15px system-ui,sans-serif;margin:2rem;max-width:1400px}}table{{border-collapse:collapse;display:block;overflow-x:auto}}th,td{{border:1px solid #bbb;padding:.45rem;text-align:left}}img{{max-width:100%;margin:1rem 0}}summary{{font-weight:bold;cursor:pointer;padding:1rem}}code{{background:#eee}}p{{line-height:1.5}}</style></head><body>
<h1>Common-course tuning exploration</h1><p><b>Exploratory comparisons, not experimental validation or an optimized tune.</b> Full throttle is unchanged. Every entrant uses the same road and boundary parameters, with dynamic fixed-pivot flyweights and a dynamic bilateral slotted helix.</p>
<p>Campaign: <code>{html.escape(run_dir.name)}</code>. Reference for gaps: <b>{html.escape(reference)}</b>. Course length: {course.finish_m:g} m; hill: {manifest['course']['hill_angle_deg']:g} degrees; cyclic wavelength: {course.wavelength:g} m.</p>
...
<h2>Detailed views</h2>{detail}<p><a href="campaign.json">Configuration and hashes</a> · <a href="time_gaps.csv">Distance time gaps</a> · <a href="leaderboard.csv">Complete metrics</a></p></body></html>'''
