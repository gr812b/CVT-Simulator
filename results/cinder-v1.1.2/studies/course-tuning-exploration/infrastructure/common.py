...
def source_fingerprint() -> dict:
    files=sorted(p for p in STUDY_ROOT.rglob('*.py') if 'artifacts' not in p.parts and '__pycache__' not in p.parts)
    return {p.relative_to(STUDY_ROOT).as_posix():sha_file(p) for p in files}
