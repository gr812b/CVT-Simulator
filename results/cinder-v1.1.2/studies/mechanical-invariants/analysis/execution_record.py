"""Portable input/output identity for a completed mechanical-invariants run.
...
def write_execution_record(artifacts: Path, inputs: dict[str, bytes], *,
    ...
    record = {
        "schema_version": 1,
        "run_id": "mechanical-invariants-" + identity[:16],
...
def verify_execution_record(artifacts: Path) -> dict:
    ...
    # The saved guards are evaluated by the current core. Refuse silent drift
    # in that implementation even when the retained evidence itself is intact.
    core = STUDY / "infrastructure" / "core.py"
    core_name = core.relative_to(REPOSITORY).as_posix()
    if digest(core) != record["input_sha256"][core_name]:
        raise ValueError("Current audit core differs from the executed input snapshot.")
    return record
