"""Geometry checks and mechanism maps for selected spring/ramp final cases."""
...
from infrastructure.tunes import component,_segment
...
def inspect_definition(document: dict, course: dict, execution: dict, out: Path) -> dict:
    ...
    for s in np.linspace(g.deadzone_shift,g.max_shift,101):
        ...
        rows.append({'active_shift_fraction':(float(s)-g.deadzone_shift)/(g.max_shift-g.deadzone_shift),
            ...
            'flyweight_dJ_dx_kg_m':f.shaft_inertia_gradient,
            'centrifugal_force_3000rpm_N':.5*(3000*2*np.pi/60)**2*f.shaft_inertia_gradient,
