...
def _plot_lines(out: Path, datasets, key, ylabel, title, low_mm=None, high_mm=None):
    ...
    for car, rows, windows, course in datasets:
        ...
        from analysis.report import TUNE_COLOURS
        colour=TUNE_COLOURS.get(car)
        line,=ax.plot(tx,yy,label=car,linewidth=1.25,**({'color':colour} if colour is not None else {}))
