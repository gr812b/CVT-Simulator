# Revision 2: course-feature discovery and shift curves

**Start with [V2_QUICKSTART.md](V2_QUICKSTART.md).** It covers the separate long-flat,
moderate-second-hill and stronger-cyclic screens, the six additional entrants,
and primary-versus-secondary-RPM plots for every car. The original road and fleet
files are unchanged. Existing artifacts can be replotted without simulation.

The original single-course runner remains available as documented below. Latest
local checks are in `provenance/V2_TEST_REPORT.md`; the original `TEST_REPORT.md`
records revision-1 testing only.
...
```powershell
python studies/course-tuning/exploration/scans/scan_courses.py --grades 35 38 40 --wavelengths 4 6 --cars R00 W90 B01 D02 D03
python studies/course-tuning/exploration/scans/scan_courses.py --grades 35 38 40 --wavelengths 4 6 --cars R00 W90 B01 D02 D03 --jobs 4 --execute
```
