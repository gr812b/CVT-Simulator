...
def arguments():
    ...
    p.add_argument('--record-campaign',type=Path,help='Write the resolved campaign path for orchestration')
    ...
    p.add_argument('--cars',nargs='+',help='IDs or family names (comma-separated also accepted); default all entries in the selected fleet')
...
def main():
    a=arguments();spec=load_json(HERE/'exploration.json');fleet=load_json(a.competitors)['competitors']
    ...
    if a.record_campaign:
        a.record_campaign.parent.mkdir(parents=True,exist_ok=True)
        a.record_campaign.write_text(str(run_dir)+'\n',encoding='utf-8')
    print(f'Course: {course.summary_text()}\nOutputs: {run_dir}',flush=True)
