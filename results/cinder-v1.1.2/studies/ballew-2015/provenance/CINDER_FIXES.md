...
implementation defects become obvious. These benchmark-exposed corrections predate the original 1.0.0 results and remain
part of the frozen `cinder-v1.1.2` mechanics. They are **not study-local patches**
and must not be re-applied by the results code. Later 1.1.2 energy-consistency
changes are documented separately in `CINDER_1_1_2_UPDATE.md`.
...
and the engaged mechanical snapshot path inherited from the original v1.0.0 implementation.
...
The transition logic introduced for v1.0.0 and retained in v1.1.2 tries, in order, physically admissible successors
...
`cinder-cvt==1.1.2`; that package already contains the corrections. The Ballew
study should not vendor alternate CINDER mechanics or insert `cvtModel/src` onto
`sys.path`.

The study therefore:

1. verifies the clean 1.1.2 environment;
