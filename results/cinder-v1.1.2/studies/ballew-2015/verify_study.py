...
BENCHMARK = ROOT / "infrastructure" / "benchmark"
...
def required_files() -> list[Path]:
    return [
        ...
        ROOT / "study.json",
        ROOT / "run.py",
        ROOT / "analysis" / "run_controller_reconstruction.py",
        ROOT / "analysis" / "run_convergence.py",
        BENCHMARK / "actuation.py",
        BENCHMARK / "belt.py",
        BENCHMARK / "case.py",
        BENCHMARK / "constants.py",
        BENCHMARK / "controller.py",
...
def verify_manifest() -> None:
    ...
    for directory, entries in groups:
        for name, expected in entries.items():
            ...
            if expected_size is not None and path.stat().st_size != int(expected_size):
                raise RuntimeError(
                    f"size mismatch for {path.relative_to(ROOT)}: {path.stat().st_size} != {expected_size}"
                )
            expected_sha = expected.get("sha256")
            if expected_sha is not None and sha256(path) != expected_sha:
                raise RuntimeError(f"SHA-256 mismatch for {path.relative_to(ROOT)}")
            expected_blob = expected.get("git_blob_sha1")
            if expected_blob is not None and git_blob_sha1(path) != expected_blob:
                raise RuntimeError(f"Git-blob mismatch for {path.relative_to(ROOT)}")
    ...
    if sha256(thesis) != source["sha256"]:
        raise RuntimeError("Ballew thesis SHA-256 mismatch.")


def verify_no_old_runtime_dependencies() -> None:
    forbidden = (
        "cvtModel/src",
...
def main() -> int:
    ...
    if missing:
        print("Missing required Ballew study files: " + ", ".join(missing), file=sys.stderr)
        return 1
    try:
        verify_manifest()
        verify_no_old_runtime_dependencies()
        subprocess.run([sys.executable, str(REFERENCE / "prepare_reference_data.py"), "--check"], check=True)
