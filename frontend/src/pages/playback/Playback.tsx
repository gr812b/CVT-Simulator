...
const PlaybackContent = ({ run }: { run: NonNullable<ReturnType<typeof useSimulationRun>['completedRun']> }) => {
  const navigate = useNavigate(); const table = run.result.report_table; const timeValues = useMemo(() => reportAxisTimes(table), [table]); const replayController = useMemo(() => new ReportReplayController(timeValues), [timeValues]); const replayRef = useRef(replayController); const categories = useMemo(() => buildReportGraphs(table), [table]);
