...
For the implicit-differentiation step below, we use the compact notation
$r_p, r_s, dr_p/ds,$ and $dr_s/ds$. Because this derivation is based on
rate relationships, any constant radial offset between radius references
vanishes directly under differentiation ($\mathrm{d}(\text{constant})/\mathrm{d}s=0$),
so the same derivative form is obtained.
