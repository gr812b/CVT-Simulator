...
\paragraph{Tangency Angle}

Let $\lambda$ denote the (nonnegative) angle between the line of centers and the straight
tangent belt segment (as defined in \Fig{fig:belt_geometry_placeholder}). Then,
...
With $\lambda \ge 0$, the wrap angles on the smaller- and larger-radius pulleys are
...
&= \pi(r_p + r_s)
 + 2\lambda |r_s - r_p|
 + 2\sqrt{C^2 - (r_s - r_p)^2}. \label{eq:belt_length_subbed}
\end{align}
...
Rearranging \Eq{eq:belt_length_final} yields a geometric constraint of the form
...
It is common in online references and simplified treatments to assume
...
Under this assumption, the belt-length equation reduces to a much
simpler approximate form.

However, this small-angle approximation can introduce substantial
geometric error when $(|r_s - r_p|)/C$ is not negligible.
Appendix~\ref{sec:approximation-validation-placeholder}
...
\Eq{eq:drsds_general}, following the same sequence as the handwritten derivation:
differentiate term-by-term and apply the product rule to the inverse-sine term.

% --------------------------------------------------------------
\paragraph{Derivative of the inverse-sine term}

Consider the factor
\[
|r_s-r_p|\sin^{-1}\!\left(\frac{|r_s-r_p|}{C}\right).
\]
Differentiation depends on whether $r_s\ge r_p$ or $r_s<r_p$ because of the
absolute value. We therefore proceed piecewise.

% --------------------------------------------------------------
\paragraph{Case 1: $r_s \ge r_p$}

If $r_s\ge r_p$, then $|r_s-r_p| = r_s-r_p$ and
\[
\sin^{-1}\!\left(\frac{|r_s-r_p|}{C}\right)
=
\sin^{-1}\!\left(\frac{r_s-r_p}{C}\right).
...
First compute the needed inner derivative:
\begin{equation}
\frac{\partial}{\partial r_p}\sin^{-1}\!\left(\frac{r_s-r_p}{C}\right)
=
-\frac{1}{\sqrt{C^2-(r_s-r_p)^2}},
\qquad
\frac{\partial}{\partial r_s}\sin^{-1}\!\left(\frac{r_s-r_p}{C}\right)
=
\frac{1}{\sqrt{C^2-(r_s-r_p)^2}}.
\label{eq:asin_derivs_case1}
\end{equation}

Now apply the product rule to $(r_s-r_p)\sin^{-1}((r_s-r_p)/C)$:

\begin{align}
\frac{\partial}{\partial r_p}
\left[(r_s-r_p)\sin^{-1}\!\left(\frac{r_s-r_p}{C}\right)\right]
&=
(-1)\sin^{-1}\!\left(\frac{r_s-r_p}{C}\right)
+(r_s-r_p)\left(-\frac{1}{\sqrt{C^2-(r_s-r_p)^2}}\right), \label{eq:prod_rp_case1}\\[6pt]
\frac{\partial}{\partial r_s}
\left[(r_s-r_p)\sin^{-1}\!\left(\frac{r_s-r_p}{C}\right)\right]
&=
(+1)\sin^{-1}\!\left(\frac{r_s-r_p}{C}\right)
+(r_s-r_p)\left(\frac{1}{\sqrt{C^2-(r_s-r_p)^2}}\right).
\label{eq:prod_rs_case1}
\end{align}
...
\begin{align}
\frac{\partial}{\partial r_p}\left[2\sqrt{C^2-(r_s-r_p)^2}\right]
&=
2\cdot \frac{(r_s-r_p)}{\sqrt{C^2-(r_s-r_p)^2}}, \label{eq:sqrt_rp_case1}\\[6pt]
\frac{\partial}{\partial r_s}\left[2\sqrt{C^2-(r_s-r_p)^2}\right]
&=
-2\cdot \frac{(r_s-r_p)}{\sqrt{C^2-(r_s-r_p)^2}}.
\label{eq:sqrt_rs_case1}
\end{align}

% --------------------------------------------------------------
\paragraph{Assemble $\partial f/\partial r_p$ and $\partial f/\partial r_s$ (Case 1)}

Differentiate \Eq{eq:f_def_again} term-by-term. The $\pi(r_p+r_s)$ term contributes $\pi$
to both partial derivatives. The inverse-sine product term contributes $2(\cdot)$, and the
square-root term contributes \Eq{eq:sqrt_rp_case1}--\Eq{eq:sqrt_rs_case1}. Substituting
and simplifying shows that the square-root contributions cancel the $ (r_s-r_p)/\sqrt{\cdot}$
pieces from the product-rule derivatives, leaving:
...
\qquad (r_s\ge r_p).
\label{eq:partials_case1}
\end{equation}

% --------------------------------------------------------------
\paragraph{Case 2: $r_s < r_p$}

If $r_s<r_p$, then $|r_s-r_p| = r_p-r_s$ and
\[
\sin^{-1}\!\left(\frac{|r_s-r_p|}{C}\right)
=
\sin^{-1}\!\left(\frac{r_p-r_s}{C}\right).
\]

Proceeding identically (product rule on $(r_p-r_s)\sin^{-1}((r_p-r_s)/C)$ and chain rule
on the square-root term) yields the swapped-sign results:
...
\pi
+
2\sin^{-1}\!\left(\frac{r_p-r_s}{C}\right)
},
\qquad
\boxed{
\frac{\partial f}{\partial r_s}
...
\qquad (r_s<r_p).
\label{eq:partials_case2}
\end{equation}

% --------------------------------------------------------------
\paragraph{Summary (piecewise partial derivatives)}

Combining \Eq{eq:partials_case1} and \Eq{eq:partials_case2}, the belt constraint partials are

\begin{equation}
\frac{\partial f}{\partial r_p}
=
\begin{cases}
\pi - 2\sin^{-1}\!\left(\dfrac{r_s-r_p}{C}\right), & r_s \ge r_p,\\[10pt]
\pi + 2\sin^{-1}\!\left(\dfrac{r_p-r_s}{C}\right), & r_s < r_p,
\end{cases}
\label{eq:df_drp_piecewise}
\end{equation}
...
\pi - 2\sin^{-1}\!\left(\dfrac{r_p-r_s}{C}\right), & r_s < r_p.
\end{cases}
\label{eq:df_drs_piecewise}
\end{equation}

These expressions are valid for both regimes and do not assume an ordering
between $r_p$ and $r_s$.
