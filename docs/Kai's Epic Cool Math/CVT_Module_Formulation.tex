...
force through a ramp (See \Fig{fig:primary_ramp}). We parameterize the ramp by an axial-to-radial profile
\begin{equation}
r_f = r_f(s),
...
As $r_f(s)$ is a function, the local ramp slope is $dr_f/ds$.
...
\end{minipage}
\caption{Primary ramp geometry. \textbf{Left:} The flyweight radius is parameterized by $r_f(s)$, where $s$ is axial travel. The local ramp angle is defined by $\tan\alpha_p(s)=dr_f/ds$, so steeper profiles (larger $dr_f/ds$) redirect a larger fraction of radial force into axial closing force. \textbf{Right:} Sample profiles of $r_f(s)$ and its derivative $dr_f/ds$ as functions of axial shift.}
