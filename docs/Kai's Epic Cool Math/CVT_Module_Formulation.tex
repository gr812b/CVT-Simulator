...
\section{Torque Transmission and Coupling}
\label{sec:torque_transmission}

The axial force models developed in \Sec{sec:pulley_axial_models} depend on the
torque transmitted through the CVT. However, this torque has not yet been
defined as a system quantity.

The engine applies torque at the primary pulley, while the vehicle load resists
motion at the secondary. These components do not interact directly; their only
mechanical connection is through the belt. To determine how torque is
transmitted between them, the belt must be treated as the medium through which
power flows.

We proceed by first defining the torque path through the system, then examining
how power is carried by the belt, and finally introducing a single coupling
torque that characterizes the CVT interface.

% ==============================================================
\subsection{Torque Path Through the Belt}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{./illustrations/torque/cvt_overview_forces.png}
    \caption{Torque flow through the CVT system. The primary pulley applies
    torque $\tau_p$ to the belt, while the belt applies torque $\tau_s$ to the
    secondary pulley.}
...
These quantities represent the torque entering and leaving the belt transmission
path, respectively.
...
Since the belt is the only connection between the pulleys, it must carry the
mechanical power transmitted through the system.

Under \AssRef{assump:constant_belt_length}, the belt moves along its path with a
single transport speed $v_b(t)$.
Since the belt is modeled as having constant length with no longitudinal
deformation, different tangential speeds at different points along the belt
would imply local stretching or compression, which this model does not permit.

Torque transmission arises from tangential forces
acting between the belt and the pulley surfaces. Let $F_t$ denote the effective
tangential force transmitted through the belt.
...
The same tangential force $F_t$ acts at the effective contact radii of each
pulley. The torques at the primary and secondary are therefore
\begin{align}
\tau_p &= r_{p,\mathrm{eff}} \, F_t, \\
\tau_s &= r_{s,\mathrm{eff}} \, F_t.
\end{align}

Since both torques arise from the same transmitted force, $F_t$ can be eliminated
directly. From the primary expression,
\begin{equation}
...
\tau_s
=
r_{s,\mathrm{eff}} \left( \frac{\tau_p}{r_{p,\mathrm{eff}}} \right)
=
\frac{r_{s,\mathrm{eff}}}{r_{p,\mathrm{eff}}}
\, \tau_p.
\end{equation}
...
% ==============================================================
\subsection{Definition of Coupling Torque}

The torques $\tau_p$ and $\tau_s$ both arise from the same transmitted belt
force. It is therefore natural to define a single \emph{coupling torque} that
characterizes the interaction between the pulleys:
\begin{equation}
\tau_c \equiv \tau_p.
\end{equation}
...
The coupling torque $\tau_c$ defined above is the torque actually transmitted
through the belt.
...
For this reason, one further quantity is needed: a measure of the relative slip
velocity.
...
This quantity compares the two tangential surface speeds that the pulleys would
impose on the belt if each were enforcing adherence.
...
\end{equation}
then the pulley motions are compatible with a common adhered belt motion, so a
sticking state is kinematically possible.
...
The next section derives the no-slip torque $\tau_{\mathrm{ns}}$.
The following section derives the traction limits $\tau_-$ and $\tau_+$ and
thereby makes the traction-limited branch explicit.
