...
$r_{p,\min}$ & minimum primary outer radius \\
$r_{s,\max}$ & maximum secondary outer radius \\
...
$r_{p,\min}$ & Primary minimum outer radius & -- & \si{\meter} & Radius at fully open primary \\
$r_{s,\max}$ & Secondary maximum outer radius & -- & \si{\meter} & Radius at fully closed secondary \\
...
for numerical integration. A practical regularized form uses three ideas:
\begin{itemize}
  \item a smooth Coulomb-like saturation near zero relative speed,
  \item a clamped no-slip torque request,
  \item and a blend between those two limits across a small mismatch region.
\end{itemize}
This preserves the intended stick--slip behavior while avoiding a hard
discontinuity at $v_{\mathrm{rel}}=0$. Implementation details and practical
parameter-selection guidance are provided in
\Sec{sec:appendix-slip-regularization}.
\end{remarkbox}


% --------------------------------------------------------------
\begin{remarkbox}[Remark (Torque deficit, frictional heating, and transient inertial power transfer)]
As an aside, this work is formulated primarily from force and torque balance,
not from a full energy-based loss accounting framework. Even so, viewing the
slip law through an energy lens is useful as a logical validation step: it
helps confirm that the model behavior remains consistent with the law of conservation of
energy.

When the no-slip request exceeds traction capacity,
\begin{equation}
\Delta\tau_{\mathrm{loss}}
=
\max\!\left(0,\,|\tau_{\mathrm{ns}}|-\tau_{\max}\right),
\label{eq:torque_deficit}
\end{equation}
which quantifies the instantaneous torque shortfall between the requested
no-slip coupling and the admissible traction-limited coupling.

During slip, mechanical energy is dissipated at the contact as heat at the rate
\begin{equation}
\dot Q_{\mathrm{fric}}
=
|F_t\,v_{\mathrm{rel}}|
=
\left|\frac{\tau_c}{r_{\mathrm{int}}}\,v_{\mathrm{rel}}\right|,
\label{eq:slip_heat_rate}
\end{equation}
where $r_{\mathrm{int}}$ is the local effective contact radius used to map
torque to tangential force. In traction-limited slip,
$|\tau_c|=\tau_{\max}$, so larger speed mismatch directly increases thermal
generation.

It is also possible to observe slip while $|\tau_{\mathrm{ns}}|<\tau_{\max}$,
because slip can persist temporarily until the existing mismatch decays.
In that interval, the transmitted interface power can exceed the
instantaneous engine shaft output without violating energy conservation.
From the primary rotational balance,
\begin{equation}
\tau_c\,\omega_p
=
\tau_{\mathrm{eng}}\,\omega_p
-
\frac{\dd}{\dd t}\!\left(\frac{1}{2}I_p\omega_p^2\right),
\label{eq:inertial_power_transfer}
\end{equation}
so when the engine-side rotating inertia is decelerating
($\dd(\tfrac{1}{2}I_p\omega_p^2)/\dd t<0$), released kinetic energy supplements
the instantaneous engine power. This effect is transient and vanishes once the
slip mismatch collapses and the system re-enters stick.
\end{remarkbox}
