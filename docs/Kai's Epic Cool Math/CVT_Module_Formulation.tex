...
Because direct experimental drivetrain data for the present vehicle
configuration was not available at the time of writing, this section does not
attempt a quantitative validation against measured traces. Instead, the purpose
of the results is to establish physical credibility of the model by showing
that

\begin{enumerate}
    \item the coupled equations produce a coherent nominal launch trajectory,
    \item force and torque balances extracted from that trajectory behave in the
    physically expected direction, and
    \item targeted tuning changes produce interpretable changes in launch and
    shift behaviour.
\end{enumerate}

The results are therefore organized around a single nominal launch case, from
which a small number of representative operating points are extracted and used
for diagnostic plots. Two additional comparison cases are then introduced to
show how the predicted behaviour changes under modified clutch tuning. This
approach keeps the section focused on the most important mechanisms without
turning it into an exhaustive plot-by-plot replay of the full derivation.

Throughout this section, the centrifugal belt terms derived earlier are
retained in the governing equations, but they are interpreted through their
effect on the overall shift and torque-transfer behaviour rather than being
isolated in a separate subsection.

\FloatBarrier

% --------------------------------------------------
\subsection{Evaluation Setup and Comparison Cases}
\label{sec:results_setup}

All simulations use the nominal parameter set listed in
\Cref{tab:simulation_parameters}, except where explicitly modified for the
comparison cases below. The primary scenario considered is a
full-throttle launch from rest on a level surface.\footnote{Edit this sentence
if a different launch condition is ultimately used, e.g.\ incline, prescribed
load, or alternate throttle input.}
...
Three cases are considered in the results section. Case N is the nominal
baseline and is used for the detailed diagnostic discussion. Cases F and H are
targeted tuning variations chosen to probe the sensitivity of the model to
primary and secondary clutch tuning, respectively.

\begin{table}[H]
\centering
\caption{Simulation cases used in the results section. All parameters not listed are identical to the nominal set of \Cref{tab:simulation_parameters}.}
\label{tab:results_cases}
\small
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Case} & \textbf{Description} & \textbf{Modified parameter(s)} \\
\midrule
N & Nominal baseline & none \\
F & Lighter flyweights & $m_f = \text{[insert value]} < m_{f,\mathrm{nom}}$ \\
H & Steeper helix / reduced torque reactivity &
modified helix profile $\theta_s(s)$ or helix-angle parameter
\text{[insert description]} \\
\bottomrule
\end{tabular}
\end{table}

To avoid introducing arbitrarily chosen static states, all detailed force and
torque diagnostics are evaluated at a small set of representative operating
points extracted directly from the nominal launch trajectory. These points are

\begin{enumerate}
    \item \textbf{Point A: engagement onset}, defined as the first instant of
    sustained torque transfer,
    \item \textbf{Point B: shift initiation}, defined as the first instant of
    sustained positive shift acceleration or ratio change,
    \item \textbf{Point C: mid-shift}, chosen near the middle of the active
    upshift region, and
    \item \textbf{Point D: terminal/high-ratio operation}, chosen near
    shift-out or, if more informative, at the first slip event.
\end{enumerate}

These same operating points are referenced throughout the following
subsections, so that all force and torque interpretations are tied back to a
single internally consistent trajectory.

In addition to the trajectory plots themselves, the following summary metrics
will be used when comparing cases:

\begin{itemize}
    \item engagement engine speed,
    \item shift-initiation time,
    \item time to a chosen vehicle-speed target,
    \item minimum torque-capacity margin, and
    \item any observed slip duration or peak slip metric.
\end{itemize}

The exact subset ultimately reported can be adjusted once the final plots are
available.
