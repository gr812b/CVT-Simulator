...
\subsection{Nominal Launch Overview}
\label{sec:results_nominal_launch}

We begin with the nominal launch trajectory, which serves as the backbone for
the remainder of the section. The goal of this first figure is not yet to
interpret every internal quantity, but simply to establish the overall sequence
of events predicted by the coupled model.

\begin{figure}[H]
\centering
% \includegraphics[width=\textwidth]{figures/results_nominal_overview.png}
\caption{Placeholder: overview of the nominal launch case. A recommended layout is
three vertically stacked panels showing
(a) shift position $s(t)$ or ratio $R(t)$,
(b) engine and secondary angular velocities (or engine speed and vehicle speed),
and
(c) markers identifying operating points A--D used later in the section.}
\label{fig:nominal_launch_overview}
\end{figure}

\Cref{fig:nominal_launch_overview} should show the expected launch progression:
initial engagement from rest, the onset of active ratio change, continued
upshift through the mid-launch region, and eventual approach toward a
high-ratio operating condition. Once the final plots are inserted, the text
here should briefly identify the characteristic phases visible in the launch
history and note where points A--D occur on the trajectory.

A useful tone here is descriptive rather than evaluative. For example:
\emph{Point A marks the onset of sustained belt torque transfer, Point B marks
the beginning of active upshift, Point C lies in the middle of the primary
upshift region, and Point D corresponds to near-terminal ratio operation.}
This gives the reader the basic storyline before the section turns to force and
torque interpretation.

\FloatBarrier

% --------------------------------------------------
\subsection{Shift Dynamics at Selected Operating Points}
\label{sec:results_shift_diagnostics}

The axial dynamics are governed by \Eq{eq:s_ddot_complete}, in which the
generalized shift force is the difference between the primary-side closing
forces and the secondary-side resisting forces. A central question for the
results section is therefore whether the predicted shift trajectory is
consistent with the force balance implied by that equation.

Rather than plotting every force contribution over the entire launch, the force
balance is examined at the representative operating points A--D identified in
\Cref{fig:nominal_launch_overview}. This keeps the discussion readable while
still tying the observed ratio motion directly back to the governing mechanics.

\begin{figure}[H]
\centering
% \includegraphics[width=\textwidth]{figures/results_shift_force_timehistory.png}
\caption{Placeholder: nominal launch shift diagnostics. A recommended layout is
two panels showing
(a) shift position $s(t)$ or ratio $R(t)$ with points A--D marked, and
(b) net generalized shift force $\sum F_s$ or shift acceleration $\ddot s$
over the same time interval.}
\label{fig:shift_force_timehistory}
\end{figure}

\begin{figure}[H]
\centering
% \includegraphics[width=\textwidth]{figures/results_shift_force_breakdown_points.png}
\caption{Placeholder: force decomposition at selected operating points.
A useful presentation is either grouped bar charts or stacked contribution plots
showing the terms
$F_{p,\mathrm{ax}}$,
$F_{c,p}$,
$F_{s,\mathrm{ax}}$, and
$F_{c,s}$
at points A--D.}
\label{fig:shift_force_breakdown_points}
\end{figure}

The intended interpretation is as follows.

At \textbf{Point A}, the force balance should still be close to the initial
engagement condition, with little or no sustained upshift motion.
At \textbf{Point B}, the net generalized force should first become
consistently positive, indicating the start of active primary closing and
secondary opening.
At \textbf{Point C}, the positive shift-driving force should be clearly
established, corresponding to continued ratio evolution through the main launch
region.
By \textbf{Point D}, the generalized force is expected to weaken again as the
system approaches a high-ratio state, unless the selected point instead
corresponds to a slip-limited event.

This subsection is the natural place to report the two most important static
checks extracted from the nominal launch:

\begin{enumerate}
    \item the \textbf{engagement condition}, identified by the first sustained
    rise in transmitted torque and corresponding change in force balance, and
    \item the \textbf{shift-initiation condition}, identified by the first
    sustained sign change in $\sum F_s$ or first persistent nonzero
    $\dot s$.
\end{enumerate}

If the final plots make it readable, one short sentence may also be added here
to note how the predicted equilibrium tendency changes over the launch
trajectory, but this subsection should remain focused on the force balance
itself rather than turning into a separate quasi-static study.

\FloatBarrier

% --------------------------------------------------
\subsection{Torque Transfer and Slip Margin During Launch}
\label{sec:results_torque_transfer}

The torque-transfer formulation is one of the key closures of the model. The
required no-slip coupling torque is given by \Eq{eq:tau_ns_final}, while the
actual transmitted torque is selected by the regime law in
\Eq{eq:tau_c_complete}. The main diagnostic question is therefore whether the
nominal launch remains within the admissible traction interval, and if not,
when and how the model transitions to a slip-limited torque state.

\begin{figure}[H]
\centering
% \includegraphics[width=\textwidth]{figures/results_torque_bounds_nominal.png}
\caption{Placeholder: nominal launch torque-transfer diagnostics. A recommended
layout is two panels showing
(a) the no-slip torque demand $\tau_{\mathrm{ns}}$, the actual transmitted
torque $\tau_c$, and the active traction bounds over time, and
(b) the corresponding torque-capacity margin.}
\label{fig:torque_bounds_nominal}
\end{figure}

\begin{figure}[H]
\centering
% \includegraphics[width=\textwidth]{figures/results_slip_metric_nominal.png}
\caption{Placeholder: slip diagnostic for the nominal launch, such as the slip
metric $v_{\Delta}$, a binary regime indicator, or both. If no slip occurs in
the nominal case, this figure can instead emphasize the closest approach to the
traction limit.}
\label{fig:slip_metric_nominal}
\end{figure}

The text here should be written to accommodate either of two outcomes.

If the nominal launch remains in the sticking regime throughout, then the main
result is that the demanded no-slip torque remains inside the admissible
traction interval, with the minimum torque margin indicating how close the
system comes to slip. In that case, the section still serves an important
purpose by showing that the regime logic is active and that the sticking
solution is not simply assumed.

If the nominal launch does enter slip, then this subsection should identify the
first slip event, describe whether it is brief or sustained, and relate it back
to the operating-point logic introduced earlier. In that case, the key sentence
to include is that once the no-slip demand exits the admissible interval, the
coupling torque no longer follows $\tau_{\mathrm{ns}}$ and instead saturates at
the active slipping bound selected by \Eq{eq:tau_c_complete}.

Either way, this is the subsection in which the reader should become convinced
that the torque-transfer closure is behaving as intended during a full dynamic
launch.

\FloatBarrier

% --------------------------------------------------
\subsection{Effect of Tuning Changes}
\label{sec:results_case_comparison}

Having established the nominal launch behaviour, we now compare it against the
two targeted tuning variations of \Cref{tab:results_cases}. The aim here is not
to perform an exhaustive parameter sweep, but simply to show that the model
responds in physically interpretable directions when the clutch tuning is
changed.

\begin{figure}[H]
\centering
% \includegraphics[width=\textwidth]{figures/results_case_overlay_launch.png}
\caption{Placeholder: comparison of the three cases N, F, and H. A recommended
layout is two or three panels showing overlays of
(a) ratio $R(t)$ or shift position $s(t)$,
(b) engine speed and/or vehicle speed,
and optionally
(c) torque-capacity margin or slip metric.}
\label{fig:case_overlay_launch}
\end{figure}

\begin{table}[H]
\centering
\caption{Placeholder: summary metrics comparing the nominal and modified tuning cases.}
\label{tab:results_summary_metrics}
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Case}
& $\omega_{p,\mathrm{eng}}$
& $t_{\mathrm{shift,start}}$
& $t_{v=v^\ast}$
& $\min(\text{torque margin})$
& \textbf{Slip duration} \\
\midrule
N & -- & -- & -- & -- & -- \\
F & -- & -- & -- & -- & -- \\
...
The expected qualitative trends are straightforward.

For \textbf{Case F} (lighter flyweights), the reduced centrifugal flyweight
force should delay clutch engagement and weaken the primary closing tendency at
a given engine speed. This would be expected to shift the launch behaviour
toward higher engagement speed and later upshift onset, with any additional
changes in launch time or torque margin depending on how the engine operating
point is affected.

For \textbf{Case H} (steeper helix / reduced torque reactivity), the secondary
should oppose ratio change less strongly under load. This would be expected to
alter the balance between primary closing and secondary resisting forces,
thereby changing the predicted shift trajectory and possibly the torque-capacity
margin during the launch.

Once the final plots are available, the main task in this subsection is simply
to confirm whether those directional expectations are borne out. The discussion
should stay compact: one short paragraph for Case F, one short paragraph for
Case H, and a closing sentence about which tuning change produced the larger
effect in the reported metrics.
...
Taken together, the results should support the following claims.

First, the coupled model produces a coherent nominal launch trajectory whose
main phases can be interpreted directly in terms of the derived force and
torque balances. Second, the operating-point diagnostics tie the predicted
ratio motion back to the axial-force closure of \Eq{eq:s_ddot_complete} without
requiring separately invented static states. Third, the torque-transfer plots
show whether the no-slip demand remains admissible or whether the model enters a
slip-limited regime, thereby directly exercising the regime logic of
\Eq{eq:tau_c_complete}. Finally, the comparison cases demonstrate that the
model responds to targeted tuning changes in the physically expected direction.

This level of evidence does not replace future validation against measured
vehicle data, but it does provide a clear internal check that the present
formulation behaves as a mechanically reasonable CVT/drivetrain model over the
launch scenarios considered here.
\section{Conclusion}
