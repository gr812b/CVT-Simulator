...
The goal statement GS\ref{GS3} are solved by Instance Models \ref{inst:4}, \ref{inst:5}, \ref{inst:6} and \ref{inst:7}\\
  ...
  {
    \[\frac{dv_\text{car}}{dt} = \frac{\frac{R_\text{CVT} R_\text{gearbox}}{r_\text{wheel}} \tau_\text{engine}(\frac{v_\text{car} R_\text{CVT} R_\text{gearbox}}{r_\text{wheel}})}{m_v + m_d} - g\sin{\theta_\text{inc}} - \frac{\rho C_D A}{2(m_v + m_d)} v_\text{car}^2 \]
  }
