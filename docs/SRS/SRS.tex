...
\item Provide the input data related to the vehicle and driver, as well as the limited environment.
\end{itemize}
...
{Hooke's Law (Compressional)}% #3 label
{$F = k \Delta x$}% #4 equation
{$F$ is the force (N)\\
  $k$ is the spring constant (N/m)\\
  $\Delta x$ is the displacement (m)}% #5 description
{In the context of this project Hooke's law is used in both a torsional and linear way.}% #6 Notes
{\citet{hookeslaw}}
{\hpref{IM:PCF}}% #8 Referenced by
...
{None.}% #6 Notes
{\citet{CentrifugalForceCalculator2024}}% #7 Source
{\hpref{IM:PCF}}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{None.}% #6 Notes
{\citet{AirResistanceFormula}}% #7 Source
{\hpref{IM:PCF}}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{None.}% #6 Notes
{\citet{HorsepowerCalculation}}% #7 Source
{}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{None.}% #6 Notes
{\citet{GearRatioExplanation}}% #7 Source
{\hpref{IM:A}}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{None.}% #6 Notes
{\citet{FrictionWikipedia}}% #7 Source
{\hpref{IM:A}, \hpref{IM:PCF}}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{None.}% #6 Notes
{\citet{BritannicaDensity}}% #7 Source
{\hpref{IM:A}}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{None.}% #6 Notes
{\citet{CapstanEquation2021}}% #7 Source
{\hpref{IM:SV}}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{\citet{NewtonsSecondLaw}}% #7 Source
{\hpref{IM:A}, \hpref{IM:SA}, \hpref{IM:SV}}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{}% #6 Notes
{\citet{SpringDesignFormulas}}% #7 Source
{}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{$\mathit{KE} = \frac{1}{2} m v^2$}% #4 equation
{$\mathit{KE}$ is the kinetic energy (J)\\
  $m$ is the mass (kg)\\
  $v$ is the velocity (m/s)
}% #5 description
{None.}% #6 Notes
{\citet{KineticEnergyPhysicsClassroom}}% #7 Source
{}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
\noindent
\deftheory
{TM:GE}% #2 refname of theory
{Gavitational Potential Energy}% #3 label
{$\mathit{GE} = mgh$}% #4 equation
{
$\mathit{GE}$ is the gravitational potential energy (J)\\
  $m$ is the mass (kg)\\
  $g$ is the gravitational constant (9.81 m/s$^2$)\\
  $h$ is the height (m)
}% #5 description
{None.}% #6 Notes
{\citet{GravitationalPotentialEnergyOnlineMath}}% #7 Source
{}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
\begin{figure}[H]
  \begin{center}
      \includegraphics[scale=0.25]{ShiftDistToRadius.png}
      \caption{Sheave Geometry}
      \label{Fig_Sheave}
  \end{center}
\end{figure}
...
{The user will input the geometry of the ramp, which will then be interpreted to generate $f_\text{prim}$}% #6 Notes
{N/A}% #7 Source
{}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
\defgeneral
{GD:SSSF}
{Secondary Spring Side Force} % #1 label
{N = \si{\kilogram \metre\per\square\second}} % #2 units
...
\defgeneral
{GD:AVE}
{Angular Velocity of the Engine} % #1 label
{rad/s} % #2 units
{
...
{None.}% #7 Source
{\hpref{IM:A}, \hpref{IM:SV}}% #8 Referenced by

\defdata
{DD:CSC} % #1 refname of data
...
{This is defined by Baja SAE via \citet{BajaSAEKohlerEngine2022}}% #7 Source
{}% #8 Referenced by
...
{\noindent}The goal statement GS\ref{GS2} are solved by Instance Models \hpref{IM:SD}\\
The goal statement GS\ref{GS3} are solved by Instance Models \hpref{IM:PCF}, \hpref{IM:SCF}, \hpref{IM:SA} and \hpref{IM:SV}\\
  \definstance
  {IM:A}
  {Acceleration}
...
Using equations for \hpref{TM:KE}{kinetic energy}, \hpref{TM:GE}{gavitational potential energy}, \hpref{TM:AR}{air resistance} and \hpref{TM:ET}{engine power}, the acceleration of the vehicle is derived using the law of conservation of energy.
\[
P = \frac{dK}{dt} + \frac{dU}{dt} + \frac{dF_D}{dt} \tag*{\text{(see \hyperref[TM:KE]{TM:KE}, \hyperref[TM:GE]{TM:GE}, \hyperref[TM:AR]{TM:AR})}}
\]
...
\]}
{$v_\text{car}$}
{$a_\text{car}$ is the vehicles acceleration (m/s$^2$)}
{-}
...
\]}
{$d_\text{car}$}
{$v_\text{car}$ is the vehicles velocity (m/s)}
{-}

\definstance
{IM:PCF}
{Primary Clamping Force}
{$d_{\text{shift}}$, $\omega_\text{eng}$, $m_\text{fly}$, $r_{\text{fly}}$, $f_{\text{prim\_height}}(d_{\text{shift}})$, $f_{\text{prim\_angle}}(d_{\text{shift}})$, $k_{\text{prim}}$, $d_{\text{prim}}$} % Input
{
  \begin{multline*}
...
We can ignore the radial force as it acts perpendicular to the ramp and does not contribute to the axial force. (see \hpref{A:IFF})
...
{$d_\text{shift}$, $T_\text{eng}$, $R_\text{cvt}$, $k_{\text{sec\_tor}}$, $\theta_{\text{sec}}$, $r_{\text{sec\_ramp}}$, $f_{\text{sec\_ramp}}(d_{\text{shift}})$, $f_{\text{sec\_shift}}(d_{\text{shift}})$, $k_{\text{sec\_comp}}$, $d_{\text{sec}}$} % Input
{
  \begin{multline*}
...
{Secondary clamping force $F_{\text{sec\_clamp}}$ (N)} % Output
{
  $d_{\text{shift}}$: Displacement of the sheave as a function of time. (m)\\
...
\[
F_H = \frac{T_{eng}(t) R_{cvt}(t)}{2r\tan(\beta)}  \tag*{\text{(see \hpref{GD:SSSF})}} % TODO: Fix reference
\]
...
\[
F_S = \frac{k_{\text{tor}}(\theta_0 + \theta)}{2r\tan(\beta)} + k_\text{comp}(x_0 + x) \tag*{\text{(see \hpref{GD:HSF})}} % TODO: Fix reference
\]
...
{IM:SA}
{Shift Acceleration}
{$d_\text{shift}, \omega_\text{eng}, T_\text{eng}, R_\text{cvt}, m_\text{sheaves}$} % Input
{\[
...
\]} % Equation
{shift acceleration $a_\text{shift}$} % Output
{
...
One can derive the acceleration of the sheaves using \hpref{TM:N2}{Newton's Second Law}. The forces acting on the built include 
...
\[mv\frac{dv}{dt} = F_{\text{prim\_clamp}}(d_\text{shift}, \omega_\text{eng}) - F_{\text{sec\_clamp}}(d_\text{shift}, T_\text{eng}, R_\text{cvt})\]
...
\]} % Equation
{shift velocity $v_\text{shift}$} % Output
{
...
\]} % Equation
{Shift Distance $d_\text{shift}$} % Output
{
...
The equation for the torque of the taut side is derived using the \hpref{TM:CE}{capstan equation}:
...
\begin{itemize}
    \item \textbf{ISO/IEC 25002:2024}: Software quality models ensuring reliability and maintainability \cite{ISO26262Standard}. This will be ensured via NFR\ref{NFR:MA}. 
\end{itemize}
...
\item[LC:TMP \label{LC:TMP}:] The effect of temperature on material properties might be included in the future, (A\ref{A_1}) Stakeholders would be interested in seeing this information in the future.
\item[LC:EB \label{LC:EB}:] The elasticity of the belt might be considered in the future, (A\ref{A_3}) This would provide a more accurate model of the CVT system.
\item[LC:FL \label{LC:FL}:] Frictional losses in non-critical components might be considered in the future, (A\ref{A_6}) This would provide a more accurate model of the CVT system.
...
\begin{itemize}
  ...
  \item[ULC:AL \label{ULC:AL}:] The assumption that the CVT axis are in line is unlikely to change because tuning is simply not valuable in a system as poor as this. (A\ref{A_8})
  \item[ULC:FT \label{ULC:FT}:] The assumption that the engine operates at full throttle is unlikely to change as different behavior is simply not ideal. (A\ref{A_7})
  \item[ULC:GF \label{ULC:GF}:] The assumption that the influence of gravitational forces is negligible is unlikely to change as it heavily reduces complexity and plays an incredibly small part on spinning objects at high rotational speeds. (A\ref{A_2})
...
\hline
	& \hpref{A:NMVT}& \hpref{A:NGI}& \hpref{A:RB}& \hpref{A:NVE}& \hpref{A:IWT}& \hpref{A:NFL}& \hpref{A:FTPE}& \hpref{A:PAA}& \hpref{A:NBS}& \hpref{A:IFF}\\
\hline
\tref{TM:HL}        & & & & & & & & & & \\ \hline
\tref{TM:CF}        & & & & & & & & & & \\ \hline
\tref{TM:AR}        & & & & & & & & & & \\ \hline
