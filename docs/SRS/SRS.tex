...
Apr 01 & 0.6 & Added Security Requirements \\
...
the instance cvt_simulator.models. 
...
including theoretical models, general definitions, data definitions, assumptions, and instanced cvt_simulator.models.
...
{\citet{hookeslaw}}
{\hpref{IM:PCF}, \hpref{IM:SCF}}% #8 Referenced by
...
{\hpref{IM:A}, \hpref{IM:SCF}, \hpref{IM:SA}, \hpref{IM:BA}}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{\hpref{IM:A}, \hpref{IM:SCF}}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{$\tau_1 = \tau_2 e^{\mu \theta}$}% #4 equation
{$\tau_1$ is the tension on the first side of the belt (N)\\
  $\tau_2$ is the tension on the second side of the belt (N)\\
  $\mu$ is the coefficient of friction\\
  $\theta$ is the angle of wrap of the belt (radians)}% #5 description
{None.}% #6 Notes
{\citet{CapstanEquation2021}}% #7 Source
{\hpref{IM:BA}}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{\hpref{IM:A}, \hpref{IM:SA}, \hpref{IM:BA}}% #8 Referenced by
{None}% #9 Preconditions
{}% #1 derivation - not applicable by default
...
{$F_H = \frac{\tau_\text{eng} R_{\text{CVT}}}{2r\tan(\beta)}$} % #3 equation
{$F_H$ is the side force due to the helix (N) \\
& $\tau_{\text{eng}}$ is the engine torque (N-m) \\
...
\[ \tau_\text{CVT} = \tau_\text{eng} \cdot R_{\text{CVT}} \tag*{\text{(see \hyperref[TM:GR]{TM:GR})}}\]

Since this is a torque, we can convert it to a force by dividing by the radius of the ramps. Further, since the ramps are angled at $\beta$, we divide by $\tan(\beta)$ to get the force in the direction of the helix (see \hpref{A:IF}). Finally, since only one of the two pulleys is moveable, and the force is acting on both, we must divide by a factor of 2.
...
& Due to the no slip assumption \hpref{A:NS}, the engine velocity is directly correlated to the speed of the vehicles through the gearbox ratio and CVT. Since the engine torque at each angular velocity is pre-defined, we can then get these at each point.
}  % #4 description
{-} % #5 source
{\hpref{IM:PCF}} % #6 referenced by
...
{$d_\text{shift}$, $\tau_\text{eng}$, $R_{\text{CVT}}$, $k_{\text{sec\_tor}}$, $\theta_{\text{sec}}$, $r_{\text{sec\_ramp}}$, $f_{\text{sec\_ramp}}(d_{\text{shift}})$, $f_{\text{sec\_shift}}(d_{\text{shift}})$, $k_{\text{sec\_comp}}$, $d_{\text{sec}}$} % Input
{
  \begin{multline*}
...
{The secondary clamping force $F_{\text{sec\_clamp}}$ (N)} % Output
{
  $d_{\text{shift}}$: Displacement of the sheave as a function of time. (m)\\
...
\[
F_H = \frac{\tau_{eng}(t) R_{\text{CVT}}(t)}{2r\tan(\beta)}  \tag*{\text{(see \hpref{GD:SSSF})}} % TODO: Fix reference
\]
...
{IM:SA}
{Shift Acceleration}
{$d_\text{shift}, \omega_\text{eng}, \tau_\text{eng}, R_{\text{CVT}}, m_\text{sheaves}$} % Input
{\[
...
\]} % Equation
{The shift acceleration $a_\text{shift}$} % Output
{
...
\]} % Equation
{The shift Velocity $v_\text{shift}$} % Output
{
...
\]} % Equation
{The distance shifted $d_\text{shift}$} % Output
{
...
% TODO: Clean up \tau_T and \tau_s, shouldnt's exist like this? These are forces that change over time, its why we derived them below
\definstance
{IM:BA}
{Belt Acceleration}
{$\tau_{\text{eng}}, r_{\text{prim}}, m_\text{belt}$ } % Input
{$a_{\text{belt}} = \frac{\tau_{\text{eng}}}{m r_{\text{prim}}}$} % Equation
...
\[\tau_{\text{eng}} = (\tau_T - \tau_s) r_{\text{prim}} \quad \Rightarrow \quad \tau_T - \tau_s = \frac{\tau_{\text{eng}}}{r_{\text{prim}}}\] % TODO: Add TM and reference here
Similarly, for the load torque: \\
\[\tau_{\text{load}} = (\tau_T - \tau_s) r_{\text{sec}} \quad \Rightarrow \quad \tau_T - \tau_s = \frac{\tau_{\text{load}}}{r_{\text{sec}}}\] % TODO: Add TM and reference here
...
\item[R:MM \label{R:MM}:] The system shall simulate the behavior of the McMaster Baja Team's Continuous Variable Transmission(CVT) using mathematical cvt_simulator.models.
...
The following requirements were derived from the \href{https://github.com/gr812b/CVT-Simulator/blob/main/docs/HazardAnalysis/HazardAnalysis.pdf#page=40}{HazardAnalysis} document.
...
Many key assumptions are made to meet the requirement of developing these mathematical cvt_simulator.models. The following assumptions are also factored in to the calculations of the CVT ratio, clamping force, RPM, torque, engine input, distance, speed and acceleration(\hpref{R:AV}). 
\\\\
...
\begin{enumerate}
  ...
  For Unity, we need to learn how to make a GUI interface, animate 3D models, create and display graphs, and learn how to interface with a Python script to run the mathematical cvt_simulator.models.
  ...
  For learning the Python required, we will do research on relevant libraries and functions that will help us implement our mathematical cvt_simulator.models.
