...
Nearly coincident shaft-speed or shift curves do not establish that the same contacts
and supports carried the motion. During a brief primary separation, for
example, the primary belt force and torque vanish from the active balances
even if the later upshift changes little
(\Cref{eq:deadzone_primary_disengaged}). Numerical refinement must therefore
resolve both the continuous motion and the changes in mechanical arrangement.
The test uses the ten-second reference launch, compared with a tighter LSODA
solution at \(\mathrm{rtol}=10^{-7}\), \(\mathrm{atol}=10^{-10}\), and
a 2.5 ms maximum step. This is a numerical reference, not an exact solution.
The unchanged physical case and tested controls are specified in
\Cref{app:study_solver_convergence}.

The comparison first checks the ordered event names, outgoing modes and
recorded state updates, rather than only counting transitions. When that
signature agrees, corresponding smooth segments are compared at equal
fractional progress between their endpoint events. This keeps a small timing
offset from comparing an incoming velocity with an already-reset outgoing
velocity. The offset is not discarded: event-time errors and the duration
spent in different regimes are checked separately. All five states, including
shift speed, enter the aligned state errors. The definitions and acceptance
limits are given in
\Cref{eq:solver_event_aligned_rms,tab:solver_convergence_guards}.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/results/verification/solver_refinement.pdf}
  \caption{Formal numerical refinement. (a) Each cell is one of the 40 tested
  tolerance/maximum-step combinations. Colour shows the five-state aligned
  RMS; circles additionally require every state, timing and history guard
  to pass; F marks the failed integration. (b) Each error is divided by its own guard limit, so values above
  one fail. Crosses identify different signatures, for which aligned state
  and corresponding event-time errors are undefined. Regime-duration error
  remains defined. Gold marks the research setting. No boundary is inferred
  between sampled settings; maximum step is a cap on adaptive integration.}
  \label{fig:verification_solver_refinement}
\end{figure}

{\clubpenalty=10000
Twenty-five of the 40 combinations pass every guard. In particular, all
20 cells with \(\mathrm{rtol}\leq10^{-4}\) pass across the tested
5--100 ms step caps, using \(\mathrm{atol}=10^{-3}\mathrm{rtol}\).
At the research setting of \(10^{-4}\), \(10^{-7}\) and 10 ms, the
aligned RMS is \(4.27\times10^{-7}\) and the largest event-time difference
is \(7.99~\mu\mathrm{s}\); the entire 16-transition signature agrees.
The coarsest combination, \(\mathrm{rtol}=10^{-2}\) with a 100 ms cap,
instead fails in an inadmissible primary flyweight-contact state. That failed
calculation is retained in \Cref{fig:verification_solver_refinement}(a);
it does not demonstrate physical contact loss on the converged trajectory.\par}

Absolute tolerance deserves a separate check because the shaft speeds,
millimetre-scale shift position and shift speed have very different numerical
scales. In panel~(b), \(10^{-5}\) and \(10^{-6}\) give different histories.
All four tested values from \(3\times10^{-7}\) down to \(10^{-8}\)
recover the reference signature, but \(3\times10^{-8}\) still exceeds
the maximum-state-error limit. The research value \(10^{-7}\) passes all
guards. Thus neither a tighter scalar tolerance nor recovery of the event
sequence alone guarantees improvement in every error measure.

The separate exploratory sweep shows why this distinction matters to the
mechanical interpretation. Its gross-motion diagnostic compares shaft speeds,
belt speed and shift position in absolute time, excluding shift speed; it is
not the formal five-state aligned norm. Among its 609 completed runs with a
different signature, the median gross RMS is only \(1.225\times10^{-4}\).
\Cref{fig:verification_gross_hybrid_agreement} places that population beside
a replay of the settings at its median, selected before examining the
reconstructed history.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/results/verification/gross_motion_hybrid_history.pdf}
  \caption{Gross motion and contact history. (a) All 4,681 completed exploratory
  runs, grouped by exact signature agreement, not transition count; the 264
  failed runs have no trajectory norm. Native solver points are a work proxy,
  not function evaluations. The star selects the median different-signature
  setting. (b) Its replay and the tight reference have nearly coincident
  ten-second shift curves. (c) Their native absolute-time histories differ
  during engagement and seating. The shortest intervals cannot be resolved
  at this scale; their durations are retained in
  \Cref{tab:solver_separation_intervals}. Traces are not joined across resets.}
  \label{fig:verification_gross_hybrid_agreement}
\end{figure}

The replay has 12 transitions rather than 16. After first engagement, the
primary separates and regains belt contact three times before settling at the
low-ratio seat, compared with five times in the reference. Seating occurs at
64.830 rather than 66.012 ms, while the later shift curves remain almost
indistinguishable in panel~(b). The different contact-free intervals nevertheless
remove and restore the primary belt loading at different instants. Their
small contribution to a ten-second gross-motion average cannot establish
agreement of those local mechanics.

The numerical resolution needed therefore depends on what is being inferred
from a trajectory. A setting adequate to reproduce the overall upshift can
still be inadequate for explaining contact or support changes along it.
The research setting passes the state and history checks on this launch; other
trajectories retain their own study-specific numerical settings and checks.
\FloatBarrier
...
throttle and zero road grade for ten seconds. Only the LSODA controls change.
...
Research setting & $10^{-4}$ & $10^{-7}$ & 10 ms \\
...
nonzero velocity jump. The retained reference and research caches were also
...
\bottomrule
\end{tabular}
\caption{Formal guards and the retained research-setting result. All guards
...
Of 4,681 completed dense runs, 4,072 have the exact reference signature and
...
\begin{tabular}{@{}crr@{}}
\toprule
Contact-free excursion & Reference duration [$\mu$s] & Replay duration [$\mu$s] \\
\midrule
1 & 1878.269 & 1412.204 \\
2 & 331.672 & 192.146 \\
3 & 60.418 & 7.219 \\
...
the research setting retains 1,156 points and takes 7.257 s for hybrid
...
tables; both-side caches permit direct recomputation of the research-setting
