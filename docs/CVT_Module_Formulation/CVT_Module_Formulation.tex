...
while that simulation is advanced. The complete fixed-data set is collected in
\Cref{tab:cvt_fixed_data}, while
\Cref{tab:cvt_instantaneous_results_by_regime} lists the instantaneous
quantities returned in each mechanical regime.
\Cref{tab:model_quantity_roles} summarizes the different roles these quantities
play during one model evaluation.
...
& State derivative \(\dot{\vect{x}}(t)\) and other mechanical quantities required for the current
  evaluation, including regime-admissibility measures
& Are determined from the fixed data, current CVT condition, and
  shaft-interface data; determine how the state can advance and whether the
  current regime can continue. \\
\bottomrule
\end{tabular}
\end{table}
...
shaft-interface data provide its current external loading, and
\(\bigl(\vect{x},q\bigr)\) gives the internal condition to which that loading
is applied. The instantaneous results are the response of that condition. One
evaluation can therefore be stated compactly:
...
\begin{enumerate}
    ...
    \item The CVT mechanics use the fixed CVT data, current internal condition,
          ...
          response. This provides \(\dot{\vect{x}}(t)\) together with the
          quantities needed to test whether the current regime remains
          admissible.
