...
The nominal case used throughout this section, R00, is the unmodified Baja
reference CVT defined in \Cref{app:common_physical_configuration}, with the
bilateral secondary helix topology of \Cref{app:reference_helix_topology}. Its
primary shaft is driven by the full-throttle Kohler CH440 boundary of
\Cref{app:primary_engine_boundary}, while its secondary is coupled to the
locked-final-drive reference vehicle of \Cref{app:secondary_vehicle_boundary};
the road grade therefore enters the secondary shaft through the vehicle load in
\Cref{eq:reference_vehicle_torque}. Every case begins from the same state in
\Cref{eq:results_reference_initial_state}. The vehicle, final drive, engine
boundary, belt, fixed pulley geometry, contact coefficients, and boundary
inertias remain common to all cases. The only differences are the declared CVT
hardware changes collected later in \Cref{tab:results_course_cases}.

The road is also common to every case. It is prescribed as a function of
travelled distance rather than clock time, so two vehicles encounter the same
grade at the same position even if they arrive there at different times or
with different CVT states. No state is reset when one part of the road ends:
the motion produced on the opening flat is the motion carried into the first
climb, and each later feature begins from the state left by the previous one.
\Cref{fig:results_course_overview} defines the complete distance-indexed
profile used for that comparison.
...
Each part of the road is included for a different mechanical reason. The
opening flat gives the CVT enough distance to engage and establish its shift
characteristic before a large external disturbance is introduced. The
\(38^\circ\) climb then asks whether that trajectory can backshift far enough
and transmit the torque required by a severe load; deliberately unfavourable
hardware can therefore expose either a poor operating point or a genuine
traction limit. The level recovery retains the state produced by that climb
while allowing the transmission to move back toward high ratio. The six-cycle
section then applies repeated longitudinal load changes to a CVT whose position
and contact state have already been shaped by the preceding road. The long
\(18^\circ\) hold serves a different purpose: it gives the backshift transient
time to decay so that the interior ratio, engine speed, and climbing speed can
be compared under a sustained load. The descent finally reverses the direction
of shaft power for the finishing cases, allowing the same signed mechanics to
be followed into engine-absorbing operation without introducing a separate CVT
model.

With that external problem fixed, the hardware cases are defined in
\Cref{tab:results_course_cases}. The table establishes the case labels used
throughout the remainder of the section and lists only what changes from the
nominal R00 configuration; every unlisted quantity retains the R00 value from
...
\begin{table}[H]
  ...
  from the nominal R00 reference configuration are listed. Ramp distances are
  ...
  \begin{tabular}{@{}
    ...
    \midrule
    R00 & Nominal Baja reference configuration of
      \Cref{app:common_physical_configuration,app:reference_helix_topology}. \\
...
The cases are not intended to form a ranking of complete tunes. They are chosen
so that different physical ways of changing the transmission can be separated
before their consequences are compared on the same road. W85 changes the
centrifugal action through flyweight mass. P300 changes how the opposing
primary spring force grows through the shift while matching its force at the
selected engagement geometry. RC10, R26B7, and RC40L instead change the ramp
surface that the roller follows, allowing the shape of the primary actuation
characteristic to be altered in different parts of the travel. H28 moves the
comparison to the secondary side by changing the helix geometry that converts
torque reaction into axial force.

The remaining cases are included for diagnosis rather than as candidate
performance tunes. U55, D01, and D02 deliberately push different parts of the
mechanism far enough to expose limitations that a well-behaved reference case
would not reveal. D02\_M and D02\_P then return to one of those failures by
restoring one primary contribution at a time. They therefore test a stronger
question than whether a bad response can be removed: once the cause has been
identified, do two plausible mechanical corrections lead to the same
transmission behaviour? With those roles established before any trajectory is
interpreted, the level opening provides the simplest place to see how the
hardware first changes the response.
...
Much of the local physics required for these extensions already has a foundation 
in the literature reviewed in \Cref{sec:literature-review}. The challenge is to 
incorporate those mechanisms while retaining the coupled transient response of 
the belt, pulleys, and actuators. Some existing descriptions assume steady running, 
prescribe part of the motion, or determine a component’s position through 
instantaneous force balance. These assumptions can make a particular mechanism 
easier to study, but they may also remove interactions that the present formulation 
is intended to resolve. Where that occurs, the additional physics must be coupled 
to the equations of motion so that belt loading and hardware motion continue to 
determine one another under the applied shaft loading.
