...
% ==============================================================
\section{Design and Tuning Consequences}
\label{sec:results_design_consequences}

% Optional final section.  Populate only if the final Results scope supports a
% compact comparison whose purpose is interpretation rather than optimization.

\subsection{Representative Tune Comparison}
\label{sec:results_tune_comparison}
% TODO: small set of deliberately distinct tunes, if retained in final manuscript.
...
% Future-work section for the Conclusion and Future Work chapter.
% Reference basis: CVT_Module_Formulation(20260920-195252).tex.
% Uses the manuscript's existing natbib, cleveref, and AssRef commands.
% New bibliography entries are supplied in CINDER_Future_Work_References.bib.

\section{Future Work}
\label{sec:future_work}

The present formulation makes it possible to follow how the shafts, actuators,
and belt respond together as the CVT changes operating condition. The
assumptions that make this description compact also identify where it can be
extended. Some omitted forces can be added to the existing free bodies.
Other changes would give the belt motions and deformations that its present
coordinates cannot describe. The useful question is therefore not simply
which assumptions can be removed, but what those changes would allow us to
explain about the transient response.

Much of the required local physics already has a foundation in the literature
reviewed in \Cref{sec:literature-review}. The opportunity is to bring those
mechanisms into the same problem in which shaft speeds, sheave motion, belt
loading, and changes of contact are determined together. This need not
preserve the present state vector or every equation. It should preserve the
mechanical interaction that motivated them: a change in belt loading changes
the actuator response, and the resulting motion changes the loading again.
Experiments are needed alongside these extensions, both to test the present
model and to establish which additional mechanisms materially change its
predictions.

\subsection{Friction in the Clamping Mechanisms}
\label{sec:future_work_mechanism_friction}

The same mechanism position need not produce the same clamping force while
the CVT shifts in opposite directions. In the present model, acceleration can
already create this distinction because some of the applied loading is
required to accelerate the actuator. Friction in the ramps, helix, and guides
would add a different source of resistance. It opposes relative motion and
may hold the mechanism at rest until the available driving force is
sufficient to start it moving. These effects are excluded by
\AssRef{assump:ideal_actuator_mechanics}, but their points of entry are already
visible in the primary free body and secondary helix balance
(\Cref{eq:primary_movable_sheave_balance,eq:secondary_reacted_helix_torque_structure}).

There are established actuator descriptions on which to build.
\citet{KimEtAl2002} include contact friction in their mechanical-actuator
relations, and \citet{Cammalleri2005} distinguish the friction directions for
incipient upshift and downshift. Incorporating such forces into the present
dynamic balances would make it possible to investigate their effect on
breakaway, shift reversal, and the recovery of engine speed after a load
increase. The existing contact reactions provide a starting point, but they
must be determined with the added friction present. Applying a friction
correction to the already solved frictionless reaction would not, in general,
produce the same force balance. For a roller mechanism, bearing drag and
rolling resistance would also have to be distinguished from sliding at the
ramp contact.

Friction in the actuator is not the only possible cause of a different
upshift and backshift response. \citet[p.~645]{KimEtAl2002} explain the
different coefficients in their experimentally identified shift law through
the direction of radial belt friction at the primary. During downshift, that
friction can support a condition resembling the self-locking behaviour of
steady running; during upshift, its reversal changes the belt's radial and
circumferential motion. Their explanation concerns the belt--groove contact,
not an isolated measurement of actuator friction. A useful investigation would
therefore distinguish the force that the actuator can deliver from the force
that the moving belt requires. Comparing matched upshift and backshift
manoeuvres at different rates, with independently characterised actuator
friction, would help establish how much of the directional response comes
from actuator friction, belt contact, and inertia. It would also show when these effects
delay the return to a useful engine operating region.

Static actuator friction would require more than an additional dissipated
power. A sheave could remain held away from a travel stop, and motion reversal
could introduce a new sticking or sliding condition inside the actuator. The
regime framework of \Cref{sec:shift_regime_dynamics} provides a way to organise
such changes, but the present set of regimes would need to be extended to
include them. Ordinary bearing, seal, and windage losses are a simpler related
extension of \AssRef{assump:no_parasitic_losses}. These can be attached to the
bodies and relative motions that produce them without introducing a global
efficiency multiplier. Their allocation must agree with the shaft boundaries
so that a loss included in an external machine model is not counted again
inside the CVT.
...
normal loading, but one traction utilisation \(\lambda_j\) and one overall
contact state describe each wrap
(\Cref{eq:lambda_local_definition} and
\AssRef{assump:wrap_contact_resolution}). They therefore cannot distinguish a
part of the belt that adheres from another part that slides, or a region where
the local friction direction reverses. Resolving those distinctions would
change the forces returned to the sheaves as well as the description of slip.

The mechanisms identified by \citet{Gerbert1996} give a physical reason to make
this change. Longitudinal extension changes the motion of the tension-carrying
layer. Radial compression changes its running radius. Shear and warping allow
the rubber between that layer and the sheave to deform, while the rapid change
of curvature at entry and exit changes the motion of the contacting surface.
These are distinct contributions to belt speed loss, particularly when the
belt thickness is appreciable relative to the pulley radius. Importantly, a
difference between cord speed and pulley surface speed need not imply sliding
of the contacting rubber: some relative motion can be accommodated by shear
while the surface adheres. Gerbert also shows how seating and unseating can
produce friction opposing the main torque-transmitting direction at the
primary. A single wrap-level slip speed cannot recover these local motions.

Radial migration introduces a further connection. Friction acting along a
sheave face can have both radial and circumferential components, so resistance
to changing running radius and resistance to tangential slip draw on the same
contact traction. They should not be introduced as two independent forces,
each with the full available friction capacity. The distributed groove
mechanics of \citet{KongParker2006} and the shifting analysis of
\citet{Sorge2008} provide foundations for treating the local sliding direction
and its consequences for normal loading and axial thrust. Such a description
would replace the circumferential-only reduction in
\AssRef{assump:gross_coulomb_traction} with a local relation between surface
motion and traction.

The corresponding loss calculation would need to follow the same mechanics.
\citet{Bertini2014} distinguish sliding, material hysteresis, and losses
associated with belt entry and exit, using a reduced contact description and
experimentally identified parameters. Their entry and exit losses concern
material passing through pulley contact during circulation, rather than the
initial clutching event. A transient extension could determine how these
losses change during a manoeuvre and whether the same local mechanisms also
change clamp demand or shift timing. Local contact work would then account
for resolved sliding; adding a separate empirical loss for the same motion
would count it twice.

These improvements need not all be introduced at once. A pressure- or
speed-dependent traction law could be tested within the present reduced
contact description, although it would not create local adhesion regions or
separate cord and surface motion. Conversely, spatially varying contact does
not by itself require a spatially varying friction coefficient, and local
material-speed variation is not exclusively an elasticity effect. The
inextensible continuum kinematics developed by
\citet{CarboneMangialardiMantriota2005} already couple local transport to
changing geometry. Longitudinal creep requires extension, and shear-mediated
surface motion requires a corresponding section model. Each intermediate
extension should therefore be judged by the particular motion it recovers.

\subsection{Face Torque and Dynamic Helix Response}
\label{sec:future_work_face_torque}

The two secondary faces provide a particularly direct connection between
local contact and the complete transient response. The movable face rotates
relative to the shaft as it shifts, so the belt generally encounters a
different surface velocity on each face. The current formulation retains
that relative rotation in the actuator and shaft dynamics. It also uses the
power-equivalent representative contact speed in
...
parts of the wrap. In some of their calculated conditions, even the
integrated torque on the faster half-pulley is negative. Their analysis
retains the contact consequences of relative face motion while neglecting
inertia other than centrifugal loading. The present formulation retains the
movable assembly's inertial demand but reduces the face contacts. The two
descriptions therefore address different consequences of the same motion.

The point at which they meet is already explicit in
...
\Cref{eq:secondary_helix_closing_force}. The changed axial acceleration alters
the relative face motion, which changes the contact conditions that supplied
the torque. The shaft balance must respond at the same time. Face torque and
helix inertia cannot therefore be assessed fully as independent corrections
to an unchanged trajectory.

The comparisons in \Cref{sec:results_secondary_helix} make this interaction
worth investigating during both smooth shifting and rapid disturbances. They
show that an inertial contribution to helix reaction and a change in the
resulting trajectory are different measures of importance, especially when a
manoeuvre approaches a contact or support transition. A controlled comparison
could vary the face-contact description and the retained helix dynamics
separately and together, while preserving the physical hardware inventory
and shaft loading. Comparing ordinary upshift with rapid backshift and torque
reversal would determine when the combined response differs from what either
change suggests on its own. The separate face torques, helix reaction,
sheave motion, and traction-state changes would be more informative here
than total transmitted torque alone.

The contact power must change consistently with the torque description.
The reduction in \Cref{eq:secondary_face_tangential_power} relies on equal
torque shares; a face-resolved model should instead evaluate the power of
each actual torque at its own face speed. Signed face torques and
powers remain meaningful even when a percentage torque split becomes
misleading near zero total torque. The objective is not to establish unequal
sharing anew, but to determine how its known contact mechanics interact with
the secondary's inertia, and where the present aggregate reduction remains
adequate.

\subsection{Belt Deformation and Material History}
\label{sec:future_work_belt_material}

Following the local motions described above requires reconsidering how the
belt carries force between the pulleys. With the fixed length of
\AssRef{assump:constant_belt_length}, the geometric compatibility relation in
\Cref{eq:belt_length_compatibility} determines the secondary position from the
shared shift coordinate. An extensible belt could accommodate part of a
sheave displacement through elongation instead. Tension would then depend on
deformation, and the present position map in
\Cref{eq:secondary_coordinate_map} would no longer supply the complete
coupling. Independent sheave coordinates and distributed belt deformation
may be needed, with both pulleys still responding to their own force balances.

The single transport coordinate in \AssRef{assump:single_belt_speed} is a
related but separate reduction. A local velocity field would allow tension
changes and deformation to propagate around the loop rather than being
represented only through one global transport acceleration. Its continuity
and momentum equations would have to agree with the evolving belt geometry.
The resulting extension could investigate whether deformation changes the
onset of shifting, the development of transient slip, or the way a sudden
load is transmitted between the pulleys. It would also provide a controlled
way to examine the different clamp-to-shift responses identified in
\Cref{sec:results_ballew_interpretation}.

The belt section offers several levels of refinement. The present cord line
has a prescribed location, but the cord and rubber do not have separately
resolved mass distributions under
\AssRef{assump:fixed-belt-cross-section}. A more representative section could
first improve the centre of mass and inertial description without introducing
all local deformations. Allowing compression, bulging, and shear would then
change the belt's width, contact surface, and relationship between sheave
position and running radius. Bending stiffness would affect the path at entry
and exit, while the section's reinforcement and geometry would determine how
these deformations transmit forces and moments along the belt. The material
and component tests described by \citet{Ballew2015} and \citet{Messick2018}
provide useful foundations for identifying these properties separately.

Retaining a section effect need not mean resolving every cord or cog throughout
the complete belt. \citet{Gerbert1996} uses detailed sectional analysis to
construct an effective shear relation. A similar approach could retain the
section's influence through resultant forces, moments, deformed contact
geometry, and material history. The required questions are whether a
deformation matters, whether its dissipation matters, and whether its own
inertia must be resolved on the timescale of interest. These need not have
the same answer. A section may deform enough to change contact while its
fast internal motion can still be reduced, provided that reduction preserves
the response seen by the rest of the CVT.

Elasticity alone does not supply material loss. It stores and returns energy;
viscoelasticity introduces a dependence on deformation history and can
dissipate energy during repeated stretching, compression, shear, and bending.
Messick measures temperature- and frequency-dependent rubber properties but
uses an elastic approximation in the mechanical model, while Ballew
identifies independent damping measurements as unfinished work
\citep{Messick2018,Ballew2015}. A transient constitutive description should
therefore be supported by cyclic material or component tests rather than by
an unexplained damping coefficient chosen to smooth a CVT trace. Because the
belt circulates, the material history must follow each section through the
spans and wraps. The associated energy balance must distinguish recoverable
strain energy from dissipation, extending the accounting in
\Cref{eq:verification_energy_balance}. During a transient, a reduction in
output power can reflect energy being stored rather than energy lost as heat.

Deformation and inertia would also allow belt vibration to be studied.
\citet{Messick2018} identifies crank-angle-dependent engine torque as a
possible excitation that a steady model cannot predict. A belt model driven
by an appropriately resolved shaft torque could determine whether such
excitation changes contact loading and traction or remains a small
oscillation around the slower shift response. This is another reason to
retain a general transient description when using detailed belt relations
from the literature. Messick's constant-mass-flow and steady-path assumptions,
for example, cannot simply be carried into this problem. Likewise, Ballew's
axial-force search determines sheave geometry by matching belt reaction to
applied clamp rather than advancing movable-sheave axial inertia
\citep{Messick2018,Ballew2015}. Their local mechanics are valuable foundations,
but the extended system must still determine how the belt and hardware
accelerate together.
...
the belt actually follows through the machine. The circular wraps and straight
spans of \AssRef{assump:prescribed_belt_path} make the current geometry
well-defined, but they remove possible motions before the force balances are
solved. The checks in \Cref{eq:wrap_contact_admissibility} can identify a
negative tension or local normal load. They cannot determine the slack or
partially detached configuration that would replace the assumed path. A
model that determines belt position and contact extent from its mechanics
could continue through such conditions instead of only identifying the limit
...
fully closed secondary through
\Cref{eq:deadzone_secondary_lock}. A belt with its own deformation and contact
motion could instead determine installed pretension, slack take-up, support
at the primary hub, intermittent sheave contact, and possible secondary slip.
Initial engagement would then depend on the belt configuration and motion
actually reached, rather than only on the reduced disengaged constraint.
These are changes to the physical description of engagement, not simply
refinements of its numerical event time.

There is substantial precedent for solving belt paths and contact boundaries
\citep{KongParker2005,KongParker2006,JulioPlante2011}. The remaining task is to
combine that freedom with the mechanically determined actuator response and
the operating conditions considered here. The difficulty reported by
\citet{JulioPlante2011} in obtaining stabilised predictions near the
clutching-to-shifting transition also makes low-load engagement an important
test, rather than a condition to bypass by starting every simulation from a
convenient running state. A useful development route would establish the
belt's response under simple end loads and isolated contact before assembling
the complete CVT. Its force and deformation description should remain
meaningful when pulley contact is removed.

The required geometric freedom depends on the question. Planar motion may be
enough for some studies of tension redistribution and slackness. Axial belt
migration, skew, and section twist require out-of-plane motion and a contact
description that follows section orientation. Where the hardware permits the
whole secondary to float axially, that motion must also be distinguished
from opening or closing its groove. Pulley compliance and departures from
fixed, aligned shafts are separate possible extensions of \AssRef{assump:rigid_hardware} and
\AssRef{assump:axis_alignment}. The chain-CVT studies of
\citet{CarboneMangialardiMantriota2005} and \citet{DuanEtAl2016} demonstrate
that hardware deformation can alter contact, but do not establish its
importance relative to rubber-belt deformation in the present system.
Structural estimates and measured alignment should determine whether rigid,
aligned pulleys remain a sufficient approximation for the intended loads.

\subsection{Contact Loss and Finite-Speed Events}
\label{sec:future_work_events}

The actuator contacts can change as well as the belt contacts. The flyweight
construction already requires a compressive ramp reaction in
\Cref{eq:appB_contact_force_admissible}. If continued contact would require
the ramp to pull, the flyweight must be allowed to separate. Its subsequent
motion cannot remain fixed by the same sheave-position map. Predicting
lift-off and return would require independent motion while contact is absent,
followed by a mechanically consistent recontact calculation. Simply clipping
a negative reaction to zero would not supply that missing motion.

The secondary has a related distinction. The reference mechanism uses the
zero-clearance bilateral helix described in
\Cref{app:reference_helix_topology}; a change in signed reaction can be carried
by the opposite slot flank. A one-sided helix could instead lose contact,
while a slot with finite clearance could permit free relative motion before
the opposite flank is reached. The reaction changes examined in
\Cref{sec:results_secondary_hybrid_response} motivate investigating these
alternatives during reversal. They should remain distinct from belt lift-off:
the sign of a helix reaction does not by itself establish loss of normal
loading at the belt.

When a new contact is reached at finite relative speed, the present
impulse--momentum balance provides a reduced description of the resulting
velocity change
(\Cref{eq:finite_event_generalized_impulse_momentum}). Two separate
extensions would make it possible to examine the limits of that description.
...
could determine whether that stick can actually be maintained or whether the
event produces tangential slip. The outgoing constraints in
\Cref{eq:finite_event_outgoing_constraints} would then depend on the admissible
contact impulses as well as the proposed mechanical arrangement. Simultaneous
contacts would have to be checked together, followed by the existing test of
whether the outgoing continuous regime can persist. This builds on the
frictional-impact framework discussed by \citet{Stewart2000RigidImpact}.

The second extension concerns the duration of contact. The instantaneous,
perfectly inelastic treatment in
\AssRef{assump:inelastic_constraint_activation} determines an outgoing motion
and an energy change, but cannot resolve indentation, peak force, rebound,
or the vibration generated during capture. Selected compliant contacts could
be advanced through time to investigate those quantities. Such a model would
need appropriate contact stiffness and dissipation, together with the belt
or hardware deformation that participates in the event. The event-energy
loss in \Cref{eq:finite_event_energy_loss} does not by itself determine how
that energy is distributed among those mechanisms.

These are different investigations. Friction-limited impulses can be studied
without first replacing every event by compliant contact, and adding normal
compliance alone does not settle the tangential friction question. Comparing
both descriptions against measured incoming and outgoing motion would help
identify where the reduced event law is sufficient for the overall shift
trajectory and where contact duration or load peaks require more detail.
The aim is to extend the range of physical behaviour that can be followed
without losing the compact cross-regime description where it remains useful.

\subsection{Temperature During Transient Operation}
\label{sec:future_work_temperature}

The contact and material losses discussed above also raise a question about
their effect on temperature. Under
\AssRef{assump:material_surface_state}, the assigned properties remain fixed
throughout a run. Yet the rubber-property measurements of
\citet{Messick2018} show that stiffness and dissipation depend on temperature
and loading frequency. A first investigation could therefore compare matched
manoeuvres at different initial belt and sheave temperatures, using
independently characterised properties. This would establish thermal
sensitivity before deciding whether temperature must also evolve during each
simulation.

The temperature of the belt as a whole is only part of that question. Local
contact can heat over much shorter distances and times than the bulk material.
\citet{Persson2006FlashTemperature} describes how this flash heating feeds
back into rubber friction through the temperature-dependent material
response. The physical mechanism is relevant to a transient contact question,
but its importance for a belt--sheave interface must be established from the
actual compound, surface, pressure, and sliding history. A nearly unchanged
bulk temperature during a short manoeuvre would not, by itself, demonstrate
that local flash heating is negligible. Nor does the existence of flash
heating in other rubber contacts establish that it controls CVT traction.

The useful distinction is between changes in the background temperature
carried by a belt section and rapid heating local to its contact. For the
former, measured heating and cooling times and surface-to-interior gradients
would help determine whether a fixed temperature, a lumped thermal state, or
a spatial description is needed. For the latter, clutching, brief slip, and
traction recovery provide cases in which to investigate whether recent
sliding changes the force available during the event. The relevant sliding
velocity is the local relative motion at each face, not simply the belt's
transport speed. A local thermal approximation could be coupled to the
resolved contact without requiring the complete belt mesh to resolve every
roughness scale.

Heat generation and redistribution would have to remain consistent with the
mechanical work. Sliding and viscoelastic dissipation provide heat sources,
but the fraction entering the belt or sheaves, its subsequent conduction,
and cooling are additional physical questions. The energy balance in
\Cref{eq:verification_energy_balance} supplies an accounting foundation,
not a contact-temperature law. In particular, an unresolved instantaneous
event loss cannot be converted into a predicted flash-temperature peak
without a duration and spatial distribution for its dissipation. Temperature
measurements would also need to distinguish the exposed surface from the
hidden contact and belt interior; a surface image does not directly measure
either internal temperature or an unresolved contact-temperature spike.
These studies could establish where thermal feedback changes the transient
mechanics and where the time-invariant property assumption remains adequate.

\subsection{Experimental Validation and Identification of Physical Causes}
\label{sec:future_work_validation}

The present model can be tested before all of these extensions are added.
Because its shaft speeds and shift motion respond to applied loading, an
acceleration experiment supplies a useful transient without requiring a
prescribed ratio history. A characterised inertia connected to the secondary
would provide a simple first load: its acceleration constrains the torque
received from the CVT once the rig losses are accounted for. With a measured
or independently characterised primary torque boundary, the model could
predict the resulting shaft and sheave histories from the initial condition.
The inertia and losses assigned to the rig must follow the same interface
definition as the model
(\Cref{sec:system_definition,eq:shaft_boundary_interface}); secondary hardware
already retained inside the CVT must not be counted again in the external
inertia.

Such a test could examine initial engagement, low-ratio acceleration, and
active upshift. It would not establish accuracy in every regime. Controlled
load changes and unloading would be needed to examine backshift, release,
and repeated transitions, while reverse-power behaviour requires a boundary
that can actually produce it. The value of the general formulation is that
these experiments can be expressed through the same mechanical interfaces,
not that one experiment validates them all. Existing rubber-belt loss tests
and loaded chain-CVT shift tests provide useful precedents for varying speed,
load, and initial operating condition
\citep{ChenLeeSung1998,Bertini2014,YildizEtAl2018}, with the different
actuation and boundary conditions kept explicit.

The observations should include more than shaft speeds. In
\Cref{sec:results_ballew_closed_loop}, feedback can approach the reference
shaft histories while producing a substantially different primary clamp.
Replaying the reference force in
\Cref{sec:results_ballew_force_replay} removes that adjustment and exposes a
different response. Although this is a model comparison rather than an
experiment, it gives a concrete reason to measure sheave displacement and,
where practical, belt speed, torque, or selected actuator loads. Otherwise,
a correct-looking speed ratio can conceal different geometric shifting and
slip. A force supplied as an input must also remain distinct from a force
predicted and independently measured as an output.

Supported operation deserves its own measurements. Once a sheave reaches a
stop, its belt loading need not equal the actuator force because the support
carries the remainder of the axial balance.
\citet{GrzegozekDobajKot2016} identify this limitation in their comparison of
rubber-belt models: regulator characteristics alone do not determine the belt
load at a travel limit without the additional reaction. The reactions in
\Cref{tab:structural_stop_reactions} therefore offer a specific validation
question, not merely another part of the operating sequence. Likewise,
measurements constraining movable-secondary torque or helix reaction would
help test face sharing more directly than total shaft torque alone.

Independent characterisation is equally important. Geometry, spring loading,
masses, inertias, and parasitic losses can be measured or bounded before a
whole-CVT comparison. Belt extensions would require the longitudinal,
compressive, shear, and bending responses relevant to the selected model,
including cyclic behaviour where material history is retained. The
measurement approaches in \citet{Ballew2015} and \citet{Messick2018} provide
starting points. Contact tests should similarly cover the pressures,
relative velocities, temperatures, and surface conditions actually being
studied. Identifying constitutive parameters from such tests is different
from adjusting unmeasured friction or damping until a complete CVT trace
agrees. Parameters identified independently can then be tested against
other manoeuvres or hardware settings without being reselected for each
comparison.

This separation would also make the benchmark discrepancy more informative.
Matched boundary histories and, as far as the descriptions permit, matched
initial tension and deformation states would allow elasticity, dissipation,
local contact, and path freedom to be changed deliberately. The case compared
in \Cref{sec:results_ballew_interpretation} is Ballew's case without the added
linear inter-node damper, so a smoother response cannot be assigned to that
damper. Controlled model variants and measurements are needed to distinguish
the remaining explanations. Sensor calibration, timing alignment, and input
uncertainty should accompany the comparison, particularly when acceleration
is inferred from measured speed or when the conclusion depends on the order
of closely spaced contact events. Agreement should be assessed in the
motion, loads, event times, and accumulated energy that distinguish the
proposed physical causes.

\subsection{Verified Reductions and Design Studies}
\label{sec:future_work_reductions_design}

More detailed mechanics would introduce new numerical questions as well as
new physical ones. A discretised belt can oscillate because of
its elasticity and inertia, but also because of inadequate spatial or time
resolution. The stiffness and stability difficulties described by
\citet{Ballew2015} make that distinction important. Verification should
therefore extend the state, constraint, and energy checks of
\Cref{sec:results_mechanical_invariants,sec:results_energy_consistency,sec:results_solver_convergence}
to the new deformation and contact variables. Spatial refinement, time-step
and tolerance refinement, and tests of any contact regularisation should
establish whether the features used for physical interpretation persist.
Numerical damping introduced for robustness must be distinguished from
measured material dissipation.

The same work can identify where a simpler model is sufficient. The belt
comparisons in \Cref{sec:results_reduced_belt_mechanics} distinguish the size
of a term evaluated on a full trajectory from the error produced by removing
it and integrating again. This distinction should be retained when evaluating
new reductions. A local belt-inertia term cannot be removed independently of
the transport and tension balances in which the same mass participates;
likewise, eliminating a fast sectional motion need not eliminate its
compliance or hysteresis. Limiting cases and comparisons under common loading
would establish which reduced descriptions preserve the motions, contact
transitions, and energy transfers of interest. Such evidence would give a
physical basis for choosing model complexity rather than treating every
added state as an improvement.

Once the relevant predictions have been tested, the formulation also provides
a basis for design studies across complete operating histories. Changes to a
ramp, flyweight mass, spring, or helix can be evaluated through the forces and
inertias they actually alter, following the component-based design motivation
of \citet{Cammalleri2005} and \citet{Skinner2020}. A useful design comparison
would examine engagement, acceleration, and recovery after changing load,
rather than only the engine-speed target reached during one smooth shift.
Including measured parameter uncertainty would determine whether a proposed
improvement persists across plausible hardware variation. The same studies
could reveal when lower contact resistance, altered inertia, or a different
force characteristic improves one manoeuvre at the expense of another.

Together, these directions would extend both the behaviour the model can
represent and the evidence available to interpret it. The aim remains the
one established in \Cref{sec:problem-motivation}: to follow an observed
response back through the belt, contacts, and actuators to its mechanical
cause. A more detailed description is valuable when it changes that
explanation or makes a previously inaccessible behaviour predictable. A
verified reduction is valuable when it preserves the explanation with less
complexity. Independent measurements would then determine which description
is needed for the response being studied.


% Role names follow the NISO Contributor Role Taxonomy (CRediT):
% https://credit.niso.org/contributor-roles-defined/
% The roles describe the present research, not the earlier capstone project.
