...
The preceding sections established the axial and rotational equations of motion
for the two pulley assemblies. Both stopped at the belt. The axial balances
still contain the belt normal loads \(N_p\) and \(N_s\), while the rotational
balances still contain the belt torques \(\tau_p\) and \(\tau_s\). The belt
transport state \(v_b\) is also retained, but its acceleration has not yet been
determined.

These unresolved quantities all belong to the same remaining mechanical body.
So far, the belt has appeared only through the actions that it exerts on the
pulley assemblies. Newton's third law allows the same interactions to be viewed
from the opposite side: whatever force the belt applies to a pulley, the pulley
applies back to the belt. Rather than trying to determine the four belt actions
from the pulley free bodies that require them, we can therefore isolate the belt
and ask what forces and motions are mechanically compatible with the belt
itself.

The task of this section is to build that belt description. The belt is one
closed moving body connecting the two pulley assemblies, so its internal force
and motion cannot be established one pulley at a time without also satisfying
the rest of the loop. We therefore begin with the complete belt, identify how
its different portions must be related, and then work inward to the local
mechanics needed to connect them.

\subsection{Following Tension Around the Closed Belt}
\label{sec:following_belt_tension}

The belt already has a prescribed geometric path. At any engaged configuration,
it wraps around the primary pulley, crosses a straight span to the secondary,
wraps around the secondary, and returns along the other straight span. What has
not yet been determined is how force is carried through that path.

For the perfectly flexible belt of \AssRef{assump:prescribed_belt_path}, the
internal force carried along the belt is its longitudinal tension. Let \(T\)
denote the magnitude of that tension at a point on the belt. If the tension were
the same everywhere, one number would describe the entire loop. In general,
however, there is no reason to assume that the force carried by one part of the
belt must equal the force carried by another. The useful question is therefore
not simply ``what is the belt tension?'', but rather:

\begin{quote}
    As we follow the belt once around the closed loop, how can its tension
    change from one location to the next?
\end{quote}

The geometry suggests how to organize that question. The belt path contains
four distinct portions: two straight spans and two wrapped portions on the
pulleys. The tangent points mark the boundaries between them, as shown in
\Cref{fig:belt_loop_tensions}. We can therefore record the tension at those
four boundaries and ask what relation each intervening portion imposes between
the tension entering it and the tension leaving it.
...
\begin{figure}[H]
    ...
    \fbox{%
        \parbox{0.90\linewidth}{%
            \centering
            Draft figure placeholder: complete CVT belt loop showing the
            primary and secondary wraps, upper and lower straight spans,
            positive belt-travel direction, wrap angles, and the four
            boundary tensions \(T_{p,u}\), \(T_{s,u}\), \(T_{s,l}\),
            and \(T_{p,l}\).
        }%
    }
...
Begin at the secondary exit and follow the positive belt-transport direction
defined in \Cref{sec:rotational_sign}. The belt enters the upper span with
tension \(T_{s,u}\) and reaches the primary with tension \(T_{p,u}\). It then
passes through the primary wrap and leaves into the lower span with tension
\(T_{p,l}\). The lower span carries the belt back to the secondary with tension
\(T_{s,l}\), and the secondary wrap returns to the starting point at
\(T_{s,u}\).

These are four samples of one continuous belt tension, not four separate
states. If we can determine how the tension changes through each of the four
portions of the path, those relations can later be joined around the loop. The
straight span is the natural place to begin because its mechanics are the
simpler of the two cases.

Consider one of the free spans in isolation. Between its two tangent points,
the belt does not touch either pulley. Along the span direction, the rest of
the belt therefore acts on this isolated portion only through the tensions at
its two ends. The two tensions point in opposite directions. If their
magnitudes are equal, their tangential effects cancel. If their magnitudes are
different, the isolated belt portion experiences a net tangential force.

What should such a net force do? The span contains belt mass moving with the
transport speed \(v_b\). A difference between its endpoint tensions must
therefore appear as a change in the tangential momentum of that belt mass. This
is why the span is treated as a Newton's-second-law free body: the quantity we
want---the change in tension from one end to the other---is precisely the net
force acting on the belt portion between those ends.

First imagine that the two tangent points are fixed in space. For a span of
length \(l_i\), the belt mass between them is \(\rho_b A_b l_i\). If the
transport speed is constant, this mass has no tangential acceleration, so no
difference between the two endpoint tensions is required. If the transport
speed is increasing, the belt mass must instead be accelerated in the direction
of travel. For fixed endpoints, Newton's second law would then give the simple
result
\[
...
The actual CVT requires one extra check before this result can be used. During a
shift, the pulley radii change and the tangent points move. The instantaneous
span can therefore grow or shrink, so the belt material lying between its two
endpoints at one instant is not exactly the same material lying between them at
the next. We must determine whether this moving boundary changes the tangential
momentum balance.
...
common belt transport speed \(v_b\). With fixed cross-sectional area \(A_b\)
and density \(\rho_b\), its mass per unit length is \(\rho_b A_b\).

For the belt occupying this moving span, Newton's second law in tangential
momentum form is
\begin{equation}
    \sum F_{i,t}
    =
    \frac{dP^{\mathrm{sys}}_{i,t}}{dt}.
    \label{eq:span_momentum_balance}
\end{equation}

The force side is already known from the free body. The downstream belt pulls
in the positive direction with \(T_{i,\mathrm{out}}\), while the upstream belt
pulls in the opposite direction with \(T_{i,\mathrm{in}}\). Hence
\[
    \sum F_{i,t}
    =
    T_{i,\mathrm{out}}-T_{i,\mathrm{in}}.
...
The momentum side must account for the moving tangent points. Let
\(v^{\mathrm{in}}_{\mathrm{st},i}\) and
\(v^{\mathrm{out}}_{\mathrm{st},i}\) be the velocities of the two tangent
stations projected along the instantaneous span. The momentum balance through
the region between those moving stations is then
\begin{equation}
\begin{aligned}
    T_{i,\mathrm{out}}-T_{i,\mathrm{in}}
    &=
    ...
    \rho_b A_b v_b
    \left[
        \left(v_b-v^{\mathrm{out}}_{\mathrm{st},i}\right)
        -
        \left(v_b-v^{\mathrm{in}}_{\mathrm{st},i}\right)
    \right].
\end{aligned}
\label{eq:span_moving_boundary_balance}
\end{equation}

The first term is the rate of change of the tangential momentum presently
contained between the two tangent points. The second accounts for belt material
crossing those moving boundaries. The same endpoint motion changes the span
length according to
\begin{equation}
    \dot l_i
    =
    v^{\mathrm{out}}_{\mathrm{st},i}
    -
    v^{\mathrm{in}}_{\mathrm{st},i}.
    \label{eq:span_length_rate}
\end{equation}

This relation makes the effect of the moving boundaries transparent. Expanding
the stored-momentum term introduces
\(\rho_b A_b \dot l_i v_b\), because the amount of belt contained in the span
is changing. The boundary flux contributes exactly the opposite quantity,
\(-\rho_b A_b \dot l_i v_b\). The changing span length therefore changes
which belt material lies between the tangent points, but the associated
momentum transport accounts for that change exactly. No additional tangential
force remains.

The span balance consequently reduces to
\begin{equation}
    T_{i,\mathrm{out}}-T_{i,\mathrm{in}}
    =
    \rho_b A_b l_i \dot v_b.
    \label{eq:span_tension_balance}
\end{equation}

The fixed-endpoint intuition was therefore correct even though the actual
tangent points move. For a free span, the difference between the tension at its
two ends is exactly the net force required to accelerate the belt mass
currently occupying that span. If \(\dot v_b=0\), the two endpoint tensions
are equal. The tangent points may still move as the CVT shifts, but their motion
does not create an additional tangential term in this reduced belt model.

This balance concerns only motion along the instantaneous span. The span itself
also changes orientation as the pulley geometry shifts. Under
\AssRef{assump:prescribed_belt_path}, that transverse reorientation is imposed
by the instantaneous belt geometry rather than retained as an independent belt
dynamic degree of freedom.

Applying \Cref{eq:span_tension_balance} to the two spans in the travel
directions shown in \Cref{fig:belt_loop_tensions} gives
\begin{equation}
\begin{aligned}
    T_{p,u}-T_{s,u}
    &=
...
\end{aligned}
\label{eq:span_tension_balances}
\end{equation}

We now know how tension changes through both free portions of the belt. The
wrapped portions remain. Unlike a free span, a wrapped belt segment is in
contact with a pulley throughout its path and continually changes direction as
it follows the circular arc. Its entry and exit tensions therefore cannot be
related from the span balance alone. To determine what happens between those
two boundary values, the next step is to isolate a small element of belt inside
a pulley wrap and establish its local equations of motion.
