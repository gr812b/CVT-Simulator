...
% Populate these three identifiers when the Results archive is frozen.
\newcommand{\ResultsTag}{\texttt{TBD-after-results-freeze}}
\newcommand{\ResultsCommit}{\texttt{TBD-after-results-freeze}}
\newcommand{\ResultsDOI}{\texttt{TBD-after-results-freeze}}
...
The formulation developed in \Cref{chap:derivation} is implemented in the
open-source CINDER simulator.  All Results studies in this manuscript use
CINDER release \CINDERtag, pinned to commit \CINDERcommit.  The tagged source
is available from
\url{https://github.com/gr812b/CVT-Simulator/tree/cinder-v1.1.2}.
Implementation details that are required to reproduce the numerical realization
of the analytical equations
are documented separately in \Cref{app:numerical_implementation}.  In
particular, that appendix records the exact belt-geometry root solves, the
fixed-$\lambda$ algebraic closure, affine actuator assembly, the nonlinear
static-traction solve, and the hybrid time-integration procedure.

The Results are organized around two additional layers of reproducibility data.
First, parameters that describe the common physical CVT rather than a particular
experiment are collected in \Cref{app:reference_configuration}.  The master
physical-configuration table, \Cref{tab:results_common_parameters}, records each
quantity together with its value and provenance.  This keeps dimensions,
material properties, inertias, friction parameters, and actuator definitions out
of the individual Results sections unless a study deliberately changes one of
them.  A study-specific deviation therefore has a precise meaning: it is an
override of the reference configuration rather than an independent, partially
specified model.

Second, execution settings and study definitions are indexed in
\Cref{app:study_reproducibility}.  Common solver settings are recorded in
\Cref{tab:results_common_numerics}, while the study and case registry in
\Cref{tab:results_case_registry} identifies the scientific purpose, host or
boundary condition, base configuration, and explicit overrides for every
paper-facing case. Several verification
studies contain many operating points or reference cases.

\subsection{Versioned software and archival boundary}
\label{sec:results_versioned_software}

The simulator release and the Results archive are treated as separate
reproducibility objects.  The simulator tag fixes the implementation of the
mechanical model and numerical algorithms.  The Results archive fixes the
study definitions, configuration files, analysis scripts, and the generated
paper-facing artifacts.  At final Results freeze, the latter will be identified
by the following versioned records:

\begin{itemize}
    \item Results release/tag: \ResultsTag,
    \item Results commit: \ResultsCommit,
    \item archival DOI: \ResultsDOI.
\end{itemize}

The placeholders above are deliberately centralized in the preamble so that the
final archival identifiers can be inserted without editing the scientific text.
The Git repository remains useful for inspection and development history, but
the frozen commit and archival DOI will define the canonical source for
reproducing the published Results.

\subsection{Configuration hierarchy}
\label{sec:results_configuration_hierarchy}

Each Results study is defined relative to a common physical configuration and,
where applicable, a common numerical configuration.  The reference physical
model is given in \Cref{tab:results_common_parameters}; common integration,
contact-solver, event, and reporting settings are given in
\Cref{tab:results_common_numerics}.  A Results subsection therefore needs to
state only the quantities that are scientifically varied or otherwise differ
from those references.  Full values are retained in the appendices and archived
configuration files so that the main text can focus on the question being
tested rather than repeatedly restating the complete simulator input set.

The same convention applies to external hosts.  Engine, vehicle, prescribed
actuator, flywheel, or dynamometer-like systems are not folded into the CVT
parameter table.  They are identified with the study that uses them in
\Cref{tab:results_case_registry}, consistent with the shaft-boundary separation
introduced in \Cref{sec:system_definition}.  Consequently, replacing a host or
actuator for a benchmark does not create a second CVT formulation; it changes a
specified component or boundary around the same mechanical plant.

\subsection{Study artifacts and traceability}
\label{sec:results_artifact_traceability}

Every manuscript figure or table generated from simulation will be traceable to
a frozen study definition and archived source artifact.  The final mapping is
recorded in \Cref{tab:results_artifact_index}.  The intended publication path is
therefore

\begin{equation}
    \text{tagged CINDER implementation}
    \;\longrightarrow\;
    \text{frozen study configuration}
    \;\longrightarrow\;
    \text{machine-readable result}
    \;\longrightarrow\;
    \text{paper-facing figure or table}.
    \label{eq:results_reproducibility_chain}
\end{equation}

This separation is important during the exploratory stage of the work because a
study directory may contain intermediate sweeps or diagnostic plots that do not
survive into the paper.  The publication archive will retain the final
configuration and evidence needed to reproduce each reported claim; exploratory
material is not itself part of the manuscript evidence unless it is explicitly
indexed in \Cref{tab:results_artifact_index}.

% ==============================================================
\section{Mathematical Closure and Verification}
\label{sec:results_verification}

% TODO after Results freeze: write claim-driving prose and insert final artifacts.

\subsection{Nonlinear Traction-Closure Structure}
\label{sec:results_closure_structure}
% TODO: conditioning / root structure / local branch discussion.

\subsection{Mechanical Invariants and Operating-Domain Audit}
\label{sec:results_mechanical_invariants}
% TODO: broad invariant/capability suite and coverage summary.

\subsection{Energy Consistency}
\label{sec:results_energy_consistency}
% TODO: work, stored energy, dissipation, and event bookkeeping closure.

\subsection{Solver Convergence}
\label{sec:results_solver_convergence}
% TODO: tolerance, maximum-step, event-sequence, and timing refinement.
...
implementation.  The scalar belt-length roots are evaluated with SciPy's
\texttt{scipy.optimize.brentq} routine \citep{Virtanen2020}.

At construction time, the specified zero-shift radii $r_{p,0}$ and $r_{s,0}$
are used to solve once for the fixed pulley centre distance $C$.  With
$\Delta r_0=r_{s,0}-r_{p,0}$, the lower bound is
...
which prevents overlap of the two outer belt envelopes.  The upper bound
follows from the exact wrap geometry.  Since
...
the two straight spans can use at most
$L_b-\pi(r_{p,0}+r_{s,0})$ of the belt.  Therefore
...
The residual is checked at both endpoints before iteration.  An endpoint is
accepted when the absolute belt-length residual is no greater than
$10^{-12}\,\mathrm{m}$; otherwise a sign change is required and Brent iteration
uses an absolute root-location tolerance of $10^{-12}\,\mathrm{m}$.  These
physical bounds avoid a broad numerical search.

Once $C$ is fixed, construction performs one additional broad solve at
$s=s_{\max}$ to determine the opposite endpoint of the secondary travel.  The
primary radius is explicit,
...
and the secondary endpoint is solved in
...
The lower bound keeps the complete belt cross-section outside the pulley axis;
the two upper bounds prevent envelope overlap and enforce
\Cref{eq:appendix_minimum_wrapped_length}.  The solved maximum-shift radius and
the specified zero-shift radius are then stored as the runtime bracket,
...
Thus every ordinary geometry evaluation solves only one scalar radius root on
a tight, precomputed physical interval.  Remaining radius references are fixed
offsets, while first and second radius derivatives are evaluated analytically
from the differentiated belt-length closure rather than by finite differences.
At $s=s_{\mathrm{dz}}$, separate deadzone-side and engaged-side evaluations
provide the appropriate one-sided derivative while preserving the common
boundary position.
...
mechanics after eliminating the two pulley torques.  The implementation retains
those torques as simultaneous unknowns,
...
and keeps the two traction rows explicit,
...
Eliminating $\tau_p$ and $\tau_s$ recovers the reduced six-variable system, so
no physical degree of freedom is added.  Retaining torque keeps torque-reactive
mechanisms coupled directly to the simultaneous solve and avoids ever needing
to recover $N_j$ by dividing a solved torque by $\lambda_j$.  At
$\lambda_j=0$, the explicit traction row simply requires $\tau_j=0$ while the
remaining balances determine $N_j$.

\subsection{Equilibrated linear solve}
\label{app:num_equilibrated_solve}

The closure mixes accelerations, torques, forces, and normal resultants, so the
raw matrix contains substantially different coefficient scales.  CINDER
therefore equilibrates the algebraically identical system before the dense
solve.  For $\vect{A}\vect{z}=\vect{b}$,
...
matrix.  The scaled system is solved directly and mapped back to physical
unknowns.  Residuals are still evaluated against the unscaled equations.
Diagnostic solves additionally expose rank and raw/equilibrated condition
numbers.  The scaling changes numerical conditioning, not the mechanics.
...
\Cref{eq:wrap_zero_traction_solution}.  Zero signed traction is therefore not a
new mechanical regime: a clamped contact may pass continuously through
$\lambda_j=0$ while retaining $N_j>0$.  The implementation preserves that
continuity and changes only how the limiting coefficients are evaluated.
...
and uses \texttt{expm1}-based expressions outside that neighbourhood.  This
uses the regularity already proved by the formulation rather than introducing
an artificial zero-traction topology change.
...
\section{Affine Assembly of Mechanical Contributions}
\label{app:num_affine_assembly}

The actuator mechanics derived in
\Cref{sec:axial_actuation_balances,sec:pulley_rotational_dynamics} must remain
coupled to the simultaneous mechanical solve rather than being evaluated as
external scalar forces.  Once the current state, geometry, and trial traction
pair are frozen, the retained mechanism equations are affine in the
instantaneous closure unknowns even though their coefficients may depend
nonlinearly on position or speed.  A contribution is therefore represented as
...
For example, if a mounted mechanism uses a local pulley coordinate $x_j(s)$,
...
At a frozen state, the first term is a known bias and the second is a gain on
the global shift-acceleration unknown.  The same interface allows force and
torque contributions to depend on $\dot\omega_j$, $\tau_j$, or $N_j$ without
an inner iteration or a lagged estimate.

The particular primary flyweight, spring, secondary helix, and spring mechanics
derived in \Cref{sec:axial_actuation_balances} are implemented through this
interface; the abstraction does not replace those laws.  It permits additional
mechanically compatible pulley elements while preserving their simultaneous
coupling to the common closure channels.
...
\Cref{sec:sticking_traction_closure} leaves the required static traction
utilizations to be determined nonlinearly.  A trial traction pair is passed to
the fixed-$\lambda$ solve of \Cref{app:num_fixed_lambda}; the resulting
relative-acceleration residuals then close the contacts assumed to stick.  The
same bounded routine is used in one or two dimensions: stick--stick frees both
traction utilizations, a mixed branch frees only the sticking interface, and
both-slip requires no nonlinear solve.

\subsection{Search domain, Jacobian, and acceptance}
\label{app:num_contact_search}

The numerical search interval is deliberately wider than the physical static
friction interval so that an over-capacity static requirement can be found and
then rejected by the physical admissibility test in
\Cref{sec:contact_admissibility_transitions}.  Absent continuation information,
the initial guess is zero.  CINDER uses SciPy's bounded
\texttt{scipy.optimize.least\_squares} routine \citep{Virtanen2020} with the
trust-region reflective method, a three-point finite-difference residual
Jacobian, and Jacobian-based scaling.  The tagged defaults use
\texttt{ftol}=\texttt{xtol}=\texttt{gtol}=$10^{-12}$ and at most 100 residual
evaluations.  A candidate is accepted only after a fresh mechanical evaluation
satisfies the declared sticking residual tolerance.

\subsection{Continuation between consecutive contact solves}
\label{app:num_contact_continuation}

Within one smooth contact topology, the required traction root normally changes
continuously between nearby right-hand-side evaluations.  CINDER therefore
stores the previous accepted root and local residual Jacobian.  One bounded
Newton correction is attempted first; a successful step updates the Jacobian
with a rank-one good-Broyden correction.  If the prediction is unavailable,
reaches a search bound, fails mechanically, or misses the residual tolerance,
the code falls back to the full bounded least-squares solve.  This reduces
repeated nonlinear work and promotes tracking of the locally connected root in
a residual map that need not be globally single-rooted; it does not add physical
friction memory.

\subsection{Numerical hysteresis at reattachment}
\label{app:num_contact_hysteresis}

The physical switching conditions remain those in
\Cref{sec:contact_admissibility_transitions}.  Numerically, reusing exactly the
same static-capacity surface for loss and regain of stick can produce event
chatter.  The implementation therefore leaves stick loss at the physical
zero-margin boundary but normally requires a positive restick reserve of
$10^{-3}$ in traction-utilization margin.  If that reserve would create a
no-successor condition, the transition logic retries the sticking candidate at
the physical static boundary.
...
The software preserves the shaft boundary defined in
\Cref{sec:system_definition}.  Engines, vehicles, dynamometers, controllers, or
other host systems may carry states of their own and are evaluated from the
current simulation state, but only the applied torque and equivalent referred
inertia are returned to the CVT at each shaft.  The same CVT implementation can
therefore be embedded in different surrounding systems without moving those
systems inside the transmission formulation.

Within a fixed regime, the resulting first-order initial-value problem is
advanced through SciPy's \texttt{scipy.integrate.solve\_ivp} interface
\citep{Virtanen2020}.  The generic runner is method-configurable.  The reported
transient study drivers select LSODA, whose automatic selection between stiff
and nonstiff multistep formulas follows the strategy of \citet{Petzold1983}.
Relative and absolute tolerances and maximum-step limits are study settings and
are therefore reported with the Results configuration rather than treated as
mechanical-model constants.

The integration mesh is adaptive: the right-hand side can be evaluated several
times at internal trial states before one step is accepted.  A plotted sampling
interval is therefore not an integration time step.

\subsection{Hybrid segmentation and event localization}
\label{app:num_hybrid_segmentation}

The regime structure in \Cref{sec:shift_regime_dynamics} is implemented as a
sequence of smooth \texttt{solve\_ivp} segments.  Each active regime exposes
only its reachable terminal, direction-aware events.  When one is localized,
the segment ends, the transition resolver selects the successor regime and any
required successor state, and integration restarts at the same physical time.
Events localized to the same endpoint within the configured association
tolerance are resolved together.  Roundoff-scale boundary snapping and
protected evaluation of rejected trial stages are used only to keep numerical
probes inside valid geometry; they do not move the accepted physical event
surface.

\subsection{Finite-speed boundary updates}
\label{app:num_finite_speed_update}

The finite-speed event map derived in
\Cref{sec:finite_speed_shift_boundary_arrival} is implemented directly.  CINDER
assembles the event-specific $\vect{J}^{-}$, $\vect{J}^{+}$, $\vect{W}$, and
$\vect{C}^{+}$ objects and solves the resulting dense linear or KKT system.  The
outgoing velocity is inserted into the successor state.  Constraint and
momentum residuals are checked afterward, and the perfectly inelastic kinetic-
energy change is recorded with the transition.

\subsection{Integration mesh and reported output}
\label{app:num_reporting_grid}

The adaptive native trajectory is preserved separately from the user-facing
reporting grid.  Uniform reports use SciPy dense output separately within each
smooth segment, with exact segment endpoints retained explicitly.  Interpolation
is therefore never performed across a discrete state reset.

\paragraph{Code reference}
The shaft ports are implemented in
\CINDERcode{cvtModel/src/cinder/model/system/ports.py}{ports.py}; segmented
integration is in
\CINDERcode{cvtModel/src/cinder/execution/hybrid/hybrid.py}{hybrid.py};
finite-speed projection is in
\CINDERcode{cvtModel/src/cinder/execution/hybrid/cvt_impact.py}{cvt_impact.py};
and reporting materialization is in
...
\endfoot
\bottomrule
\caption{Correspondence between the analytical formulation and numerical implementation choices.}
\label{tab:appendix_numerical_correspondence}\\
\endlastfoot
Implicit exact belt geometry
& SciPy \texttt{brentq}; construction-time bounds and cached runtime radius bracket
& Preserve the exact geometry while keeping repeated solves tightly bounded. \\[5pt]
Six-variable fixed-$\lambda$ closure
& Eight-variable closure retaining pulley torques and explicit traction rows
& Preserve torque coupling and regular zero-traction algebra. \\[5pt]
$\vect{A}\vect{z}=\vect{b}$
& Row/column equilibration and dense direct solve
& Reduce avoidable mixed-unit scale sensitivity without changing the physical equations. \\[5pt]
Continuous $\lambda_j\to0$ wrap limit
& Regular $\Phi_-$/$\Psi_-$ evaluation using local series and \texttt{expm1}
& Avoid cancellation without introducing a new regime. \\[5pt]
Derived actuator equations
& Affine bias-plus-gain component interface
& Keep dynamic actuator coupling inside the simultaneous solve while allowing compatible elements. \\[5pt]
Static-traction root
& SciPy bounded trust-region \texttt{least\_squares}, local continuation, and residual acceptance
& Solve mixed and stick--stick closure while tracking the nearby branch efficiently. \\[5pt]
Continuous regime equations
& Adaptive SciPy \texttt{solve\_ivp} segments with explicit hybrid transitions
& Keep smooth integration separate from topology changes. \\[5pt]
Finite-speed momentum projection
& Direct event-specific linear/KKT solve with residual and energy audits
& Apply the event relation derived in \Cref{sec:finite_speed_shift_boundary_arrival}. \\
\end{longtable}
\normalsize
\end{center}


\chapter{Reference Configuration and Parameter Provenance}
\label{app:reference_configuration}

This appendix will contain the physical parameters that define the common CVT
used by the Results studies.  It is separated from the study registry so that a
parameter appears once with its provenance even when many studies reuse it.
Values in \Cref{tab:results_common_parameters} are intentionally left as
placeholders until the Results branch and final paper-facing configurations are
frozen.

\section{Common Physical Configuration}
...
\setlength{\LTleft}{0pt}
\setlength{\LTright}{0pt}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.13\textwidth}>{\raggedright\arraybackslash}p{0.15\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}@{}}
\toprule
Group & Quantity & Symbol & Value & Source / provenance \\ \midrule
\endfirsthead
\toprule
Group & Quantity & Symbol & Value & Source / provenance \\ \midrule
\endhead
...
\endfoot
\bottomrule
\caption{Common physical configuration for the Results chapter.  Populate from the frozen Results configuration and retain the provenance of each value.}
\label{tab:results_common_parameters}\\
\endlastfoot
Belt geometry and material & \textit{TBD after Results freeze} & --- & --- & --- \\[5pt]
Pulley geometry & \textit{TBD after Results freeze} & --- & --- & --- \\[5pt]
Primary actuation and inertia & \textit{TBD after Results freeze} & --- & --- & --- \\[5pt]
Secondary actuation and inertia & \textit{TBD after Results freeze} & --- & --- & --- \\[5pt]
Contact and friction & \textit{TBD after Results freeze} & --- & --- & --- \\
\end{longtable}
\normalsize
\end{center}

% TODO after Results freeze:
% - replace group placeholders with one row per frozen physical parameter;
% - identify measured, manufacturer-specified, derived, or assumed provenance;
% - keep host-specific quantities out of this table unless truly common.

\chapter{Study Configurations and Reproducibility Index}
\label{app:study_reproducibility}

This appendix will index the numerical settings, study-specific configurations,
and archived artifacts used by \Cref{chap:results}.  The schemas are frozen now
so that Results cleanup becomes a population task rather than a redesign of the
paper's reproducibility structure.

\section{Common Numerical Configuration}
\label{app:common_numerical_configuration}

\begin{table}[H]
\centering
\small
\caption{Common numerical settings used by Results studies unless a study-specific override is listed in \Cref{tab:results_case_registry}.}
\label{tab:results_common_numerics}
\begin{tabular}{@{}p{0.34\textwidth}p{0.20\textwidth}p{0.36\textwidth}@{}}
\toprule
Setting & Value & Notes \\ \midrule
Integration method & \textit{TBD} & Populate from frozen study configuration \\
Relative tolerance & \textit{TBD} & --- \\
Absolute tolerance & \textit{TBD} & --- \\
Maximum step & \textit{TBD} & --- \\
Reporting interval / grid & \textit{TBD} & Distinct from adaptive integration mesh \\
Static-traction solver tolerances & \textit{TBD} & --- \\
Hybrid event association tolerance & \textit{TBD} & --- \\
\bottomrule
\end{tabular}
\end{table}

\section{Study and Case Registry}
\label{app:study_case_registry}

\begin{center}
\small
\setlength{\LTleft}{0pt}
\setlength{\LTright}{0pt}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}p{0.08\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}p{0.14\textwidth}>{\raggedright\arraybackslash}p{0.14\textwidth}@{}}
\toprule
Study & Case ID & Scientific purpose & Host / boundary & Overrides & Archive path \\ \midrule
\endfirsthead
\toprule
Study & Case ID & Scientific purpose & Host / boundary & Overrides & Archive path \\ \midrule
\endhead
...
\endfoot
\bottomrule
\caption{Registry of paper-facing Results studies and cases.  Case identifiers and paths will be populated after the Results archive is frozen.}
\label{tab:results_case_registry}\\
\endlastfoot
Closure structure & TBD & Required-$\lambda$ root structure and conditioning & TBD & TBD & TBD \\[5pt]
Mechanical invariants & TBD & Operating-domain capability / invariant audit & TBD & TBD & TBD \\[5pt]
Energy consistency & TBD & Work--energy bookkeeping closure & TBD & TBD & TBD \\[5pt]
Solver convergence & TBD & Numerical refinement / hybrid-sequence stability & TBD & TBD & TBD \\[5pt]
Independent benchmark & TBD & Ballew transient model-to-model benchmark & TBD & TBD & TBD \\[5pt]
Primary dynamics & TBD & Effect of retained primary actuator inertia/dynamics & TBD & TBD & TBD \\[5pt]
Secondary helix & TBD & Dynamic helix terms and contact topology & TBD & TBD & TBD \\[5pt]
Reduced belt mechanics & TBD & Contribution of retained transient belt terms & TBD & TBD & TBD \\[5pt]
Launch / upshift & TBD & Coupled forward launch trajectory & TBD & TBD & TBD \\[5pt]
Backshift & TBD & Load-induced coupled backshift & TBD & TBD & TBD \\[5pt]
Engine braking & TBD & Reverse-power coupled operation & TBD & TBD & TBD \\[5pt]
Tune comparison & TBD & Optional compact design consequence study & TBD & TBD & TBD \\
\end{longtable}
\normalsize
\end{center}

\section{Manuscript Artifact Index}
...
\setlength{\LTleft}{0pt}
\setlength{\LTright}{0pt}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.28\textwidth}@{}}
\toprule
Manuscript item & Generating study / case & Archived source artifact & Generating script / notes \\ \midrule
\endfirsthead
\toprule
Manuscript item & Generating study / case & Archived source artifact & Generating script / notes \\ \midrule
\endhead
...
\endfoot
\bottomrule
\caption{Traceability index from paper-facing figures and tables to frozen Results artifacts.}
\label{tab:results_artifact_index}\\
\endlastfoot
\textit{TBD after Results freeze} & --- & --- & --- \\
\end{longtable}
\normalsize
\end{center}

% TODO after Results freeze:
% - expand the case registry to the actual final paper-facing cases;
% - populate exact host/boundary definitions and every study override;
% - populate artifact paths only after exploratory/intermediate outputs are cleaned;
% - include checksums or archive object identifiers if used in the final release.
