...
which contacts and supports are active and which motions they permit. This is
the additional information carried by the mechanical regime introduced in
\Cref{sec:mechanical_regimes_state_evolution}.

The same boundary also makes clear that structural support and tangential
contact state answer different mechanical questions. While the secondary stop
holds the engaged mechanism at \(s=s_{\mathrm{dz}}\), the two shafts and the
belt can continue moving. The stop settles the axial question of whether the
secondary may close farther, but it does not determine whether the belt moves
with either pulley surface or slides relative to it. Each engaged structural
regime must therefore still carry the sticking or sliding state of its two
pulley contacts from \Cref{sec:engaged_dynamic_closure}.

Opening into the deadzone changes more than the axial support condition.
Once the primary separates from the belt, there is no primary belt--sheave
contact left to stick or slide, and the primary belt load disappears with it.
The same shift coordinate also describes a different physical motion:
within the deadzone, changing \(s\) moves only the primary sheave, whereas
after engagement it also moves the secondary and changes both pulley radii.
A structural regime change can therefore either constrain an existing motion
or remove an interaction from the free body entirely. The continuous
governing equations must change with that arrangement.

% ---------------------------------------------------------------------------
\subsection{Governing Equations in Each Structural Regime}
\label{sec:structural_regime_governing_equations}

\Cref{sec:shift_coordinate_boundaries} identified two ways in which the
physical arrangement can change. The primary can separate from the belt,
removing a contact that previously carried force and torque, or a movable
sheave can reach a travel stop, preventing one of the motions allowed in free
shift. The continuous equations can therefore be built by returning to the
free bodies already derived and changing only what has changed physically.
A vanished contact is removed; a newly supported motion gains the reaction
needed to enforce its constraint. The remaining mechanics stay intact.

\subsubsection{Primary Separation and Deadzone Motion}
\label{sec:deadzone_continuous_regimes}

The clearest change occurs when the primary opens into its clearance. Before
separation, the belt acts on the primary through the normal resultant \(N_p\)
and the contact torque \(\tau_p\). Once the sheave faces no longer touch the
belt, neither interaction can remain:
...
Only the belt interaction has disappeared. The primary still rotates, its
flyweights still move on their ramps, the spring still acts on the movable
assembly, and the movable hardware still carries inertia. The rotational
balance in \Cref{eq:primary_rotational_eom_belt} therefore becomes
...
while the primary axial balance in
\Cref{eq:primary_balance_in_shift_coordinate} reduces to
...
These are the same primary mechanics used while engaged, with the belt load
removed. In particular, disengagement does not freeze the actuation mechanism:
a rotating primary can still cross the clearance or be driven back toward its
opening boundary by its own flyweight, spring, and inertial loads.

Removing the primary contact also breaks something on the belt side. The
closed-loop construction in \Cref{sec:closed_belt_reassembly} relied on two
pulley wraps joined by two tension-carrying spans. Once the primary sheaves
separate, the present model does not resolve the resulting slack-belt shape,
pretension, or an independent secondary sliding state. Under
\AssRef{assump:deadzone_belt_closure}, that unresolved motion is replaced by
one explicit reduction: the belt remains seated on the fully closed secondary
and is carried with its surface.

The secondary effective radius is fixed throughout this interval. Define
...
so the adopted deadzone lock is
...
This relation is the reduction declared in
\AssRef{assump:deadzone_belt_closure}; it does not follow merely from setting
\(N_p=\tau_p=0\). It replaces the contact-level belt solution that is no
longer represented once the primary has separated. Consequently, no deadzone
value of \(\lambda_s\), \(N_s\), or detailed tension field is solved there.

Because the secondary radius is fixed, differentiating
\Cref{eq:deadzone_secondary_lock} gives
...
The belt may no longer load the primary, but its mass has not disappeared.
Combining the fixed-configuration secondary rotational balance in
\Cref{eq:secondary_rotational_fixed_configuration_limit} with the
complete-belt transport balance in
\Cref{eq:complete_belt_transport_newton} and the lock above gives
...
The final term is simply the complete belt transport inertia reflected through
the secondary radius. Thus primary separation removes the primary belt load
without making the belt dynamically irrelevant: the secondary must still
accelerate the transport motion of the entire belt.

For free motion through the clearance,
...
If the primary instead remains engaged and no stop acts, no force or permitted
motion has changed. The free engaged arrangement therefore continues to use
exactly the mechanics already closed in \Cref{sec:engaged_dynamic_closure}.

\subsubsection{Stop-Supported Motion}
\label{sec:stop_supported_regimes}

The other structural change leaves the relevant belt contacts in place but
removes an axial motion. The low-ratio seat makes the distinction especially
clear because \(s\) is the primary coordinate even though the supporting
hardware is the fully closed secondary stop. During a sustained seated
interval,
...
The secondary is therefore stationary in its axial and helix-relative motion,
but the secondary shaft, primary shaft, and belt need not be stationary.
The belt can continue transmitting torque while both pulley contacts retain
the tangential states determined by
\Cref{sec:engaged_dynamic_closure}.

Return now to the secondary axial free body in
\Cref{fig:secondary_axial_free_body}. Away from a stop, its three modeled
loads satisfy
\Cref{eq:secondary_movable_sheave_balance}. At the low-ratio seat, the
hardware adds one more force: the stop pushes the movable secondary in the
opening direction whenever the other loads would drive it farther closed.
Let \(F_{\mathrm{stop,LR}}\geq0\) denote the magnitude of that opening
reaction. With the sheave held,
...
This equation has changed purpose, not disappeared. When the secondary is
free, the same net force determines its axial acceleration. Once the stop
prescribes that acceleration to be zero, the balance instead determines the
support force required to make that motion possible.

The reaction cannot be evaluated from the spring and actuator loads alone.
The helix force in \Cref{eq:secondary_helix_closing_force} contains the
secondary angular acceleration, while
\(F_{\mathrm{belt},x_s}=N_s\cos\beta/2\) from
\Cref{eq:secondary_belt_axial_reaction}. Both \(\dot\omega_s\) and \(N_s\)
belong to the coupled engaged response. For a proposed traction pair, the
continuous seated solution is therefore obtained by replacing the secondary
axial row of \Cref{eq:engaged_six_by_six_mechanical_core} with
\(\ddot s=0\). The remaining rows determine the compatible shaft and belt
accelerations and normal resultants, after which
\Cref{eq:low_ratio_stop_reaction} gives the reaction carried by the stop.

This construction makes the other two travel stops immediate. To write them
compactly, define the non-stop axial force on each movable assembly by
...
At the low-ratio seat,
\Cref{eq:low_ratio_stop_reaction} is simply
\(F_{\mathrm{stop,LR}}=F_{s,\mathrm{free}}\) evaluated from the constrained
solution. At the upper primary stop,
...
the primary axial row is replaced instead, and the opening reaction supplied
by the stop is
...
At the fully open primary stop, the primary is already disengaged, so
\Cref{eq:deadzone_primary_disengaged} also applies. The sustained condition is
...
The secondary--belt lock in \Cref{eq:deadzone_secondary_lock} and the
fixed-configuration primary rotational balance in
\Cref{eq:primary_rotational_fixed_configuration_limit} remain active, while
the primary axial balance now determines the support force instead of
\(\ddot s\). Because this stop pushes in the positive, closing direction, its
reaction is
...
The three cases are summarized in
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}
        ...
        & \textbf{Reaction recovered from}
        & \textbf{Stop force direction}
        & \textbf{Required reaction}
        \\
        \midrule
        Engaged low-ratio seat
        & Secondary axial balance
        & Opening secondary
        & \(F_{\mathrm{stop,LR}}=F_{s,\mathrm{free}}\)
        \\[4pt]
        Engaged upper primary stop
        & Primary axial balance
        & Opening primary
        & \(F_{\mathrm{stop,max}}=F_{p,\mathrm{free}}\)
        \\[4pt]
        Fully open primary stop
        & Primary axial balance, \(N_p=0\)
        & Closing primary
        & \(F_{\mathrm{stop},0}=-F_{p,\mathrm{free}}\)
        \\
        \bottomrule
    \end{tabular}
    \caption{Axial reactions required to sustain the three stop-supported
    structural regimes. Each reaction is evaluated after solving the
    corresponding constrained continuous mechanics.}
    \label{tab:structural_stop_reactions}
\end{table}

A physical stop can supply only the pushing reaction shown in the table. Each
supported regime is therefore admissible only while its recovered reaction is
nonnegative:
...
A negative value would require the stop to pull the sheave toward itself, so
that structural constraint cannot be sustained. Notice also that none of
these reactions is a belt normal load. A stop can carry part of the actuator
loading while the belt is still governed by \(N_p\) or \(N_s\); force carried
directly into the hardware does not increase the corresponding tangential
traction capacity.

The five arrangements in
\Cref{tab:shift_structural_arrangements} now all have continuous equations.
Those equations determine the accelerations after a structural regime is
active, together with the reaction required to remain at a stop. They do not,
however, alter a velocity that already exists when the boundary is reached.
If a sheave arrives with finite axial speed, imposing
\(\dot s=0\) and \(\ddot s=0\) states what the supported regime requires but
does not explain how the incoming motion becomes compatible with it. That
remaining change of velocity must be obtained from the momentum carried into
the transition.
