...
To examine that question, the main comparison carries several mechanically
different CVT configurations from the same initial condition through one
continuous road course. The same external problem is applied to every
configuration, so differences between their trajectories develop under a common
engine, vehicle, and road history rather than from different tests. The drive is
treated as one continuous experiment: conditions encountered later in the
course act on the shaft speeds, ratio, contact state, and hardware motion left by
the conditions before them. This provides a common setting in which each new
load can probe a different part of the coupled transmission while any resulting
difference can still be followed back into the mechanics that produced it.

The quantities defining that common external problem are fixed before the CVT
configurations are compared. The vehicle and shaft interfaces use the Baja
reference setup documented in the appendices. The primary shaft is driven by
the full-throttle Kohler CH440 boundary of
\Cref{app:primary_engine_boundary}, while the secondary is coupled to the
locked-final-drive vehicle boundary of
\Cref{app:secondary_vehicle_boundary}; road grade therefore enters the
secondary shaft through the vehicle load in
\Cref{eq:reference_vehicle_torque}. Every trajectory begins from the common
state in \Cref{eq:results_reference_initial_state}. The common baseline for the
CVT itself is the physical reference configuration of
\Cref{app:common_physical_configuration}, with the bilateral secondary helix
topology of \Cref{app:reference_helix_topology}. These definitions provide the
same vehicle, final drive, engine boundary, belt and contact model, reference
pulley geometry, and boundary inertias for the comparison.

The road is prescribed as a function of travelled distance rather than clock
time. Every configuration therefore encounters the same grade at the same
position even when differences in vehicle speed cause it to arrive there at a
different time. \Cref{fig:results_course_overview} defines that common road.
...
\begin{figure}[H]
  ...
  finish. The cyclic region is a longitudinal road-load input only; suspension
  motion, wheel unloading, tire normal-load variation, and airborne vehicle
  dynamics are not represented.}
  \label{fig:results_course_overview}
\end{figure}

The road features are chosen to place different demands on the same
transmission rather than to prescribe a sequence of CVT responses. The opening
flat provides room for engagement and upshift before a large road load is
introduced. The \(38^\circ\) climb then asks the CVT to respond to a severe
increase in load, where the available ratio range and belt traction may both
become important. The following level section allows the transmission to
recover before the six-cycle region applies repeated longitudinal load changes.
The long \(18^\circ\) climb serves a different purpose: its sustained load gives
the initial backshift time to develop toward an interior operating condition,
so the transient response can later be distinguished from the ratio, engine
speed, and vehicle speed maintained on the climb. The descent finally changes
the direction in which the road can exchange power with the drivetrain, giving
the same signed mechanics a chance to continue into reverse-power operation.

With the vehicle, shaft boundaries, initial state, and road fixed in this way,
the selected CVT hardware is the only prescribed difference between runs. The
configurations used for that comparison are defined in
\Cref{tab:results_course_cases}. The nominal case, R00, is the unmodified Baja
reference CVT. Every other row lists only the hardware changed from that
baseline; unlisted quantities retain the reference values of
\Cref{app:common_physical_configuration,app:reference_helix_topology}. The mass
percentages refer to the replaceable flyweight tip mass rather than the total
flyweight mass, and the corresponding mass moments are changed consistently.
...
\begin{table}[H]
  ...
  Only changes from R00 are listed for the remaining cases. Ramp distances are
  physical coordinates along the ramp, not movable-sheave travel.}
...
The cases are chosen to probe several ways in which physical hardware can alter
the coupled response, not because a particular course outcome is assumed in
advance. W85 and P300 change the primary centrifugal--spring balance through
different routes: one changes the flyweight mass properties, while the other
changes how the opposing spring force develops with sheave position after being
matched to the nominal force at one geometry. RC10, R26B7, and RC40L modify the
primary ramp over different portions of the roller path, providing three ways
to examine how a change in the contacted geometry is carried into the actuator
response. H28 moves the comparison to the secondary side by changing the helix
geometry that couples reacted torque, axial force, and secondary motion.

The remaining cases broaden those comparisons and provide controlled contrasts
for the more demanding parts of the road. U55 extends the primary flyweight-mass
change well beyond W85, while D01 and D02 combine larger departures from the
nominal hardware so that the same course can reveal whether different internal
balances lead to distinguishable limiting behaviour. D02\_M and D02\_P are
one-change variants of D02: each restores a different primary-side quantity to
its nominal value. They are included so that, if the D02 response exposes a
useful limitation, the consequences of changing those two contributions can be
compared without changing the rest of that configuration.
...
\begin{table}[tbp]
  ...
  \begin{tabular}{@{}lrrrrr@{}}
    ...
    Case & \shortstack{Primary rpm\\at 20\%} & \shortstack{Primary rpm\\at 50\%} &
            \shortstack{Primary rpm\\at 80\%} & \shortstack{Secant slope\\[2pt]\mbox{[rpm/1000 rpm]}} &
...
Much of the local physics required for these extensions already has a
foundation in the literature reviewed in \Cref{sec:literature-review}. The
challenge is to incorporate those mechanisms while retaining the coupled
transient response of the belt, pulleys, and actuators. Some descriptions
assume steady running, prescribe part of the motion, or determine a
component's position through instantaneous force balance. Where these
restrictions remove an interaction that the present formulation is intended
to resolve, the local relations must be reconsidered and coupled to the
equations of motion. Belt loading must affect how the hardware accelerates,
while that motion changes the contact conditions and loading in return.
...
The secondary actuator also depends on how belt torque is divided between
its two sheave faces. The movable face rotates relative to the shaft as it
shifts, so the belt generally encounters a different surface velocity on each
face. The current formulation retains this relative rotation
...
\subsection*{Belt Deformation, Transport, and Material History}
\label{sec:future_work_belt_material}

The belt couples the two pulleys through its length, motion, and loading.
...
examining this coupling. It already evolves belt-node motion and both pulley
speeds together, but its running-radius search matches belt axial reaction
to applied clamp without advancing movable-sheave axial inertia. Coupling
those belt mechanics to axial equations of motion would allow sheave and
actuator inertia to participate in the response. Such a comparison could
help explain the different clamp-to-shift responses identified in
\Cref{sec:results_ballew_interpretation}.

The single transport coordinate in \AssRef{assump:single_belt_speed} is a
separate kinematic reduction. Relaxing it would allow belt transport to vary
around the loop. Such variation need not originate in longitudinal
extension: the inextensible continuum kinematics of
\citet{CarboneMangialardiMantriota2005} already allow local transport to vary
with changing geometry. With compliance and inertia retained, a spatial
description could also follow the propagation of changes in tension and
deformation after a sudden load change. Steady-path and constant-mass-flow
restrictions, such as those used by \citet{Messick2018}, would need to be
reconsidered when following this response through time.
...
geometry are prescribed. Resolving compression, bulging, shear, and warping would
...
A belt model could retain these sectional effects without representing every
cord or cog around the loop. \citet{Gerbert1996} uses sectional analysis to construct an
effective shear relation. Similar reductions could carry sectional forces,
moments, contact geometry, and material history into a belt model. If a section
adjusts much faster than the overall belt motion, its deformation may be
treated without a separate inertial state even when it changes contact.
Whether deformation,
...
unfinished work \citep{Messick2018,Ballew2015}. A transient constitutive
description would carry the deformation history of each belt section through
the spans and wraps, while the energy accounting in
\Cref{eq:verification_energy_balance} would extend to
...
\citet{Messick2018} identifies crank-angle-dependent engine torque as a
possible source of vibration outside the scope of his steady model. A transient belt model
could determine whether torque ripple materially changes contact loading
and traction. Physical vibration would need to remain distinguishable from
numerical oscillation through spatial and
temporal refinement, particularly given the stiffness difficulties reported
by \citet{Ballew2015}.

\subsection*{Local Belt Contact and Traction}
\label{sec:future_work_local_contact}

A richer belt description would need contact relations that connect those
motions to the forces on the sheaves. The present wrap equations resolve
tension and normal loading, but one traction utilisation \(\lambda_j\) and
one contact state describe each wrap
...
local friction direction reverses. Resolving these differences would change
the torque carried by a pulley and the belt load opposing its actuator.

These contact states depend on the motion of the contacting surface.
The sectional mechanics described by \citet{Gerbert1996} show that rubber
between the tension-carrying layer and the sheave can deform in shear while
its surface adheres. A difference between cord speed and pulley surface
speed therefore need not imply sliding. Seating and unseating also change
the surface motion as the belt enters and leaves a wrap; Gerbert shows that
they can produce friction opposing the main torque-transmitting direction
at the primary. The deformation established in the belt description must
therefore enter the contact kinematics if these motions are to be
distinguished from sliding.
...
torque transmission, and axial thrust change together during shifting.
A pressure- or speed-dependent friction law could also be examined within
the present reduced model; separating adhering and sliding regions would
require spatial contact resolution. Applied separately to the secondary
faces, these relations could supply the face torques needed by the helix
balance.

The distinction between surface sliding and internal deformation also
matters for power loss. The contact calculation would determine work
dissipated by sliding, while the material description would determine the
energy stored or dissipated within the belt. \citet{Bertini2014} distinguish
sliding, material hysteresis, and losses at entry to and exit from the
pulley wraps. Those entry and exit processes recur as the belt circulates,
even after initial clutching is complete. Following them during a transient
would connect changes in power loss to the same mechanics that determine
clamp demand and shift timing.

\subsection*{Belt Path, Support, and Re-engagement}
\label{sec:future_work_belt_path}

Local traction can be refined while the belt path remains prescribed. The
circular wraps and straight spans
...
Bearing, seal, and windage resistance are excluded by
...
secondary face-torque coupling, movable-face torque or helix reaction
...
specified, measurements can test predicted shaft and sheave histories through
engagement, acceleration, upshift, backshift, and traction recovery. Different
arrangements provide access to different loads and measurements; the useful
comparison is between the quantities the model predicts and those the
experiment can independently observe.

Independent material and component characterisation is a further part of
this work. Geometry, inertia, and spring characteristics establish the
mechanical inputs. Actuator friction requires measurements through holding,
breakaway, and reversal, while belt tests must distinguish elastic storage
from dissipation under relevant stretching, compression, shear, and bending.
The tests described by \citet{Ballew2015} and \citet{Messick2018} provide
foundations, with further work needed on damping and dependence on loading
history. Belt--sheave friction and parasitic losses also require independent
characterisation over the relevant loads, speeds, and temperatures. These measurements would
allow complete-CVT experiments to examine the coupled response without
reselecting the physical parameters for each maneuver.

\subsection*{Recovery and Assessment of the Reduced Model}
\label{sec:future_work_reductions_design}

These extensions would be most useful if their relation to the present
model remained explicit. Wherever the reduction is well defined, restoring
the same assumptions and closure relations should recover CINDER's equations
and reproduce its response within numerical accuracy under corresponding
loading and initial conditions. Agreement
in that case would establish that the added mechanics connect consistently
to the existing description. A remaining difference would need to be traced
to the reduction, the solution procedure, or an identified change in the
physical model.

Recovery may require several restrictions to be imposed together. Making
a belt very stiff, for example, does not by itself impose the prescribed
path, single transport coordinate, or wrap-level contact description used
here. The same forces and inertias must also be reduced consistently in
every balance where they participate. Establishing this correspondence
would give a clear basis for a second comparison: allowing the additional
mechanisms to act under the same physical loading, then examining how much
they change the predictions of interest.

With the additional mechanisms active, the comparison would address when
the simpler model is sufficient.
The comparisons in \Cref{sec:results_reduced_belt_mechanics} already show why
the effect on the integrated response matters, beyond the size of an omitted
term evaluated along one trajectory. Differences in clamp demand, shift
timing, traction recovery, or contact transitions would show which
predictions are sensitive to the added mechanics. Experimental comparisons
would then help establish whether the extra detail improves those
predictions. Where its effect is small enough for the question being studied,
the present description may remain sufficient. Each extension could thereby
help establish both where CINDER remains useful and where more detail is
needed, providing a basis for choosing an appropriate model for a component
or design study.
