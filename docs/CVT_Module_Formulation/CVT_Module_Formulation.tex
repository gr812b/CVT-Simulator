...
$R_g$ & 1 & Instantaneous geometric CVT ratio $r_s/r_p$ used in the ratio analysis. \\
...
Advancing the CVT from one instant to the next requires four kinds of
information that differ in where they originate and whether they persist between
evaluations. Fixed CVT data define the selected mechanism and remain unchanged
while the simulation is advanced. The current CVT condition carries the internal
state and regime retained from the preceding instant. Shaft-interface conditions
are supplied by the systems attached to the two shafts for the present
evaluation. The CVT mechanics then determine the instantaneous quantities needed
to advance that condition and test whether the active regime remains admissible.

\Cref{tab:model_quantity_roles} summarizes these four roles.
...
The corresponding symbols are collected in the front-matter lookup tables:
fixed CVT data in \Cref{tab:cvt_fixed_data}, the current CVT condition in
\Cref{tab:cvt_current_condition}, shaft-interface conditions in
\Cref{tab:cvt_shaft_interface_conditions}, and instantaneous CVT quantities in
...
\begin{enumerate}
    ...
    \item The CVT mechanics use the fixed CVT data, current internal condition,
          and shaft-interface conditions to determine the instantaneous mechanical
