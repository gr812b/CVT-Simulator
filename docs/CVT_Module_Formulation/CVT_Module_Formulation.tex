...
simply in whether the belt and pulley surface have relative tangential motion.
At pulley \(j\), the belt moves through the wrap with signed transport speed
\(v_b\), while the pulley surface at the effective contact radius moves
tangentially at \(r_{j,\mathrm{eff}}\omega_j\). Their difference defines the
...
moves ahead of the pulley surface in the positive belt direction, while a
negative value means it moves the other way. For the secondary wrap,
...
Sticking begins at the opposite kinematic condition,
...
force. It only says that the belt and pulley surface are moving together at
that instant. Static friction may still supply a nonzero reaction, with
...
That speed match must also persist. If the belt speed and pulley-surface speed
begin changing at different rates, relative motion appears immediately.
Differentiating \Cref{eq:tangential_relative_speed} gives the relative
...
the acceleration of a pulley surface at fixed radius. The final term appears
...
The two Coulomb branches now meet again in one useful way. A sliding contact
supplies a numerical \(\lambda_j\) directly, while the sticking solve tests
candidate values of \(\lambda_j\). Once one value is available for each wrap,
the mechanical balances use them in exactly the same way. Beneath either
contact branch is therefore the same simpler problem: determine the
instantaneous response associated with a specified pair
\((\lambda_p,\lambda_s)\).

With that pair specified, the belt torques are not additional unknowns.
\Cref{eq:generic_belt_contact_torque} already gives
...
so each torque follows directly from the corresponding normal resultant
\(N_j\). The axial belt reactions are likewise already written in terms of
\(N_p\) and \(N_s\). The geometry has reduced both sheave motions to the
single shift coordinate \(s\), and the four temporary belt-boundary tensions
were eliminated when the loop was closed in
\Cref{eq:closed_loop_tension_compatibility}. What remains to be determined at
the current instant is therefore
...
These six quantities are already tied together by six relations derived on
their natural free bodies. The two shaft accelerations appear in the primary
and secondary rotational balances,
\Cref{eq:primary_rotational_eom_belt,eq:secondary_rotational_eom_belt}. The
shared shift acceleration and the two normal resultants enter the primary and
secondary axial balances,
\Cref{eq:primary_balance_in_shift_coordinate,eq:secondary_balance_in_shift_coordinate}.
The belt transport balance in \Cref{eq:complete_belt_transport_newton}
determines \(\dot v_b\) from the net tangential action of the two wraps, while
\Cref{eq:closed_loop_tension_compatibility} supplies the second independent
condition carried by the closed belt.

Once \(\lambda_p\) and \(\lambda_s\) are treated as specified values, their
nonlinear dependence is no longer part of this inner calculation. They appear
only as known coefficients. Substituting the contact torques above, together
with the radius accelerations already obtained in
\Cref{eq:primary_radius_acceleration,eq:secondary_radius_acceleration}, leaves
the six existing relations linear in the six entries of \(\mathbf z\).
...
The \(6\times6\) form is simply the assembled version of mechanics already
derived: two rotational balances, two axial balances, the complete-belt
transport balance, and the closed-loop belt compatibility. The current state,
geometry, applied boundary loads, and specified traction pair set the
coefficients; solving the system returns the accelerations and normal
resultants for that instant.

This is the calculation used inside the sticking root of
\Cref{sec:sticking_traction_closure}. A candidate static traction pair is
inserted into \Cref{eq:engaged_six_by_six_mechanical_core}, the resulting
mechanical response is solved, and the corresponding
\(a_{\mathrm{rel},j}\) values provide the sticking residuals. When a contact is
sliding, the same calculation applies without that outer search because
kinetic friction has already supplied its \(\lambda_j\).

One technical point matters when a sticking root passes through zero traction.
The nonzero-\(\lambda_j\) wrap relation contains reciprocal terms, but its
combined limit at \(\lambda_j=0\) is finite. The zero-traction form already
derived in \Cref{eq:generic_wrap_boundary_tension_sum_zero_lambda} should
therefore be used at that point, so the mechanical solve remains continuous as
a trial static utilization changes sign.

The different engaged contact combinations do not require different
mechanical equations. They differ only in how the pair
\((\lambda_p,\lambda_s)\) supplied to this common solve is obtained.
