...
Together, these rules determine how the state passes from one mechanical
regime to the next.  While the current regime remains admissible, its
continuous balances advance the state.  Loss of a support or contact
admissibility condition ends that interval and determines which neighboring
mechanics can become active.  If the new regime already admits the incoming
velocity, the state passes into it continuously; if a newly active constraint
does not, \Cref{eq:finite_event_kkt_system} supplies the compatible outgoing
velocity and the impulse required to establish it.  That outgoing state then
provides the initial condition for the next interval of continuous motion.
