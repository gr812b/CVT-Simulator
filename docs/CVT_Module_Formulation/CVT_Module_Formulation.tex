...
substantially from an equal split during shifting
...
y_{\mathrm{cm}}
&=
\frac{\displaystyle\int_0^{h_b} y\,b(y)\,dy}
...
The choice of reference changes the absolute radius, but not how quickly that
radius changes. The two pulley-radius rates are now determined by \(s\) and
\(\dot s\). However, the secondary multiplier
...
The only additional expansion needed is the radius acceleration already implied
by the geometry,
...
For one trial traction pair, solving this \(6\times6\) system determines the
complete mechanical response associated with that pair.
...
\section{Shift Limits, Engagement, and Deadzone Dynamics}
\label{sec:shift_limits_deadzone}

The freely shifting closure developed in \Cref{sec:engaged_dynamic_closure}
describes the CVT only while the belt is engaged with both pulleys and the
primary movable sheave remains between its physical travel limits. Real
hardware has two additional features that this interior solution does not
contain. The primary can open far enough that its sheaves no longer touch the
belt, and it can close only until its hardware reaches a mechanical stop.
Between those two limits, first belt contact introduces another boundary: the
primary can touch and clamp the belt while the belt is still supported at its
minimum primary radius.

These boundaries do not require a new CVT model at every operating point.
Instead, they determine which parts of the formulation developed above remain
active, which kinematic quantities are constrained, and which reactions appear
when a physical boundary prevents further motion. The useful question is
therefore not how to construct several unrelated regimes, but how the complete
equations reduce as the primary travels from fully open, through first contact
and free shift, to fully closed.

The natural place to begin is the range of the shift coordinate itself.

% ---------------------------------------------------------------------------
\subsection{Physical Boundaries of the Shift Coordinate}
\label{sec:shift_coordinate_boundaries}

The shared coordinate \(s\) is the axial displacement of the primary movable
sheave. Three positions divide its physically meaningful range. Let
...
the fully closed primary position. With the convention established in
\Cref{sec:geometry}, the lower mechanical stop is chosen as the origin,
...
the movable primary sheave travels toward or away from the belt without
changing the belt running radii. The belt remains supported at the low-ratio
geometry: its primary-side path is fixed at the minimum radius and the
secondary remains fully closed at its maximum belt radius. The distance
\(s_{\mathrm e}-s_0\) is therefore a primary deadzone in axial travel rather
than a ratio change.

Once \(s=s_{\mathrm e}\), the movable primary face first reaches the belt.
Further primary closure can then force the belt away from its minimum primary
radius and, through the fixed-length geometry, open the secondary. Under the
fixed-cross-section idealization used throughout the model, the primary can
continue closing only until the gap consumed after first contact equals the
belt width at its inner surface. The available engaged primary travel is
therefore

\begin{equation}
    \boxed{
        s_{\mathrm u}-s_{\mathrm e}
        =
        b_{\mathrm{in}},
    }
...
\begin{equation}
    \boxed{
        s_{\mathrm u}
        =
        s_{\mathrm e}+b_{\mathrm{in}}.
    }
    \label{eq:upper_shift_stop_position}
\end{equation}

Equation \eqref{eq:upper_shift_stop_position} represents the idealized case in
which the primary reaches its geometric closing limit when the sheave hardware
at the base of the groove meets. If a real variator contains an earlier
mechanical stop, its measured travel replaces \(b_{\mathrm{in}}\) in
\Cref{eq:active_shift_travel_limit}. The present formulation also assumes that
the secondary has not reached an independent shaft or opening stop at
\(s=s_{\mathrm u}\). The upper travel limit is therefore set by the primary
alone.

Two additional reductions are required at the low-ratio end. First, belt
pretension is neglected while the primary is disengaged. The primary shaft may
support the belt radially, but that support is not assigned a tangential
traction law and transmits no primary belt torque. Second, the slack-belt
shape is not introduced as a new distributed degree of freedom. The belt is
taken to remain seated on the fully closed secondary and to move with its
surface at the fixed low-ratio radius \(r_{s,\mathrm e}\). Thus

\begin{equation}
    \boxed{
        v_b
        =
        r_{s,\mathrm e}\omega_s
        \qquad
        \text{while the primary is disengaged}.
    }
    \label{eq:deadzone_secondary_lock}
\end{equation}

This is a deadzone closure assumption, not a consequence of the engaged wrap
equations. It replaces the tension-loop and two-contact description that no
longer applies once the primary sheaves have separated from the belt.

A final distinction concerns what happens when a boundary is reached with
nonzero \(\dot s\). The fully open and fully closed limits are metal-on-metal
mechanical stops. Their axial impacts are treated as perfectly inelastic, so
arrival resets the shift velocity to zero. First belt contact at
\(s=s_{\mathrm e}\) is different. The sheave is meeting the compliant belt and
coupling into the secondary rather than striking a rigid travel stop. No
kinematic reset of \(\dot s\) is imposed there. The incoming primary axial
momentum is carried into the newly active engaged equations, which determine
whether the sheave continues to close, is decelerated by the belt and
secondary, or subsequently separates again.

With these boundaries established, the complete operating range can be built
from the fully open configuration upward.

% ---------------------------------------------------------------------------
\subsection{Fully Open Primary Stop}
\label{sec:fully_open_primary_stop}

At the lowest shift coordinate, the primary movable assembly cannot travel any
farther in the opening direction. The belt is not in contact with the primary
sheave faces, so the stop acts only on the primary mechanism. This makes the
fully open configuration the simplest constrained regime.
...
and \(\lambda_p\) has no meaning because no primary belt--sheave contact
exists.

The primary rotational balance from
\Cref{eq:primary_rotational_eom_belt} therefore reduces to
...
The term proportional to \(\dot s\,\omega_p\) vanishes because the movable
assembly is fixed at the stop.

The primary axial equation is not discarded. Instead, the stop supplies the
reaction required to satisfy that equation with \(\ddot s=0\). Let
\(R_0\) denote the stop reaction acting in the positive, closing direction.
Substituting \(N_p=0\), \(\dot s=0\), and \(\ddot s=0\) into the primary
balance gives
...
\begin{equation}
    ...
    =
    \frac{1}{2}\omega_p^2
    \frac{dJ_{f,p}}{ds}\bigg|_{s_0}
    -
    k_p\left(x_{p,\mathrm{pre}}+s_0\right)
    +
    R_0,
\end{equation}
...
The stop is unilateral. It may push the movable assembly closed, but it cannot
pull it open. The constrained state is therefore admissible only while
...
At low primary speed, the spring tends to hold the sheave against the open
stop and \(R_0>0\). As \(\omega_p\) rises, the flyweight closing action grows
until \(R_0=0\). Beyond that point, holding the sheave at \(s_0\) would require
the stop to pull in the opening direction. The constraint must release, and
the primary enters free deadzone motion.

The belt and secondary remain governed by the deadzone lock in
\Cref{eq:deadzone_secondary_lock}. Since the secondary geometry is fixed in
this regime, the belt transport mass can be reflected directly to the
secondary shaft. The remaining equations are
...
\begin{equation}
    \boxed{
        \dot v_b
        =
        r_{s,\mathrm e}\dot\omega_s.
    }
    \label{eq:deadzone_belt_acceleration_lock}
\end{equation}

Thus the only free accelerations at the fully open stop are
\(\dot\omega_p\), \(\dot\omega_s\), and \(\dot v_b\). The shift acceleration
is fixed to zero, and \(R_0\) is recovered from the omitted free primary axial
motion.

% ---------------------------------------------------------------------------
\subsection{Free Motion Through the Primary Deadzone}
\label{sec:free_primary_deadzone}

Once the lower stop releases, the primary movable assembly can travel toward
the belt. The belt has still not been reached, so none of the primary
belt-contact equations should be retained merely because the sheave is moving.
The free deadzone is therefore obtained by restoring the primary axial degree
of freedom while leaving the primary belt reactions absent.
...
while \(s\) and \(\dot s\) are free state variables. The full moving-primary
rotational balance remains active:
...
The second term is retained because the flyweights can redistribute their mass
about the shaft while the movable sheave travels through the deadzone. Loss of
belt contact does not freeze the primary actuation mechanism.

The primary axial balance is likewise the ordinary balance from
\Cref{eq:primary_balance_in_shift_coordinate} with the belt load removed:
...
The secondary and belt do not acquire independent deadzone dynamics. They
continue to satisfy
\Cref{eq:deadzone_secondary_rotation,eq:deadzone_belt_acceleration_lock}.
Consequently the free deadzone has four instantaneous derivatives,
...
\Cref{eq:deadzone_primary_rotation,eq:deadzone_primary_axial,%
eq:deadzone_secondary_rotation,eq:deadzone_belt_acceleration_lock}
determine them directly. No \(N_p\), \(\lambda_p\), primary belt torque, or
closed-loop tension compatibility is present.

This regime ends when the moving primary sheave reaches
\(s=s_{\mathrm e}\). At that point the reason for the reduced equations
disappears: a primary belt contact now exists.

% ---------------------------------------------------------------------------
\subsection{First Belt Contact and the Low-Ratio Boundary}
\label{sec:first_contact_low_ratio}

First contact changes the topology of the mechanical problem. Before
\(s_{\mathrm e}\), the primary movable sheave and the belt evolve without a
sheave-face contact between them. At \(s_{\mathrm e}\), a primary normal
reaction and tangential traction can appear, and the primary motion can begin
to drive the belt against the fully closed secondary. The important point is
that contact alone is not a rigid axial stop.

\subsubsection{First Contact With Continuous Shift Momentum}
\label{sec:first_contact_momentum}

Let \(t_{\mathrm e}\) be the instant at which a closing primary first reaches

\[
    s(t_{\mathrm e})=s_{\mathrm e}.
\]

No impact projection is imposed on the shift velocity. The continuous state is
...
The superscripts \((-)\) and \((+)\) denote the states immediately before and
after first contact. The accelerations are allowed to change discontinuously
because the belt normal force and traction become active at the event, but no
finite amount of axial momentum is deleted by setting \(\dot s\) to zero.

The engaged equations from \Cref{sec:engaged_dynamic_closure} are then
evaluated at the same state. In particular, the eight engaged unknowns
...
are again determined by the six engaged mechanical equations and the two
contact relations appropriate to the instantaneous stick--slip combination.
The primary relative speed need not vanish at first contact. If it does not,
the primary begins in a sliding contact state; adhesion is not imposed merely
because the surfaces have touched.

This treatment lets the mechanics decide what first contact does. The incoming
primary axial momentum may be sufficient to continue into \(s>s_{\mathrm e}\)
and begin moving the belt radially outward. Alternatively, the belt,
secondary spring, helix, and reflected inertias may decelerate the primary
motion. Any dissipation comes from mechanisms already retained in the model,
most directly kinetic slip, rather than from an artificial zero-velocity reset
at the engagement event.

If the trajectory leaves \(s_{\mathrm e}\) in the closing direction, the belt
lifts from its minimum-radius support and the CVT enters ordinary free engaged
shift. If instead the system is at \(s=s_{\mathrm e}\) with zero shift speed
and the low-ratio support is still required, the boundary itself becomes an
active unilateral constraint.

\subsubsection{Sustained Low-Ratio Seat}
\label{sec:sustained_low_ratio_seat}

A sustained low-ratio state exists only when the belt is engaged with the
primary while remaining at the minimum primary radius. The secondary is then
also at its fully closed, maximum-radius configuration. Physically, this
limiting geometry can be supported by the primary shaft beneath the belt and
by the secondary hardware at its closed limit. The reduced one-coordinate
formulation does not attempt to resolve how that support load is distributed
between those physical contacts. Their net effect is represented by a single
generalized low-ratio reaction \(R_{\mathrm e}\), acting in the positive shift
direction and preventing motion to a ratio lower than the geometric engagement
limit.
...
This condition does not contradict
\Cref{eq:first_contact_state_continuity}: a first-contact event with
\(\dot s>0\) is not projected into
\Cref{eq:low_ratio_seat_kinematics}. The seated equations apply only once the
system is actually at rest on the low-ratio boundary.

Both pulley contacts exist in the seated regime, so the tangential contact
logic of \Cref{sec:engaged_dynamic_closure} remains active. For a chosen
\((\lambda_p,\lambda_s)\), the free linear unknowns are now
...
The secondary has no relative helix motion while \(s\) is fixed, so its
rotational balance reduces to

\begin{equation}
    \boxed{
        \tau_{\mathrm B,s}
        -
        r_{s,\mathrm{eff}}\lambda_sN_s
        =
        \left(
            I_{\mathrm B,s}
            +
...
The secondary axial balance remains a physical force balance even though the
shared shift coordinate is fixed. Setting \(\dot s=\ddot s=0\) in
\Cref{eq:secondary_balance_in_shift_coordinate} and substituting
\(\tau_s=-r_{s,\mathrm{eff}}\lambda_sN_s\) gives
...
\begin{aligned}
    ...
    \frac{d\theta_s}{dx_s}
    \left[
        -\frac{r_{s,\mathrm{eff}}\lambda_sN_s}{2}
        +
        k_{s,\theta}
        \left(
            \theta_{s,\mathrm{pre}}-\theta_s
...
with all secondary geometry evaluated at \(s=s_{\mathrm e}\).
...
\begin{equation}
    \boxed{
        m_b\dot v_b
        =
        \lambda_pN_p+\lambda_sN_s,
    }
    \label{eq:low_seat_belt_transport}
\end{equation}

and the closed-loop tension compatibility from
\Cref{eq:closed_belt_tension_compatibility}, evaluated at the fixed
low-ratio geometry with
...
The primary axial balance supplies the remaining equation by recovering the
generalized low-ratio support reaction. This choice assigns the unresolved
boundary reaction to the primary shared-coordinate balance; it should not be
read as a face-resolved prediction of the separate shaft and secondary-stop
loads. With \(\dot s=\ddot s=0\),
...
\boxed{
    ...
    \frac{1}{2}\omega_p^2\frac{dJ_{f,p}}{ds}
    -
    k_p\left(x_{p,\mathrm{pre}}+s_{\mathrm e}\right)
    -
    \frac{N_p}{2}\cos\beta
    +
    R_{\mathrm e}.
...
The low-ratio support can prevent the belt from moving farther inward, but it
cannot pull the belt away from the shaft. With \(R_{\mathrm e}\) defined in
...
is therefore the condition for a sustained seated state. If the constrained
solution would require \(R_{\mathrm e}<0\), the support would have to act in
the opposite direction to keep the belt at its minimum radius. The seat must
release and the ordinary freely shifting engaged equations become active.

Primary belt contact is separately unilateral. A seated solution also
requires
...
If the primary normal resultant falls to zero and the sheave motion is opening,
the primary can separate from the belt and the deadzone equations again
become the appropriate description.

For the \(\lambda_j\) values, nothing new is introduced at the low-ratio
seat. A slipping contact supplies its kinetic value directly, while a sticking
contact supplies the no-slip acceleration condition and requires the same
one- or two-dimensional root logic developed in \Cref{sec:engaged_dynamic_closure}.
The only change inside each trial is that \(\ddot s\) is fixed at zero and the
boundary reaction \(R_{\mathrm e}\) replaces it among the linear unknowns.

% ---------------------------------------------------------------------------
\subsection{Freely Shifting Engaged Interior}
\label{sec:free_engaged_interior}

Once the low-ratio support is no longer required, the belt has lifted from its
minimum-radius support and the primary is free to move through the engaged
range

\begin{equation}
    s_{\mathrm e}<s<s_{\mathrm u}.
    \label{eq:free_engaged_shift_interval}
\end{equation}

No boundary reaction acts on the shift coordinate. The free variables are
exactly the eight instantaneous quantities collected in
\Cref{eq:engaged_instantaneous_unknowns},
...
The governing equations are likewise exactly those assembled in
\Cref{sec:engaged_dynamic_closure}: the primary and secondary rotational
balances, the two axial balances, complete-belt transport, closed-loop tension
compatibility, and the two stick-or-slip contact relations. No additional
equation is needed in the interior because no travel boundary is active.

This interval therefore requires no new derivation. Its importance here is
only to locate the freely shifting system within the complete physical travel:
it is the interior solution between the low-ratio support and the upper
primary stop.

% ---------------------------------------------------------------------------
\subsection{Fully Closed Primary Stop}
\label{sec:fully_closed_primary_stop}

The freely shifting equations cease to apply once the primary reaches its
maximum closing travel. At

\[
    s=s_{\mathrm u},
\]

the primary hardware reaches a rigid metal stop. Unlike first belt contact,
this is a true kinematic termination of the shift coordinate. Arrival is
therefore treated as a perfectly inelastic axial impact,
...
Both pulley contacts remain engaged, and the secondary is assumed not to have
reached an independent shaft or opening limit. The governing equations are
therefore the same fixed-shift engaged equations used at the low-ratio seat,
with all geometry evaluated at \(s=s_{\mathrm u}\). For a chosen
\((\lambda_p,\lambda_s)\), the free linear unknowns are
...
where \(R_{\mathrm u}\) is the upper-stop reaction acting in the opening
direction.
...
The secondary axial balance is
\Cref{eq:low_seat_secondary_axial} evaluated at \(s=s_{\mathrm u}\), while
the belt obeys
...
together with the fixed-geometry form of
\Cref{eq:closed_belt_tension_compatibility}.

The primary axial balance now recovers the stop reaction with the opposite
sign from the low-ratio support. A positive \(R_{\mathrm u}\) acts in the
opening direction and opposes further closure:
...
\boxed{
    ...
    \frac{1}{2}\omega_p^2\frac{dJ_{f,p}}{ds}
    -
    k_p\left(x_{p,\mathrm{pre}}+s_{\mathrm u}\right)
    -
    \frac{N_p}{2}\cos\beta
    -
    R_{\mathrm u}.
...
The stop may push open but cannot pull closed, so the constrained state is
...
If the recovered reaction reaches zero and the unconstrained tendency is
toward decreasing \(s\), the metal stop is no longer needed and the CVT
releases directly into free engaged backshift. The stick--slip logic at the
two pulley contacts is unchanged by this release; only the shift constraint is
removed and \(\ddot s\) again becomes an unknown of the free engaged closure.

% ---------------------------------------------------------------------------
\subsection{Complete Operating-Regime Map}
\label{sec:complete_operating_regime_map}

The physical travel limits now complete the formulation over the entire
primary range. What initially appears to be a collection of special cases
reduces to five operating regimes connected by the same few ideas: loss or
creation of primary belt contact, free motion of the shared shift coordinate,
and unilateral reactions at physical boundaries.
...
\updownarrow
\\[4pt]
\text{engaged low-ratio seat}
\\[4pt]
\updownarrow
\\[4pt]
\text{engaged free shift}
\\[4pt]
\updownarrow
\\[4pt]
\text{engaged upper stop}.
\end{array}
}
    \label{eq:complete_cvt_regime_map}
\end{equation}

The diagram describes the physical ordering of the five regimes, not a demand
that every boundary event contain a finite-duration intermediate state. In
particular, a closing deadzone trajectory that reaches first belt contact with
\(\dot s>0\) carries that velocity through
\Cref{eq:first_contact_state_continuity}. If the newly engaged equations admit
continued closing motion, the low-ratio boundary releases immediately and the
trajectory enters free engaged shift without an artificial pause at
\(\dot s=0\).
...
\centering
\small
\begin{tabular}{|p{0.14\textwidth}|p{0.16\textwidth}|p{0.22\textwidth}|p{0.28\textwidth}|}
\hline
\textbf{Regime}
&
\textbf{Shift condition}
&
\textbf{Free instantaneous quantities}
&
\textbf{Governing relations}
\\
\hline

Deadzone lower stop
&
\(s=s_0,\ \dot s=\ddot s=0\)
&
\(\dot\omega_p,\dot\omega_s,\dot v_b,R_0\)
&
Primary rotation with \(\tau_p=0\);
deadzone secondary--belt rotation;
\(\dot v_b=r_{s,\mathrm e}\dot\omega_s\);
primary axial balance recovers \(R_0\).
\\
\hline
...
\(\dot\omega_p,\dot\omega_s,\dot v_b,\ddot s\)
&
Primary rotation and axial motion with
\(N_p=\tau_p=0\);
deadzone secondary--belt rotation;
\(\dot v_b=r_{s,\mathrm e}\dot\omega_s\).
\\
...
&
\(s=s_{\mathrm e},\ \dot s=\ddot s=0\)
for a sustained seated interval
&
\(\dot\omega_p,\dot\omega_s,\dot v_b,N_p,N_s,R_{\mathrm e}\),
plus any static \(\lambda_j\)
&
Fixed-shift engaged rotational, secondary axial, belt transport, and
tension-loop equations; primary axial balance recovers
\(R_{\mathrm e}\); two contact relations determine the \(\lambda_j\).
\\
\hline
...
\(s,\dot s\) free
&
\(\dot\omega_p,\dot\omega_s,\dot v_b,\ddot s,N_p,N_s,\lambda_p,\lambda_s\)
&
The complete freely shifting engaged closure of
...
Engaged upper stop
&
\(s=s_{\mathrm u},\ \dot s=\ddot s=0\)
&
\(\dot\omega_p,\dot\omega_s,\dot v_b,N_p,N_s,R_{\mathrm u}\),
plus any static \(\lambda_j\)
&
Fixed-shift engaged rotational, secondary axial, belt transport, and
tension-loop equations; primary axial balance recovers
\(R_{\mathrm u}\); two contact relations determine the \(\lambda_j\).
\\
\hline
\end{tabular}
\caption{Governing content of the five CVT operating regimes. A slipping
contact supplies its \(\lambda_j\) directly, while each sticking contact adds
the static traction unknown and corresponding no-slip root condition developed
in \Cref{sec:engaged_dynamic_closure}.}
\label{tab:complete_cvt_regime_summary}
\end{table}

Three types of transition can now be distinguished without introducing new
continuous mechanics. A metal stop is reached when \(s\) reaches \(s_0\) or
\(s_{\mathrm u}\); the axial impact is taken as perfectly inelastic and the
corresponding unilateral reaction determines whether the stop can continue to
hold. Primary belt contact appears or disappears at \(s=s_{\mathrm e}\);
first contact preserves the incoming shift velocity rather than replacing it
with an impact condition. Finally, the low-ratio support is active only while
the belt remains engaged at the minimum primary radius and its generalized
reaction is nonnegative.

The five regimes are therefore not five independent models. The deadzone uses
one reduced primary-disengaged system in which the primary sheave moves without
belt load and the belt is kinematically carried by the fully closed secondary.
The three engaged regimes all use the contact closure of
\Cref{sec:engaged_dynamic_closure}; the low-ratio and upper boundaries simply
replace the free shift acceleration by a boundary reaction when the
corresponding unilateral constraint is active. Between them lies the
unconstrained engaged system already derived.

The operating limits complete the mechanical domain of the CVT. The primary
may begin at rest against its open stop, release when the flyweights overcome
the spring, cross the deadzone, contact the belt without losing its axial
momentum, remain at or depart from the low-ratio boundary according to the
coupled belt--secondary response, shift freely while both pulley contacts are
engaged, and finally terminate its closing travel against the upper mechanical
stop. Every continuous segment is therefore governed by an explicit reduction
of the same pulley, belt, and contact mechanics developed in the preceding
sections.
