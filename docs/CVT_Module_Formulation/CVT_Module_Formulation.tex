...
With the retained motions and coordinates now defined, the next question is
what information about the CVT must be carried from one instant to the next.
The formulation uses five continuous state variables, collected in the state
vector,
...
including the primary deadzone before engagement, is established in
\Cref{sec:pulley_coordinate_mapping}.

The corresponding shift speed \(\dot{s}\) must also be retained because the
moving sheaves and actuation hardware carry inertia. Pulley radii and actuator
configurations are instead obtained from the current shift position through
the geometric and mechanism relations developed in
\Cref{chap:derivation}.
...
for an additional belt-phase coordinate.
...
which the allowable motion need not be the same. The next section introduces
