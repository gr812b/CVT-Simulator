...
At this point, each assumed contact regime produces a complete instantaneous
response. For a sticking contact, however, satisfying
\(a_{\mathrm{rel},j}=0\) only identifies the static traction required to
maintain that state; it does not yet show that the contact can supply it. The
remaining question is therefore whether the required reaction is admissible,
and how to respond when it is not.

The static branch of \Cref{eq:lambda_local_definition} gives that first check
directly. A sticking solution is admissible only while
...
If the root found in \Cref{sec:sticking_traction_closure} lies inside this
bound, the contact can supply the reaction needed to keep
\(a_{\mathrm{rel},j}=0\), and sticking may continue. If the required
\(\left|\lambda_j\right|\) exceeds \(\mu_s\), no admissible static reaction can
preserve the sticking constraint. The assumed sticking regime has then reached
its limit and the contact must release into sliding.

A sliding contact has no analogous static-capacity check. While
\(v_{\mathrm{rel},j}\neq0\), its sign selects the kinetic branch through
\Cref{eq:sliding_traction_utilization}, and the contact remains sliding as long
as that relative motion persists. The next ambiguity appears only when
\(v_{\mathrm{rel},j}\) returns to zero. At that instant the previous sliding
direction disappears, so the kinetic branch alone no longer determines the
continuation of the contact.

At \(v_{\mathrm{rel},j}=0\), the sticking solution provides the natural
test. Solving for the static \(\lambda_j\) with
\(a_{\mathrm{rel},j}=0\) asks whether the belt and pulley can continue moving
together from that instant onward. If the resulting utilization satisfies
\Cref{eq:sticking_admissibility}, the contact transitions to sticking. If it
does not, sticking is unavailable and a kinetic branch must carry the contact
away from zero relative speed.

At the instant of release or slip reversal,
\(v_{\mathrm{rel},j}=0\), so its sign cannot select that kinetic branch
directly. Instead, the branch must be consistent with the relative acceleration
it produces. Since \(\sigma_j\) is opposite the sliding direction, an emerging
sliding branch must satisfy
...
The resulting \(a_{\mathrm{rel},j}\) then gives the sign of the relative
motion immediately after the transition.

These tangential transitions are therefore nonimpulsive. The belt and pulley
reach the regime boundary with compatible position and velocity, and the
change of contact state changes the force relation—and hence the subsequent
acceleration—rather than requiring an instantaneous velocity reset. This is
the nonimpulsive regime transition described in
\Cref{sec:mechanical_regimes_state_evolution}.

The two contact checks cannot be applied independently. Changing the state
of either wrap changes its \(\lambda_j\), which changes the entries of
\(\mathbf A\) and \(\mathbf b\) and therefore the response at both pulley
contacts. A transition at one wrap can consequently change the static traction
required at the other. After any contact changes state, the new traction pair
must be resolved, the mechanical response recomputed, and both contacts checked
again.

An admissible fully engaged regime is therefore one in which every sticking
contact can supply the static reaction required to maintain
\(a_{\mathrm{rel},j}=0\), while every sliding contact has relative motion
consistent with its kinetic branch. When either condition fails, the contact
regime changes and the common mechanical response is solved again under the new
traction pair.
