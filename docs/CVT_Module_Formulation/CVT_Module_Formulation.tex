...
No velocity reset is needed for any of these releases. A sustained stop
already has \(\dot s=0\), so that velocity is also admissible in the free
regime that replaces it. Position and velocity pass through unchanged, as in
\Cref{eq:nonimpulsive_regime_transition}, and only the active continuous
equations change. Primary separation from an established low-ratio seat is
likewise nonimpulsive because it begins from the same fixed-shift state; once
the contact is no longer admissible, the deadzone equations simply become
active.

Release is therefore a change of active equations without an instantaneous
velocity transfer. The opposite direction is different: a free regime can
arrive at a boundary carrying a velocity that the newly active constraint or
kinematic mapping cannot admit.
