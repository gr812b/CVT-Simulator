...
The four contact combinations above show how to obtain an instantaneous
response for any assumed tangential regime. They do not yet show whether that
regime can actually be maintained, nor what to do when it cannot.

For a sticking contact, the root in
\Cref{sec:sticking_traction_closure} gives the static utilization required to
keep the belt and pulley moving together. Sticking is possible only if the
contact can supply that reaction,
...
If the required utilization lies inside this limit, the contact can supply
the reaction needed to remain stuck. If it would have to exceed the limit, no
admissible static reaction can keep the belt and pulley moving together, and
the contact must begin to slide.

A sliding contact is simpler while
\(v_{\mathrm{rel},j}\neq0\). The sign of the relative velocity fixes the
sliding direction, and \Cref{eq:sliding_traction_utilization} fixes the
corresponding kinetic traction. The next decision only appears when the
relative velocity returns to zero. At that instant its sign no longer tells us
what happens next: the belt and pulley may remain together, or relative motion
may develop again.

The first possibility to test is sticking. With
\(v_{\mathrm{rel},j}=0\), solve the sticking condition
\(a_{\mathrm{rel},j}=0\) for the static utilization required at that instant,
then check \Cref{eq:sticking_admissibility}. If the required
\(\lambda_j\) is admissible, the contact captures into sticking. If it is not,
sticking is unavailable and the contact must continue on a kinetic branch.

At the instant of release or slip reversal,
\(v_{\mathrm{rel},j}=0\), so the relative velocity itself cannot yet choose
that kinetic branch. The relative acceleration does. It tells which way the
relative velocity will begin to move away from zero. For a candidate kinetic
sign \(\sigma_j\), consistency therefore requires
...
because \(\sigma_j\) is defined opposite to the sliding direction. The valid
kinetic branch is the one whose solved acceleration sends
\(v_{\mathrm{rel},j}\) into the corresponding direction of slip.

The order of the regime checks follows directly from these two tests. Suppose,
for example, that the CVT is in sliding--sliding and the primary relative
velocity reaches zero. The secondary sliding branch is retained provisionally
while the primary is tested as sticking. If the resulting static
...
branch consistent with \Cref{eq:sliding_onset_sign_consistency} is used
instead. In either case, the resulting traction pair defines a new common mechanical
response, and the conditions at both contacts are rechecked. If both contacts reach a regime boundary together, the
same logic is applied to the combined candidate regime rather than resolving
the two contacts independently.

These tangential transitions are nonimpulsive in the sense introduced in
\Cref{sec:mechanical_regimes_state_evolution}. The belt and pulley reach the
event with continuous position and velocity; changing the active contact law
changes the acceleration that follows rather than resetting the state
instantaneously.
