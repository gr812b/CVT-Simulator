...
retained direction.  For secondary rotation, the \(\omega_s\) column
has coefficient one in both the shaft-side and movable-side rotation
rows.  Their angular-momentum changes therefore enter together:
\begin{equation}
...
This is also where the upper-stop exchange described at the outset
reappears in the reduced balance.  The pin--track reaction changes the
shaft-side and movable-side angular momenta by equal and opposite
amounts as their relative rotation is removed.  Because both changes
enter the \(\omega_s\) direction with the same coefficient, that
internal helix impulse cancels from their sum.  It therefore requires
no additional component of \(\boldsymbol\Gamma\), even though the
transfer remains present in the two individual momentum changes.

% F6
The shift column shows why the coefficients need not be unity.  In the
engaged map, a shift rate contributes
\((d\theta_s/ds)\dot s\) to the movable secondary's angular velocity,
so its angular-momentum change contributes
\[
    \frac{d\theta_s}{ds}\Delta H_{s,M}
\]
to the shift-direction balance.  The other entries of the shift column
act in the same way on the translational and rotational momentum
changes produced by shift.  These are the same kinematic conversion
factors used by the compatible-work relations: a coefficient that maps
a retained velocity into a physical velocity also maps that physical
momentum change back into the corresponding retained direction.
...
For the upper primary stop, the same system now resolves the secondary-speed
...
The rows identify the retained direction in which the generalized momentum
acts, while the columns identify the retained velocity that causes it.  The
upper-left entry is therefore caused by \(\omega_s\) and acts in the
\(\omega_s\) direction: both secondary assemblies rotate with the shaft.
The upper-right entry is caused by \(\dot s\) but also acts in the
\(\omega_s\) direction, because shift rotates the movable assembly through
the helix.  The lower-left entry is the reciprocal coupling, in which shaft
rotation contributes to the shift-direction momentum.  The lower-right entry
is caused by \(\dot s\) and acts in the shift direction, giving the movable
