...
\chapter{Reference Inputs and Shaft-Boundary Models}
...
rotational inertia referred to that shaft.  In the notation of the formulation,
a boundary evaluation returns
\begin{equation}
\mathcal{B}_j(t,\vect{x},\vect{x}_h)
\;\longmapsto\;
\left(\tau_{j,\mathrm{ext}},I_{j,\mathrm{ext}}\right),
\qquad j\in\{p,s\},
...
\begin{equation}
\tau_{j,\mathrm{ext}}=\tau_{j,0},
\qquad
I_{j,\mathrm{ext}}=I_{j,0},
...
the selected \(\tau_{j,0}\) and \(I_{j,0}\) values are listed with those cases in
...
component,
\begin{equation}
\tau_{p,\mathrm{ext}}=\tau_{\mathrm{eng}}(\omega_p),
\qquad
I_{p,\mathrm{ext}}=I_{\mathrm{eng}},
...
\end{equation}
The boundary forms used in the Results campaign include
\begin{align}
\tau_{\mathrm{add}}(t)
&=\tau_{\mathrm{base}}(t)+u(t)\,\Delta\tau,\\
\tau_{\mathrm{target}}(t)
&=[1-u(t)]\tau_{\mathrm{base}}(t)+u(t)\tau_{\mathrm{target}},\\
\tau_{\mathrm{scale}}(t)
&=[1+u(t)(k_\tau-1)]\tau_{\mathrm{base}}(t),\\
\tau_{\mathrm{prescribed}}(t)
&=[1-u(t)]\tau_i+u(t)\tau_f.
...
This appendix collects the study-specific case definitions and numerical settings that support the Results chapter.  The working paper references the
Results tree on \ResultsTag; the single ref used by \verb|\ResultsCode| and
\verb|\ResultsTree| will be replaced by the frozen Results revision before
publication.

A shared Results decoder is used by the Baja-reference studies.  It keeps the
secondary helix force, spring, inertia, and kinematics of CINDER v1.1.2 but
allows the signed helix reaction to pass through zero to the opposite slot
flank; see
...
The results reported in \Cref{sec:results_mechanical_invariants} use the case
set defined by
...
The standard constructed bench cases use constant referred inertias of
$ I_{p,0}=\SI{0.03}{\kilogram\meter\squared}$ and
$ I_{s,0}=\SI{0.05}{\kilogram\meter\squared}$.  The two targeted anchors
use the explicit boundary values in \Cref{tab:invariants_rare_anchor_boundaries}.
...
Canonical contact bench & $3\times10^{-5}$ & $3\times10^{-8}$ & 1 ms & 30 ms \\
Targeted contact anchors & $3\times10^{-5}$ & $3\times10^{-8}$ & 0.5 ms & 30 ms \\
...
The geometry-domain audit uses 401 points over the physical shift range and a
\SI{1e-6}{\meter} finite-difference displacement for derivative checks.
...
Configuration details for \Cref{sec:results_energy_consistency} are defined in
\ResultsCode{studies/energy-consistency/study.json}{energy-consistency/study.json}.
The physical case is the reference configuration of
\Cref{app:common_physical_configuration} with the reference engine and
flat-road vehicle boundaries of
\Cref{app:primary_engine_boundary,app:secondary_vehicle_boundary}.
...
Maximum final $\lvert R_E\rvert/\lvert W_{\mathrm{ext}}\rvert$ & $10^{-4}$ \\
...
Configuration details for \Cref{sec:results_solver_convergence} are defined in
\ResultsCode{studies/solver-convergence/study.json}{solver-convergence/study.json}.
The benchmark trajectory uses the reference configuration and reference shaft
boundaries of \Cref{app:reference_configuration}.
