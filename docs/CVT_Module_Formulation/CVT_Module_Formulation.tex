...
% Replacement ending for Section 3.6.2
...
The stop reactions derived in
\Cref{sec:structural_regime_governing_equations} provide that condition
directly. For any of the three stops, the supported regime remains admissible while
...
primary axial equation in
\Cref{eq:deadzone_primary_axial_motion} at the same instantaneous state. Let
\(\ddot s_{\mathrm{cf}}\) denote the acceleration returned by this contact-free
evaluation. It is not used to evolve the seated regime; it is only the acceleration the
primary would have if belt contact were removed at that instant. Since positive \(s\)
closes the primary, separation into the deadzone requires
...
Consider free engaged upshift reaching the upper primary stop at \(s=s_{\max}\) with
\(\dot s^- > 0\). Immediately after the stop becomes active, the supported regime
requires
...
The continuous upper-stop equations cannot produce this change. They begin with the stop
already active and use \(\dot s=\ddot s=0\) to determine the subsequent shaft and belt
accelerations. The finite change from \(\dot s^-\) to \(\dot s^+\) occurs before that
continuous solution can begin.

Setting \(\dot s\) to zero by itself would satisfy the stop kinematically, but it would
not describe what the impact did. During engaged shift, \(\dot s\) is also the primary
movable-sheave speed, drives the flyweight pivot motion, moves the secondary through
\(x_s(s)\), rotates the secondary movable assembly through \(\theta_s(x_s)\), and gives
the belt material on both wraps a radial velocity through \(r_j(s)\). Each of those
motions belongs to a mass or rotational inertia. One instant before impact they
therefore store kinetic energy and contribute inertially to the impulse--momentum
balance. Arresting the shift must account for that stored motion rather than simply
deleting the scalar state entry that describes it.

The fully open primary stop at \(s=0\) presents the same basic problem in the deadzone:
an incoming primary shift velocity is incompatible with the stopped state. The low-ratio
boundary at \(s=s_{\mathrm{dz}}\) shows why the state transfer cannot be reduced to a
stop rule for \(\dot s\). During engaged backshift, \(\dot s^-<0\) means that the
primary is opening while the secondary is closing and rotating relative to its shaft
through the helix. When the secondary reaches its fully closed stop, the secondary axial
and helix-relative motions cannot continue, but there is no primary stop preventing the
primary from opening farther into the deadzone.

A blanket reset \(\dot s^+=0\) would therefore arrest primary motion that the outgoing
deadzone can support. Copying \(\dot s^-\) unchanged would do the opposite: it would
continue secondary axial, helix-relative, and belt-wrap radial motions that the deadzone
does not support. The stop acts on physical hardware, not on the symbol \(s\). The
shared coordinate is useful because it describes the coupled shift, but it does not
itself carry momentum.

The post-event state must therefore be built from the motions of the physical masses and
inertias represented by the reduced state. Their incoming motion can redistribute into
physical velocities that remain available after the event, while the newly active
constraint supplies the impulse needed to remove incompatible motion. Under the
perfectly inelastic idealization of \AssRef{assump:inelastic_constraint_activation}, any
kinetic energy not retained in the compatible outgoing motion is the unresolved impact
loss.

\subsubsection{Physical Velocities Carried into the Event}
\label{sec:boundary_event_physical_velocities}

The state already contains the four rates from which those physical motions are
constructed. Collect them as
...
Here \(\boldsymbol{\nu}\) is only the reduced velocity vector carried by the CVT state.
Its entries are not a list of all material velocities in the mechanism. A single entry,
especially \(\dot s\), can appear in several different physical motions.

The primary provides the simplest examples. The shaft-carried hardware and flyweight
mass distribution rotate at \(\omega_p\), with the configuration-dependent shaft-axis
inertia \(J_{f,p}(s)\) introduced in \Cref{sec:primary_pulley_rotational_dynamics}. The
primary movable assembly translates at \(\dot s\), and the flyweight pivot motion
follows the same mechanism map used in the axial balance,

\begin{equation}
    \dot q_f
    =
    \frac{dq_f}{ds}\dot s.
    \label{eq:boundary_event_flyweight_velocity}
\end{equation}

The secondary adds a rotational coupling. From \Cref{eq:secondary_movable_sheave_speed},
its movable assembly has absolute angular speed
...
\begin{equation}
    ...
    \frac{d\theta_s}{ds}
    =
    \frac{d\theta_s}{dx_s}\frac{dx_s}{ds},
    \label{eq:boundary_event_secondary_movable_speed}
\end{equation}

while its axial speed is

\begin{equation}
    \dot x_s
    =
    \frac{dx_s}{ds}\dot s.
    \label{eq:boundary_event_secondary_axial_speed}
\end{equation}

The distinction matters at a stop. Removing \(\dot s\) removes both the secondary
translation and the helix-relative part of \(\omega_{s,M}\), but it need not remove the
shaft speed \(\omega_s\). The angular momentum carried by the movable assembly can
therefore participate in the impulsive redistribution instead of being silently
discarded.

The belt carries one more shift-associated motion. For the belt element on pulley \(j\),
...
\begin{equation}
    ...
    +
    v_b\hat{\mathbf e}_\theta.
    \label{eq:boundary_event_wrap_velocity_recall}
\end{equation}

The tangential component \(v_b\) is the transport speed of the complete belt. During
engaged shift, the material carried on each circular wrap also moves radially with

\begin{equation}
    \dot r_j
    =
    \frac{dr_j}{ds}\dot s.
    \label{eq:boundary_event_wrap_radial_speed}
\end{equation}

Using the element mass in \Cref{eq:wrap_element_mass}, the belt mass carried on wrap
\(j\) is
...
\begin{equation}
    ...
    \qquad
    j\in\{p,s\},
    \label{eq:boundary_event_wrap_mass}
\end{equation}

so the retained wrap-radial kinetic energy is
...
\begin{equation}
    ...
    \left(
        \frac{dr_j}{ds}\dot s
    \right)^2.
    \label{eq:boundary_event_wrap_radial_energy}
\end{equation}

This term is distinct from the complete-belt transport energy \(m_bv_b^2/2\): the two
speeds are the orthogonal components already visible in
\Cref{eq:boundary_event_wrap_velocity_recall}. It also reflects the fidelity of the
prescribed belt path rather than a claim that these are the only motions a real belt can
possess. Under \AssRef{assump:prescribed_belt_path}, the model retains the radial motion
of material carried on the circular wraps but omits additional distributed inertia from
straight-span reorientation, out-of-plane skew and twist, and axial belt motion. The
straight spans still participate in the common tangential transport represented by
\(v_b\).

The retained physical velocity modes are collected in
\Cref{tab:boundary_event_physical_velocity_inventory}.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}
        ...
        @{}}
        \toprule
        \textbf{Retained motion}
        & \textbf{Physical velocity}
        ...
        \bottomrule
    \end{tabular}
    \caption{Physical velocity modes retained during a shift-regime boundary event. Equivalent inertias supplied through the primary or secondary shaft boundary are included with the corresponding shaft-side rotational mode.}
    \label{tab:boundary_event_physical_velocity_inventory}
\end{table}

The primary flyweight entries use the mechanism maps \(q_f(s)\), \(J_{f,p}(s)\), and
\(I_f\) already introduced in the main actuator and pulley dynamics rather than the
detailed roller geometry that generated them. The event law therefore depends on the
kinetic modes supplied by the actuator model, not on the geometry of one particular
flyweight implementation. An alternative actuator described by its own
configuration-dependent velocity and inertia maps enters the same projection without
changing its form.

The second column of \Cref{tab:boundary_event_physical_velocity_inventory} is the
physical information needed by the event law. Stack those scalar linear and angular
velocities into a vector \(\mathbf y\). For the engaged-side ordering shown in the
table,

\begin{equation}
    \mathbf y_{\mathrm{eng}}
    =
    \begin{bmatrix}
        \omega_p \\
        \dot s \\
        ...
        (dr_s/ds)\dot s
    \end{bmatrix}.
    \label{eq:boundary_event_engaged_physical_velocity_vector}
\end{equation}

Each entry \(y_k\) is therefore one physical velocity mode paired with the mass or
rotational inertia in the third column of the table. In the deadzone, the same physical
inventory is retained, but the secondary axial, helix-relative, and two wrap-radial
shift coefficients are zero. At a fixed event configuration every listed speed is linear
in the four reduced rates of \(\boldsymbol\nu\), so define the kinematic velocity map
\(\mathbf J\) by
...
\begin{equation}
    ...
    J_{k\ell}
    \equiv
    \frac{\partial y_k}{\partial \nu_\ell}.
    \label{eq:boundary_event_physical_velocity_map}
\end{equation}

\(\mathbf y\) therefore records how the retained physical masses and inertias are
actually moving, while \(\mathbf J\) records how the reduced state velocities produce
those motions. The matrix is only a compact stacking of the kinematic relations already
shown. For example, during engaged shift the secondary movable rotation and primary-wrap
radial motion contribute the rows

\begin{equation}
    \mathbf j_{s,M,\mathrm{eng}}
    =
    \begin{bmatrix}
        0 & 1 & 0 & d\theta_s/ds
    \end{bmatrix},
    \qquad
    \mathbf j_{b,r,p,\mathrm{eng}}
    =
    \begin{bmatrix}
        0 & 0 & 0 & dr_p/ds
    \end{bmatrix}.
    \label{eq:boundary_event_representative_J_rows}
\end{equation}

In the deadzone, the corresponding shift couplings vanish because the secondary axial
position, helix angle, and represented belt radii are fixed. The same physical
components are retained, but the relevant rows of the deadzone-side map contain zero
shift coefficients. An event crossing between two such kinematic descriptions therefore
uses one map for the incoming motion and one for the outgoing motion:
...
\begin{equation}
    ...
    =
    \mathbf J^+\boldsymbol\nu^+,
    \label{eq:boundary_event_pre_post_velocity_maps}
\end{equation}

where the superscripts \(-\) and \(+\) denote the instants immediately before and after
the event. A physical mode that becomes kinematically inactive is represented by a zero
row coefficient in \(\mathbf J^+\), rather than by deleting its mass or inertia from the
event. A change in \(\mathbf J\) can therefore remove or activate physical motion as the
kinematic topology changes. Additional relations among the outgoing reduced velocities,
such as a metal stop that arrests the entire shift coordinate or a retained no-slip
condition, are imposed separately below.

Let \(\mathbf W\) be the diagonal kinetic-weight matrix associated with the same
ordering of \(\mathbf y\): a linear velocity entry is weighted by its mass, and an
angular velocity entry by its rotational inertia. The same physical component basis is
used on both sides of the event. Its kinetic energy is therefore

\begin{equation}
    \mathcal T^{\pm}
    =
    \frac{1}{2}
    (\mathbf y^{\pm})^{\mathsf T}
    \mathbf W
    \mathbf y^{\pm}
    =
    \frac{1}{2}
    (\boldsymbol\nu^{\pm})^{\mathsf T}
    (\mathbf J^{\pm})^{\mathsf T}
    \mathbf W
    \mathbf J^{\pm}
    \boldsymbol\nu^{\pm}.
...
The combination \((\mathbf J^{\pm})^{\mathsf T}\mathbf W\mathbf J^{\pm}\) is the reduced
kinetic metric seen by \(\boldsymbol\nu^{\pm}\). It is not an independently assumed
inertia. It is what results after the masses and inertias in
\Cref{tab:boundary_event_physical_velocity_inventory} are expressed through the four
state velocities.

\subsubsection{Impulse and the Outgoing Velocity Constraints}
\label{sec:boundary_event_impulse_projection}

The physical velocity map tells us how every retained mass and inertia is moving as the
boundary is reached. The outgoing regime supplies a different kind of information: which
velocities are forbidden after the event. A metal stop requires the stopped shift
velocity to vanish; an impulsively retained sticking contact requires its belt--pulley
relative velocity to vanish; the deadzone closure requires the secondary--belt relative
velocity to vanish. Each is a scalar linear condition on the outgoing reduced velocity.
Represent one such condition by a row vector \(\mathbf c^+\), chosen so that

\begin{equation}
    \mathbf c^+\boldsymbol\nu^+=0
    \label{eq:boundary_event_single_constraint}
\end{equation}

is the physical velocity condition imposed immediately after the event. Stacking every
active row gives the outgoing constraint matrix \(\mathbf C^+\):

\begin{equation}
    \mathbf C^+\boldsymbol\nu^+
    =
    \mathbf 0.
    \label{eq:boundary_event_outgoing_constraints}
\end{equation}

Thus \(\mathbf C^+\) does not describe inertia or force. Its rows identify the outgoing
relative velocities that the newly active mechanical constraints require to vanish.

For the ordering in \Cref{eq:boundary_event_velocity_vector}, a metal stop contributes

\begin{equation}
    \mathbf c_{\mathrm{stop}}
    =
    \begin{bmatrix}
        0 & 0 & 0 & 1
    \end{bmatrix},
    \label{eq:boundary_event_stop_constraint_row}
\end{equation}

which imposes \(\dot s^+=0\) when the complete engaged or deadzone shift coordinate is
arrested. Tangential no-slip constraints are written using the same representative
contact velocities as the continuous contact mechanics. Since the primary has no
helix-relative face rotation,

\begin{equation}
    \mathbf c_{p,\mathrm{stick}}
    =
    \begin{bmatrix}
        -r_{p,\mathrm{eff}} & 0 & 1 & 0
    \end{bmatrix},
    \label{eq:boundary_event_primary_stick_row}
\end{equation}

while \Cref{eq:representative_contact_angular_speed} gives
\(\bar\omega_s=\omega_s+\dot\theta_s/2\) on the secondary, so

\begin{equation}
    \mathbf c_{s,\mathrm{stick}}
    =
    \begin{bmatrix}
        0 &
        -r_{s,\mathrm{eff}} &
    ...
    \end{bmatrix}.
    \label{eq:boundary_event_secondary_stick_row}
\end{equation}

These rows impose the instantaneous no-slip relation
\(v_b=r_{j,\mathrm{eff}}\bar\omega_j\). Under \AssRef{assump:impact_tangential_contact},
only a pulley contact already sticking immediately before the event contributes its
no-slip row to the impulse problem. A sliding contact contributes no impulsive stick
condition, and a newly formed pulley contact is not automatically made to stick. The
ordinary tangential contact closure is evaluated again after the projection to determine
the continuous regime that follows.

The deadzone secondary-belt lock is different. It is not a preserved stick/slip state
but the reduced belt closure that defines deadzone motion. Whenever the outgoing shift
regime is deadzone, it contributes

\begin{equation}
    \mathbf c_{s,\mathrm{dz}}
    =
    \begin{bmatrix}
        0 &
        -r_{s,\mathrm{eff,dz}} &
        ...
        0
    \end{bmatrix},
    \label{eq:boundary_event_deadzone_lock_row}
\end{equation}

so that \(v_b^+=r_{s,\mathrm{eff,dz}}\omega_s^+\), consistent with
\Cref{eq:deadzone_secondary_lock}.

Each row of \(\mathbf C^+\) is enforced by an impulsive reaction during the unresolved
event. Let \(\boldsymbol\Gamma\) collect those constraint impulses, with one entry
corresponding to each row of \(\mathbf C^+\). Applied shaft torques, springs, and other
bounded forces act over no resolved duration and therefore contribute no finite impulse
under \AssRef{assump:inelastic_constraint_activation}.

Multiplying a physical velocity change by its corresponding entry of \(\mathbf W\) gives
the linear or angular momentum change of that retained mode. The vector

\[
\mathbf W \left( \mathbf y^+-\mathbf y^- \right)
\]

therefore collects the mass- or inertia-weighted velocity changes of the physical
inventory in \Cref{tab:boundary_event_physical_velocity_inventory}. Because the outgoing
reduced velocity \(\boldsymbol\nu^+\) moves those physical components through \(\mathbf
J^+\), premultiplying by \((\mathbf J^+)^{\mathsf T}\) gathers the same momentum changes
into the outgoing reduced directions. Impulse--momentum balance then gives

\begin{equation}
    (\mathbf J^+)^{\mathsf T}\mathbf W
    \left(
        \mathbf J^+\boldsymbol\nu^+
        -
        \mathbf J^-\boldsymbol\nu^-
    \right)
    +
    (\mathbf C^+)^{\mathsf T}\boldsymbol\Gamma
    =
    \mathbf 0.
    \label{eq:boundary_event_impulse_balance}
\end{equation}

The first term accounts for the combined inertial momentum change of all retained
physical modes. The second accounts for the impulses supplied by the velocity
constraints that become active through the event. The stop is therefore allowed to
absorb momentum in the motion it arrests, while coupled shaft, belt, and mechanism
velocities can change together wherever the outgoing kinematics permit it.

Combining \Cref{eq:boundary_event_impulse_balance} with the outgoing constraints in
\Cref{eq:boundary_event_outgoing_constraints} gives
...
=
\begin{bmatrix}
    (\mathbf J^+)^{\mathsf T}\mathbf W\mathbf J^-\boldsymbol\nu^-
    \\
    \mathbf 0
\end{bmatrix}.
...
The position is continuous through the event, so \(s^+=s^-\). The four outgoing
velocities are not copied or reset independently;
\Cref{eq:boundary_event_velocity_projection} solves them together with the impulses
required by the outgoing constraints.

The perfectly inelastic idealization does not require kinetic energy to be conserved.
Instead,
...
The momentum projection determines how much of the incoming motion remains as compatible
shaft, belt, and shift motion. The difference \(E_{\mathrm{event}}\) is the kinetic
energy removed by the unresolved capture and represents the local deformation, damping,
heat, sound, and structural vibration excluded by the instantaneous rigid-event model.
This energy check is also essential numerically: the state transfer may dissipate
kinetic energy, but it must not create it.

\subsubsection{Shift-Regime Event Rules}
\label{sec:shift_regime_transition_applications}

Each physical boundary now reduces to the same information: which kinematic map
describes the moving components immediately before the event, which map describes them
immediately afterward, and which outgoing relative velocities must vanish. Those choices
determine \(\mathbf J^-\), \(\mathbf J^+\), and the rows of \(\mathbf C^+\) in
\Cref{eq:boundary_event_velocity_projection}.

\paragraph{Fully open primary stop.}
Free deadzone opening reaches \(s=0\) with \(\dot s^-<0\). The physical topology remains
deadzone on both sides, so \(\mathbf J^-\) and \(\mathbf J^+\) use the same deadzone
mapping. The outgoing constraints contain the primary stop row and the already active
deadzone secondary-belt lock. The projection arrests the primary shift motion without
independently resetting \(\omega_p\), \(\omega_s\), or \(v_b\). The lower-stop closure
is then evaluated at the projected state to recover \(F_{\mathrm{stop},0}\). If the
required reaction is already inadmissible, the impact has still occurred, but no
sustained stop-supported interval follows and the system immediately returns to free
deadzone motion.

\paragraph{Closing from deadzone into engagement.}
Closing motion reaches \(s=s_{\mathrm{dz}}\) with \(\dot s^->0\). Before contact, \(\dot
s\) moves the primary only; the secondary axial position, helix-relative angle, and
represented belt radii are fixed. Immediately after engagement, those physical motions
become coupled to the outgoing shift speed through the engaged mapping. Thus \(\mathbf
J^-\) is the deadzone map and \(\mathbf J^+\) is the engaged map. Carrying \(\dot s^-\)
straight across would instantaneously create secondary and wrap-radial kinetic energy
without accounting for the momentum needed to accelerate those modes.

The secondary-belt relation already imposed in the deadzone is retained through the
capture, expressed on the engaged side with the representative secondary contact speed.
The new primary contact contributes no impulsive stick row merely because the sheaves
have touched. After the projection, the engaged closure evaluates the primary and
secondary relative speeds and selects the admissible continuous tangential contact
regime.

\paragraph{Engaged backshift to the low-ratio boundary.}
Free engaged backshift reaches \(s=s_{\mathrm{dz}}\) with \(\dot s^-<0\). The secondary
movable sheave is the member that reaches the metal stop. Its axial motion and its
helix-relative rotation cannot continue beyond the boundary, while the primary has no
opening stop and can continue into the deadzone.

For a finite-speed return with a real deadzone, the outgoing map is therefore the
deadzone map rather than an engaged map with a global \(\dot s^+=0\) constraint. Here
\(\mathbf J^-\) uses the engaged kinematics and \(\mathbf J^+\) the deadzone kinematics.
That change removes secondary axial motion, secondary helix-relative rotation, and both
retained wrap-radial velocities while leaving primary axial motion available. The
outgoing deadzone secondary-belt lock is imposed through \(\mathbf c_{s,\mathrm{dz}}\).

This projection also clarifies where the incoming secondary movable-sheave motion goes.
The axial component that is incompatible with the fully closed secondary is taken by the
structural impact. The helix-relative angular component is not assigned solely to the
secondary shaft: \(\omega_s^+\) and \(v_b^+\) are coupled by the outgoing secondary-belt
lock, so the compatible secondary shaft and belt transport velocities are solved
together. The primary shift velocity and its coupled flyweight motion remain available
degrees of freedom in the deadzone. The belt itself does not stop; only the radial wrap
motion associated with changing radii disappears because those radii are fixed after
separation.

If the return reaches the limiting state with no finite shift motion left to remove and
both the primary contact and secondary stop remain admissible, the continuous successor
is the engaged low-ratio seat rather than free deadzone motion.

\begin{remarkbox}[Remark (Rigid low-ratio capture)]
With perfectly rigid geometry and perfectly inelastic captures, repeated secondary-stop impacts and primary make--break events can form a shrinking sequence as the remaining removable shift kinetic energy approaches zero. This sequence does not introduce another mechanical regime. A numerical implementation may complete the limiting capture when the additional kinetic energy removed by seating falls below its working energy resolution; that threshold closes the rigid-event limit numerically rather than introducing a new physical parameter.
\end{remarkbox}

An established low-ratio seat can later leave by either release mechanism derived
earlier. If the secondary stop reaction crosses zero, the seat releases into free
engaged shift without an impact. If primary contact separates, loss of that contact is
itself nonimpulsive because the seated shift velocity is already zero. Entering the
adopted deadzone closure, however, also imposes the secondary-belt lock. When the
pre-event secondary contact already satisfies that relation, the complete transition
remains velocity-continuous. If it was sliding, the lock activation requires the same
impulse projection even though the primary-contact release itself did not.

\paragraph{Upper primary stop.}
Free engaged upshift reaches \(s=s_{\max}\) with \(\dot s^->0\). The engaged kinematic
map remains active, but the stop adds \(\mathbf c_{\mathrm{stop}}\), so \(\dot s^+=0\).
A pulley contact already sticking contributes its corresponding no-slip row under
\AssRef{assump:impact_tangential_contact}; a sliding contact does not.

Although the metal stop acts on the primary, arresting \(\dot s\) also removes every
engaged motion coupled to the shared shift coordinate: primary translation and flyweight
pivot motion, secondary translation, secondary helix-relative rotation, and both
belt-wrap radial velocities. Their incoming momenta therefore participate in the common
projection. In particular, the secondary movable assembly can transfer part of its
helix-associated angular momentum into compatible secondary-shaft motion; when a
sticking belt constraint is active, that redistribution can also couple into belt
transport and the opposite shaft. None of these velocities is reset independently.

The fixed-shift engaged closure is then evaluated at the projected state. The tangential
contact regime is selected from that state, and \(F_{\mathrm{stop,max}}\) is recovered
from \Cref{eq:upper_stop_reaction}. A nonnegative reaction admits the sustained
upper-stop regime; an inadmissible reaction gives an immediate nonimpulsive release back
into free engaged shift after the impact state has been established.

The exact geometric arrival surfaces are
...
Their physical meaning depends on the active regime and crossing direction. After every
impulsive projection, the candidate outgoing continuous closure is evaluated at
\(\boldsymbol\nu^+\). Stop reactions must remain nonnegative, the low-ratio primary
contact must satisfy the separation conditions already derived, and each engaged pulley
contact must satisfy the tangential admissibility conditions of
\Cref{sec:contact_admissibility_transitions}. The projection determines the mechanically
compatible instantaneous state; these admissibility conditions determine which
continuous mechanical regime can actually continue from it.

The resulting shift-regime transitions are summarized in
\Cref{tab:shift_regime_transition_summary}.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}
        ...
        \toprule
        \textbf{Transition}
        & \textbf{Event condition}
        & \textbf{State transfer}
        \\
        \midrule
        Free deadzone to fully open stop
        & \(s=0\), \(\dot s^-<0\)
        & Deadzone-to-deadzone inelastic projection with \(\dot s^+=0\); retain the secondary-belt lock.
        \\[4pt]
        Fully open stop to free deadzone
        & \(F_{\mathrm{stop},0}=0\), crossing toward inadmissibility
        & Nonimpulsive release; the free deadzone equations resume from the same position and velocity.
        \\[4pt]
        Deadzone to engaged motion
        & \(s=s_{\mathrm{dz}}\), \(\dot s^->0\)
        & Project from the deadzone to the engaged kinematic map; carry the existing secondary-belt relation through the capture; do not impose primary stick.
        \\[4pt]
        Engaged backshift to low-ratio boundary
        & \(s=s_{\mathrm{dz}}\), \(\dot s^-<0\)
        & At finite speed, project from engaged to deadzone so the secondary stop arrests its axial/helix motion while primary opening motion remains available; the zero-motion limit can settle on the engaged seat.
        \\[4pt]
        Low-ratio seat to free engaged shift
        & \(F_{\mathrm{stop,LR}}=0\), crossing toward inadmissibility
        & Nonimpulsive release of the secondary stop.
        \\[4pt]
        Low-ratio seat to deadzone
        & \(N_p=0\) and \(\ddot s_{\mathrm{cf}}<0\)
        & Primary contact releases without its own impulse; activate the deadzone secondary-belt lock, projecting only if that newly imposed lock is not already satisfied.
        \\[4pt]
        Free engaged shift to upper stop
        & \(s=s_{\max}\), \(\dot s^->0\)
        & Engaged-to-engaged inelastic projection with \(\dot s^+=0\); retain only no-slip constraints already active before impact.
        \\[4pt]
        Upper stop to free engaged shift
        & \(F_{\mathrm{stop,max}}=0\), crossing toward inadmissibility
        & Nonimpulsive release; the free engaged equations resume from the same position and velocity.
        \\
        \bottomrule
    \end{tabular}
    \caption{Shift-regime event conditions and state-transfer rules. Tangential contact-regime changes remain governed by \Cref{sec:contact_admissibility_transitions}.}
    \label{tab:shift_regime_transition_summary}
\end{table}

Once the projected state satisfies the successor's unilateral and tangential
admissibility conditions, the corresponding continuous equations resume from the
continuous position \(s^+=s^-\) and the transferred velocity \(\boldsymbol\nu^+\).
