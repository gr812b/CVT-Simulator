...
A launch can exercise engagement, slip and shifting without bringing either
pulley into both directions of sliding or approaching every travel boundary
from free motion. The audit therefore starts the CVT from additional
admissible states and asks whether the coupled balances and contact conditions
remain satisfied as each state evolves, including when its initial arrangement
gives way to another.

It combines a ten-second, flat-road Baja launch with
constructed cases using constant shaft torques and attached inertias. The
launch uses the engine and vehicle boundaries of
\Cref{app:primary_engine_boundary,app:secondary_vehicle_boundary}; the
constructed cases use \Cref{app:fixed_shaft_boundary}, with both shaft speeds
still determined dynamically. All retain the reference CVT and its
zero-clearance bilateral secondary helix
(\Cref{app:common_physical_configuration,app:reference_helix_topology}). The
additional cases exercise rest, deadzone motion, free shift, travel supports
and the different tangential contact states. The exact inputs, checks and
coverage record are given in \Cref{app:study_mechanical_invariants}.

All 25 required coverage classes were reached. The mechanical checks passed
at 6,257 sampled states and at each of the 81 exact post-transition states.
Coverage records which arrangements were reached; the checks test whether
their motion remains mechanically consistent.

An admissible contact arrangement need not persist for long.
\Cref{fig:verification_contact_regimes}(a) shows the time to the first change
from each of ten selected initial contact states. A positive slip sign means
the belt leads the representative pulley surface; a negative sign reverses
that relation, following
\Cref{eq:tangential_relative_speed}. The right-pointing markers indicate
cases that retain their initial arrangement for the entire \SI{30}{ms}
observation window. Their durations are lower bounds, rather than measured
exit times.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/results/verification/contact_regimes.pdf}
  \caption{Persistence and local contact loading for ten canonical initial
  contact states. (a) Time to first transition; orange marks the supplied
  initial-state anchors. (b) Least normal loading per unit wrap angle on each
  pulley over engaged states in the \SI{30}{ms} continuation. Minima may follow
  a contact change and need not coincide between pulleys. Deadzone intervals
  are excluded because they have no engaged full-wrap field.}
  \label{fig:verification_contact_regimes}
\end{figure}

The secondary-slip \((+)\) and both-slip \((-,+)\) cases instead leave
their initial arrangements after approximately \(414~\mu\mathrm{s}\) and
\(9.82~\mu\mathrm{s}\). In both, the secondary relative speed reaches
zero and the required static traction lies within its available capacity, so
the secondary begins sticking. The primary remains sticking in the first
case and sliding in the second. These two initial states were supplied as
targeted reproduction anchors, but short duration is not confined to them:
the ordinary search also finds both-slip \((+,+)\), which reaches secondary
sticking after \(31.7~\mu\mathrm{s}\). A brief branch can therefore be a
resolved part of an admissible continuation. Neither its duration nor the
way its initial state was found measures how frequently it occurs in a
vehicle drive.

The local loading in panel~(b) tests a different requirement. The sheaves may
carry a substantial total normal force while one part of a wrap is only
lightly loaded. Since \(N_j\) is the integral of that loading, its positive
value alone cannot establish that contact is maintained everywhere. In the
case initialized in both-slip \((-,-)\), the primary's least local loading
falls to \SI{0.0482}{N/rad}. At that same state, the integrated primary
normal force is still approximately \SI{411}{N}. This state occurs just
after the primary sliding direction reverses at \SI{1.757}{ms}: the belt
and primary momentarily match speeds, then their relative motion reverses.
Reversing sliding reverses the kinetic traction, changing how tension varies
along the wrap and, through the radial balance, how its normal load is
distributed. Evaluating both sides of the change therefore matters even
when the velocities remain continuous.

The small positive local load remains admissible under the prescribed
full-wrap description. A negative value would require the sheave to pull on
the belt; similarly, negative tension would require the flexible belt to
carry compression. As established in \Cref{eq:wrap_contact_admissibility},
those outcomes would require local lift-off or slack, which the retained
belt path does not describe. They would not simply select another
stick--slip branch. The audit thus checks both continuation among the
represented contact arrangements and continued use of the belt description
itself. These are verified for the sampled cases, without extending the
result to every admissible initial state or every possible transition.
\FloatBarrier

\subsection{Energy Consistency}
\label{sec:results_energy_consistency}

The shaft loading does more than transmit power through the CVT. During a
launch it also accelerates the retained inertias and changes spring
deformation. An energy balance must therefore distinguish stored energy
from dissipation, and continue to hold when an event changes the velocities.
The audit follows the ten-second, flat-road reference launch using the
settings in \Cref{app:study_energy_consistency}.

The stored-energy inventory includes the retained CVT motions, the attached
engine and referred vehicle inertias, and conservative actuator-spring
energy. The work supplied to this inventory is the signed net shaft work,
\begin{equation}
  W_{\mathrm B}(t)
  =\int_0^t
...
\end{equation}
Relative sliding removes mechanical energy through the work of kinetic
traction. A perfectly inelastic capture instead removes kinetic energy
instantaneously as the new constraint is established, according to
\Cref{eq:finite_event_energy_loss}. Work and slip loss are consequently
integrated within each continuous segment, while capture losses are added
at the events. Both sides of each velocity reset are retained. The remainder
\(R_E\) is the unexplained balance residual; it is not assigned to another
loss mechanism.

\Cref{fig:verification_energy_balance}(a) shows the work increasing mainly
as stored energy. By ten seconds, stored energy has increased by
approximately \SI{50.58}{kJ}, while \SI{1.588}{kJ} has been dissipated by
kinetic slip.
The cumulative capture loss is only \SI{0.254}{J}, but this is about ten
times the final residual. Its small share of the launch work therefore does
not make it negligible for this accounting check. The 13 nominal capture
projections reproduce their stored-energy drops to within
\(2.27\times10^{-13}~\mathrm{J}\), checked separately from continuous work.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/results/verification/energy_balance.pdf}
  \caption{Energy accounting for the reference launch. (a) Nominal net work,
  stored-energy change, and storage plus modeled losses; work and accounted
  energy overlap at this scale. (b) Signed residuals for the nominal and
  tighter ODE trajectories, both audited at 0.625 ms maximum spacing.
  (c) Detail around each trajectory's upper-stop capture; the incoming and
  outgoing residuals coincide at the open markers. (d) Continuous-segment
  defect with the nominal trajectory held fixed.
  The open point and dashed line mark the two-finest-grid, second-order
  zero-spacing estimate. Native event sides are retained; lines and work
  quadrature do not bridge velocity resets.}
  \label{fig:verification_energy_balance}
\end{figure}

The kilojoule-scale overlap cannot resolve the accuracy of the balance.
Panel (b) instead shows its signed residual in joules. At the finest audit
spacing, the nominal final residual is \(+0.0254~\mathrm{J}\) against
\SI{52.17}{kJ} of net work, or \(\epsilon_E=4.88\times10^{-7}\) under
\Cref{eq:energy_final_balance_fraction}. The sampled residual remains
within \SI{0.040}{J} in both trajectories. Panel (c) resolves the apparent
jump near the upper stop: the residual is continuous across the native
capture and changes during the adjoining continuous integration. The visible
change belongs to the continuous-segment defect rather than a mismatch in
the capture-energy update. The final ledgers are compared in
\Cref{tab:verification_energy_ledger}; spring and inertial storage are kept
separate from the irreversible channels.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lrr@{}}
    \toprule
    Quantity & Nominal [J] & Tighter ODE [J] \\
    \midrule
    Net boundary work & \(52\,168.893\) & \(52\,168.858\) \\
    Stored-energy change & \(50\,580.274\) & \(50\,580.287\) \\
    Kinetic-slip dissipation & \(1\,588.340\) & \(1\,588.339\) \\
    Inelastic capture dissipation & \(0.253859\) & \(0.253884\) \\
    Balance residual & \(+0.025445\) & \(-0.022841\) \\
    \bottomrule
  \end{tabular}
  \caption{Energy ledgers at ten seconds, both evaluated at the finest audit
  spacing. Residuals use unrounded quantities; displayed entries are rounded.}
  \label{tab:verification_energy_ledger}
\end{table}

\begin{samepage}
Two refinements distinguish the numerical trajectory from the integration
of power along it. In panel (d), reducing the quadrature spacing from 5 to
0.625 ms leaves a finite continuous-segment defect, with a zero-spacing
estimate of \(+0.024511~\mathrm{J}\). Across these grids, finer quadrature reduces the defect without removing
it. Recomputing the trajectory
with tighter ODE settings changes the final residual to
\(-0.022841~\mathrm{J}\), while the maximum normalized final-state change
is only \(1.30\times10^{-7}\)
(\Cref{eq:energy_final_state_normalization}). Thus the small remainder is
sensitive to the trajectory integration as well as to work quadrature.
These two ODE settings do not isolate its source or establish an order of
convergence.\par
\end{samepage}

The result supports consistency of the retained energy accounting at the
observed residual scale. It does not establish a complete physical loss
budget: an omitted loss contributes to neither side of the computed balance,
so close agreement cannot establish that the omissions in
\AssRef{assump:no_parasitic_losses} are small. The residual fraction
measures accounting accuracy, not transmission efficiency. Convergence of
the motion and contact history requires the separate comparison that follows.
\FloatBarrier
...
\(\gamma=0\) throughout the 10 s interval. Both integrations use LSODA;
only the tolerances and maximum time step in
\Cref{tab:energy_study_numerics} change between them. The released kinetic
metric includes the attached engine and referred vehicle inertias, as well
as the retained CVT motions. Conservative axial- and torsional-spring
potentials complete the stored-energy inventory.
...
the net work supplied through the shaft boundaries. The 1 J denominator
floor is inactive in this launch.

\begin{samepage}
The ODE-refinement final-state metric compares the nominal and tight solutions componentwise. For
...
of the energy defect.\par
\end{samepage}


Power is integrated separately within each continuous hybrid segment using
composite trapezoids on retained dense output. For requested maximum spacing
\(h\), a segment of duration \(\Delta t_i\) receives
\(\max(\lceil\Delta t_i/h\rceil,12)\) intervals. Its stored-energy change
uses the exact native endpoint states. Equal-time incoming and outgoing
states are retained separately in both exported traces. No quadrature
interval crosses a reset: cumulative work and slip loss remain continuous,
while the recorded capture loss is added only on the outgoing side. Each
recorded capture is checked against its exact stored-energy drop; transitions
without a capture projection are retained in the complete event ledger too.

Let \(D(h)\) denote the sum of the continuous-segment defects
\(W_i-\Delta E_i-E_{\mathrm{slip},i}\). The four quadrature grids reuse the
same nominal trajectory. The reported second-order zero-spacing estimate is
\begin{equation}
  D_0=\frac{4D(h)-D(2h)}{3},
  \qquad h=0.625~\mathrm{ms},
  \label{eq:energy_quadrature_extrapolate}
\end{equation}
using only the two finest grids. This extrapolates the defect of the fixed
numerical trajectory; it is neither an error bar nor the quadrature error
remaining at \(h\). The tighter ODE trajectory is audited at the same
finest spacing. Signed work from velocity drift at a contact declared
sticking remains a numerical diagnostic. It is neither kinetic-slip
dissipation nor a correction to \(R_E\).
...
Maximum final balance fraction $\epsilon_E$ & $10^{-4}$ \\
...
ledgers and publication figure. Its \texttt{--plot-only} option checks the
retained input/output hashes, energy identities, both native event sides,
and refinement quantities before regenerating the figure. Exact executed
inputs, package versions and output hashes accompany the study's execution
record; the reproduction commands and figure provenance are documented
with the maintained study.
