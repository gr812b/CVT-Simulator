...
The reverse crossing exposes a different problem. Consider free engaged
upshift reaching the upper primary stop at $s=s_{\max}$ with
$\dot s^- > 0$, where superscripts $-$ and $+$ denote the limiting states
immediately before and after the boundary event. The mechanism arrives while
still shifting into the stop, whereas the supported regime admits only
...
The incoming velocity therefore cannot simply be carried into the next
continuous solution. Unlike the releases above, the boundary itself must
change the velocity state before the upper-stop equations can take over.

The incoming motion can become inadmissible for two different reasons. The
first is that the relation between the state velocities and the physical
hardware itself changes. At the low-ratio boundary $s=s_{\rm dz}$, an
engaged-to-deadzone transition has exactly this character. During engaged
shift, a nonzero $\dot s$ moves both pulley mechanisms: in particular,
...
\begin{equation}
    ...
    \frac{dx_s}{ds}\dot s,
    \qquad \text{engaged},
    \label{eq:finite_event_secondary_engaged_motion}
\end{equation}

and the same shift motion also drives the secondary helix rotation and the
changes in the represented belt radii. In the deadzone, $\dot s$ still moves
the primary, but the secondary remains fully closed,
...
\begin{equation}
    ...
    \dot x_s = 0,
    \qquad \text{deadzone}.
    \label{eq:finite_event_deadzone_motion}
\end{equation}

Thus the same numerical shift velocity represents different physical motion
on the two sides of this boundary. A finite-speed crossing cannot be handled
by merely retaining the same velocity numbers: momentum carried by secondary
shift, helix, or radial belt motion on the engaged side must be reconciled
with a deadzone state in which $\dot s$ no longer produces those motions.

A metal stop creates a different incompatibility. Reaching the upper primary
stop does not change what $\dot s$ means physically. If the stop were absent,
the same nonzero $\dot s$ would continue to produce the engaged shift motion.
The stop instead forbids that otherwise meaningful motion by imposing
\Cref{eq:upper_stop_post_event_velocity}. What happens to the momentum carried
into the event then depends on whether each physical motion has another
allowed way to continue.

The primary movable sheave provides the simplest case. Its axial velocity is

\begin{equation}
    \dot x_p = \dot s.
    \label{eq:finite_event_primary_axial_velocity}
\end{equation}

Once the stop requires $\dot s^+=0$, no remaining velocity can produce
primary axial motion, so $\dot x_p^+=0$. That motion has no surviving outlet;
its momentum must be arrested by the impulsive stop reaction.

The secondary movable sheave behaves differently because its absolute
rotation has two kinematic contributions. During engaged shift,
...
\begin{equation}
    ...
    \label{eq:finite_event_secondary_movable_velocity}
\end{equation}

Immediately before the stop, the movable sheave therefore carries rotational
motion at
...
\begin{equation}
    ...
    \qquad
    \omega_{s,M}^+ = \omega_s^+.
    \label{eq:finite_event_secondary_movable_before_after}
\end{equation}

The two sides of \Cref{eq:finite_event_secondary_movable_before_after} are not
being set equal. They identify the physical rotation available to the movable
sheave on each side of the event; the unknown value $\omega_s^+$ is still to
be determined. The helix-relative part cannot survive because $\dot s^+=0$,
but rotation with the secondary shaft can. If the helix-relative contribution
is positive, the movable sheave therefore arrives with more angular momentum
than it would carry at $\omega_s^-$ alone. Because the sheave can still rotate
through $\omega_s^+$, that additional angular momentum tends to increase the
outgoing secondary speed; a negative contribution tends to decrease it. The
exact change depends on the remaining coupled inertia and any other impulsive
restrictions active at the same event.

Tangential sticking is another restriction of this same kind, but it need not
involve $\dot s$. If a pulley interface is already sticking and that lock is
retained through the zero-duration event, its outgoing belt and pulley contact
velocities must remain equal. The two motions are therefore not free to emerge
with independent speeds, and the contact may exchange a tangential impulse to
maintain the lock. A metal stop and one or both retained sticking conditions
may act during the same event. Sliding interfaces and newly formed contacts
are not automatically constrained to stick.

A finite-speed regime change can therefore alter the admissible motion in two
distinct ways. It can change how the state velocities move the physical
hardware, as between engaged operation and the deadzone, and it can restrict
motions whose physical meaning is otherwise unchanged, as at a metal stop or
a retained sticking contact. Either effect changes where the momentum carried
into the boundary is able to go. Physical motions with an allowed outgoing
route can contribute their momentum to the surviving motion; when every such
route is removed, the required momentum change must instead be supplied by the
impulsive reactions that establish the outgoing regime.

Immediately before such an event, the incoming velocity state and the momentum
carried by the moving hardware are known. Immediately after it, the velocity
state is unknown but must obey the kinematics and restrictions of the outgoing
regime. The impact calculation must determine that outgoing state while
accounting for where each incoming physical motion can still exist and where
an impulsive reaction is required to remove or couple it.
