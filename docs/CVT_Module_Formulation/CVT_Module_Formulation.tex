...
the axial coordinate \(x_p\). Ramp contact connects the two. While that
contact is maintained, each sheave position fixes the corresponding
flyweight pose.
...
\begin{remarkbox}[Constructing the flyweight maps]
...
\begin{figure}[H]
    ...
    panel, mark the positive circumferential and axial directions, then show
    the signed circumferential run \(r_h\,d\theta_s\), axial rise \(dx_s\),
    track tangent, and signed angle \(\alpha_s\). In a companion panel at the
    same angle, resolve the pin--track reaction into the circumferential
    component that produces \(\tau_{s,\mathrm{helix}}\) and the axial component
...
Because the motion ratio is signed,
\Cref{eq:secondary_helix_force_conversion} applies to either helix
handedness without an additional sign choice.
...
\(\tau_{s,\mathrm{helix}}\geq0\). If the evaluated torque would be negative,
the loaded flank cannot pull the pin: that contact constraint must be released
until recontact or, where clearance permits, the opposite flank is engaged.
...
\begin{remarkbox}[Validity of the active-motion balances]
The local equations derived above govern free sheave motion while the stated
flyweight--ramp and pin--helix contacts are active. The shared-coordinate
mapping below additionally requires the belt to be engaged. At the primary
open stop, the secondary closed stop, belt engagement or disengagement,
flyweight separation or recontact, or a helix-flank change, the affected
motion instead requires a constraint reaction or a separate contact mode.
These events change which balance is active; they do not alter the
free-travel balances derived here.
\end{remarkbox}
...
coordinates, \(x_p\) and \(x_s\). Once the belt is engaged, however, its
fixed-length geometry makes those motions compatible. Choosing the primary
displacement as the shared shift coordinate leaves the primary balance
unchanged and supplies the secondary motion through the map \(x_s=x_s(s)\).
...
Within the active-contact, free-travel branch, the primary and secondary now
have complete local equations of motion. The primary converts shaft speed into
closing force through the changing shaft-axis inertia of the flyweights. The
secondary converts belt and spring torque into axial force through the helix.
In both cases, part of the actuator action accelerates internal rotating
hardware before the remainder can load the belt.
