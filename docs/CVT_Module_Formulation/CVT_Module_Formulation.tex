...
To see what that response actually consists of, return to the balances already
derived. The primary and secondary rotational equations,
\Cref{eq:primary_rotational_eom_belt,eq:secondary_rotational_eom_belt},
contain the shaft accelerations \(\dot\omega_p\) and \(\dot\omega_s\), together
with the belt torques \(\tau_p\) and \(\tau_s\). Once the traction pair is
specified, however, those torques are not independent unknowns. From
...
so each belt torque follows directly from the corresponding normal resultant
\(N_j\).

The two axial balances,
\Cref{eq:primary_balance_in_shift_coordinate,eq:secondary_balance_in_shift_coordinate},
then contain the shared shift acceleration \(\ddot s\) and the two normal
resultants \(N_p\) and \(N_s\); the secondary balance also uses
\(\dot\omega_s\), which is already present in the rotational equations. The
complete-belt transport balance in
\Cref{eq:complete_belt_transport_newton} adds the belt acceleration
\(\dot v_b\). Finally, the closed-loop tension compatibility in
\Cref{eq:closed_loop_tension_compatibility} supplies the remaining independent
relation among the belt acceleration, shift acceleration, and normal
resultants.

The radial accelerations appearing in the belt relations are not additional
unknowns: \Cref{eq:primary_radius_acceleration,eq:secondary_radius_acceleration}
already express them through \(s\), \(\dot s\), and \(\ddot s\). For a specified 
\((\lambda_p,\lambda_s)\), the instantaneous mechanical response is therefore described by
...
These are exactly the quantities carried by the six relations above. With
\(\lambda_p\) and \(\lambda_s\) fixed, each relation is linear in
\(\mathbf z\), so they can be collected as
...
Writing the six rows explicitly requires a little additional notation. For
this matrix only, primes shorten derivatives: \(x_s'\), \(r_j'\), \(q_f'\),
and \(J_{f,p}'\) denote derivatives with respect to \(s\), while
\(\theta_s'\) denotes differentiation with respect to \(x_s\); double primes
follow the same convention. The only expressions worth abbreviating are the
two analytical wrap factors repeated in the closed-loop row, which are written
as
...
Read from top to bottom, the matrix follows the same mechanics just gathered
above: the first two rows are the pulley rotational balances, the next two are
the axial balances, the fifth is the complete-belt transport balance, and the
sixth is the closed-loop tension compatibility.
...
derived. Solving \Cref{eq:engaged_six_by_six_mechanical_core} gives the two
shaft accelerations, belt acceleration, shift acceleration, and the two normal
resultants. A sliding \(\lambda_j\) reaches this solve directly from kinetic
friction; a candidate sticking \(\lambda_j\) enters in the same place, after
which the resulting \(a_{\mathrm{rel},j}\) is returned to the sticking
residual from \Cref{sec:sticking_traction_closure}.

The contact regime therefore changes how each \(\lambda_j\) is obtained, but
not the six mechanical balances solved once the traction pair is specified.
