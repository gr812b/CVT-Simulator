...
equal face-torque split have been adopted, power consistency fixes this
representative speed. The arithmetic mean appears specifically because the
...
For any admissible tangential contact state,
...
Reaching one of these boundaries therefore changes the structural regime of the CVT,
in the same sense that a change of sticking or sliding state changed the tangential regime
...
Both are required if the model is to pass through a structural boundary
...
Before the governing equations can change with the structural regime,
...
A boundary position does not, by itself, determine which structural regime
...
structural regime must therefore be carried as part of the mechanical
...
The same boundary also shows that structural regime and tangential contact
regime describe different parts of the mechanics. While the secondary stop
...
or slides relative to it. Each engaged structural regime must therefore still
be combined with the corresponding tangential contact states from
...
matter. The structural map and the tangential contact states together define
the set of mechanical regimes that the model must track as the CVT evolves.

% ---------------------------------------------------------------------------
\subsection{Governing Equations in Each Structural Regime}
\label{sec:structural_regime_governing_equations}

Knowing the active structural regime still does not determine how the state
evolves within it. Each arrangement needs a continuous set of balances and
kinematic conditions that determine the accelerations it permits. Most of
the mechanics need not be derived again, because the underlying pulley,
belt, and actuator models have not changed.

Only the free bodies and constraints that physically change need to be
updated. With the primary in the deadzone, the belt--sheave interaction is
absent and its force and torque must be removed. At a travel stop, the
relevant contact remains, but the stopped sheave can no longer accelerate
through the boundary and the hardware supplies the reaction required to hold
it there. Following the mechanism from fully open, through engagement and
free shift, to the upper stop lets each of these changes enter exactly where
it occurs.
...
The same remains true while the primary crosses the clearance toward
\(s=s_{\mathrm{dz}}\). The first modification is therefore direct: every
interaction that requires primary belt contact is removed, while the primary
hardware itself remains dynamically active. Hence,
...
The primary still rotates, the flyweights still move on their ramps, the
spring still acts on the movable assembly, and the translating hardware still
carries inertia. Removing only the belt torque from
\Cref{eq:primary_rotational_eom_belt} gives
...
while removing the primary belt force from
\Cref{eq:primary_balance_in_shift_coordinate} gives
...
Primary separation creates a second issue on the belt side. The engaged
belt mechanics relied on a closed path carrying tension through both pulley
wraps. In the deadzone, the secondary is already fully closed, so it cannot
close farther to take up belt length and generate additional tension against
the primary-side support. At the same time, the present belt model contains
no longitudinal strain state or pretension law from which such a tension
could otherwise be recovered. Although the loose belt may still rest on the
minimum-radius primary support, that contact is not represented as a clamped
belt--sheave interaction.

Resolving the actual slack-belt shape and the loads created as slack is taken
up would therefore require additional belt elasticity, pretension, and
free-span geometry beyond the reduced model adopted here. The deadzone
description instead uses the single approximation declared in
\AssRef{assump:deadzone_belt_closure}: the belt remains seated on the fully
closed secondary and is carried with its represented contact surface. Because
the secondary has no axial or helix-relative motion in this regime,
\(\dot\theta_s=0\), and
\Cref{eq:representative_contact_angular_speed} reduces to
\(\bar\omega_s=\omega_s\). The deadzone lock is therefore
...
Since this radius is fixed, the corresponding belt acceleration is
...
The belt is unloaded from the primary, but its transport inertia is still
present. Combining the fixed-configuration secondary rotational balance in
\Cref{eq:secondary_rotational_fixed_configuration_limit} with the complete
belt transport balance in \Cref{eq:complete_belt_transport_newton} gives
...
fixed secondary radius. Primary disengagement therefore removes the primary
belt load without making the belt dynamically irrelevant: the secondary must
still accelerate the transport motion of the entire belt.

For \(0<s<s_{\mathrm{dz}}\), the primary is free to move through the
clearance, so
\Cref{eq:deadzone_primary_rotation,eq:deadzone_primary_axial_motion,%
eq:deadzone_belt_acceleration_lock,eq:deadzone_secondary_rotation}
determine
\(\dot\omega_p\), \(\dot\omega_s\), \(\dot v_b\), and \(\ddot s\).
At \(s=0\), the same contact-free mechanics meet the primary opening stop.
A sustained stop condition requires
...
Let \(F_{\mathrm{stop},0}\geq0\) denote the force exerted by this stop in the
positive, closing direction. The primary axial balance then becomes
...
so the reaction required to hold the primary open is
...
The stop therefore carries whatever part of the spring opening load is not
balanced by the flyweights. If the required reaction is negative, the hardware
would have to pull the primary open rather than push it closed, so the fully
open stop cannot support that continuous state.
...
engaged belt mechanics become active again. The secondary, however, is still
at its fully closed position. Primary contact can therefore exist while the
shift remains held at the minimum engaged ratio.
...
The fixed shift also gives \(\dot\theta_s=0\), so the representative secondary
contact speed again reduces to \(\bar\omega_s=\omega_s\). The shafts and belt
may nevertheless continue to rotate and accelerate, and the two pulley
contacts retain the sticking or sliding closure developed in
\Cref{sec:engaged_dynamic_closure}.

The hardware supporting this boundary is the fully closed secondary stop.
Return to the secondary axial free body in
\Cref{fig:secondary_axial_free_body}. The helix and spring act toward
secondary closure, while the belt acts toward opening. Let
\(F_{\mathrm{stop,LR}}\geq0\) denote the opening reaction supplied by the
stop. With the secondary held,
...
The secondary balance has not been discarded; its role has changed. While
the sheave is free, the imbalance of these three modeled loads determines its
axial acceleration. Once the stop requires that acceleration to vanish, the
same imbalance determines the support force that the hardware must carry.

That reaction cannot be evaluated before the rest of the engaged mechanism is
solved. The belt term is
\(F_{\mathrm{belt},x_s}=N_s\cos\beta/2\), so it depends on the unknown
secondary normal resultant. Even with the shift fixed, the helix force in
\Cref{eq:secondary_helix_closing_force} still depends on the transmitted
secondary torque and on \(\dot\omega_s\). The belt loading, shaft
acceleration, and stop reaction are therefore parts of the same coupled
response rather than three quantities that can be found independently.

The constraint tells us how to reorganize that response. In the mechanical
core of \Cref{eq:engaged_six_by_six_mechanical_core}, set
\(\ddot s=0\) and replace the secondary axial row by this imposed condition.
The primary axial row remains: no primary stop is available to absorb its
force imbalance, so at the held geometry that balance instead helps determine
the primary belt load. The remaining rotational and belt equations then
determine
\(\dot\omega_p\), \(\dot\omega_s\), \(\dot v_b\), \(N_p\), and \(N_s\)
for the proposed traction pair. Substituting those solved quantities back
into \Cref{eq:low_ratio_stop_reaction} gives the force carried by the
secondary stop. The sticking or sliding values of \(\lambda_p\) and
\(\lambda_s\) are selected with the same contact-state logic already developed
in \Cref{sec:engaged_dynamic_closure}.

The distinction between stop reaction and belt load is important. The stop
can carry part of the secondary spring and helix loading directly into the
structure without transmitting that force through the belt faces. Hence
\(F_{\mathrm{stop,LR}}\) does not add to \(N_s\) and does not increase the
secondary traction capacity. The seated regime is mechanically supportable
only while
...
Once the secondary is away from its closing stop while primary belt contact
is retained, this support reaction is absent and both sheaves again follow
the shared belt geometry. That is precisely the free engaged arrangement
already closed in \Cref{sec:engaged_dynamic_closure}; no additional
continuous equation is introduced for the interior shift.

\subsubsection{Upper Primary Stop}
\label{sec:upper_primary_stop_regime}

Free upshift can continue until the primary reaches
\(s=s_{\max}\). Belt contact remains present, but the supporting hardware has
now changed sides: further closure is prevented by the primary closing stop.
A sustained upper-stop state therefore satisfies
...
Let \(F_{\mathrm{stop,max}}\geq0\) denote the opening reaction exerted by this
stop on the primary movable assembly. The primary axial balance becomes
...
The same constrained-solve idea now applies with the opposite axial row.
Set \(\ddot s=0\) in
\Cref{eq:engaged_six_by_six_mechanical_core} and replace the primary axial
row, because that is the balance containing the new stop reaction. The
secondary axial row remains active and, together with the rotational and belt
equations, determines the coupled fixed-shift response. The solved primary
belt load is then substituted into
\Cref{eq:upper_stop_reaction} to recover the force carried by the stop.

As at low ratio, the stop reaction is a separate structural load path rather
than additional belt clamp force. The upper stop can sustain the regime only
while
...
The three supported arrangements are summarized in
\Cref{tab:structural_stop_reactions}.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}
        ...
        @{}}
        \toprule
        \textbf{Supported regime}
        & \textbf{Stopped member}
        & \textbf{Reaction direction}
        & \textbf{Balance that recovers the reaction}
        \\
        \midrule
        Fully open primary stop
        & Primary
        ...
        \bottomrule
    \end{tabular}
    \caption{Hardware reactions for the three stop-supported structural
    regimes. In each case, the stopped member's original axial balance is
    evaluated after the corresponding constrained continuous response has
    been obtained.}
    \label{tab:structural_stop_reactions}
\end{table}

A physical stop can push only in the direction shown in the table. The three
supported regimes therefore require
...
The continuous mechanics are now defined for all five structural
arrangements: the fully open stop, free deadzone, low-ratio seat, free engaged
shift, and upper primary stop. These equations determine the accelerations and
support reactions once a regime is established. A different issue remains
whenever a boundary is reached with nonzero incoming shift speed:
\(\dot s=0\) describes the supported state, but it does not account for the
velocity that existed immediately before contact with the constraint. That
velocity change is not part of the continuous force balance derived here.
