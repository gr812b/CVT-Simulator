...
The secondary pulley is \emph{torque-reactive}: its axial force is produced when torque is carried through the secondary helix mechanism. This makes the secondary different from the primary. The primary force is mainly speed-sensitive, because its flyweight force grows with rotational speed. The secondary force is generated by a helical constraint and therefore depends strongly on torque.

The secondary assembly consists of a fixed sheave, a movable sheave, a helical slot between them, and a spring. The fixed sheave is attached to the secondary shaft. The movable sheave can slide axially, but it is captured by the helical slot, so it cannot rotate independently of the fixed sheave. If the movable sheave moves axially, it must also rotate relative to the fixed sheave according to the helix geometry defined previously.

The same secondary spring contributes in two ways:
\begin{itemize}
    \item It is compressed axially as the sheaves separate.
    \item It is twisted torsionally as the helix produces relative rotation between the sheaves.
\end{itemize}

Thus, the secondary axial force has two sources: direct axial spring compression, and torque reacted through the screw-like helix.

\begin{figure}[H]
\centering
\includegraphics[width=0.5\linewidth]{./illustrations/pulleyForces/secondary_3d_model.png}
\caption{Secondary CVT assembly showing the helix mechanism and spring. The helix is modeled as a captured slot, so axial displacement of the movable sheave is coupled to relative rotation between the movable and fixed sheaves.}
\label{fig:secondary_cvt_3d}
\end{figure}

% --------------------------------------------------------------
\paragraph{Physical Mechanism: Why Torque Produces Axial Force}

The helix can be understood using a screw analogy. Spinning a screw freely in space does not create axial force. Spinning a screw into a block that is free to rotate with it also produces little axial force, because the block can simply rotate with the screw. Axial force appears when relative rotation is resisted.

The secondary helix works the same way. Torque applied to the movable sheave tends to rotate it relative to the fixed sheave. Because the movable sheave is captured by the helix, that relative rotation cannot occur independently of axial motion. The helix must therefore react the torque, and because the helix is inclined, that reaction produces an axial component.

This is the key physical idea: the secondary does not produce axial force because it is spinning. It produces axial force because torque tries to rotate the movable sheave relative to the fixed sheave, and that relative motion is constrained by a helical path.

% --------------------------------------------------------------
\paragraph{Torque Applied to the Movable Sheave}

Let $\tau_{b,M}$ denote the signed torque applied by the belt to the movable secondary sheave. This is a local torque on the movable-sheave free body. It is the torque that tries to rotate the movable sheave relative to the fixed sheave, and therefore the torque that can load the helix.

This distinction matters. A torque that only spins the fixed and movable sheaves together does not create relative motion and therefore does not load the helix. The helix is loaded by the part of the torque path that must be reacted between the movable sheave and the fixed sheave.

The relationship between $\tau_{b,M}$ and the total secondary torque is handled later by the torque-transfer model. In this section, $\tau_{b,M}$ is treated as the local belt-applied torque entering the secondary helix mechanism.

A positive value of $\tau_{b,M}$ is defined as torque that tends to produce positive relative helix rotation. Since $\tau_{b,M}$ is signed, reverse loading is handled by the same equations with $\tau_{b,M}<0$.

% --------------------------------------------------------------
\paragraph{Helix Velocity Constraint}

The helix geometry has already been parameterized as
...
where $\theta_s$ is the relative rotation between the movable and fixed sheaves. For the force model, the most intuitive use of this constraint is through angular velocity.

Let $\omega_F$ be the angular velocity of the fixed secondary sheave. Since the fixed sheave is attached to the secondary shaft, this is the secondary shaft speed:
...
Let $\omega_M$ be the angular velocity of the movable secondary sheave. With the sign convention used here, positive helix rotation means that the movable sheave lags the fixed sheave. Therefore, the relative helix speed is
...
At the same time, because $\theta_s$ is a function of the shift coordinate $s$,
...
Combining \Eq{eq:secondary_relative_speed} and \Eq{eq:secondary_theta_dot} gives the movable-sheave speed:
...
This equation is the kinematic meaning of the captured helix. If the secondary shifts, the movable sheave cannot simply translate; its angular speed must also differ from the fixed sheave speed by the amount required by the helix slope.
...
This angular acceleration matters because some of the torque applied to the movable sheave is used to rotationally accelerate that sheave. Only the remaining torque is reacted through the helix and converted into axial force.

% --------------------------------------------------------------
\paragraph{Torsional Spring Torque}

As the helix rotates the sheaves relative to each other, the secondary spring is twisted. Let $k_{s,\theta}$ denote the torsional stiffness of the spring and let $\theta_{s,0}$ denote its preload deflection.

The torsional spring torque applied to the movable sheave is written as
...
This is a signed torque in the same helix coordinate as $\tau_{b,M}$. The sign convention for $\theta_s$ and the preload term determines whether the spring torque assists or opposes the belt-applied torque at a given instant.

% --------------------------------------------------------------
\paragraph{Rotational Equation of Motion of the Movable Sheave}

Let $I_M$ be the rotational inertia of the movable secondary sheave about the secondary shaft axis. The movable sheave has three torques acting on it:
\begin{itemize}
    \item the belt-applied torque, $\tau_{b,M}$,
    \item the torsional spring torque, $\tau_{s,\mathrm{spring}}$,
    \item the reaction torque applied by the helix.
\end{itemize}

Let $\tau_{\mathrm{helix}\rightarrow M}$ denote the torque applied by the helix to the movable sheave. Newton's second law in rotation gives
...
The helix reaction is what turns the screw analogy into an axial force. If there were no helix reaction, the belt-applied torque and spring torque would simply angularly accelerate the movable sheave. The helix produces axial force because it must react the portion of torque that is not used to accelerate the movable sheave.

The equal and opposite torque applied by the movable sheave into the helix is
...
Solving \Eq{eq:secondary_rotational_n2l} for this reacted torque gives
...
Substituting \Eq{eq:secondary_torsion} and \Eq{eq:secondary_movable_alpha},
...
This equation has a direct interpretation: the helix reacts the belt-applied torque and the torsional spring torque, except for the portion needed to angularly accelerate the movable sheave.

% --------------------------------------------------------------
\paragraph{Torque-to-Axial Conversion Through the Helix}

The reacted helix torque is converted into axial force by the helix geometry. From the mapping $\theta_s=\theta_s(s)$, a small axial displacement $\delta s$ corresponds to a relative rotation
...
Therefore, the axial force associated with the reacted torque is
...
This is the screw relation written in the shift coordinate. A larger value of $d\theta_s/ds$ means that a small axial movement requires more relative rotation, so a given reacted torque produces a larger axial force.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{./illustrations/pulleyForces/secondary_helix_torque_to_axial.png}
\caption{Helix free-body concept. Torque applied to the movable sheave is reacted through the helix. Because the helix is inclined, the reacted torque produces an axial force proportional to $d\theta_s/ds$.}
\label{fig:secondary_helix_torque_to_axial}
\end{figure}

Substituting \Eq{eq:secondary_helix_reaction_torque_expanded} into \Eq{eq:secondary_helix_force_general} gives
...
The sign of this force follows the signed torque balance. If the belt-applied torque and spring torque drive the helix in the positive direction, the helix force is positive. If reverse loading overcomes the spring contribution, the same equation naturally gives force in the opposite direction.

% --------------------------------------------------------------
\paragraph{Secondary Compression Spring}

The secondary spring also acts directly in compression between the sheaves. At $s=0$, it is pre-compressed by an amount $x_{s,0}$. As the sheaves separate with increasing $s$, its compression increases.
...
With axial stiffness $k_{s,x}$ and compression

\[
\Delta x=x_{s,0}+s,
\]

the direct axial spring force is
...
The total secondary axial force used in the shift equation is the sum of the direct compression spring force and the helix-generated force:
...
The first term is direct axial spring compression. The bracketed term is the torque reacted through the helix: belt-applied torque plus torsional spring torque, minus the torque required to rotationally accelerate the movable sheave. Multiplication by $d\theta_s/ds$ converts that reacted torque into axial force.

Because this expression contains $\ddot{s}$, the secondary force is not only a lookup from position and torque. When this force is substituted into the axial shift equation of motion, the terms involving $\ddot{s}$ should be collected with the other shift-inertia terms.
...
Earlier (see \Eq{eq:axial_newton_base}), the evolution of the shift coordinate was introduced as
...
\begin{enumerate}
    ...
    \item Replacing the lumped mass $m$ with the equivalent inertia associated with coordinated shift motion,
    \item Accounting for the fact that the secondary helix force itself contains an $\ddot{s}$ term.
\end{enumerate}

The last point requires one additional algebraic step. The secondary helix force does not depend only on position and torque: because the movable secondary sheave is forced to rotate as it shifts, its angular acceleration appears in the force expression. Since that angular acceleration contains $\ddot{s}$, the secondary force must be substituted into the shift equation before solving explicitly for $\ddot{s}$.

% --------------------------------------------------------------
\paragraph{Equivalent Inertia Associated with $s$}

Newton's second law applies directly to physical masses in translation. Since $s$ parameterizes coordinated motion of several bodies, we construct an equivalent mass $m_{\mathrm{eq}}$ such that the total kinetic energy associated with shift can be written in the form of \theoryRef{th:kinetic_energy_trans}:

\[
T_{\text{shift}}
=
\frac12\,m_{\mathrm{eq}}\,\dot s^2.
\]

The movable primary sheave translates with speed $\dot{s}$ in the positive coordinate direction. The movable secondary sheave translates with equal magnitude speed, but in the opposite physical direction, so its kinetic energy also contributes proportionally to $\dot{s}^2$.
...
where $A_b$ is the trapezoidal cross-sectional area from \Eq{eq:Ab_trap} and $L_b$ is the constant belt length.

The belt moves continuously with a single belt speed, but the axial component of that motion associated with a change in $s$ is determined by geometry. From the axial-radial coupling in \Eq{eq:drpds_piecewise}, a change in sheave separation $s$ is shared between the two conical faces supporting the belt wedge. Thus, the belt's axial velocity relative to the pulley axis is
...
% --------------------------------------------------------------
\paragraph{Base Equivalent Mass}

The translational equivalent mass associated with shift is
...
This is the base equivalent mass before including the rotational inertia reflected through the secondary helix.

% --------------------------------------------------------------
\paragraph{Secondary Force as an Acceleration-Dependent Force}

The updated secondary force model depends on the belt-applied torque to the movable sheave, the secondary angular acceleration, and the shift acceleration. Therefore, for the shift equation it should be viewed as
...
F_{s,\mathrm{ax}}
\left(
s,\dot{s},\tau_{b,M},\dot{\omega}_s,\ddot{s}
\right).
\label{eq:secondary_force_arguments}
\end{equation}

From \Eq{eq:secondary_axial}, the secondary force can be written as
...
This equation contains $\ddot{s}$ inside the bracket. To make the shift equation explicit, separate the secondary force into two parts:
...
where the acceleration-independent part is
...
This split is only algebraic; it does not change the secondary model. It separates the term
...
acts as additional inertia in the shift coordinate.
...
\begin{itemize}
  ...
  \item $F_{s,\mathrm{ax}}(s,\dot{s},\tau_{b,M},\dot{\omega}_s,\ddot{s})$ from \Eq{eq:secondary_axial},
...
Substituting the split form of the secondary force gives
...
Now collect the terms multiplying $\ddot{s}$ on the left-hand side:
...
This collected form shows how the secondary helix contributes to the shift dynamics. It adds the secondary force contribution in the numerator while reflecting the movable sheave's rotational inertia into the denominator.

% --------------------------------------------------------------
\paragraph{Shift Equation of Motion}

Substituting \Eq{eq:meq_def} and solving for $\ddot{s}$ gives the final axial shift acceleration:
...
-
\Big(
\widehat{F}_{s,\mathrm{ax}}(s,\dot{s},\tau_{b,M},\dot{\omega}_s)
+
F_{c,s}(s,v_b)
\Big)
}{
...
The numerator is the net generalized force driving shift: primary force and primary belt centrifugal contribution act in the positive $s$ direction, while the secondary force and secondary belt centrifugal contribution oppose it. The denominator is the effective inertia associated with changing $s$: the translating sheaves, the axial component of belt motion, and the movable secondary sheave inertia reflected through the helix.

The secondary torque input is $\tau_{b,M}$, the belt-applied torque on the movable secondary sheave. The quantity $\dot{\omega}_s$ is the angular acceleration of the fixed secondary sheave and shaft, determined by the rotational dynamics. Thus, the shift equation remains coupled to the torque-transfer and secondary rotational equations, while the dependence on $\ddot{s}$ has been resolved explicitly in \Eq{eq:s_ddot_final}.
