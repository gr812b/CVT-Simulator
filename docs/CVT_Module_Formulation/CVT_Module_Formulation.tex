...
The radius \(r_j\) defined in \Cref{eq:geometry_radius_convention} follows the
outer surface of the belt. The mass of the belt element instead travels with
the centroid of its trapezoidal cross-section. Let \(y\) measure radially
inward from the outer surface through the belt height \(h_b\). Under
\AssRef{assump:fixed-belt-cross-section}, the cross-section retains the outer
and inner widths \(b_{\mathrm{out}}\) and \(b_{\mathrm{in}}\), so its width at
\(y\) is
...
\begin{equation}
    ...
    \label{eq:belt_cross_section_width_distribution}
\end{equation}
...
\begin{align}
    ...
    \label{eq:belt_trapezoid_area}
    \\
    A_b y_{\mathrm{cm}}
    &=
    \int_0^{h_b} y\,b(y)\,dy.
    \label{eq:belt_trapezoid_first_moment}
\end{align}

Substitution of \Cref{eq:belt_cross_section_width_distribution} gives
...
\begin{equation}
    ...
    \label{eq:belt_cross_section_centroid}
\end{equation}
...
\begin{equation}
    ...
    =
    r_j-y_{\mathrm{cm}}.
    \label{eq:belt_mass_centroid_radius}
\end{equation}

Since the cross-section is fixed, \(y_{\mathrm{cm}}\) is constant. The
centroid consequently inherits the radius rate and acceleration already
obtained from the pulley geometry,
...
The integrand also gives a useful contact check. The belt and sheave can press
against one another but cannot sustain a tensile normal force, so the
prescribed continuous wrap requires \(dN_j/d\theta\geq0\) throughout the
contact arc. A negative value would indicate loss of the assumed full-wrap
contact rather than a negative physical pressure.
...
Let \(r_{j,\mathrm{eff}}\) denote the effective moment arm of this tangential
belt--sheave contact. It is distinct from \(r_{j,\mathrm{cm}}\), which follows
the belt mass and therefore appears in the inertial terms above. The belt
exerts the equal-and-opposite tangential force on the pulley, so the signed
belt torque used in the rotational balances is
...
surface, so \Cref{eq:belt_mass_centroid_radius} and
...
and its resultant \(N_j\). The axial belt reaction and transmitted pulley
torque then follow as two components of the same contact resultant rather than
as independent constitutive laws. This idea also has strong precedent. Detailed
...
The most restrictive contact condition is adhesion, in which the belt and
pulley surface move together. It provides the natural place to determine what
the complete coupled problem requires.

% --------------------------------------------------------------
\subsection{Sticking Contact and the Full Coupled Problem}
\label{sec:sticking_engaged_closure}

If the belt adheres to pulley \(j\), the two surfaces cannot have different
tangential speeds at the contact radius. The sticking condition is therefore
...
During an established sticking interval this equality is already satisfied by
the current state. The instantaneous accelerations must preserve it.
...
closed-loop compatibility already identified above. For one trial traction
pair, solving this \(6\times6\) system determines the complete mechanical
response associated with that pair.
...
The resulting \(\lambda_p\) and \(\lambda_s\) are therefore not prescribed
static friction forces. They are the contact reactions required to make the
six mechanical balances compatible with adhesion at both pulleys.
...
For pulley \(j\), the relative tangential speed is
...
where \(\sigma_j=\pm1\) is chosen so that the tangential contact action opposes
the established relative motion. Unlike the static case, \(\lambda_j\) is now
known directly from the contact state rather than determined as an unknown
reaction.
...
% --------------------------------------------------------------
\subsection{Admissibility and Changes of Contact State}
\label{sec:engaged_contact_state_changes}

The equations above determine an instantaneous solution for an assumed contact
combination. That assumption can persist only while the solution remains
consistent with the physical condition used to construct it. The same
quantities already obtained from the closure therefore determine when a
contact description must change.
...
sliding must continue with the direction required by the subsequent relative
motion.
...
The freely shifting engaged CVT now has a complete instantaneous closure. The
two pulley assemblies and the belt contribute six mechanical equations, while
the two tangential contacts supply the remaining information needed to close
the system.
...
For any admissible freely shifting engaged contact combination, the current
state and applied shaft boundaries therefore determine
