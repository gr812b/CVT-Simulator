...
With the tangential contact states resolved, the equations now determine how the freely 
shifting, engaged CVT moves under the loading applied at its two shafts. The pulley 
rotations, belt transport, and sheave motion are coupled through the balances derived above.

But the pulley geometry already established limits on that axial travel. The predicted 
motion can bring a sheave against a metal stop, or open the primary far enough to lose 
belt contact and enter the deadzone. The stop can hold a sheave that was previously 
moving, while primary separation allows it to travel without loading the belt. The mechanism 
has therefore entered a different mechanical regime, with different forces and permitted 
motions. Its behaviour must be established under these conditions, including how changes in
loading allow it to leave a stop or engage the belt again.

Reaching a stop also exposes a second issue: the sheave need not arrive at rest. Knowing how 
it behaves once held does not explain how its incoming motion is brought to an end. That 
motion carries momentum and is coupled, through the actuator mechanisms, to rotation of other 
components. Preventing further axial travel therefore leaves a momentum exchange to account 
for. The transition itself must be described alongside the motion that follows, so that momentum 
is carried consistently through the change in mechanical regime.


\subsection{Shift Domain and Structural Regimes}
\label{sec:shift_coordinate_boundaries}

The first question is where the freely shifting, engaged motion must change.
The geometry has already established the travel limits of both sheaves and
the primary clearance, through which the primary can move while the secondary
remains fully closed. We can follow that same travel through \(s\), now
examining where the primary loses belt contact and where a stop can support a
sheave. Each change alters the forces acting on the mechanism, giving a
different mechanical regime to describe.
...
\Cref{tab:shift_structural_arrangements} collects the two intervals of free
primary motion and the three positions at which a stop can hold the shift.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}
        ...
        & Present
        & Primary closing stop holds the engaged mechanism at \(s=s_{\max}\).
        \\
        \bottomrule
    \end{tabular}
...
The table locates where each arrangement is possible. Reaching a listed
boundary, however, does not establish that its stop is holding the mechanism.
At \(s=s_{\mathrm{dz}}\), the primary can remain seated against the belt,
begin closing and open the secondary away from its stop, or begin opening
into the clearance. The mechanical regime defined in
\Cref{sec:mechanical_regimes_state_evolution} records which of these contacts
and constraints are active.

Even when the mechanism remains seated, the stop has only fixed the axial
condition. Its reaction holds the sheave while the pulleys and belt can keep
moving. The belt may move with a pulley surface or slide relative to it, so
selecting the seat has left the tangential contact states undecided. Each of
the three arrangements with primary belt contact must therefore also carry
the sticking or sliding state of each pulley contact from
\Cref{sec:engaged_dynamic_closure}.

Those sticking and sliding states are possible only while the contact
remains. If the primary instead opens into its clearance, there is no primary
contact left to stick or slide, and no belt load acts on it. The belt still
contacts the secondary. Continuing into the deadzone therefore requires
revising the governing balances with the primary belt interaction removed.
