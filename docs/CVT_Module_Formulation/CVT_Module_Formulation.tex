...
can vary smoothly within a finite range rather than changing through a fixed set
of gear steps. This is useful because an internal combustion engine (ICE) does
not produce the same torque and power at every speed. Instead, the engine has a
useful operating region where it can produce power more effectively, as shown
conceptually in the left panel of \Cref{fig:cvt_engine_motivation}.
...
speed rises through one gear, drops when the transmission shifts, and then rises
again in the next gear. This repeated sweep through the engine speed range is a
direct consequence of having only a small number of available ratios. The right
panel of \Cref{fig:cvt_engine_motivation} contrasts this stepped behavior with
the smoother operating path made possible by a CVT.

A CVT gives the drivetrain a continuously adjustable speed relationship between
the engine and the driven wheels. Since the ratio can vary smoothly within its
available range, the engine can remain closer to a desired operating region
while vehicle speed changes. This is the main benefit of a CVT: it gives the
...
For traction to develop, the belt must be pressed into the sheave faces, as
shown in \Cref{fig:belt_sheave_forces}. This requires clamping force. The
clamping force creates contact pressure between the belt and the pulley, and
that contact pressure allows friction to transmit tangential force around the
pulley. In practical terms, transferring more torque through the CVT requires
enough clamping force and enough available traction at the belt-pulley
interface.
...
CVT modelling is not a single, uniform problem. A model developed to size a variator, estimate belt efficiency, design a control strategy, or predict launch behaviour may involve the same physical components, but it will usually make different choices about which quantities are prescribed and which are solved. Some models are primarily design tools. Some focus on belt forces, axial thrust, or power loss at fixed operating points. Others study ratio change, belt slip, or detailed belt-pulley contact mechanics. A useful review must therefore do more than list papers chronologically: it must identify which part of the CVT problem each body of work is trying to resolve.

The progression followed here is from design intuition, to component force balance, to belt contact, to ratio change, to full transient launch behaviour. This ordering mirrors the way the modelling problem becomes difficult. A belt-sheave CVT can first be understood as a variable-radius transmission, then as a set of mechanical actuators that generate axial force, then as a friction-limited belt contact problem, and finally as a coupled dynamic system whose engine speed, belt state, shift position, and vehicle motion evolve together. The literature contains strong foundations for each of these pieces, but those foundations are distributed across several modelling traditions.

\subsection{Framework for Classifying CVT Models}
\label{sec:lit-classification}

The most important difference between CVT models is often the boundary they draw around the system. A steady belt-contact model may resolve pressure, slip, and tension distributions around a pulley while prescribing the pulley speeds and axial forces. A design model may capture the influence of springs, flyweights, and helix geometry while treating the transmission as a sequence of equilibrium operating points. A drivetrain model may include engine and vehicle dynamics while using a simplified belt representation. Each choice is reasonable for some questions and limiting for others.

The first useful classification axis is contact fidelity. At low fidelity, torque capacity may be estimated using a capstan-style tension relationship, modified for the wedge action of a V-groove. More detailed models resolve radial and tangential friction, sliding angle, belt penetration, bending stiffness, local normal pressure, and tension variation around the wrap \citep{Gerbert1996,KimKim1989,Sorge1996,KongParker2005,KongParker2006}. At still higher fidelity, the belt can be discretized into nodes or finite elements so that local contact, belt stiffness, and transient belt motion are represented directly \citep{JulioPlante2011,Ballew2015}.

A second axis is actuation. In many automotive CVTs, axial clamping force is hydraulic, electronic, or otherwise externally controlled. In many small powersport CVTs, the axial forces are generated mechanically: the primary responds mainly to speed through flyweights and springs, while the secondary responds to torque through a helix or cam mechanism and spring preload. This distinction matters because a mechanically actuated CVT does not directly command ratio. The ratio is a result of the interaction between actuator forces, belt forces, transmitted torque, and load.

A third axis is dynamic closure. Some models prescribe pulley speeds, ratio, load, or axial force. Others solve rotational equations of motion for the pulleys, include engine torque and inertia, or couple the output to a vehicle or dynamometer load. These choices are especially important during launch, where the engine, belt, secondary shaft, and vehicle may not yet share the no-slip kinematic relationship assumed during ordinary engaged operation. A model may include ratio change without resolving engine dynamics, or include engine torque without resolving the contact-state transitions that occur during clutching.

The final axis is the treatment of launch and clutching. Many models begin from a fully engaged condition and study behaviour at a known operating point or during a controlled shift. Launch is more difficult because the belt may begin in a slipping or partially engaged state, clamp force is still developing, the vehicle load is changing rapidly, and the transmission may transition between slipping, traction-limited, and near no-slip behaviour. This regime is central to tuning small mechanically actuated CVTs, but it is also one of the hardest regimes to model cleanly.

\subsection{Practical Design and Tuning Foundations}
\label{sec:lit-practical-design}

The practical understanding of mechanically actuated rubber V-belt CVTs developed largely through machine design and tuning practice. General machine design references such as \citet{BudynasNisbett2015} provide the background needed to treat the CVT as a real mechanical assembly: belts, springs, clutches, brakes, shafts, friction interfaces, fatigue, uncertainty, and design iteration all matter. Not every effect in a real machine is included in a compact CVT model, but these references are useful reminders that every modelling assumption excludes some physical behaviour. A good model must therefore be explicit about which mechanical effects are retained and which are idealized.

Practical tuning literature provides a different kind of foundation. \citet{Aaen1979} describes mechanically actuated powersport CVTs using the language of engagement speed, shift speed, flyweights, primary springs, secondary springs, torque sensing, backshift, and speed variance. This vocabulary is important because it is the language in which many CVTs are actually tuned. It also captures a useful qualitative picture of operation: the engine rises toward engagement, the belt begins transmitting torque, the system shifts while attempting to hold the engine near a desired speed range, and backshift occurs when load increases or vehicle speed falls.

These sources are not formal dynamic derivations, but they are valuable because they build intuition for a machine whose behaviour is otherwise difficult to predict. In practice, the same hardware change can affect engagement, shift rate, belt grip, backshift, engine speed, and vehicle acceleration at the same time. This is why purely qualitative tuning rules are often insufficient: the system is strongly coupled, and the effect of a component change depends on the operating condition in which it is observed.

\subsection{Design-Oriented and Quasi-Static CVT Models}
\label{sec:lit-design-models}

Design-oriented CVT models attempt to turn practical component knowledge into quantitative predictions. They commonly relate pulley geometry, belt length, wrap angle, speed ratio, flyweight force, spring preload, helix force, belt tension, and torque capacity. Their main value is that they connect tunable hardware to expected operating behaviour.

\citet{Cammalleri2005} developed a design approach for a speed-torque-controlled rubber V-belt variator using a centrifugal driver and torque-sensitive driven pulley. This work is important because it treats the mechanical actuator architecture directly, rather than assuming that the axial forces are arbitrary inputs. It provides a formal design framework for choosing actuator geometry so that the variator follows a desired speed-torque behaviour.

More recent design-oriented studies approach the problem from the system development side. \citet{ArangoMunoz2020} propose a numerical design method for rubber V-belt CVTs based on vehicle requirements, road specifications, and desired performance. \citet{LaBattaglia2022} focus on the kinematic design of motorcycle rubber V-belt CVTs, emphasizing the front pulley roller profile as a critical step in matching transmission behaviour to vehicle performance. These works are useful because they show how CVT design can be organized around vehicle-level requirements rather than isolated component choices.

Within Baja-style applications, \citet{Skinner2020} provides a practical force-balance and tuning-oriented model. The work emphasizes quantifiable tuning objectives such as engagement speed, shift-out speed, static RPM shifting, and efficient power transfer, and relates those objectives to the primary and secondary subsystems. This is useful as an applied tuning reference, but it remains primarily an operating-point and force-balance model.

The common strength of these works is that they make CVT design less arbitrary. They identify the parameters that matter and show how geometry, springs, flyweights, helices, and belt forces can be related. Their limitation is tied to their purpose: by focusing on design points or quasi-static force balance, they do not generally describe the complete time history of launch, clutching, slip, shift motion, and vehicle acceleration.

\subsection{Rubber V-Belt Contact, Axial Force, and Efficiency Models}
\label{sec:lit-contact-efficiency}

The next layer of the problem is the belt-pulley interface itself. A rubber V-belt does not transmit torque as an ideal kinematic constraint. It wedges into the sheaves, develops a tension distribution around the wrap, experiences radial and tangential friction, deforms under load, and dissipates energy through several mechanisms. This makes the contact problem much richer than the simple statement that the belt does not slip.

Classical belt theory gives the starting point: friction and wrap angle determine the maximum tension ratio that can be supported before gross sliding occurs. For V-belts, this picture is modified by the groove angle and the associated wedge effect. However, real belt slip includes more than a single capstan-style traction limit. \citet{Gerbert1996} presents a unified treatment of belt slip in which creep, radial compliance, shear deflection, and seating/unseating effects all contribute to speed loss. This is an important result because it shows why a simplified traction model can be useful for system simulation while still omitting several mechanisms that affect measured efficiency and slip.

Axial thrust in rubber V-belt drives has also been studied extensively. \citet{KimKim1989} developed a theoretical analysis of axial forces in a rubber V-belt CVT using simplified assumptions about the contact regions and belt force distribution. \citet{Sorge1996} developed a compact model for axial thrust in V-belt drives, emphasizing the role of radial penetration and the connection between belt forces and pulley thrust. These works are especially useful for understanding why axial force cannot be treated as a purely actuator-side quantity: it is also a result of the belt's interaction with the sheave groove.
...
\citet{Messick2018} extends this tradition for rubber V-belt CVTs by building on the Kong and Parker framework and validating the resulting axial-force and efficiency predictions experimentally. Messick's work is best understood as a detailed rubber belt force and efficiency model rather than as a full transient launch model. Its value is in the contact-region formulation, belt property measurement, operating-range analysis, and dynamometer validation. It also clearly identifies its own operating scope: shifting is treated as a quasi-equilibrium process rather than as a response-time problem.

Efficiency-focused studies make the same point from another direction. \citet{ChenLeeSung1998} experimentally investigated speed and torque losses in a rubber V-belt CVT, while \citet{Bertini2014} developed an analytical model for rubber V-belt CVT power losses including sliding, hysteresis, and entry/exit effects. These works show that efficiency depends on multiple interacting mechanisms, not just on whether the belt is below a gross slip threshold.

The contact and efficiency literature therefore gives strong support for the physical mechanisms involved in torque transmission, axial thrust, and power loss. It also shows why local belt fidelity can become very complex. A system-level dynamic model must choose how much of this contact detail to retain, because resolving every local belt effect can obscure the simpler question of how the complete drivetrain evolves during launch.

\subsection{Mechanical Actuation and Rubber-Belt Shift Mechanics}
\label{sec:lit-shift-mechanics}

Once the belt can transmit torque and generate axial reactions, the next question is how the transmission ratio changes. For rubber V-belt variators, shifting is not simply a sequence of independent steady states. During ratio change, the belt moves radially through the sheave grooves while also moving circumferentially around the pulleys and transmitting torque.

\citet{KimEtAl2002} studied the steady-state and transient characteristics of a rubber belt CVT with mechanical actuators. This work is an important bridge between actuator force models and transient behaviour. Its transient formulation, however, relies on an empirical shift-rate relationship. Such a relationship can be useful for matching observed behaviour, but it gives less direct mechanical intuition than a derivation in which the shift coordinate follows from force balance and inertia.

\citet{Sorge2008} provides a more focused theoretical treatment of rubber belt variator shift mechanics. A central point of that work is that radial belt motion during shift changes the contact problem itself. Belt mass conservation, radial motion, and circumferential transport must be considered together, and opening and closing pulleys need not behave symmetrically. \citet{CammalleriSorge2009} later develop approximate closed-form solutions for this shift mechanics problem, making the theory more accessible for engineering calculations.

The secondary helix introduces an additional coupling because the driven pulley can generate axial force as a function of transmitted torque. \citet{SorgeCammalleri2011} study the helical shift mechanics of rubber V-belt variators and provide a dedicated treatment of how helix geometry affects the driven pulley response. This is particularly relevant to torque-sensing CVTs, where the secondary is not merely a passive spring-loaded pulley but a mechanism that converts torque reaction into clamping and shift resistance.

Together, these works show that rubber-belt shift mechanics has its own structure and cannot be reduced entirely to steady belt contact. They also show why the problem is difficult to make both predictive and intuitive. Empirical shift laws, quasi-static force balances, and local contact formulations each capture part of the behaviour, but the connection between actuator forces, transmitted torque, belt motion, and ratio evolution remains a central modelling challenge.

\subsection{Transient Rubber-Belt Models and the Launch Problem}
\label{sec:lit-transient-rubber}

Transient rubber-belt models address time-dependent behaviour more directly, but the word transient covers several different levels of closure. Some models simulate local belt-node motion, some simulate shift response under prescribed or controlled axial forces, and some add engine or vehicle dynamics. These are related problems, but they do not all resolve launch in the same sense.

Launch is particularly demanding because the system is not simply shifting between two already engaged operating states. The belt may be slipping, the engine is accelerating, the primary force is developing, the secondary force depends on transmitted torque, and the vehicle load is changing as the vehicle begins to move. The model must therefore manage both the torque-transfer problem and the ratio-change problem at the same time.

\citet{JulioPlante2011} developed an experimentally validated transient model of rubber-belt CVT mechanics using a discretized belt formulation. This is one of the strongest rubber-belt dynamic references because it resolves local belt behaviour directly rather than relying only on lumped contact assumptions. Its strength is physical detail at the belt level. The associated cost is complexity: the behaviour emerges from many local element interactions, which can make the global mechanism chain harder to interpret from a tuning perspective.

\citet{Ballew2015} continues in the discretized-belt direction and is especially relevant because it expands the degrees of freedom to include engine torque output and vehicle loads such as rolling resistance and aerodynamic drag. Ballew also explicitly criticizes formulations that prescribe input pulley speed, since doing so removes an important rotational degree of freedom. In that respect, Ballew moves closer to a drivetrain-level transient simulation than models that impose the input speed as a known function of time.

Ballew's work also illustrates the practical cost of high-fidelity belt discretization. Numerical stability becomes a major issue, and additional damping between belt nodes is introduced to control resonance. In the reported simulations, the primary axial force is generated by a PI controller with feed-forward gain, and the secondary axial force is fixed to simplify the simulation \citep{Ballew2015}. This makes the work a strong reference for transient rubber-belt simulation, engine/vehicle coupling, belt material measurement, and numerical issues, but less direct as a mechanically self-actuated flyweight-and-helix launch model.

The SAE work by \citet{YiLeeKim2011} also addresses shift dynamics with an elastic rubber V-belt. Since the full derivation is less accessible than the larger theses and journal papers, it is most useful here as evidence that elastic-belt shift dynamics has been pursued in application-oriented simulation work, rather than as the main basis for the present modelling choices.

The transient rubber-belt literature therefore provides important precedents. It shows that rubber-belt CVTs can be modelled dynamically and that engine and vehicle effects can be incorporated. It also shows the tradeoff between belt-level fidelity and system-level interpretability. Highly detailed belt-node models can capture local mechanics, but they may require prescribed, controlled, or simplified actuation inputs and can become difficult to connect back to the small set of tuning variables used in practice.

\subsection{Adjacent Metal-Belt and Chain CVT Dynamic Models}
\label{sec:lit-metal-chain}

The broader CVT literature contains more mature dynamic models for metal push-belt and chain CVTs, especially in automotive applications. These systems are not direct equivalents of dry rubber V-belt CVTs, but they are important because they show how far dynamic CVT modelling has been developed in adjacent architectures.

\citet{SrivastavaHaque2009} review the dynamics and control of belt and chain CVTs, covering both modelling and control strategies for friction-limited transmissions. Their review makes clear that CVT behaviour depends strongly on architecture: rubber V-belts, metal push belts, and chains differ in contact mechanics, actuation, deformation, torque capacity, and control objectives.

For metal belt CVTs, \citet{CarboneMangialardiMantriota2005} study the influence of pulley deformation on shifting behaviour. Their model explains the distinction between creep-mode and slip-mode shifting and shows that pulley deformation can be essential to reproducing slow-shift behaviour under Coulomb-friction assumptions. \citet{YildizEtAl2017} later provide experimental validation of the Carbone--Mangialardi--Mantriota model under steady-state conditions and shift manoeuvres, supporting the usefulness of this modelling framework for controlled metal-belt CVTs.

\citet{SrivastavaHaque2006} also model the dynamics of metal V-belt CVTs, while \citet{Bonsen2004} analyze slip behaviour and the relationship between slip and transmitted torque. These works are useful analogues for understanding traction-limited behaviour, slip regimes, and dynamic ratio change, even though the transmission architecture differs.

Chain CVTs provide another adjacent comparison. \citet{DuanEtAl2016} develop a physics-based chain CVT model beginning from the kinematics and dynamics of an infinitesimal chain segment. Their model includes speed-dependent friction, pulley axial dynamics coupled with chain radial movement, and pulley deformation. This is one of the stronger examples of a physically coupled CVT model because rotational dynamics, axial dynamics, contact mechanics, and friction state are treated together.

The important distinction is that these models are usually developed for lubricated automotive metal-belt or chain systems with hydraulic or electronic clamping. Their contact physics and actuation mechanisms differ from dry rubber V-belt systems using mechanical flyweights, springs, and torque-sensitive helices. They therefore provide valuable dynamic analogues, but they do not remove the need for a rubber-belt mechanical-actuation formulation.

\subsection{Synthesis of Prior Work}
\label{sec:lit-synthesis}

The literature establishes a clear set of foundations. Practical and design references explain the tuning variables and component relationships. Design-oriented models connect geometry and actuator choices to expected operating behaviour. Rubber V-belt contact models explain axial thrust, tension distribution, slip mechanisms, and efficiency losses. Shift-mechanics studies show that ratio change introduces radial belt motion and actuator coupling. Transient rubber-belt models demonstrate that local belt dynamics can be simulated in time. Metal-belt and chain models show that coupled axial, rotational, and traction dynamics can be modelled rigorously in adjacent architectures.

The remaining difficulty is not any single missing equation, but the closure of the whole system under the assumptions appropriate to a mechanically actuated dry rubber V-belt CVT. Launch and clutching sit at the intersection of several effects: engine acceleration, primary engagement, secondary torque reaction, belt traction, shift motion, and vehicle load. Treating any one of these as prescribed can be reasonable for a focused study, but it changes the problem being solved.
...
\textbf{Primary limitation for launch modelling} \\
\hline
    ...
    \citet{KimEtAl2002}, \citet{Sorge2008}, \citet{CammalleriSorge2009}, \citet{SorgeCammalleri2011} &
    Medium &
    Mechanical actuation considered &
    Often simplified &
    Approximate, analytical, or empirical transient behaviour &
    Partial drivetrain representation \\

    Transient rubber-belt models &
    \citet{JulioPlante2011}, \citet{Ballew2015}, \citet{YiLeeKim2011} &
    High local belt/contact fidelity &
    Prescribed, controlled, or simplified force generation &
    Moderate to high &
    Dynamic, with launch and actuation assumptions &
    Complexity, numerical stability, or simplified actuator closure \\
    ...
    Different architecture, contact physics, and actuation \\
...
The literature review in Section~\ref{sec:literature-review} motivates a specific modelling objective: to build a physically interpretable dynamic formulation for a mechanically actuated dry rubber V-belt CVT during launch. The model is not intended to be the highest-fidelity representation of rubber belt contact, nor a complete loss model for efficiency prediction. Its purpose is to close the dominant system-level mechanisms into a single time-domain formulation so that launch behaviour, shift behaviour, torque transfer, and vehicle response can be studied together.

\subsection{Aim}
\label{sec:aim}

The technical aim of this work is to derive a coupled nonlinear dynamic model of the CVT and drivetrain from the engine-side pulley to the vehicle wheel. The model is formulated so that the primary pulley speed, secondary pulley speed, belt torque-transfer state, axial shift state, and vehicle motion evolve from the governing equations rather than being prescribed independently. This allows launch, shift, and traction-limited behaviour to be simulated directly in time.
...
The system considered in this work is a mechanically actuated dry rubber V-belt CVT coupled to a simplified vehicle drivetrain. The CVT consists of a speed-sensitive primary pulley, a torque-sensitive secondary pulley, a rubber V-belt, and a downstream drivetrain connected to the vehicle. The model focuses on launch and shift behaviour under one-dimensional longitudinal vehicle motion.

The model includes the following effects:

\begin{itemize}
\item variable pulley geometry and belt-length closure;
\item ratio kinematics and ratio-rate relationships;
\item primary flyweight actuation and primary spring preload;
\item secondary helix torque feedback and secondary spring loading;
\item centrifugal belt effects on the pulley wraps;
\item axial shift dynamics with effective moving mass and sheave inertia contributions;
\item rotational equations of motion for the primary and secondary pulleys;
\item engine torque input and engine-side inertia;
\item vehicle load torque from longitudinal dynamics;
\item no-slip torque demand;
\item traction-limit checks for the primary and secondary contacts;
\item slip-state selection and traction-limited torque transfer.
\end{itemize}

Several effects are intentionally idealized or excluded. The belt is treated through a lumped geometric and traction model rather than a full distributed rubber contact model. Local belt deformation, detailed radial penetration, bending hysteresis, temperature effects, belt wear, sheave compliance, detailed tribology, and full three-dimensional vehicle dynamics are not resolved. These omissions are deliberate. They keep the model low-dimensional enough to remain interpretable while still capturing the main mechanical couplings that govern launch and shift behaviour.

This scope also makes the formulation extensible. Because the assumptions are stated explicitly and the model is derived step by step, later work can replace individual closures with higher-fidelity alternatives. For example, the traction-limit model could be replaced with a more detailed distributed contact model, the belt could be made elastic, pulley deformation could be introduced, or thermal and efficiency losses could be added without changing the overall purpose of the framework.
...
\begin{enumerate}
\item A unified nonlinear time-domain model for a mechanically actuated rubber V-belt CVT coupled to engine-side and vehicle-side dynamics.

\item A pulley geometry and belt-length closure formulation that relates axial shift position to pulley radii, wrap angles, transmission ratio, and ratio rate.

\item A first-principles primary axial-force model incorporating centrifugal flyweight actuation, ramp geometry, and spring preload.

\item A torque-reactive secondary axial-force model incorporating helix geometry, transmitted torque, and spring loading.

\item An axial shift equation of motion in which the shift coordinate is treated as a dynamic state rather than an instantaneous equilibrium variable.

\item A belt centrifugal-force correction that modifies the effective clamping contribution on the pulley wraps at speed.

\item A rotational dynamics formulation for the primary pulley, secondary pulley, and belt torque path, allowing engine speed and output speed to evolve dynamically.

\item A no-slip torque-demand calculation that determines the torque required to enforce kinematic compatibility between the belt and pulleys.

\item A traction-admissibility framework that compares no-slip torque demand against the available friction-limited torque capacity at each pulley.

\item A slip-branch formulation for traction-limited operation when no-slip compatibility cannot be maintained.

\item Coupling of the CVT model to one-dimensional vehicle longitudinal dynamics, allowing road-load and launch simulations.

\item A physically interpretable derivation structure intended to make the model usable as both a simulation framework and a foundation for future higher-fidelity extensions.


\end{enumerate}

The central contribution is the system-level closure. Rather than prescribing the ratio, prescribing the shift rate, or prescribing the pulley speeds, the model allows the launch trajectory to emerge from the interaction of mechanical actuation, belt traction, rotational inertia, and vehicle load. This makes the formulation especially useful for studying how tuning changes propagate through the mechanism chain from component forces, to torque admissibility, to slip behaviour, to vehicle acceleration.
