...
open-source CINDER simulator.  The numerical details required to carry that
formulation into an executable model are documented in
\Cref{app:numerical_implementation}, including the solution of the implicit belt
geometry, the coupled mechanical and traction closure, actuator assembly, and
hybrid time integration.  The Results reported in this chapter are generated
with the frozen CINDER release identified at the end of this section.

CINDER was developed around a Baja SAE-scale mechanically actuated CVT, and the
reference configuration in \Cref{app:reference_configuration} is based closely
on that physical system.  Much of the chapter therefore occupies the speed,
torque, power, ratio, inertia, and loading scales of a small off-road vehicle
rather than attempting a broad sweep over CVT applications.  The quantitative
magnitudes reported for those cases are most directly supported within and near
that operating regime; their extrapolation to substantially different machines
is not established by the present Results.

The supporting material is separated according to what it defines.
\Cref{app:numerical_implementation} records the numerical implementation of the
CVT equations, while \Cref{app:reference_configuration} collects the recurring
physical configuration and parameter provenance.  The exact study and case
definitions, including numerical settings and study-specific configurations,
are indexed in \Cref{app:study_reproducibility}.  The same appendix traces the
manuscript figures and tables to their archived source artifacts through
\Cref{app:artifact_index}.

The shaft-boundary models used by the Results are documented separately in
\Cref{app:results_boundary_models}.  As established in
\Cref{sec:system_definition}, the CVT model ends at the primary and secondary
shafts; a complete simulation must additionally specify the mechanical systems
acting through those interfaces.  The cases in this chapter use several such
descriptions, ranging from simplified engine and vehicle models grounded in
physical systems to prescribed torque or load histories and attached inertias
used for controlled numerical experiments.  The boundary-model appendix records
the equations, assumptions, parameters, and provenance of these external
systems, keeping them explicit without folding them into the CVT formulation.

The simulator implementation and the Results record are frozen separately.
The former identifies the exact CINDER source used to evaluate the model; the
latter identifies the study definitions, analysis code, and archived outputs
used in the manuscript.  Their publication records are
...
\toprule
Record & Identifier \\ \midrule
CINDER release & \CINDERtag\; (\texttt{\CINDERcommit}) \\
Tagged CINDER source &
\url{https://github.com/gr812b/CVT-Simulator/tree/cinder-v1.1.2} \\
Results release & \ResultsTag\; (\texttt{\ResultsCommit}) \\
Results archive & \ResultsDOI \\ \bottomrule
\end{tabular}
\end{table}

\noindent
The release tags identify the named frozen versions, the commits identify the
exact repository revisions, and \ResultsDOI provides the persistent archival
record of the published Results package.
