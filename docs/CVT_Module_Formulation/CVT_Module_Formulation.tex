...
The motion of a CVT can only be described once the mechanical system itself is
clear. A physical CVT is connected to other machinery and contains more detail
than a model can usefully resolve. A model must therefore identify which
hardware belongs to the CVT, how the surrounding machinery acts on it, and
which features of the real hardware are retained.

This chapter establishes that shared mechanical description. It defines the
modeled CVT and its connections to the surrounding machinery, states the
assumptions used to idealize the hardware, and fixes the coordinates and sign
conventions used to describe its motion. Together, these choices define the
mechanical problem represented by the formulation.
...
the belt runs and shifts. It follows the curved modeled belt path, but it does
...
information that must be carried from one instant to the next. Combined with
the fixed model data, the current shaft-interface conditions, and the active
mechanical constraints, they are sufficient for the formulation to determine
