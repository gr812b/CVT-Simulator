...
The derivation has brought the belt, pulleys, and actuators into a single
coupled model. The first numerical studies examine whether that model produces
motion consistent with all of its governing relations. The forces exchanged
between components must satisfy their respective balances, energy must be
accounted for as it is transferred, stored, and dissipated, and changes of
contact or support must produce admissible outgoing states.

This verification provides the basis for interpreting the transient mechanisms
investigated later in the chapter. A plausible shift trajectory alone does not
establish that the internal mechanics are consistent or that their evolution
is adequately resolved. The studies therefore examine both the consistency of
the assembled model and the numerical reliability of its response across the
conditions tested here.

\subsection{Mechanical Invariants and Operating-Domain Audit}
\label{sec:results_mechanical_invariants}

The first test was deliberately broader than the nominal vehicle trajectory.
CINDER contains distinct equations for the deadzone, free shifting, structural
stops, sticking contact, either single-contact slip state, and both-contact
slip. If those branches are to represent one mechanical model rather than a
collection of locally valid equations, each must admit states that satisfy the
same geometry, tension, contact, and closure requirements.

The operating-domain audit therefore searched for and propagated states in
each required class. Across 6,257 audited states and 81 hybrid transitions, all
25 requested coverage classes were realized. These included static rest,
deadzone motion, engagement, free shift in both directions, the low-ratio seat
and upper stop, forward and reverse shaft rotation, both individual slip
directions at each pulley, and all four signed both-slip combinations. No
accepted state violated the retained mechanical admissibility conditions, no
exact hybrid-successor check failed, and the largest row-scaled residual of the
engaged mechanical closure was \(1.11\times10^{-14}\).

The different contact regimes were not equally easy to realize in the reference
configuration. Several appeared naturally and could remain on their requested
branch for the full audit interval, whereas others required targeted initial
conditions and rapidly evolved toward restick. Secondary positive slip, for
example, persisted for approximately \(414~\mu\mathrm{s}\), while the
\((-,+)\) both-slip state persisted for only approximately
\(9.8~\mu\mathrm{s}\) in its canonical test. These observations are specific
to the investigated CVT and loading set; they are not taken to imply that the
same regimes are intrinsically rare for every CVT. They do show, however, that
some portions of the hybrid state space occupy much narrower dynamical regions
than the ordinary stick and single-slip states.

The limiting condition in these difficult cases was also informative. Loss of
admissibility often first appeared in the distributed wrap contact, through a
local normal load approaching zero, rather than through loss of the integrated
pulley-normal resultant. Thus the audit exercised not only the gross regime
logic but also the distributed contact mechanics retained within each pulley
wrap.

The resulting coverage is useful beyond verification. It demonstrates that the
same formulation can move continuously among a comparatively large set of
mechanical behaviors without replacing the underlying model or prescribing the
contact state externally. Later studies use only a subset of this capability,
but the broader audit establishes that those trajectories sit inside a much
larger mechanically admissible hybrid domain.

\subsection{Energy Consistency}
\label{sec:results_energy_consistency}

Mechanical admissibility does not by itself ensure that the coupled equations
exchange energy consistently. CINDER therefore records the work crossing the
two shaft boundaries, the change in modeled stored energy, and each retained
irreversible loss channel along the complete hybrid trajectory.

For the reference trajectory, the net external work was
\(52.168893~\mathrm{kJ}\). Of this, \(50.580274~\mathrm{kJ}\) remained as a
change in modeled stored energy and \(1.588340~\mathrm{kJ}\) was dissipated by
kinetic belt--pulley slip. Discrete impact and capture events dissipated a much
smaller \(0.253859~\mathrm{J}\). The final unexplained balance was only
\(0.025444~\mathrm{J}\), or \(4.88\times10^{-7}\) of the magnitude of the
external work.

This decomposition is also useful for interpreting what CINDER regards as an
energy loss. Conservative actuator springs and the retained inertial elements
store and return energy; they do not contribute directly to dissipation.
Irreversible loss in the present formulation enters through kinetic slip and
through the perfectly inelastic velocity updates applied at finite-speed
capture events. These are the loss mechanisms represented by the model, not an
exhaustive inventory of losses in a physical CVT. Belt hysteresis, bearing and
seal drag, windage, and other parasitic mechanisms omitted from the formulation
are therefore absent from this accounting.

The residual magnitude was consistent with numerical integration and
quadrature error. Refinement of the audit quadrature gave a Richardson estimate
of \(0.024510~\mathrm{J}\), essentially the same scale as the observed final
defect. Individual discrete events satisfied their energy relation to within
\(2.27\times10^{-13}~\mathrm{J}\), and tightening the trajectory integration
changed the final state by only \(1.30\times10^{-7}\) in the normalized metric
while reversing the sign, but not the scale, of the remaining energy residual.

The energy audit therefore serves two purposes. It verifies that the retained
mechanics close energetically to numerical accuracy, and it establishes a
common bookkeeping framework for later comparisons in which different CVT
configurations may redistribute energy between useful boundary work, stored
mechanical energy, and the modeled dissipative channels.

\subsection{Solver Convergence}
\label{sec:results_solver_convergence}

The preceding tests examine the mechanics represented by CINDER. A separate
question is whether the simulated trajectory depends materially on the
numerical controls used to integrate those mechanics. This distinction is
particularly important for a hybrid system: two trajectories can remain nearly
identical in shaft speed and shift position while differing in the timing or
even the sequence of brief contact-state transitions.

The convergence study therefore compared both the continuous state trajectory
and the discrete hybrid history. A tight numerical reference used
\(\mathrm{rtol}=10^{-7}\), \(\mathrm{atol}=10^{-10}\), and a
\(2.5~\mathrm{ms}\) maximum step. The formal sweep then varied relative
tolerance over eight levels and maximum step over five levels. Every tested
maximum step from \(5\) to \(100~\mathrm{ms}\) satisfied all review criteria
once \(\mathrm{rtol}\le10^{-4}\), forming a contiguous converged region rather
than an isolated passing point.

The retained research setting,
\[
\mathrm{rtol}=10^{-4},\qquad
\mathrm{atol}=10^{-7},\qquad
\Delta t_{\max}=10~\mathrm{ms},
\]
lay comfortably within this region. Relative to the tight reference, its
event-aligned normalized trajectory RMS error was
\(4.27\times10^{-7}\), its maximum normalized state error was
\(6.96\times10^{-5}\), and the largest corresponding event-time error was only
\(7.99~\mu\mathrm{s}\). The complete 16-transition signature was reproduced,
with an exact regime-history mismatch fraction of
\(8.46\times10^{-6}\).

Comparison across hybrid trajectories requires some care. A direct
absolute-time norm can compare the state immediately before an event in one
solution with the state immediately after the corresponding event in another
solution when the two event times differ by only microseconds. Such a
comparison produces an artificial \(O(1)\) pointwise error at the reset even
when the two hybrid trajectories have otherwise converged. The reported state
errors were therefore evaluated between corresponding smooth segments using a
common normalized phase coordinate, while event timing and regime-history
differences were measured separately.

The broader sweep also exposed a useful distinction between mechanics-level and
outcome-level convergence. At sufficiently loose settings, brief stick--slip
transitions could be missed or altered while the macroscopic RPM, belt-speed,
and shift trajectories remained visually close to the converged solution.
Gross vehicle response can therefore appear robust beyond the point at which
the internal contact mechanics have ceased to converge. Because CINDER is
intended to resolve those internal mechanics, the stricter hybrid criterion is
used throughout the remaining studies.

\subsection{Closure Conditioning and Root Structure}
\label{sec:results_closure_structure}

The engaged contact problem contains two distinct numerical layers. For any
proposed pair of contact tractions, the instantaneous mechanical response is
obtained from the linear \(8\times8\) closure described in
\Cref{app:num_fixed_lambda}. When one or both contacts are assumed to stick,
the corresponding traction values must then be selected so that the sticking
residuals vanish.

The linear mechanical closure was well behaved throughout the practical domain
examined. No rank-deficient point was found anywhere in the sampled physical
or expanded traction maps, and the system remained solvable even through the
strong conditioning structures visible near the edges of the admissible
region. Among the representative physical maps, the largest scaled
\(8\times8\) condition number was approximately \(3.4\times10^{5}\).
Consequently, for a specified traction pair, the implemented closure produced a
single instantaneous mechanical solution throughout the investigated operating
domain. The retained torque variables and the reason for the eight-variable
implementation, rather than the reduced form used in the formulation, are
described in \Cref{app:num_fixed_lambda}.

The nonlinear sticking problem was similarly unambiguous at the representative
operating states. Fifteen frozen states were each solved repeatedly from a
broad set of initial traction guesses, giving 375 multistart attempts in total.
Within each state, every accepted solution collapsed to the same physical root.
Thus the ordinary operating cases showed no evidence that the contact solver
was selecting arbitrarily among several admissible sticking solutions.

A deliberately broader search nevertheless revealed that global uniqueness is
not guaranteed by the reduced contact equations. The stick--stick solution
surface can fold, allowing a fixed loading slice to intersect the same
continuous solution manifold twice. A corresponding fold was also found in a
mixed contact state, where one interface was already sliding and the remaining
scalar sticking residual possessed two roots. These structures are therefore
properties of the nonlinear closure itself rather than numerical noise.

Their practical context is important. The multiple-root examples were found
only after explicitly searching for them over extended operating and traction
domains. In the mixed-contact search, 62 frozen states reduced to only one
unique geometry exhibiting the scalar fold, and one of the two roots lay close
to local contact loss. No spontaneous root ambiguity of this type has appeared
in the maintained transient studies. The result should therefore not be read
as evidence that ordinary CINDER trajectories are routinely non-unique; it
identifies a genuine global limit of the reduced sticking closure that becomes
visible near selected, often boundary-adjacent regions.

The numerical solution strategy is consistent with that structure. During
time integration, the contact solve uses the previously accepted traction root
as a continuation estimate and first attempts to remain on the locally
connected branch before reverting to the full bounded nonlinear solve. The
continuation and fallback procedure is described in
\Cref{app:num_contact_continuation}. This does not remove the possibility of a
fold, but it prevents each right-hand-side evaluation from behaving as an
independent global root-selection problem while the existing branch remains
available.

The conditioning study therefore gives both a positive and a limiting result.
The fixed-traction \(8\times8\) mechanics are nonsingular and uniquely solvable
throughout the investigated practical domain, and representative sticking
states converge robustly to a single accepted physical root. At the same time,
the nonlinear closure possesses rare folded regions in a wider state space.
Their existence motivates branch-aware continuation and provides a concrete
boundary on the global uniqueness that can be claimed for the reduced contact
model.
...
% TODO after Results freeze: independent model-to-model benchmark against Ballew.

\subsection{Benchmark Reconstruction}
\label{sec:results_ballew_reconstruction}
% TODO: reconstructed geometry, inputs, and implementation assumptions.

\subsection{Force-Replay Protocol}
\label{sec:results_ballew_force_replay}
% TODO: prescribed forcing comparison.

\subsection{Reconstructed Closed-Loop Protocol}
\label{sec:results_ballew_closed_loop}
% TODO: closed-loop comparison using reconstructed external plant/actuator logic.

% ==============================================================
\section{Dynamic Mechanics Retained by CINDER}
\label{sec:results_dynamic_mechanics}

% TODO after Results freeze: quantify when retained transient terms matter.

\subsection{Primary Actuation Dynamics}
\label{sec:results_primary_actuator_dynamics}
% TODO: quasi-static versus dynamic primary-actuation study.

\subsection{Secondary Helix Dynamics and Contact Topology}
\label{sec:results_secondary_helix}
% TODO: dynamic helix terms and selected-flank / bilateral topology study.

\subsection{Reduced-Belt Transient Mechanics}
\label{sec:results_reduced_belt_mechanics}
% TODO: belt acceleration, shift acceleration, curvature, moving-radius, density continuation.
...
\chapter{Reference Configuration and Boundary Models}
\label{app:reference_configuration}

The numerical studies use a common Baja-class CVT and host configuration unless
an individual study states an override.  This appendix records the physical
parameters shared by that reference configuration and derives the shaft-boundary
models used to drive and load the CVT.  Study-specific initial states, boundary
overrides, search domains, and numerical settings are kept with the individual
study definitions in \Cref{app:study_reproducibility}.

The working Results configuration is stored in
\ResultsCode{defaults/baja/simulation_case.json}{defaults/baja/simulation_case.json}.
The parameter tables below summarize the quantities that are repeatedly used in
the paper; the machine-readable document remains the complete definition.
...
\setlength{\LTleft}{0pt}
\setlength{\LTright}{0pt}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.40\textwidth}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedleft\arraybackslash}p{0.17\textwidth}@{}}
\toprule
...
\endfoot
\bottomrule
\caption{Common physical parameters of the Baja-class reference CVT.}
\label{tab:results_common_parameters}\\
\endlastfoot
\multicolumn{4}{@{}l}{\textbf{Belt and pulley geometry}} \\
Belt & Outer length & $L_b$ & \SI{0.953262}{\meter} \\[3pt]
...
Pulley geometry & Deadzone shift & $s_{\mathrm{dz}}$ & \SI{0.0024892}{\meter} \\[3pt]
Pulley geometry & Maximum shift & $s_{\max}$ & \SI{0.01905}{\meter} \\[5pt]
\multicolumn{4}{@{}l}{\textbf{Belt--pulley contact}} \\
Contact & Static traction coefficient & $\mu_s$ & $0.65$ \\[3pt]
Contact & Kinetic traction coefficient & $\mu_k$ & $0.55$ \\[5pt]
\multicolumn{4}{@{}l}{\textbf{Primary actuation and inertia}} \\
Primary & Number of flyweights & $n_f$ & $3$ \\[3pt]
Primary & Mass per flyweight & $m_f$ & \SI{0.263646}{\kilogram} \\[3pt]
Primary & Fixed rotating hardware inertia & --- & \SI{0.0032202838}{\kilogram\meter\squared} \\[3pt]
...
\multicolumn{4}{@{}l}{\textbf{Secondary actuation and inertia}} \\
Secondary & Fixed rotating hardware inertia & $I_{s,F}$ & \SI{0.00232626}{\kilogram\meter\squared} \\[3pt]
...
Secondary & Initial torsional preload & $\theta_{s,0}$ & $300^\circ$ \\[3pt]
Secondary & Helix reference radius & $r_h$ & \SI{0.04445}{\meter} \\[3pt]
Secondary & Constant helix track angle & $\alpha_s$ & $70^\circ$ \\[3pt]
Secondary & Movable-face share of belt torque & --- & $0.5$ \\
\end{longtable}
\normalsize
\end{center}

The full primary ramp and flyweight geometry is retained in the same reference
configuration and is not duplicated in this table.  The primary flyweight
mass-property construction is documented separately in the flyweight appendix.

Maintained Results studies instantiate the reference configuration through the
shared Results decoder in
\ResultsCode{defaults/reference_model/reference_case.py}{defaults/reference_model/reference_case.py}.
For the Results studies this decoder permits the signed secondary-helix reaction
to pass through zero and be carried by the opposite slot flank.  The helix force,
spring, inertia, and kinematic equations themselves are unchanged; this is an
implementation detail of the study harness rather than an additional model
equation.

\section{Reference Shaft Boundary Models}
\label{app:results_boundary_models}

The CVT equations are closed at the primary and secondary shafts by boundary
models that return a signed external torque and an equivalent inertia referred
to that shaft.  The reference Baja configuration uses a full-throttle engine
boundary on the primary and a rigid final-drive vehicle boundary on the
secondary.  These definitions are external to the CVT mechanics and can be
replaced study by study without changing the internal CVT equations.

\subsection{Primary full-throttle engine boundary}
\label{app:primary_engine_boundary}

The primary boundary contributes the net crankshaft torque and the rotational
inertia of the engine-side hardware directly at the primary shaft,
...
\(I_{\mathrm{eng}}=\SI{0.05}{\kilogram\meter\squared}\).
The positive full-throttle torque curve is specified by the tabulated points in
\Cref{tab:results_engine_curve}.  CINDER evaluates the curve with a
shape-preserving piecewise cubic Hermite interpolant.  Two bounded braking tails
allow the primary to be driven externally outside the positive-torque speed
range: the low-speed extension passes through
\((0,0)\), \((500~\mathrm{rpm},-5~\mathrm{N\,m})\), and
\((1000~\mathrm{rpm},0)\); above the final zero-torque point at
\(4000~\mathrm{rpm}\), the curve transitions to
\(-28~\mathrm{N\,m}\) at \(5500~\mathrm{rpm}\) and remains at that value
from \(7000~\mathrm{rpm}\) upward.  Non-positive shaft speed returns zero
engine torque.

\begin{table}[H]
\centering
\small
\caption{Positive-speed full-throttle torque points of the reference primary boundary.}
...
The implementation of the interpolation and shaft boundary is given by
\CINDERcode{cvtModel/src/cinder/model/boundaries/engine/torque_curve.py}{engine/torque_curve.py}
and
\CINDERcode{cvtModel/src/cinder/model/boundaries/shaft/__init__.py}{boundaries/shaft/__init__.py}.

\subsection{Secondary locked-final-drive vehicle boundary}
\label{app:secondary_vehicle_boundary}

The reference secondary boundary assumes a rigid final drive and pure rolling at
the wheel.  With the final-drive ratio defined by

\begin{equation}
G
=
\frac{\omega_s}{\omega_w},
\label{eq:reference_final_drive_ratio}
\end{equation}

the wheel speed and vehicle speed are
...
=
\frac{r_w}{G}\omega_s.
\label{eq:reference_vehicle_speed}
\end{equation}

The host retains the accumulated secondary-shaft angle \(\psi_s\),
...
=
\frac{r_w}{G}\psi_s,
\label{eq:reference_vehicle_distance}
\end{equation}

so a road profile may be evaluated as a function of vehicle distance.  Let
\(\gamma(x)\) be the road angle, positive uphill.  The longitudinal force
applied to the vehicle by grade, rolling resistance, and aerodynamic drag is
...
F_{\mathrm{roll}}
&=
-C_{\mathrm{rr}}mg\cos\gamma\,
\frac{v}{\sqrt{v^2+v_\epsilon^2}},
\\
F_{\mathrm{aero}}
&=
-\frac{1}{2}\rho C_dA_f\lvert v\rvert v,
\\
F_{\mathrm{road}}
&=
F_{\mathrm{grade}}+F_{\mathrm{roll}}+F_{\mathrm{aero}}.
\label{eq:reference_road_load}
\end{align}

The regularization speed \(v_\epsilon\) makes the rolling-resistance direction
continuous through zero vehicle speed.  The corresponding torque applied to the
secondary shaft is obtained by wheel-force and final-drive conversion,

\begin{equation}
\tau_{s,\mathrm{ext}}
=
\frac{r_w}{G}F_{\mathrm{road}}.
\label{eq:reference_secondary_road_torque}
\end{equation}

Vehicle translation and wheel rotation are rigidly tied to \(\omega_s\), so
their inertias are reflected to the secondary shaft.  Including the direct
secondary-side drivetrain inertia \(I_{s,\mathrm{dir}}\), the equivalent
boundary inertia is
...
I_{s,\mathrm{dir}}
+
\frac{I_{\mathrm{wheel}}}{G^2}
+
m\left(\frac{r_w}{G}\right)^2.
\label{eq:reference_secondary_reflected_inertia}
\end{equation}

For the reference parameters in \Cref{tab:results_vehicle_boundary_parameters},
this gives
\(I_{s,\mathrm{ext}}=\SI{0.431610}{\kilogram\meter\squared}\).
Because the translational mass already appears in this reflected inertia, the
road-load expression in \Cref{eq:reference_road_load} contains only externally
applied forces and no separate vehicle-acceleration term.

\begin{table}[H]
\centering
\small
\caption{Parameters of the reference locked-final-drive vehicle boundary.}
\label{tab:results_vehicle_boundary_parameters}
\begin{tabular}{@{}p{0.50\textwidth}p{0.18\textwidth}r@{}}
\toprule
Quantity & Symbol & Value \\ \midrule
Vehicle mass & $m$ & \SI{300}{\kilogram} \\
Wheel rotational inertia & $I_{\mathrm{wheel}}$ & \SI{1.10781744}{\kilogram\meter\squared} \\
Final-drive ratio & $G$ & $7.556$ \\
Wheel rolling radius & $r_w$ & \SI{0.2794}{\meter} \\
Rolling-resistance coefficient & $C_{\mathrm{rr}}$ & $0.015$ \\
...
Gravitational acceleration & $g$ & \SI{9.80665}{\meter\per\second\squared} \\
...
The final-drive kinematics, road-load force model, and secondary boundary are
implemented in
\CINDERcode{cvtModel/src/cinder/model/boundaries/vehicle/final_drive.py}{vehicle/final_drive.py},
\CINDERcode{cvtModel/src/cinder/model/boundaries/vehicle/road_load.py}{vehicle/road_load.py},
and
\CINDERcode{cvtModel/src/cinder/model/boundaries/shaft/__init__.py}{boundaries/shaft/__init__.py}.

\chapter{Study Configurations and Reproducibility}
\label{app:study_reproducibility}

The Results archive contains study-specific case construction, numerical
settings, and analysis code.  This appendix is therefore organized by study,
rather than by one nominal configuration table that obscures differences among
the verification problems.  The links below currently point to the working
\texttt{develop} Results tree through the \verb|\ResultsCode| and
\verb|\ResultsTree| helpers; they will be changed to the frozen Results revision
when the paper-facing archive is finalized.

The four internal-verification studies use CINDER \CINDERtag.  Their maintained
entry points live under
\ResultsTree{studies}{results/cinder-v1.1.2/studies}.

\section{Mechanical-Invariant Operating-Domain Audit}
\label{app:study_mechanical_invariants}

The mechanical-invariant study combines one reference Baja trajectory with
constructed fixed-boundary bench cases, structural-boundary arrivals, rare
contact-state reproduction anchors, geometry checks, and negative controls.
Its maintained definition and implementation are
\ResultsCode{studies/mechanical-invariants/study.json}{mechanical-invariants/study.json},
\ResultsCode{studies/mechanical-invariants/run.py}{mechanical-invariants/run.py},
and
\ResultsCode{studies/mechanical-invariants/REFERENCE.md}{mechanical-invariants/REFERENCE.md}.
Reusable operating cases are owned by
\ResultsCode{defaults/verification/operating_cases.json}{defaults/verification/operating_cases.json}.

The constructed bench cases replace the engine and vehicle boundaries with the
fixed shaft boundary

\begin{equation}
\tau_{j,\mathrm{ext}}=\bar\tau_j,
\qquad
I_{j,\mathrm{ext}}=\bar I_j,
\label{eq:invariants_fixed_shaft_boundary}
\end{equation}

while leaving the CVT mechanics unchanged.  The implementation is
\CINDERcode{cvtModel/src/cinder/model/boundaries/shaft/__init__.py}{FixedShaftBoundary}.

\subsection{Numerical settings by case family}

\begin{center}
\small
\setlength{\LTleft}{0pt}
\setlength{\LTright}{0pt}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedleft\arraybackslash}p{0.10\textwidth}>{\raggedleft\arraybackslash}p{0.11\textwidth}>{\raggedleft\arraybackslash}p{0.11\textwidth}>{\raggedleft\arraybackslash}p{0.12\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}@{}}
\toprule
Case family & Duration & $\mathrm{rtol}$ & $\mathrm{atol}$ & Max. step & Audit / sampling \\ \midrule
\endfirsthead
\toprule
Case family & Duration & $\mathrm{rtol}$ & $\mathrm{atol}$ & Max. step & Audit / sampling \\ \midrule
\endhead
...
\endfoot
\bottomrule
\caption{Numerical settings used by the mechanical-invariant audit.}
\label{tab:invariants_numerical_settings}\\
\endlastfoot
Reference Baja trajectory & \SI{10}{\second} & $3\times10^{-5}$ & $3\times10^{-8}$ & \SI{10}{\milli\second} & \SI{2}{\milli\second} audit grid \\
Standard contact search & \SI{30}{\milli\second} & $3\times10^{-5}$ & $3\times10^{-8}$ & \SI{1}{\milli\second} & \SI{0.5}{\milli\second} audit grid \\
Rare-state anchors & \SI{30}{\milli\second} & $3\times10^{-5}$ & $3\times10^{-8}$ & \SI{0.5}{\milli\second} & \SI{0.25}{\milli\second} audit grid \\
Free-shift search & \SI{20}{\milli\second} & $3\times10^{-5}$ & $3\times10^{-8}$ & \SI{1}{\milli\second} & \SI{0.5}{\milli\second} audit grid \\
Structural-boundary arrivals & \SI{30}{\milli\second} & $3\times10^{-5}$ & $3\times10^{-8}$ & \SI{0.5}{\milli\second} & \SI{0.25}{\milli\second} audit grid \\
Static-rest case & \SI{50}{\milli\second} & $3\times10^{-5}$ & $3\times10^{-8}$ & \SI{2}{\milli\second} & \SI{1}{\milli\second} audit grid \\
Geometry-domain sweep & --- & --- & --- & --- & 401 shift points; \SI{1e-6}{\meter} finite-difference step \\
\end{longtable}
\normalsize
\end{center}

The contact-search and structural cases use an event-time tolerance of
\(10^{-11}\,\mathrm{s}\); the 10-s reference trajectory uses
\(10^{-10}\,\mathrm{s}\).

\subsection{Requested tangential contact classes}

The sign in \Cref{tab:invariants_contact_requests} is the sign of the represented
belt--pulley relative velocity on a slipping interface.  A zero denotes a
sticking interface.

\begin{table}[H]
\centering
\small
\caption{Tangential contact classes requested by the mechanical-invariant audit.}
\label{tab:invariants_contact_requests}
\begin{tabular}{@{}p{0.32\textwidth}p{0.20\textwidth}p{0.20\textwidth}p{0.16\textwidth}@{}}
\toprule
Case ID & Primary & Secondary & Overall rotation \\ \midrule
\texttt{stick\_stick\_forward} & stick & stick & forward \\
\texttt{stick\_stick\_reverse} & stick & stick & reverse \\
\texttt{primary\_slip\_plus} & slip $(+)$ & stick & forward \\
\texttt{primary\_slip\_minus} & slip $(-)$ & stick & forward \\
\texttt{secondary\_slip\_plus} & stick & slip $(+)$ & forward \\
\texttt{secondary\_slip\_minus} & stick & slip $(-)$ & forward \\
\texttt{both\_slip\_pp} & slip $(+)$ & slip $(+)$ & forward \\
\texttt{both\_slip\_pm} & slip $(+)$ & slip $(-)$ & forward \\
\texttt{both\_slip\_mp} & slip $(-)$ & slip $(+)$ & forward \\
\texttt{both\_slip\_mm} & slip $(-)$ & slip $(-)$ & forward \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Bench search domain}

The ordinary deterministic search draws from the standard domain first and
expands only when required.  The fixed-boundary inertias used by the standard
bench are \(\bar I_p=\SI{0.03}{\kilogram\meter\squared}\) and
\(\bar I_s=\SI{0.05}{\kilogram\meter\squared}\).

\begin{table}[H]
\centering
\small
\caption{Standard and extended search domains used to construct contact states.}
\label{tab:invariants_contact_search_domain}
\begin{tabular}{@{}p{0.27\textwidth}p{0.33\textwidth}p{0.33\textwidth}@{}}
\toprule
Quantity & Standard search & Extended search \\ \midrule
Engaged shift fraction & $0.25,\ 0.50,\ 0.75$ & $0.10,\ 0.90$ \\
Belt speed [m/s] & $5,\ 8,\ 12,\ 16$ & $3,\ 20$ \\
Slip-speed magnitude [m/s] & $0.3,\ 1,\ 3$ & $0.1,\ 5$ \\
Primary boundary torque [N\,m] & $0,\ \pm5,\ \pm10,\ \pm20,\ \pm35$ & $0,\ \pm20,\ \pm35,\ \pm60$ \\
Secondary boundary torque [N\,m] & $0,\ \pm5,\ \pm10,\ \pm20,\ \pm35$ & $0,\ \pm20,\ \pm35,\ \pm60$ \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Canonical stick, free-shift, and structural cases}

\begin{center}
\small
\setlength{\LTleft}{0pt}
\setlength{\LTright}{0pt}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedleft\arraybackslash}p{0.13\textwidth}>{\raggedleft\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}p{0.33\textwidth}@{}}
\toprule
Case ID & Shift fraction & $\dot s$ [m/s] & Purpose \\ \midrule
\endfirsthead
\toprule
Case ID & Shift fraction & $\dot s$ [m/s] & Purpose \\ \midrule
\endhead
...
\endfoot
\bottomrule
\caption{Canonical engaged stick and free-shift cases in the shared verification library.}
\label{tab:invariants_canonical_engaged_cases}\\
\endlastfoot
\texttt{stick\_mid\_forward} & $0.50$ & $0$ & Interior forward stick--stick \\
\texttt{stick\_mid\_reverse} & $0.50$ & $0$ & Interior reverse-rotation stick--stick \\
\texttt{stick\_low\_forward} & $0.20$ & $0$ & Low-ratio interior stick--stick \\
\texttt{stick\_high\_forward} & $0.80$ & $0$ & High-ratio interior stick--stick \\
\texttt{stick\_mid\_stressed} & $0.50$ & $0$ & Interior stick state searched with the largest admissible fixed-shaft loads \\
\texttt{free\_shift\_closing\_30} & $0.30$ & $+0.02$ & Engaged closing / upshift \\
\texttt{free\_shift\_closing\_50} & $0.50$ & $+0.02$ & Mid-ratio closing / upshift \\
\texttt{free\_shift\_closing\_70} & $0.70$ & $+0.02$ & Late-ratio closing / upshift \\
\texttt{free\_shift\_opening\_30} & $0.30$ & $-0.02$ & Engaged opening / backshift \\
\texttt{free\_shift\_opening\_50} & $0.50$ & $-0.02$ & Mid-ratio opening / backshift \\
\texttt{free\_shift\_opening\_70} & $0.70$ & $-0.02$ & Late-ratio opening / backshift \\
\end{longtable}
\normalsize
\end{center}

The structural-boundary search starts \SI{0.05}{\milli\meter} from the target
boundary and uses shift-speed magnitudes of $0.01$, $0.02$, and
$0.05~\mathrm{m/s}$.  The four requested arrivals are listed in
\Cref{tab:invariants_structural_requests}.

\begin{table}[H]
\centering
\small
\caption{Structural-boundary transitions explicitly requested by the invariant audit.}
\label{tab:invariants_structural_requests}
\begin{tabular}{@{}p{0.38\textwidth}p{0.34\textwidth}p{0.18\textwidth}@{}}
\toprule
Case ID & Target event & Approached side \\ \midrule
\texttt{boundary\_lower\_stop\_arrival} & \texttt{cvt:lower\_stop\_reached} & lower stop \\
\texttt{boundary\_engagement\_arrival} & \texttt{cvt:engagement\_reached} & engagement \\
\texttt{boundary\_low\_ratio\_arrival} & \texttt{cvt:low\_ratio\_seat\_reached} & low-ratio seat \\
\texttt{boundary\_upper\_stop\_arrival} & \texttt{cvt:upper\_stop\_reached} & upper stop \\
\bottomrule
\end{tabular}
\end{table}

The same audit also contains a true zero-speed, zero-external-torque static
lower-stop case, an interior deadzone snapshot, invalid shift-domain and
deadzone-lock negative controls, and the one-sided engagement classifier checks.

\subsection{Rare deterministic reproduction anchors}

Two contact classes were difficult to recover by the ordinary deterministic
search and therefore retain exact release-scoped reproduction anchors.  These
anchors do not bypass the production classifier or the mechanical audit; the
state counts only if its requested branch is accepted and the subsequent hybrid
continuation remains admissible.

\begin{table}[H]
\centering
\small
\caption{Exact initial conditions of the two rare contact-state reproduction anchors.}
\label{tab:invariants_rare_anchors}
\begin{tabular}{@{}p{0.40\textwidth}rr@{}}
\toprule
Quantity & \texttt{secondary\_slip\_plus} & \texttt{both\_slip\_mp} \\ \midrule
$\omega_p$ [rad/s] & 272.7864 & 536.8700 \\
$\omega_s$ [rad/s] & 223.0145 & 173.9880 \\
$v_b$ [m/s] & 17.21139 & 17.24540 \\
$s$ [m] & 0.01446320 & 0.00292236 \\
$\dot s$ [m/s] & 0 & 0 \\
$\bar\tau_p$ [N\,m] & -38.5969 & -105.5810 \\
$\bar\tau_s$ [N\,m] & -17.9844 & -282.0736 \\
$\bar I_p$ [kg\,m$^2$] & 0.0127909 & 0.0646801 \\
$\bar I_s$ [kg\,m$^2$] & 0.0412725 & 0.00250569 \\
Initial requested-branch dwell [s] & $4.1444\times10^{-4}$ & $9.8232\times10^{-6}$ \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Audit thresholds}

Physical admissibility is defined by the model inequalities and active
constraints themselves.  The thresholds in
\Cref{tab:invariants_review_guards} are numerical/reporting tolerances used to
decide whether a computed sample satisfies those relations to the required
accuracy.
...
\toprule
Guard quantity & Threshold \\ \midrule
\endfirsthead
\toprule
Guard quantity & Threshold \\ \midrule
\endhead
...
\endfoot
\bottomrule
\caption{Numerical review guards used by the mechanical-invariant audit.}
\label{tab:invariants_review_guards}\\
\endlastfoot
Fixed-belt-length residual & $10^{-9}$ m \\
Row-scaled $8\times8$ closure residual & $10^{-10}$ \\
Sticking relative-velocity drift & $10^{-3}$ m/s \\
...
\section{Mechanical-Energy Consistency Study}
\label{app:study_energy_consistency}

The energy study uses the flat-road Baja reference for 10 s with no mechanical
parameter changes.  Its maintained definition and implementation are
\ResultsCode{studies/energy-consistency/study.json}{energy-consistency/study.json}
and
\ResultsCode{studies/energy-consistency/run.py}{energy-consistency/run.py}.
The audited identity is

\begin{equation}
W_{\mathrm{ext}}
=
\Delta\!\left(E_{\mathrm{kin}}+E_{\mathrm{cons}}\right)
+E_{\mathrm{slip}}
+E_{\mathrm{capture}}
+R_E,
\label{eq:energy_audit_identity}
\end{equation}

where \(R_E\) is the residual reported by the audit.

\begin{table}[H]
\centering
\small
\caption{Numerical configurations used by the mechanical-energy audit.}
\label{tab:energy_study_numerics}
\begin{tabular}{@{}p{0.28\textwidth}p{0.14\textwidth}p{0.14\textwidth}p{0.15\textwidth}p{0.20\textwidth}@{}}
\toprule
Case & $\mathrm{rtol}$ & $\mathrm{atol}$ & Max. step & Purpose \\ \midrule
Nominal audit & $3\times10^{-5}$ & $3\times10^{-8}$ & 10 ms & Primary reported energy balance \\
Tight integration & $9\times10^{-6}$ & $9\times10^{-9}$ & 5 ms & Final-state and residual sensitivity \\
\bottomrule
\end{tabular}
\end{table}

The continuous work integrals are independently evaluated at quadrature steps of
5, 2.5, 1.25, and 0.625 ms.  The review criteria are collected in
\Cref{tab:energy_acceptance_criteria}.

\begin{table}[H]
\centering
\small
\caption{Acceptance criteria of the mechanical-energy audit.}
\label{tab:energy_acceptance_criteria}
\begin{tabular}{@{}p{0.67\textwidth}r@{}}
\toprule
Metric & Limit \\ \midrule
$\lvert R_E\rvert/\lvert W_{\mathrm{ext}}\rvert$ at final time & $10^{-4}$ \\
Maximum absolute event-energy defect & $10^{-8}$ J \\
Maximum impact momentum residual & $10^{-7}$ \\
Maximum impact constraint residual & $10^{-8}$ \\
Normalized final-state difference, nominal vs. tight & $10^{-2}$ \\
Allowed negative computed capture loss & $10^{-9}$ J \\
Finest-to-previous continuous-defect ratio & $1.25$ \\
\bottomrule
\end{tabular}
\end{table}

\section{Solver-Convergence Study}
\label{app:study_solver_convergence}

The solver-convergence study uses the same 10-s Baja reference trajectory but
varies only numerical controls.  Its maintained definition and implementation
are
\ResultsCode{studies/solver-convergence/study.json}{solver-convergence/study.json}
and
\ResultsCode{studies/solver-convergence/run.py}{solver-convergence/run.py}.
...
\begin{tabular}{@{}p{0.23\textwidth}p{0.22\textwidth}p{0.22\textwidth}p{0.25\textwidth}@{}}
\toprule
...
The eight relative tolerances in the formal grid are
\(10^{-2}\), \(3\times10^{-3}\), \(10^{-3}\), \(3\times10^{-4}\),
\(10^{-4}\), \(3\times10^{-5}\), \(10^{-5}\), and
\(3\times10^{-6}\).  Candidate and reference trajectories are compared by
corresponding hybrid segment; event timing and regime-history disagreement are
reported separately.
...
\section{Closure-Conditioning and Root-Structure Study}
\label{app:study_closure_conditioning}

The closure study separates the conditioning of the fixed-traction
\(8\times8\) mechanical solve from the conditioning and root structure of the
stick residual map.  The maintained study definition and entry point are
\ResultsCode{studies/closure-conditioning/study.json}{closure-conditioning/study.json}
and
\ResultsCode{studies/closure-conditioning/run.py}{closure-conditioning/run.py}.
The shared operating-state vocabulary is again taken from
\ResultsCode{defaults/verification/operating_cases.json}{defaults/verification/operating_cases.json}.

The reference launch is integrated for 10 s with
\(\mathrm{rtol}=10^{-4}\), \(\mathrm{atol}=10^{-7}\), and a 10 ms maximum
step, with frozen states sampled every 2 ms.  The expensive traction-plane maps
are then evaluated independently at selected frozen states.

\begin{table}[H]
\centering
\small
\caption{Named frozen states selected for paper-facing closure maps in the maintained study configuration.}
...
\texttt{free\_shift\_opening\_70} & Late-ratio opening/backshift state from the shared verification library \\
Worst actual $A$ state & Automatically selected largest encountered mechanical-closure condition number \\
Worst actual $J_R$ state & Automatically selected largest encountered stick-root Jacobian condition number \\
\bottomrule
\end{tabular}
\end{table}
...
Physical static box & $241\times241$ & Exact static Coulomb-capacity intervals at the frozen state; mechanical/topology admissibility is overlaid separately \\
Expanded & $321\times321$ & Signed half-width $\max(2.5,4\lambda_{\max,\mathrm{phys}})$; normally approximately $[-2.6,2.6]^2$ \\
Broad diagnostic & $501\times501$ & $[-10,10]^2$; generated only for the configured showcase state \\
\bottomrule
\end{tabular}
\end{table}

The current working \texttt{develop} configuration uses a $7\times7$ multistart
grid in the physical static box and a $9\times9$ grid in the expanded box, with
a root-clustering tolerance of $10^{-6}$.  These settings will be frozen to the
paper-facing Results revision together with the final regenerated artifacts.
The physical stick-root Jacobian \(J_R\) used for conditioning is reconstructed
with an independent central derivative of the frozen residual map rather than
taking the solver's continuation Jacobian, which may contain Broyden updates.

The follow-on fold searches are analysis products of this study rather than part
of the ordinary production root solve.  Their generating scripts are maintained
under
\ResultsTree{studies/closure-conditioning/analysis}{closure-conditioning/analysis}.

% Additional Results studies will be added here in the same study-local format
% as their manuscript sections are finalized.

\section{Manuscript Artifact Traceability}
\label{app:artifact_index}

The final figure-to-artifact index will be inserted here after the paper-facing
figures are selected and regenerated.  Each entry will identify the manuscript
item, exact study/case, archived source artifact, and generating script.  Until
that figure set is frozen, the study-local definitions above are the
reproducibility record for the internal-verification section.
