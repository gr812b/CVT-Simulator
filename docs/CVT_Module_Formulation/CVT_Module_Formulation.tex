...
The formulation derived in \Cref{chap:derivation} is implemented in the
open-source CINDER simulator. The studies reported in this chapter examine the
resulting model at several scales, including local closure and invariant checks,
controlled comparisons of retained dynamic terms, reconstructed benchmark
cases, and complete transient operation. The numerical scope changes with the
study, but the governing CVT mechanics do not.

The shaft-boundary definition in \Cref{sec:system_definition} keeps the CVT
separate from the external systems that drive or load it. Engines, vehicles,
flywheels, dynamometers, and other hosts couple through those interfaces, so a
benchmark or isolated study can impose the boundary history required for the
comparison, while complete operating cases allow the surrounding system to
evolve with the CVT. Where a study intentionally replaces or suppresses a model
contribution, that change is stated explicitly with the case.

Unless stated otherwise, all studies use the reference CVT defined in
\Cref{app:reference_configuration} and the common numerical settings listed in
\Cref{tab:results_common_numerics}. The full physical parameter set and its
provenance are given in \Cref{tab:results_common_parameters}. Each Results
subsection therefore reports only the quantities intentionally changed for that
study. The corresponding host or boundary definition and all explicit changes
are collected in \Cref{tab:results_case_registry}.

All results use CINDER release \CINDERtag, pinned to commit \CINDERcommit. The
tagged source is available from
\url{https://github.com/gr812b/CVT-Simulator/tree/cinder-v1.1.2}. The numerical
details used to implement the Chapter~3 equations are documented separately in
\Cref{app:numerical_implementation}, including the belt-geometry root solves,
the fixed-$\lambda$ mechanical closure, actuator assembly, static-traction
solution, and hybrid time integration.

The tagged CINDER release identifies the model implementation, while the frozen
Results archive identifies the study definitions used to produce the reported
results. The archive contains the configuration files, analysis scripts, and
saved outputs used for the manuscript figures and tables and, at the final
Results freeze, is identified by \ResultsTag, \ResultsCommit, and \ResultsDOI.
The source for each reported figure or table is indexed in
\Cref{tab:results_artifact_index}. Together, these records identify the exact
code, inputs, scripts, and saved outputs behind each reported result.
