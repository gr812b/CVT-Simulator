...
The first climb adds a large gravitational load opposing the wheels.
Backshifting moves the belt inward on the primary and outward on the
secondary, increasing torque multiplication and allowing the engine to turn
faster relative to the slowing vehicle. The secondary helix provides a
...
progress begins after the available backshift has already been used. Its
light tips provide less centrifugal closing action, while its elevated
primary spring compression increases the opposing opening force. The fixed
ratio does not, however, fix the clamping force. As established in
\Cref{sec:low_ratio_supported_regime}, the secondary is held against its
closing stop while the primary axial balance remains active. At this fixed
...
The primary force balance also identifies two changes that can preserve
clamping as speed falls. D02\_M restores the reference tip mass and its
associated mass moments, strengthening centrifugal closing while retaining
D02's elevated primary spring compression. D02\_P retains the light tips but
restores the reference spring compression, reducing the opening force opposing
the flyweights without changing the spring rate. Both act on
the difference between centrifugal closing and spring opposition. A larger
...
The changes also alter the approach to the seat.
\Cref{fig:results_d02_hill_repairs}(a) follows that motion on the severe hill:
all three cases reach the same backshift limit, with both repairs arriving
farther along the road than D02. Their entry speeds and ratios already differ,
so these arrival distances combine the changed hardware response with the
inherited states. Once seated, the cases can be compared at a common geometry
and primary speed. Panels~(b,c) therefore show primary normal load and static
use against decreasing primary speed on the constant \(38^\circ\) slope.
Matching speeds are reached at different times and road positions in each run.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/results/course/d02_hill_repairs.png}
  \caption{Severe-hill repair comparison. (a) Shift paths, with the entry
  taper shaded and the low-ratio seat dashed. (b,c) Total primary normal load
  and static utilization during seated sticking on the \(38^\circ\) hold,
  after the seating transitions. Markers identify the comparison near
  \(2975~\mathrm{rpm}\). D02 ends at sustained primary slip onset; repair
  annotations refer to states near \SI{200}{m}.}
  \label{fig:results_d02_hill_repairs}
\end{figure}

At the marked speed, the required primary tangential force is approximately
\SI{683}{N} in all three cases. D02 has just exhausted its static capacity,
but both repairs support greater normal load at the same seated geometry and
speed. The increase is much larger when the tip mass is restored. Because
the available static force is \(\mu_sN_p\), the greater normal loads in
panel~(b) directly explain the lower fractions used in panel~(c). The repaired
contacts can therefore remain stuck as speed falls below the point where the
original D02 began sliding. The successful climb follows from restored
clamping and traction capacity at the existing ratio limit.

Both repairs continue through the hill without the sustained primary traction
loss seen in D02. Their normal loads still fall as primary speed decreases,
and the fraction of available traction required rises. Near \SI{200}{m},
D02\_M uses about \(0.40\) of its static capacity, whereas D02\_P uses about
\(0.94\). Yet both climb near \SI{3.77}{m/s} at the same low-ratio seat.
Once sticking is maintained, additional unused traction capacity does not
itself deliver more power to the wheels. The almost equal climbing speeds
therefore coexist with substantially different reserves: the preload repair
succeeds much closer to its primary traction limit under this hill load.
The different actuator balances also carry into later operation: on the
subsequent \(18^\circ\) climb, D02\_P runs farther backshifted and climbs
more slowly than D02\_M.

The hill has separated an unfavourable ratio and engine speed with unused
backshift travel from a loss of belt traction after full backshift. In the
latter case, the primary force balance identifies changes that increase
normal load and static traction capacity. Continued climbing demonstrates
that both changes supply enough capacity, while the internal contact forces
show their different remaining reserves. The next repeated
changes in road load turn attention back to the motion itself, and to how
much shift travel remains available when each new disturbance arrives.
