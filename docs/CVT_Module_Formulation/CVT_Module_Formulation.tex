...
\section{Numerical Study Framework and Reproducibility}
\label{sec:results_implementation_reproducibility}

The derivation in \Cref{chap:derivation} closes the mechanics of the CVT so
that its instantaneous response can be determined from the current state and
the loading presented at the two shafts. The Results use the CINDER simulator
both to check how those equations behave when solved numerically and to study
the response that follows from them.

Different questions naturally call for different numerical experiments. A
question about the instantaneous mechanical closure can be examined at selected
states or operating points. The effect of a particular dynamic term or
mechanism is more clearly exposed by a controlled comparison in which that
contribution is changed while the rest of the case is held fixed. Complete
operating events, by contrast, require the coupled system to evolve through
time. The studies in this chapter therefore range from local evaluations and
controlled comparisons to full transient simulations, with the scope of each
calculation chosen to match the question being tested.

The shaft interfaces defined in \Cref{sec:system_definition} allow the same CVT
to be used across those experiments. An engine, vehicle, flywheel, or
dynamometer can be attached through those interfaces, while a benchmark can
instead prescribe the boundary conditions needed for the comparison. These
choices change the experiment surrounding the transmission, not the CVT
formulation between the two shaft interfaces.

Comparisons across these experiments also require a common baseline. Unless
stated otherwise, each study uses the reference CVT defined in
\Cref{app:reference_configuration} and the common numerical settings listed in
\Cref{tab:results_common_numerics}. The complete physical parameter set and its
provenance are given in \Cref{tab:results_common_parameters}. Individual
Results sections therefore need to state only the physical or numerical
quantities that are deliberately changed. The study and case registry in
\Cref{tab:results_case_registry} records those changes together with the host or
boundary used by each case.

All results reported in this chapter use the open-source CINDER release
\CINDERtag, pinned to commit \CINDERcommit. The tagged source is available from
\url{https://github.com/gr812b/CVT-Simulator/tree/cinder-v1.1.2}. Details of
how the analytical equations are solved in software are documented separately
in \Cref{app:numerical_implementation}, including the belt-geometry root solves,
the fixed-$\lambda$ mechanical closure, actuator assembly, static-traction
solution, and hybrid time integration.

Reproducing a reported result also requires the exact study definition used
with that implementation. The frozen Results archive therefore contains the
configuration files, analysis scripts, and saved outputs used to generate the
figures and tables retained in the manuscript. At the final Results freeze, the
archive is identified by \ResultsTag, \ResultsCommit, and \ResultsDOI. The
mapping from each manuscript figure or table to its archived source is recorded
in \Cref{tab:results_artifact_index}. Together, the tagged CINDER implementation
and the frozen Results archive specify both the model that was executed and the
numerical experiment that produced each reported result.
...
From the exact span geometry in \Cref{sec:belt_loop_geometry},
...
For a contact assumed to stick, \Cref{sec:sticking_traction_closure} does not
prescribe $\lambda_j$ in advance.  The required static traction is the value
that makes the corresponding relative tangential acceleration vanish.  CINDER
therefore proposes the unknown sticking traction, evaluates the fixed-$\lambda$
mechanics from \Cref{app:num_fixed_lambda}, and uses the resulting relative-
acceleration residual to update that traction.

The same bounded nonlinear solve is used in either dimension required by the
current contact state.  Stick--stick leaves both $\lambda_p$ and $\lambda_s$
free, a mixed stick--slip state solves only for the traction at the sticking
contact, and both-slip fixes both kinetic tractions and requires no nonlinear
traction solve.
...
admissibility test in \Cref{sec:contact_admissibility_transitions}, rather than
...
\Cref{sec:contact_admissibility_transitions}.  At finite numerical precision,
however, using exactly the same zero-margin static-capacity surface for both
loss and recovery of stick can cause repeated stick--slip switching at nearly
the same state.  CINDER therefore leaves stick loss at the physical
...
\Cref{sec:system_definition}.  An attached engine, vehicle, dynamometer,
controller, or other host system may carry states of its own, but its mechanical
interface to the CVT remains the applied torque and equivalent referred inertia
at the corresponding shaft.  The shaft speeds remain states of the CVT
simulation rather than prescribed host trajectories.

Within any fixed mechanical regime, the resulting first-order initial-value
...
When arrival at a shift boundary changes the velocity relations that may hold
after the event, the finite-speed momentum map derived in
\Cref{sec:finite_speed_shift_boundary_arrival} is applied rather than simply
switching the continuous equations.  CINDER assembles the event-specific
$\vect{J}^{-}$, $\vect{J}^{+}$, $\vect{W}$, and $\vect{C}^{+}$ objects and solves
the resulting dense linear or KKT system.  The solved outgoing velocity is then
written into the successor state.

The event solve is checked against the momentum and post-event constraint
relations from the formulation.  The associated perfectly inelastic kinetic-
energy change is also recorded, so the discrete update can be audited separately
from the continuous dynamics on either side of the event.
...
Finite-speed momentum update
& Direct event-specific linear/KKT solve with momentum, constraint, and energy checks
& Apply the discrete boundary relation derived in \Cref{sec:finite_speed_shift_boundary_arrival}. \\
\end{longtable}
\normalsize
\end{center}
