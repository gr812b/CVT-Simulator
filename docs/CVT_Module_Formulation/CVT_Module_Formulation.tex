...
The secondary pulley is \emph{torque-reactive}: its axial force is produced when torque is carried through the secondary helix mechanism. The primary pulley is mainly speed-sensitive, because its flyweight force grows with rotational speed. The secondary pulley is different. Its helix responds to torque that tries to rotate the movable sheave relative to the fixed sheave.

The secondary assembly consists of a fixed sheave, a movable sheave, a helical slot between them, and a spring. The fixed sheave rotates with the secondary shaft. The movable sheave can slide axially, but it is not free to rotate independently: it is captured by the helical slot. Therefore, if the movable sheave shifts axially, it must also rotate relative to the fixed sheave according to the helix geometry introduced in \Eq{eq:secondary_theta_profile}.
...
Thus, the secondary axial force comes from two coupled sources: direct axial spring compression, and torque reacted through a screw-like helix.
...
The helix can be understood using a screw analogy. Spinning a screw freely in space does not create axial force. Spinning a screw into a block that is also free to rotate does not create much axial force either, because the block can simply rotate with the screw. Axial force appears when relative rotation is resisted.

The secondary helix works in the same way. Torque applied to the movable sheave tries to rotate it relative to the fixed sheave. Because the movable sheave is captured in a helical slot, that relative rotation cannot occur independently of axial motion. The helix must react the torque, and because the helix is inclined, that reaction has an axial component.

This is the key physical idea: the secondary does not produce axial force simply because it is spinning. It develops axial force because torque is applied to the movable sheave and reacted through a constrained helical path.

% --------------------------------------------------------------
\paragraph{Torque Applied to the Movable Sheave}

Let $\tau_{b,M}$ denote the signed torque applied by the belt to the movable secondary sheave. This is a local torque in the movable-sheave free body. It is the torque that tends to rotate the movable sheave relative to the fixed sheave, and therefore the torque that can load the helix.

This is distinct from a torque that simply rotates the entire secondary assembly. If the fixed and movable sheaves rotated together with no relative tendency, the helix would not be loaded. The helix is loaded only by torque that must be reacted between the movable sheave and the fixed sheave.

The relationship between $\tau_{b,M}$ and the total secondary torque is handled by the torque-transfer model. Here, $\tau_{b,M}$ is treated as the local belt-applied torque entering the secondary helix mechanism.

A positive value of $\tau_{b,M}$ is defined as torque that tends to produce positive relative helix rotation. Because $\tau_{b,M}$ is signed, reverse loading is handled by the same equations with $\tau_{b,M}<0$.

% --------------------------------------------------------------
\paragraph{Helix Velocity Constraint}

The helix geometry has already been parameterized by the mapping
...
where $\theta_s$ is the relative rotation between the movable and fixed sheaves. For the dynamics, the most direct way to use this constraint is through angular velocity.

Let $\omega_F$ be the angular velocity of the fixed secondary sheave and shaft, and let $\omega_M$ be the angular velocity of the movable secondary sheave. With the sign convention used here, positive helix rotation means that the movable sheave lags the fixed sheave, so
...
Since $\theta_s=\theta_s(s)$, the same relative angular speed is also
...
Combining \Eq{eq:secondary_relative_speed} and \Eq{eq:secondary_theta_dot} gives the movable-sheave angular velocity:
...
This equation is the kinematic meaning of the captured helix. If the secondary shifts, the movable sheave cannot simply translate; it must also change angular speed relative to the fixed sheave.
...
This angular acceleration is needed because some of the torque applied to the movable sheave is used to rotationally accelerate that sheave rather than being reacted through the helix as axial force.
...
This is a signed torque in the same helix coordinate as $\tau_{b,M}$. The sign of the preload and the definition of positive $\theta_s$ determine whether the spring torque assists or opposes the belt-applied torque at a given instant.

% --------------------------------------------------------------
\paragraph{Rotational Equation of Motion of the Movable Sheave}

Let $I_M$ be the rotational inertia of the movable secondary sheave about the secondary shaft axis. The torques acting on the movable sheave are:
\begin{itemize}
...
The helix reaction is what makes the screw analogy produce axial force. If there were no helix reaction, the applied torque would simply accelerate the movable sheave. The axial force appears because the helix must react whatever torque is not used to rotationally accelerate the movable sheave.
