...
Sticking requires the belt and the represented pulley surface to keep
matching tangential accelerations. As developed in
\Cref{sec:sticking_traction_closure}, a trial traction pair first determines
the coupled accelerations and contact loads; the sticking search then adjusts
that pair until the relative accelerations vanish. These are different
questions. A definite response to each prescribed pair does not establish
that only one pair can preserve sticking. Changing traction also changes
the normal loads and transmitted torques through the coupled balances, so
the returned sticking residuals need not vary monotonically with traction.

The fixed-traction calculation was examined at frozen states from the
reference launch and constructed operating cases. The implementation retains
eight unknowns, including the two pulley torques; eliminating those torques
recovers the six-variable formulation, without changing the mechanics
(\Cref{eq:appendix_eight_unknown_closure}). All sampled systems in the four
static-capacity maps and their expanded maps have rank eight.
\Cref{fig:verification_closure_robustness}(a) nevertheless shows substantial
variation in sensitivity. The largest equilibrated condition number in the
static maps is \(1.05\times10^6\), at an upper-stop trial, compared with
\(8.56\times10^3\) at that state's accepted sticking root. That extreme
trial also fails the compressive belt-loading checks. Being inside
\(\lvert\lambda_j\rvert\leq\mu_s\) has limited the traction coefficient,
but has not ensured that the resulting loads can be carried by the belt.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/results/verification/closure_robustness.pdf}
  \caption{Fixed-traction conditioning and sticking-search outcomes.
  (a) Unmasked maxima over four \(241\times241\) static-capacity maps,
  contrasted with values at the accepted roots. All use the same
  row/column-equilibrated eight-variable matrix of
  \Cref{eq:appendix_equilibrated_system}. Opening denotes the constructed
  state at \(70\%\) engaged travel and \(-20~\mathrm{mm/s}\).
  (b) All \(7\times7\) mid-shift starting guesses: 42 are accepted and
  seven are not. The star marks the common accepted root, not a starting
  guess or an iteration path. Neither panel establishes global uniqueness.}
  \label{fig:verification_closure_robustness}
\end{figure}

The nonlinear search was tested separately with 49 starting guesses at each
of 15 states. Of the 735 attempts, 666 are accepted, and every accepted
solution at a given state belongs to the same root cluster.
Panel~(b) makes both parts of that result visible at mid-shift: the accepted
starts recover one solution, but not every start succeeds. Repeating the
search from the wider traction box at the four mapped states also recovers
only their existing root clusters. These are results for the sampled states
and starting guesses; they leave open whether another state or loading can
admit more than one sticking response.

A deliberately selected late-shift opening state exposes the limit of this
single-cluster result. Its position and velocities are held fixed while the
shaft torques are varied, rather than following a vehicle trajectory.
At the selected loading, \Cref{fig:verification_closure_fold}(a) shows two
intersections of the primary and secondary zero-relative-acceleration
contours. Both traction pairs therefore preserve sticking. Both also pass
the static-friction, belt-loading and actuator-contact checks; the smaller
static margin, defined as \(\mu_s-\lvert\lambda_j\rvert\), is 0.098.
The roots predict different mechanical responses: the secondary normal
resultant is \SI{2.401}{kN} at root 1 and
\SI{1.669}{kN} at root 2, with different accelerations as well.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/results/verification/sticking_closure_fold.pdf}
  \caption{Two sticking roots at one frozen state: \(95\%\) engaged travel,
  \(-40~\mathrm{mm/s}\) opening speed and \(3000~\mathrm{rpm}\) primary
  speed. (a) Both acceleration residuals vanish at each marked intersection
  under the displayed shaft-boundary torques. (b) The required secondary
  boundary torque along the same locally corrected sticking branch, with
  primary boundary torque fixed. The horizontal line intersects it at the
  same two roots. Secondary traction is a branch coordinate, not time or
  shift position. The dotted continuation beyond loss of local wrap
  compression is mathematical only. Exact inputs and signed responses are
  given in \Cref{app:study_closure_conditioning}.}
  \label{fig:verification_closure_fold}
\end{figure}

Panel~(b) explains how the two roots arise. Along this connected branch,
increasing secondary traction first raises the secondary torque compatible
with sticking, then lowers it. The required torque turns near
\SI{197.39}{N.m}, so the selected \SI{189.96}{N.m} loading cuts both sides
of the turn. A loading above this local maximum would no longer intersect
this part of the branch. The mechanical matrix remains full-rank through
the turn: what folds is the relation selecting sticking traction, not the
linear response once traction is specified.

The distinction also applies with only one sticking contact. A separate
targeted search of 62 frozen states found one distinct scalar-fold geometry
on the primary-slip branch at \(-\mu_k\), with secondary sticking. Its selected
loading admits secondary tractions near 0.434 and 0.546. The latter leaves
only \SI{0.00616}{N/rad} of minimum local wrap-normal loading, so a positive
integrated normal force alone would conceal its proximity to contact loss.
At this zero-relative-speed onset, the primary relative acceleration has
the sign required to enter the selected slip direction
(\Cref{app:study_closure_conditioning}). The search is evidence of existence,
not a measure of how often such a state occurs in operation.

During smooth evolution, CINDER reuses the preceding accepted traction
solution to follow the nearby branch, while rechecking residuals and
admissibility (\Cref{app:num_contact_continuation,app:num_contact_search}).
The maintained transients did not report spontaneous ambiguity of the kind
exposed here, but continuation cannot remove a fold or establish what happens
after a branch ends. Thus the earlier trajectory-convergence tests can
reproduce a locally followed response even when the instantaneous state and
loading admit more than one acceleration. In that case, the numerical
continuation history contributes to branch selection; these tests do not
establish a unique physical selection rule.
\FloatBarrier
...
of its 251,001 points are numerically rank deficient. The static-map maxima
shown in \Cref{fig:verification_closure_robustness} are, in plotted order,
...
Every map retains signed normal resultants, minimum belt tension, minimum
local wrap-normal loading, actuator-contact margin and support reaction.
The archived admissibility masks use the original release's one-sided
secondary-helix contact guard. Those masks are retained with that meaning;
they are not relabeled as results for the current bilateral slotted-helix
reference. The unmasked matrix values in the figure do not depend on that
choice of contact guard. The selected roots below are also checked with
the bilateral reference and give identical mechanical responses.
...
\bottomrule
\end{tabular}
\caption{Fresh evaluation of the two retained traction pairs under installed
...
Both fixed-traction matrices have rank eight. The fresh
...
provides a checked \texttt{--import-retained} path for those three archives
and a \texttt{--plot-only} path for the delivered evidence. The import
performs only the selected frozen-root, derivative, local-curve and focused
residual-grid checks under installed \texttt{cinder-cvt==1.1.2}; it does
not import the newer live simulator. It uses the full archive's embedded
case library. Original archive/member hashes,
executed input snapshots, numerical-output hashes and exact reproduction
...
dependency lock, so plot reproduction and selected fresh checks do not
establish bitwise regeneration of the historical full search.
