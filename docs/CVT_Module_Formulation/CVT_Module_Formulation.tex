...
The belt speed \(v_b\) can change only if something pushes or pulls the belt
along its path. Those tangential actions come from the primary and secondary
pulley contacts. The belt's transport acceleration must therefore come from
the two pulley wraps.

At either wrap, the mechanism is simple. Frictional traction is spread over
the contact, and at each small part of the wrap it acts tangent to the belt.
On the pulley, each of these small forces produces a small moment about the
shaft. Adding those moments around the wrap gives the contact torque \(\tau_j\)
already used in the pulley balance. Thus, \(\tau_j\) is simply the accumulated
turning effect of the tangential contact forces.

Newton's third law places equal-and-opposite forces on the belt. Their total
moment is therefore \(-\tau_j\), and at the effective contact radius
\(r_{j,\mathrm{eff}}\) they are equivalent to the net tangential pull
\(-\tau_j/r_{j,\mathrm{eff}}\). There is no need to find the small contact
forces one by one here; the torque has already added their effects for us.

Viewed from the belt, these two pulls determine whether its transport motion
gains or loses energy. A force acting in the direction of belt travel does
work on the belt, while a force acting against the direction of travel removes
work from it. If one pulley supplies exactly the power removed by the other,
the belt simply carries that power between them and \(v_b\) remains unchanged.
If the two actions do not balance, the excess remains with the belt.

The belt assumptions leave that excess only one place to go. The fixed length
of \AssRef{assump:constant_belt_length} excludes longitudinal extension and the
strain energy it would store. The single transport coordinate of
...
This makes the complete belt a particularly simple free body. Place one
boundary around the entire loop. For the transport motion, the tension forces
exerted internally between neighbouring portions of the belt cancel around
the closed loop. The remaining tangential actions are the two pulley pulls
derived above, acting on the complete belt mass \(m_b\). The complete-belt
