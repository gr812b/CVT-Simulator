...
$r_{p,\mathrm{outer},\min}$ & minimum primary outer radius \\
$r_{s,\mathrm{outer},\max}$ & maximum secondary outer radius \\
$s_{\mathrm{dz}},\ s_{\max}$ & deadzone threshold and maximum shift \\
$\beta$ & sheave face half-angle (radial reference) \\
$d_c$ & pulley center distance \\
...
$f(r_p,r_s,d_c)$ & belt-length compatibility root function ($f=0$) \\
...
$r_{p,\mathrm{outer},\min}$ & Primary minimum outer radius & \si{\meter} & Radius at fully open primary \\
$r_{s,\mathrm{outer},\max}$ & Secondary maximum outer radius & \si{\meter} & Radius at fully closed secondary \\
...
$m_f$ & Flyweight-set mass & \si{\kilogram} & Total mass of the symmetric flyweight set \\
...
\[
\frac{\left|r_s-r_p\right|}{d_c}\ll 1,
\]
...
temporary notation is useful. The prime notation retains the independent
variables already established by the corresponding maps:
\[
    x_s'\equiv\frac{dx_s}{ds},
    \qquad
    r_j'\equiv\frac{dr_j}{ds},
    \qquad
    q_f'\equiv\frac{dq_f}{dx_p},
\]
\[
    J_{f,p}'\equiv\frac{dJ_{f,p}}{dx_p},
    \qquad
    \theta_s'\equiv\frac{d\theta_s}{dx_s},
\]
with double primes taken with respect to the same established arguments.
The only additional symbols are the two analytical wrap factors repeated in
the closed-loop row,
...
\begin{equation}
    \mathbf J_{\mathrm{eng}},\,\mathbf J_{\mathrm{dz}}
    \in \mathbb{R}^{9\times4}.
    \label{eq:finite_event_velocity_map_dimensions}
\end{equation}
For either topology, \(\mathbf J\boldsymbol{\nu}\) therefore contains the nine
...
diagonal of \(\mathbf W\in\mathbb{R}^{9\times9}\), in the same row order, performs
these multiplications together:
\begin{equation}
    \boldsymbol p
    =
    \mathbf W\mathbf J\boldsymbol{\nu}
    \in\mathbb{R}^{9},
    \qquad
    \mathbf J\in\{\mathbf J_{\mathrm{eng}},\mathbf J_{\mathrm{dz}}\}.
...
and \(\mathbf W\) is common to both sides. The incoming and outgoing connections
determine \(\mathbf J_-\) and \(\mathbf J_+\), respectively, so both maps are known for the
transition being considered:
\begin{equation}
    \mathbf J_-
    \in\{\mathbf J_{\mathrm{eng}},\mathbf J_{\mathrm{dz}}\},
    \qquad
    \mathbf J_+
    \in\{\mathbf J_{\mathrm{eng}},\mathbf J_{\mathrm{dz}}\}.
...
arrival at the upper primary stop has \(\mathbf J_-=\mathbf J_+=\mathbf J_{\mathrm{eng}}\). At the
fully open primary stop, both sides instead use \(\mathbf J_{\mathrm{dz}}\).
...
\end{equation}
If \(\mathbf J_-\neq \mathbf J_+\), even identical state-velocity vectors can correspond to
different physical motions and therefore different momenta.

\paragraph{Velocities allowed after the event}

With the momentum on both sides now expressed through \(\mathbf J_-\) and \(\mathbf J_+\), the
...
even after the appropriate outgoing map \(\mathbf J_+\) is known, the new regime can
...
remains valid, so \(\mathbf J_-=\mathbf J_+=\mathbf J_{\mathrm{eng}}\), while the already-established
stop condition is
\begin{equation}
    \dot s^+=0.
\end{equation}
...
\begin{equation}
    ...
    \end{bmatrix},
    \qquad
    \mathbf c_{\mathrm{stop}}\boldsymbol{\nu}^+=0.
...
\(\mathbf J_{\mathrm{eng}}\), while the fully open primary stop uses
\(\mathbf J_{\mathrm{dz}}\).
...
\begin{equation}
    ...
    \end{bmatrix},
    \qquad
    \mathbf c_{p,\mathrm{stick}}\boldsymbol{\nu}^+=0.
...
\begin{equation}
    ...
    \end{bmatrix},
    \qquad
    \mathbf c_{s,\mathrm{stick}}\boldsymbol{\nu}^+=0.
...
\(\mathbf J_+=\mathbf J_{\mathrm{dz}}\), and the belt remains locked to the fully closed
...
\begin{equation}
    ...
    \end{bmatrix},
    \qquad
    \mathbf c_{s,\mathrm{dz}}\boldsymbol{\nu}^+=0.
...
increasing \(s\).  Since \(\mathbf c_{\mathrm{stop}}\) identifies the retained
direction in which the stop acts, its impulse is

\begin{equation}
    \mathbf c_{\mathrm{stop}}^{\mathsf T}\Gamma_{\mathrm{stop}}
    =
    \begin{bmatrix}
        0\\
        0\\
...
\begin{equation}
    ...
    \end{bmatrix}
    =
    \mathbf c_{p,\mathrm{stick}}^{\mathsf T}\Gamma_p.
...
row \(\mathbf c_i\) of \(\mathbf C_+\) with an unknown signed impulse amplitude 
...
\(\mathbf c_i\boldsymbol{\nu}^+=0\) specifies the outgoing velocity relation, while
\(\mathbf c_i^{\mathsf T}\Gamma_i\) gives the corresponding generalized impulse.
...
\(\mathbf C_+^{\mathsf T}\boldsymbol{\Gamma}\in\mathbb R^4\) already acts in the
...
column of \(\mathbf J_+\) states how one retained velocity appears among the
...
\begin{equation}
    \left[\mathbf J_+^{\mathsf T}\Delta\boldsymbol p\right]_{\omega_s}
    =
...
\begin{equation}
    \mathbf J_+^{\mathsf T}\Delta\boldsymbol p
    \in\mathbb R^4,
...
\(\Delta\boldsymbol p\) through \(\mathbf W\mathbf J_-\boldsymbol{\nu}^-\), while
...
the restrictions already collected in \(\mathbf C_+\):
\begin{equation}
    \begin{aligned}
        \mathbf J_+^{\mathsf T}\Delta\boldsymbol p
        &=\mathbf C_+^{\mathsf T}\boldsymbol{\Gamma},\\
...
\begin{equation}
    \begin{bmatrix}
        \mathbf J_+^{\mathsf T}\mathbf W\mathbf J_+&-\mathbf C_+^{\mathsf T}\\
        \mathbf C_+&0
    \end{bmatrix}
    \begin{bmatrix}\boldsymbol{\nu}^+\\\boldsymbol{\Gamma}\end{bmatrix}
    =
    \begin{bmatrix}\mathbf J_+^{\mathsf T}\mathbf W\mathbf J_-\boldsymbol{\nu}^-\\0\end{bmatrix}.
...
\(\mathbf J_+^{\mathsf T}\mathbf W\mathbf J_+\) is the generalized inertia seen in the outgoing
velocity directions, \(\mathbf C_+^{\mathsf T}\) places the unknown constraint
impulses into those directions, and \(\mathbf C_+\) imposes the required outgoing
...
question posed at the outset. Take \(\mathbf J_-=\mathbf J_+=\mathbf J_{\mathrm{eng}}\) and first
...
\[
\frac{r_s - r_p}{d_c} \ll 1,
\]
which implies $\delta \approx 0$ and therefore
\[
\sin^{-1}\!\left(\frac{r_s - r_p}{d_c}\right) \approx 0.
\]

Under this approximation, the belt-length constraint
\Eq{eq:belt_length_final} reduces to
\begin{equation}
L_b \approx \pi(r_p + r_s) + 2\sqrt{d_c^2 - (r_s - r_p)^2}.
...
we substitute $r_p = r_{p,\mathrm{outer},\min}$ and $r_s = r_{s,\mathrm{outer},\max}$ into
\Eq{eq:belt_length_approx} and solve for $d_c$.

\paragraph{Step 1: Isolate the square root.}

Rearranging \Eq{eq:belt_length_approx}:
\begin{equation}
L_b - \pi(r_{p,\mathrm{outer},\min} + r_{s,\mathrm{outer},\max})
=
2\sqrt{d_c^2 - (r_{s,\mathrm{outer},\max} - r_{p,\mathrm{outer},\min})^2}.
...
\begin{equation}
\bigl[L_b - \pi(r_{p,\mathrm{outer},\min} + r_{s,\mathrm{outer},\max})\bigr]^2
=
4\bigl[d_c^2 - (r_{s,\mathrm{outer},\max} - r_{p,\mathrm{outer},\min})^2\bigr].
\label{eq:approx_step2}
\end{equation}

\paragraph{Step 3: Solve for $d_c^2$.}

\begin{equation}
d_c^2
=
\frac{\bigl[L_b - \pi(r_{p,\mathrm{outer},\min} + r_{s,\mathrm{outer},\max})\bigr]^2}{4}
+
(r_{s,\mathrm{outer},\max} - r_{p,\mathrm{outer},\min})^2.
...
\bigl[L_b - \pi(r_{p,\mathrm{outer},\min} + r_{s,\mathrm{outer},\max})\bigr]^2
=
L_b^2 - 2L_b\pi(r_{p,\mathrm{outer},\min} + r_{s,\mathrm{outer},\max}) + \pi^2(r_{p,\mathrm{outer},\min} + r_{s,\mathrm{outer},\max})^2,
\]
...
=
\sqrt{
\frac{L_b^2 - 2L_b\pi(r_{p,\mathrm{outer},\min} + r_{s,\mathrm{outer},\max}) + \pi^2(r_{p,\mathrm{outer},\min} + r_{s,\mathrm{outer},\max})^2}{4}
+
(r_{s,\mathrm{outer},\max} - r_{p,\mathrm{outer},\min})^2
}.
}
...
Primary outer radius $r_{p,\mathrm{outer},\min}$ & \SI{1.4255}{in} & \SI{0.03621}{m} \\
Secondary outer radius $r_{s,\mathrm{outer},\max}$ & \SI{4.0}{in} & \SI{0.1016}{m} \\
\bottomrule
\end{tabular}
\end{center}
...
$r_{p,\mathrm{outer},\min} = 0.8125 + 0.613 = \SI{1.4255}{in}$.
...
numerically yields
\[
d_{c,\mathrm{exact}} = \SI{0.25174}{m} \approx \SI{9.911}{in}.
\]

Evaluating the approximate formula \Eq{eq:C_approx} yields
\[
d_{c,\mathrm{approx}} = \SI{0.26837}{m} \approx \SI{10.566}{in}.
\]
...
\begin{align}
\Delta d_c &= d_{c,\mathrm{approx}} - d_{c,\mathrm{exact}}
= \SI{0.01663}{m}
= \SI{0.655}{in}, \\[4pt]
\varepsilon_{\mathrm{rel}}
&= \frac{\Delta d_c}{d_{c,\mathrm{exact}}}
...
the ratio
\[
\frac{r_{s,\mathrm{outer},\max} - r_{p,\mathrm{outer},\min}}{d_c}
=
\frac{0.1016 - 0.03621}{0.25174}
=
0.260
\]
is not small. The assumption $\delta \approx 0$ is therefore not
justified for this CVT geometry.

Since the center distance $d_c$ propagates into all subsequent geometric
calculations---wrap angles, tangent span lengths, and derived quantities
such as the transmission ratio---a 7\% error in $d_c$ can compound into
...
Given a known primary outer radius $r_{p,\mathrm{outer}}$ and center distance $d_c$,
...
\begin{equation}
L_b - \pi(r_{p,\mathrm{outer}} + r_{s,\mathrm{outer}})
=
2\sqrt{d_c^2 - (r_{s,\mathrm{outer}} - r_{p,\mathrm{outer}})^2}.
\end{equation}

\paragraph{Step 2: Square both sides.}

\begin{equation}
\bigl[L_b - \pi r_{p,\mathrm{outer}} - \pi r_{s,\mathrm{outer}}\bigr]^2
=
4\bigl[d_c^2 - (r_{s,\mathrm{outer}} - r_{p,\mathrm{outer}})^2\bigr].
\end{equation}
...
\[
(L_b - \pi r_{p,\mathrm{outer}})^2
- 2\pi(L_b - \pi r_{p,\mathrm{outer}}) r_{s,\mathrm{outer}}
+ \pi^2 r_{s,\mathrm{outer}}^2.
\]

Expanding the right side:
\[
4d_c^2 - 4r_{s,\mathrm{outer}}^2 + 8 r_{p,\mathrm{outer}} r_{s,\mathrm{outer}} - 4r_{p,\mathrm{outer}}^2.
\]

Collecting powers of $r_{s,\mathrm{outer}}$ yields a quadratic equation:
\begin{equation}
(\pi^2 + 4) r_{s,\mathrm{outer}}^2
- 2\bigl[\pi L_b - \pi^2 r_{p,\mathrm{outer}} + 4 r_{p,\mathrm{outer}}\bigr] r_{s,\mathrm{outer}}
+ \bigl[(L_b - \pi r_{p,\mathrm{outer}})^2 + 4r_{p,\mathrm{outer}}^2 - 4d_c^2\bigr]
= 0.
\end{equation}
...
=
\frac{
\pi L_b + (4 - \pi^2) r_{p,\mathrm{outer}}
-
2\sqrt{(\pi^2 + 4)d_c^2 - L_b^2 + 4\pi L_b r_{p,\mathrm{outer}} - 4\pi^2 r_{p,\mathrm{outer}}^2}
}{
\pi^2 + 4
}.
}
...
center distance $d_{c,\mathrm{approx}}$, so much of that initial center-distance
...
\[
r_{p,\mathrm{outer}}(0) = r_{p,\mathrm{outer},\min} = \SI{0.03621}{m}.
\]

\paragraph{Secondary Radius Comparison}

Solving the exact belt-length constraint with $d_{c,\mathrm{exact}}$ yields
\[
r_{s,\mathrm{exact}} = \SI{0.10160}{m}.
\]

Evaluating the approximate formula \Eq{eq:rs_approx} with
$d_{c,\mathrm{approx}} = \SI{0.26837}{m}$ yields
\[
...
\midrule
$r_{p,\mathrm{eff}}$ & \SI{0.0284}{m} & \SI{0.0284}{m} \\
$r_{s,\mathrm{eff}}$ & \SI{0.0938}{m} & \SI{0.0938}{m} \\
$R = r_{s,\mathrm{eff}}/r_{p,\mathrm{eff}}$ & 3.300 & 3.300 \\
\bottomrule
\end{tabular}
\end{center}
...
Thus, although $d_{c,\mathrm{approx}}$ is already 6.61\% too large, the induced
...
\begin{align}
r_{p,\mathrm{outer}}(s_{\max})
&= r_{p,\mathrm{outer},\min} + \frac{s_{\max} - s_{\mathrm{dz}}}{2\tan\beta} \nonumber\\[4pt]
...
Solving the exact belt-length constraint \Eq{eq:belt_length_constraint}
numerically with $d_{c,\mathrm{exact}} = \SI{0.25174}{m}$ yields
\[
...
\[
r_{\mathrm{eff}} = r_{\mathrm{outer}} - \frac{h_b}{2}.
\]
...
\midrule
$r_{p,\mathrm{eff}}$ & \SI{0.0691}{m} & \SI{0.0691}{m} \\
$r_{s,\mathrm{eff}}$ & \SI{0.0589}{m} & \SI{0.0485}{m} \\
$R = r_{s,\mathrm{eff}}/r_{p,\mathrm{eff}}$ & 0.853 & 0.701 \\
\bottomrule
\end{tabular}
\end{center}
...
$d_c$, the secondary outer radius $r_2$ is the unique root of the belt-length
...
=
\pi(r_1+r_2)
+ 2(r_2-r_1)\arcsin\!\left(\frac{r_2-r_1}{d_c}\right)
+ 2\sqrt{d_c^2-(r_2-r_1)^2}
- L_b
= 0,
\label{eq:belt_constraint_g}
\end{equation}
where $L_b$ is the fixed belt length. A separate invocation solves for the
center-to-center distance $d_c$ itself at initialization, using the same equation
evaluated at the boundary radii $(r_{1,\min},\, r_{2,\max})$ with $d_c$ as the
...
$|r_2 - r_1| \le d_c$, so the natural domain for $r_2$ is the open interval
$(r_1 - d_c,\, r_1 + d_c)$. Within this interval a sign change is guaranteed:
at the lower boundary $r_2 \to r_1 - d_c$, the $\arcsin$ saturates to $-\pi/2$
and the square root vanishes, giving
$g \to 2\pi r_1 - 2\pi d_c - L_b < 0$ for all physically valid parameters.
At the upper boundary $r_2 \to r_1 + d_c$, by symmetry
$g \to 2\pi r_1 + 2\pi d_c - L_b > 0$, satisfied whenever the belt length is
...
\(d_c\)---is fixed, the compatibility relation defines a family of allowable
...
\(L_b\) or \(d_c\) changes not only the reachable endpoints, but also the
...
-
\frac{8\pi^2}
{\phi_s^3r_p\sqrt{d_c^2-(r_s-r_p)^2}}
\right]
\dot r_p^2
\nonumber\\
={}&
...
model, direct measurement, or CAD. With \(n_f\) identical flyweights, the total
flyweight-set mass used in the main derivation is
\[
    m_f=n_fm_{f,1}.
\]
...
\begin{equation}
    \boxed{
    F_{\mathrm{fw},x_p}\ge0.
    }
    \label{eq:appB_contact_force_admissible}
\end{equation}
If \(F_{\mathrm{fw},x_p}<0\), the constrained solution would require the ramp to
pull on the roller. That state is physically inadmissible. The result is not
replaced by \(\max(0,F_{\mathrm{fw},x_p})\), since clipping the force would hide
...
\(F_{\mathrm{fw},x_p}=0\) marks the unilateral-contact boundary.
...
point or centre-of-mass description at radius \(r_f(x_p)\), then the total
flyweight-set mass \(m_f=n_fm_{f,1}\) gives
\begin{equation}
    J_{f,p}=m_fr_f^2
\end{equation}
