...
them. In particular, this section explains why a CVT is useful, how a
rubber V-belt CVT changes ratio, why clamping and traction matter, how the
primary and secondary pulleys generate their forces, and why the CVT must be
understood as part of a larger drivetrain system.

% --------------------------------------------------------------
\subsection{Why Use a Continuously Variable Transmission?}
\label{subsec:why_use_cvt}

A continuously variable transmission is a drivetrain element whose speed ratio
can vary smoothly between limiting values rather than changing through a finite
set of fixed gear steps. This is useful because an internal combustion engine
does not produce the same torque and power at every speed. Instead, the engine
has a useful operating region where it can produce power more effectively.

In a conventional stepped gearbox, each gear imposes a fixed relationship
between engine speed and vehicle speed. As the vehicle accelerates, the engine
speed rises through the useful range of one gear, drops when the transmission
shifts, and then rises again in the next gear. This repeated sweep through the
engine speed range is a direct consequence of having only a small number of
available ratios.

A CVT behaves differently. Because its ratio can vary continuously, it gives
the drivetrain a continuously adjustable speed relationship between the engine
and the driven wheels. This allows the engine to remain closer to a desired
operating region while vehicle speed changes. The engine is not completely
independent of vehicle speed, since the CVT still has finite ratio limits and
must obey its own mechanical constraints. However, compared with a stepped
gearbox, the CVT provides much more freedom to match engine behavior to vehicle
load.

This idea is illustrated conceptually in \Cref{fig:cvt_engine_motivation}. The
left panel shows that engine torque and power depend strongly on engine speed.
The right panel compares the repeated speed sweeps of a stepped transmission
with the smoother operating behavior made possible by a CVT.
...
Show that torque and power vary with RPM and that there is a useful operating
region.
\end{minipage}
\hfill
\begin{minipage}{0.43\textwidth}
...
torque and power that vary with speed, while a continuously variable ratio
allows the drivetrain to keep the engine closer to a useful operating region
...
There are many possible continuously variable transmission architectures. This
work focuses on a belt-sheave CVT, in which two variable-radius pulleys are
connected by a rubber V-belt. The pulley connected to the engine side is called
the primary pulley, while the pulley connected to the downstream drivetrain is
called the secondary pulley. The belt transmits motion and torque between these
two pulleys.
...
toward the fixed sheave, the groove becomes narrower. Since the belt cannot
occupy the same radial position in a narrower groove, it is pushed outward
along the conical faces. The effective belt radius therefore increases. If the
movable sheave moves away from the fixed sheave, the groove becomes wider and
the belt sits deeper between the sheaves, reducing the effective radius.

At this introductory level, the mechanism is just wedge geometry. Axial motion
of the sheave is converted into radial motion of the belt. The exact geometric
relationship between sheave displacement and belt radius is derived later; for
now, the important point is the direction of motion. Closing the sheaves makes
the belt radius larger, and opening the sheaves makes the belt radius smaller.
\Cref{fig:single_pulley_states} illustrates this idea using three positions of
the same pulley.
...
sheaves open, the belt sits deeper in the groove and the effective radius
decreases.}
...
If the belt rides outward on the primary pulley, the belt must ride inward on
the secondary pulley. Likewise, if the primary radius decreases, the secondary
radius must increase. This opposite motion is the basic geometric coupling that
allows the CVT ratio to change. The belt-length constraint defines the required
relationship between the two pulley radii, while the actual motion of the
sheaves occurs through the contact forces, springs, actuator mechanisms, and
...
drivetrain: the engine-side pulley turns relatively quickly while the
downstream side turns more slowly.
...
For traction to develop, the belt must be pressed into the sheave faces. This
requires clamping force. The clamping force creates contact pressure between
the belt and the pulley, and that contact pressure allows friction to transmit
tangential force around the pulley. In practical terms, transferring more torque
through the CVT requires enough clamping force and enough available traction at
...
traction problem. \Cref{fig:belt_sheave_forces} summarizes this force picture.
...
Different CVTs generate radius change and clamping force in different ways. In
the system studied here, the CVT is mechanically actuated: the pulley forces
arise from mechanisms inside the primary and secondary pulley assemblies rather
than from an externally commanded ratio. These mechanisms are important because
they determine both how the belt moves through the ratio range and how much
clamping force is available for torque transfer.

The primary pulley is mainly speed-sensitive. As engine speed increases,
flyweights in the primary pulley move outward under centrifugal effects.
Through ramped contact surfaces, this outward flyweight motion produces an
axial tendency to close the primary sheaves. Closing the primary sheaves pushes
the belt outward to a larger effective radius and drives the upshift direction
introduced in \Cref{fig:cvt_ratio_states}. A conceptual view of this mechanism
is shown in \Cref{fig:cvt_primary_actuation}.
...
\begin{itemize}
    ...
    \item resulting axial closing tendency.
\end{itemize}
...
this motion into an axial tendency to close the primary sheaves, increasing the
primary effective radius.}
\label{fig:cvt_primary_actuation}
\end{figure}

The secondary pulley is mainly torque-reactive. As torque is transmitted
through the secondary, a helical mechanism produces an axial clamping tendency.
This helps the secondary respond to load: when more torque is being carried,
the secondary can generate additional clamping action. Springs provide preload
and help set the baseline behavior of the mechanism.

The helix also introduces an important kinematic feature that will appear later
in the model. Axial motion of the secondary sheave is accompanied by relative
rotation between secondary sheave components. At this stage, the important
point is only that the secondary is not just translating axially; its shift
motion is coupled to rotation through the helix geometry. This coupling is
shown conceptually in \Cref{fig:cvt_secondary_actuation}.
...
\begin{itemize}
    ...
    \item spring,
    \item movable sheave,
    \item torque-reactive clamping direction,
    \item relative rotation between secondary sheave components during shift.
\end{itemize}

The figure should visually support two ideas:
\begin{enumerate}
    \item transmitted torque contributes to secondary clamping,
    \item axial shift of the secondary is coupled with relative sheave rotation.
\end{enumerate}
\vspace{0.8cm}
\end{minipage}
}
\caption{Secondary pulley actuation concept. The secondary pulley is mainly
torque-reactive: torque transmitted through the pulley loads the helix
mechanism, producing an axial clamping tendency. The helix also couples axial
shift with relative rotation between secondary sheave components.}
\label{fig:cvt_secondary_actuation}
\end{figure}

Although the primary and secondary mechanisms respond to different physical
quantities, they act on the same belt and the same shift process. The primary
is mainly driven by speed, the secondary is mainly driven by transmitted torque,
and the resulting CVT behavior depends on their interaction with the belt,
springs, pulley geometry, drivetrain inertia, and external load.

% --------------------------------------------------------------
\subsection{Full Drivetrain Context}
\label{subsec:full_drivetrain_context}

The CVT does not operate in isolation. In the system considered here, engine
torque enters the primary pulley, passes through the belt to the secondary
pulley, travels through a fixed-ratio gearbox or final drive, and ultimately
produces tractive force at the wheels. The vehicle then pushes against the
ground, and the resulting vehicle load feeds back into the drivetrain through
inertia, grade resistance, rolling resistance, and aerodynamic drag.

This system-level view is shown in \Cref{fig:drivetrain_overview}. It is
...
affects secondary clamping. Vehicle load affects the torque required at the
secondary. Belt traction determines whether the torque can be carried without
...
The later derivation formalizes this mechanism chain. The geometry model
determines how shift position changes pulley radii. The axial force models
determine how the sheaves move. The traction model determines whether torque
transfer sticks or slips. The drivetrain model determines how engine torque and
vehicle load interact through the CVT. Together, these pieces form the dynamic
state-space model developed in the following chapters.
...
\label{sec:lit-purpose}

Continuously variable transmission (CVT) modelling is not a single, uniform problem. A model developed to size a variator, estimate belt efficiency, design a control strategy, or predict launch behaviour may use the same physical components, but it will usually make very different assumptions about which quantities are prescribed and which are solved dynamically. For example, some models focus on component sizing and geometric design, some on rubber belt force distributions and axial thrust, some on efficiency and power loss at fixed operating points, and others on transient shift behaviour or detailed belt-pulley contact mechanics \citep{SrivastavaHaque2009,Gerbert1996,KimKim1989,Sorge1996,KongParker2005,KongParker2006,Messick2018}. Consequently, the most useful way to review the literature is not as a chronological list of papers, but as a progression through the major modelling problems that arise in belt-sheave CVTs.

This review is organized around those modelling problems. First, a classification framework is introduced to compare CVT models by their belt/contact fidelity, actuation closure, shift dynamics, rotational dynamics, drivetrain coupling, and treatment of launch or clutching. The review then begins with practical design and tuning references, since these sources define much of the vocabulary used by practitioners and establish the qualitative behaviour that a useful model should reproduce. It then moves to design-oriented and quasi-static models, rubber V-belt contact and efficiency models, mechanically actuated shift models, transient rubber-belt models, and finally adjacent metal-belt and chain CVT dynamic models. This ordering follows the engineering difficulty of the problem: from design intuition, to component force balance, to belt contact, to ratio change, to full transient launch behaviour.

The goal is not to claim that earlier models are incomplete in a general sense. Rather, the goal is to identify which parts of the CVT problem each class of model resolves, which assumptions are introduced to make the problem tractable, and how those assumptions affect the ability to simulate a mechanically actuated rubber V-belt CVT during launch. The literature contains extensive work on individual aspects of CVT behaviour, including belt geometry, actuator design, axial thrust, belt slip, efficiency, transient shift response, and metal/chain CVT dynamics. However, these areas have often developed around different architectures and objectives. This makes a careful classification necessary before identifying the specific gap addressed by the present formulation.

\subsection{Framework for Classifying CVT Models}
\label{sec:lit-classification}

Several modelling axes are useful for comparing prior CVT work. The first is the fidelity with which the belt-pulley contact is represented. At the simplest level, torque capacity may be estimated through capstan-style tension-ratio relations, possibly modified by the wedge effect of the V-groove. More detailed models resolve radial and tangential friction components, sliding angle, belt penetration, belt bending, local normal pressure, or the spatial distribution of tension around the wrap \citep{Gerbert1996,KimKim1989,Sorge1996,KongParker2005,KongParker2006}. At still higher fidelity, the belt may be discretized into nodes or elements, allowing local stick-slip behaviour, bending stiffness, axial stiffness, and transient belt vibration to be represented directly \citep{JulioPlante2011,Ballew2015}.

A second axis is actuation closure. Many CVT models prescribe the axial force applied to each pulley, which is appropriate for hydraulically or electronically controlled variators and for studies focused on belt contact rather than actuator mechanics. In mechanically actuated rubber V-belt CVTs, however, the axial forces are not arbitrary inputs. The primary force is generated by a centrifugal mechanism opposed by a spring, while the secondary force is generated by a combination of preload, spring force, and torque-reactive helix or cam geometry \citep{Cammalleri2005,KimEtAl2002,SorgeCammalleri2011,Aaen1979}. This distinction is important because, in a mechanical CVT, the ratio is not directly commanded. It emerges from the coupled response of the actuator forces, belt forces, transmitted torque, and vehicle load.

A third axis is the treatment of ratio change. Some models evaluate the CVT at a fixed ratio, while others solve for the ratio quasi-statically from a force balance. Some transient models use empirical shift-rate equations, where the ratio rate is related to the difference between an actual and steady force condition \citep{KimEtAl2002}. Other models resolve shifting through belt motion, pulley axial motion, or contact mechanics more directly \citep{Sorge2008,CammalleriSorge2009,JulioPlante2011,Ballew2015}. For launch simulation, the distinction is especially important because the shift coordinate may accelerate, stop, reverse, or interact with clutching slip; it cannot always be treated as a sequence of steady operating points.

A fourth axis is the treatment of rotational dynamics. In many operating-point models, pulley speeds are prescribed or related algebraically by the instantaneous ratio. This is sufficient when the objective is to evaluate a known condition, but it removes the possibility of engine speed and vehicle speed co-evolving dynamically. A more complete drivetrain model must allow the primary and secondary angular velocities to change according to torque balances, and must account for the fact that the belt can transmit only the torque allowed by the current contact and traction state. This becomes especially important during launch, where engine speed, belt speed, secondary speed, and vehicle speed may initially be far from their eventual no-slip relationship.

A fifth axis is drivetrain coupling. On the input side, a model may prescribe engine speed, prescribe engine torque, use an engine torque curve, include engine inertia, or include throttle and governor dynamics. On the output side, a model may prescribe load torque, use a dynamometer load, include road-load terms, or solve the full longitudinal vehicle response. These choices strongly affect the interpretation of model results. A CVT model with vehicle load but fixed input speed is not solving the same problem as one where engine speed is itself a state determined by engine torque and CVT reaction torque.

The final axis is launch and clutching behaviour. Many CVT studies begin from an already engaged condition, where the belt is assumed to be transmitting torque in a known ratio range. Launch is more difficult because the system may pass through clutch engagement, gross slip, traction-limited operation, and eventual no-slip motion. During this interval, clamp force, belt tension, transmitted torque, engine speed, and vehicle acceleration all evolve together. A model may therefore include shift dynamics without necessarily resolving launch, and may include rotational dynamics without necessarily resolving clutch engagement.

\subsection{Practical Design and Tuning Foundations}
\label{sec:lit-practical-design}

Before considering formal analytical models, it is useful to recognize the practical design tradition from which mechanically actuated rubber V-belt CVTs developed. Standard machine design references provide the general background for many of the components involved: springs, clutches, brakes, flexible belt elements, friction interfaces, shafts, bearings, and the broader iterative nature of mechanical design \citep{BudynasNisbett2015}. These references are not CVT-specific research works, but they establish the engineering context in which a CVT must be treated as a coupled mechanical system rather than as an isolated kinematic ratio device. Component stiffness, friction, safety factors, tolerances, wear, and uncertainty all influence the final behaviour of a real variator.

Practical clutch-tuning literature is also important because it introduces the operating language used by racers, tuners, and powersports engineers. Sources such as Aaen's clutch tuning handbook describe the behaviour of snowmobile-style rubber V-belt CVTs using terms such as engagement speed, shift speed, low ratio, high ratio, flyweights, spring preload, torque sensing, backshift, and speed variance \citep{Aaen1979}. Speed diagrams are especially useful for developing intuition. In a typical launch, engine speed first rises toward the engagement speed, the vehicle begins to move as the belt starts transmitting torque, and the CVT then attempts to shift while holding the engine near a desired operating speed. After the variator reaches its high-ratio limit, engine speed again rises with vehicle speed. This qualitative picture is central to how mechanically actuated CVTs are tuned in practice.

However, practical tuning sources are primarily behavioural and empirical. They explain which components are changed and what qualitative response is expected, but they do not generally derive a predictive dynamic model from first principles. A tuner may know that changing flyweight mass, ramp curvature, spring preload, or helix angle affects engagement speed, shift speed, and backshift response, but that knowledge does not by itself produce a coupled time-domain simulation. The practical literature therefore motivates the need for analytical and numerical models: the observed behaviour is rich, coupled, and important, but difficult to predict from intuition alone.

\subsection{Design-Oriented and Quasi-Static CVT Models}
\label{sec:lit-design-models}

Design-oriented models attempt to convert the component-level understanding of CVTs into quantitative predictions. These models commonly relate pulley geometry, belt length, wrap angle, speed ratio, flyweight force, spring preload, helix force, belt tension, and torque capacity. Their strength is that they provide a systematic bridge between physical component choices and expected operating behaviour. They are therefore useful for sizing studies, actuator design, and first-pass tuning.

Cammalleri developed a design approach for a speed-torque-controlled rubber V-belt variator using a centrifugal primary mechanism and a torque-sensitive driven pulley \citep{Cammalleri2005}. This work is important because it treats the mechanical actuation architecture directly rather than assuming externally commanded clamp forces. It provides a useful foundation for reasoning about how actuator geometry can be selected to obtain desired speed-torque behaviour. Similarly, Arango and Muñoz proposed a numerical design method for rubber V-belt CVTs that begins from vehicle requirements, road specifications, and desired performance, then proceeds toward component selection and transmission sizing \citep{ArangoMunoz2020}. La Battaglia and co-workers focused on the kinematic design of motorcycle rubber V-belt CVTs, emphasizing the front pulley roller profile and the geometric relationships needed to improve system development \citep{LaBattaglia2022}. These works are valuable because they show that rubber V-belt CVT design can be treated systematically rather than purely through trial and error.

Practical force-balance models occupy a related category. For example, Skinner developed a mechanically actuated CVT tuning model for Baja SAE applications, emphasizing engagement speed, shift-out speed, static RPM shifting, spring testing, flyweight behaviour, helix forces, and experimental testing tools \citep{Skinner2020}. Such work is useful because it connects the variables adjusted by tuners to predicted operating conditions. However, it is best understood as a design and tuning model rather than as a full transient dynamics model. The ratio is treated primarily through force-balance and operating-point calculations rather than through an explicit inertial shift coordinate coupled to rotational and vehicle dynamics.

The main limitation of design-oriented and quasi-static models is therefore not that they are incorrect, but that their scope is different. They are well suited to answering questions such as which spring, flyweight, helix, or pulley geometry should produce a desired operating trend. They are less suited to predicting launch as a continuous dynamic process. During launch, the engine accelerates, the belt may slip, clamp force builds, vehicle load changes, and the secondary torque reaction changes as transmitted torque changes. A model based only on equilibrium operating points cannot fully represent these coupled transitions.

\subsection{Rubber V-Belt Contact, Axial Force, and Efficiency Models}
\label{sec:lit-contact-efficiency}

To move beyond component-level design, it is necessary to consider the belt-pulley interface itself. A rubber V-belt CVT does not transmit torque through an ideal kinematic constraint. Torque transmission depends on belt tension, normal pressure, wedge action, friction, wrap angle, radial belt motion, centrifugal effects, belt deformation, and energy loss. These phenomena have motivated a substantial body of rubber V-belt contact and efficiency literature.

Classical belt theory establishes the basic relationship between wrap angle, friction, and allowable tension ratio. For V-belts, this picture is modified by the groove geometry, which changes the relationship between normal force, axial force, and tangential friction. However, real rubber V-belt slip is more complicated than a single scalar loss term. Gerbert's unified slip treatment decomposes belt slip into multiple mechanisms, including creep, radial compliance, shear deflection, and seating or unseating effects \citep{Gerbert1996}. This is important because it shows that a simple no-slip/slip distinction can be useful at the system level but cannot capture every source of speed loss or energy dissipation.

Kim and Kim developed influential analytical models for axial forces in rubber V-belt CVTs, using simplified contact-region assumptions to relate pulley thrust, belt tension, and torque transmission \citep{KimKim1989}. Sorge further emphasized the role of radial penetration and developed simplified expressions for axial thrust in V-belt drives \citep{Sorge1996}. These works provide important foundations for modelling the relationship between belt forces and sheave axial forces. They also demonstrate that even steady rubber V-belt operation involves nontrivial contact mechanics.

Kong and Parker developed a more detailed formulation for belt-pulley systems using distributed mechanics and boundary-value modelling \citep{KongParker2005}. Their later work extended this framework to pulley grooves and sliding friction, making it especially relevant to V-belt drives \citep{KongParker2006}. These models operate at a higher contact fidelity than most component-level CVT tuning models. They are valuable because they show how local belt equilibrium, sliding direction, and contact pressure can be resolved more rigorously, although their purpose is not to produce a compact mechanically actuated launch simulator.

Messick built on this contact-mechanics tradition in an experimentally validated rubber V-belt model for axial force and efficiency in a CVT \citep{Messick2018}. This work is especially relevant because it is motivated by Baja SAE and powersports CVTs, but its main technical contribution lies in belt force, axial-force, efficiency, and parameter-estimation modelling rather than full launch simulation. Messick's model provides quantitative information about efficiency and pulley axial forces and validates analytical predictions using a CVT dynamometer. It therefore serves as a strong reference for belt contact and efficiency modelling, but it should not be treated primarily as a mechanical actuation or full drivetrain transient model.

Efficiency-oriented studies provide another important perspective. Chen, Lee, and Sung experimentally studied transmission efficiency in a rubber V-belt CVT, separating speed and torque loss effects across operating conditions \citep{ChenLeeSung1998}. Bertini and co-workers developed an analytical power-loss model for rubber V-belt CVTs, considering sliding losses, belt hysteresis, and entry/exit effects \citep{Bertini2014}. These studies demonstrate that efficiency is governed by multiple interacting loss mechanisms, many of which are not represented if a model only enforces traction admissibility.

The contact and efficiency literature therefore provides a strong foundation for understanding torque transmission, axial force, and loss mechanisms. It also makes clear that high-fidelity local belt modelling can become quite complex. The present work does not attempt to supersede this literature in local contact fidelity. Instead, it adopts simplified but physically interpretable belt assumptions so that the full mechanically actuated drivetrain can be closed dynamically.

\subsection{Mechanical Actuation and Rubber-Belt Shift Mechanics}
\label{sec:lit-shift-mechanics}

The next modelling challenge is ratio change. A mechanically actuated rubber V-belt CVT does not directly command its speed ratio. The primary pulley responds mainly to rotational speed through flyweights and a spring, while the secondary pulley responds to torque and preload through a helix, cam, and spring system. The shift ratio emerges from the interaction of these mechanisms with belt geometry, belt tension, torque transmission, and external load.

This is the point at which practical tuning language connects directly to modelling. Flyweight mass and profile influence the speed-sensitive primary closing force; the primary spring affects engagement and shift speed; the secondary helix converts transmitted torque into axial force; and secondary preload influences backshift and belt clamping \citep{Aaen1979,Cammalleri2005,Skinner2020}. These relationships are well known in tuning practice, but the dynamic coupling between them is difficult to capture. For example, changing helix angle affects not only clamp force but also the load-dependent resistance to shift, which changes engine speed, belt torque, and vehicle acceleration.

Cammalleri's work provides a formal design treatment of mechanically actuated rubber V-belt variators, including centrifugal and torque-sensitive mechanisms \citep{Cammalleri2005}. Kim and co-workers studied steady-state and transient characteristics of a mechanically actuated rubber belt CVT, making their work especially relevant to the present problem \citep{KimEtAl2002}. However, the transient portion of that model uses an empirical shift-rate relationship rather than deriving the shift coordinate from an inertial equation of motion. This makes the model valuable as a precedent for transient mechanical CVT behaviour, but different in closure from a fully first-principles dynamic shift formulation.

Sorge's work on rubber belt variator shift mechanics is also central. Sorge emphasized that shifting is not simply a sequence of steady operating points, because radial belt motion is superimposed on the circumferential motion needed for torque transmission \citep{Sorge2008}. This distinction is important: during shift, the belt must move radially through the pulley groove while also carrying torque around the wrap. Opening and closing pulleys need not behave symmetrically, and mass conservation of the belt imposes additional constraints on the motion. Camelleri and Sorge later developed approximate closed-form solutions to make these shift-mechanics ideas more usable in engineering calculations \citep{CammalleriSorge2009}. Helix-focused work further clarifies how torque-reactive mechanisms in the driven pulley influence axial force and shift behaviour \citep{SorgeCammalleri2011}.

Together, these studies show that mechanical actuation and shift mechanics are well-established topics. They also show why the problem is difficult. Mechanical CVT shift behaviour depends on speed-sensitive primary force, torque-sensitive secondary force, belt motion, contact state, and load torque. Many prior treatments simplify this interaction by prescribing axial forces, using quasi-static balances, or introducing empirical shift-rate laws. The present work builds on this literature but seeks a different closure: an explicit dynamic shift state driven by mechanical actuator forces and resisted by belt and secondary forces within the full drivetrain model.

\subsection{Transient Rubber-Belt Models and the Launch Problem}
\label{sec:lit-transient-rubber}

Transient rubber-belt CVT models address the time-dependent behaviour that steady and quasi-static models cannot capture. However, ``transient'' can mean several different things. A model may simulate belt-node vibration, ratio change under prescribed forces, response to a torque disturbance, or full vehicle acceleration. These are related but not equivalent problems. In particular, launch and clutching are more difficult than shift between already engaged operating points.

During launch, the vehicle begins near rest, the engine accelerates toward engagement, belt clamp force develops, the belt may slip, and the secondary torque reaction depends on the torque that the belt is actually able to transmit. At the same time, road load and vehicle acceleration determine the torque demand at the secondary. The system may transition from clutching slip to traction-limited torque transfer and eventually toward no-slip operation. This makes launch a strongly coupled event involving engine dynamics, belt dynamics, shift dynamics, and vehicle dynamics simultaneously.

Julio and Plante developed one of the strongest transient rubber-belt CVT models in the literature, using a discretized belt representation with experimental validation \citep{JulioPlante2011}. This work is important because it prevents any broad claim that transient rubber V-belt CVT modelling is absent from the literature. It resolves local belt mechanics at a higher fidelity than a lumped model and is a key benchmark for any system-level formulation. However, its modelling emphasis differs from the present work: it is not primarily a compact, mechanically self-actuated launch model with flyweight and torque-reactive helix closure embedded in the drivetrain equations.

Ballew extended transient rubber-belt modelling by recreating a discretized rubber-belt CVT algorithm and adding engine and vehicle sub-models \citep{Ballew2015}. This work is particularly important because it explicitly criticizes previous algorithms that prescribe input pulley speed. Prescribing input speed fixes a rotational degree of freedom and removes part of the dynamic interaction between the input and output pulley speeds. Ballew instead uses torque as an input so that pulley angular speeds are governed by belt-pulley interface forces and applied torques. This is a significant step toward drivetrain-level transient simulation.

At the same time, Ballew's reported simulations use a controller-generated primary axial force and a simplified or fixed secondary axial force. The model is framed around the possibility of electronically controlled axial force, although it notes that mechanically governed axial force could be incorporated with changes to the model outputs and force inputs \citep{Ballew2015}. Thus, Ballew is a close precursor for transient belt dynamics and engine/vehicle coupling, but it does not close the same mechanically self-actuated primary and secondary force-generation problem addressed here.

The transient rubber-belt literature therefore occupies an important middle ground. It shows that dynamic rubber-belt simulation is possible and that detailed belt contact can be resolved in time. It also shows that engine and vehicle effects can be included. However, assumptions about actuation, secondary force, launch conditions, or operating state are often needed to keep the model tractable. This motivates a lower-dimensional but more fully closed mechanical drivetrain model, where the belt is simplified but the primary actuation, secondary actuation, rotational dynamics, shift dynamics, slip admissibility, and vehicle response are solved together.

\subsection{Adjacent Metal-Belt and Chain CVT Dynamic Models}
\label{sec:lit-metal-chain}

Although the present work is concerned with dry rubber V-belt CVTs, the adjacent metal-belt and chain CVT literature is important context. These architectures have received extensive attention in dynamic modelling and control research, especially for automotive applications. Reviews such as Srivastava and Haque show the breadth of belt and chain CVT modelling, including steady mechanics, transient dynamics, friction-limited behaviour, and control-oriented formulations \citep{SrivastavaHaque2009}. This literature demonstrates that dynamic CVT modelling is a mature field, but also that modelling assumptions depend strongly on transmission architecture.

The Carbone--Mangialardi--Mantriota (CMM) family of models is a central example in metal-belt and chain CVT dynamics \citep{CarboneMangialardiMantriota2005,YildizEtAl2017}. These models study ratio-changing behaviour, clamp-force ratio, creep-mode and slip-mode behaviour, and the effect of pulley deformation. A key result from this line of work is that pulley deformation can be essential to explaining slow shifting behaviour under Coulomb-friction assumptions. Yildiz and co-workers later validated the CMM model under steady-state and shift manoeuvres with applied torque load, showing good agreement between theoretical predictions and experiments \citep{YildizEtAl2017}. These works provide strong evidence that high-fidelity dynamic CVT closure is possible in adjacent architectures.

Duan and co-workers developed a physics-based chain CVT model that starts from the kinematics and dynamics of an infinitesimal chain segment, uses a speed-dependent friction law spanning micro-slip and macro-slip, and couples pulley axial dynamics with chain radial motion \citep{DuanEtAl2016}. The model also includes pulley deformation and shows that deformation affects sliding angle, chain tension, and normal contact force distributions. This is highly relevant methodologically because it couples rotational dynamics, axial dynamics, contact mechanics, and friction state in a physical model.

However, metal-belt and chain CVTs differ substantially from dry mechanically actuated rubber V-belt CVTs. They are often lubricated, use different belt or chain structures, and commonly rely on hydraulic or electronic clamp-force control. Their contact mechanics, deformation mechanisms, and control objectives are therefore not directly transferable. These papers should not be treated as direct rubber V-belt mechanical-actuator precedents. Instead, they are best understood as adjacent dynamic analogues: they show that coupled shift, rotation, contact, and clamp-force dynamics matter, but in systems with different belt architecture and actuation closure.

This distinction strengthens the motivation for the present work. The broader CVT field has developed sophisticated dynamic models, especially for metal-belt and chain systems. Rubber V-belt CVTs, particularly mechanically actuated dry systems used in powersports and small off-road vehicles, have a different combination of modelling needs: simpler hardware, strong mechanical self-actuation, highly load-dependent launch behaviour, and less direct control over clamp force. The modelling gap is therefore not the absence of CVT dynamics in general, but the lack of a compact mechanically self-actuated rubber V-belt launch model with comparable system-level closure.

\subsection{Synthesis of Prior Work}
\label{sec:lit-synthesis}

The literature establishes strong foundations for many parts of the CVT problem. Geometry and belt-length relationships are well understood and appear throughout design-oriented studies. Mechanical actuation through flyweights, springs, torque-sensing cams, and helices is well documented in both practical and analytical work. Rubber V-belt contact mechanics have been studied in detail, including tension variation, axial thrust, radial penetration, slip mechanisms, distributed contact forces, and efficiency losses. Transient rubber-belt models exist, including discretized belt models with experimental validation and models that incorporate engine and vehicle effects. In adjacent metal-belt and chain architectures, dynamic models have been developed with detailed friction laws, pulley deformation, axial dynamics, rotational dynamics, and experimental validation.

At the same time, the literature also shows why a complete mechanically actuated rubber V-belt launch model is difficult. Simplifications that are appropriate for one objective can become limiting for another. A design model may capture how component choices affect steady operating points, but not how the drivetrain evolves during launch. A contact model may resolve local belt forces accurately, but prescribe the operating state. A transient belt model may resolve belt-node dynamics, but use controlled or simplified axial forces. A metal or chain CVT model may solve coupled shift dynamics rigorously, but rely on different contact physics and actuation methods.

The launch problem is especially coupled. Engine speed changes according to engine torque and CVT reaction torque. Primary clamp force changes with speed and shift position. Secondary clamp force changes with transmitted torque and helix geometry. Belt torque transfer depends on the current contact and traction state. Vehicle acceleration changes the load seen by the secondary. Shift motion changes pulley radii, which changes speed ratio, belt speed, wrap angle, and torque demand. These interactions make it difficult to prescribe one part of the system without affecting the validity of the others.

The present work is positioned in this gap. It does not attempt to supersede high-fidelity local rubber-belt contact models or detailed efficiency/loss models. Instead, it uses simplified but physically interpretable assumptions to form a compact system-level dynamic model. The intended contribution is the closure of a mechanically actuated dry rubber V-belt CVT during launch: engine-side rotation, secondary/load rotation, belt torque transfer, axial shift inertia, flyweight force generation, torque-reactive secondary force generation, traction admissibility, and vehicle longitudinal motion are solved together rather than prescribed separately.

Table~\ref{tab:lit-comparison} summarizes the major model families discussed in this review.

\begin{table}[htbp]
\centering
\caption{Comparison of major CVT model families in the literature.}
\label{tab:lit-comparison}
\small
\begin{tabular}{p{0.16\textwidth} p{0.17\textwidth} p{0.14\textwidth} p{0.14\textwidth} p{0.13\textwidth} p{0.13\textwidth} p{0.13\textwidth}}
\hline
\textbf{Model family} &
\textbf{Representative works} &
\textbf{Contact fidelity} &
\textbf{Actuation treatment} &
\textbf{Drivetrain coupling} &
\textbf{Shift / launch treatment} &
\textbf{Primary limitation relative to this work} \\
\hline
    ...
    Limited &
    Qualitative speed diagrams and tuning trends &
    Not predictive dynamic models \\

    Design-oriented and quasi-static models &
    \citet{Cammalleri2005}, \citet{ArangoMunoz2020}, \citet{LaBattaglia2022}, \citet{Skinner2020} &
    Low to medium &
    Geometry and force-balance based &
    Often simplified &
    Mostly operating-point or quasi-static &
    Limited transient launch behaviour \\

    Rubber V-belt contact and efficiency models &
    ...
    Often limited &
    Mostly steady or quasi-equilibrium &
    Strong local physics, limited system-level closure \\

    Rubber-belt shift mechanics &
    \citet{Sorge2008}, \citet{CammalleriSorge2009}, \citet{SorgeCammalleri2011}, \citet{KimEtAl2002} &
    Medium &
    Mechanical actuation considered &
    Often simplified &
    Approximate or empirical transient behaviour &
    Partial drivetrain representation \\

    Transient rubber-belt models &
    \citet{JulioPlante2011}, \citet{Ballew2015} &
    High local belt/contact fidelity &
    Prescribed, controlled, or simplified force generation &
    Moderate to high &
    Dynamic, but launch and actuation assumptions remain &
    Not fully closed mechanical flyweight/helix launch models \\

    Metal-belt and chain dynamic models &
    \citet{SrivastavaHaque2009}, \citet{CarboneMangialardiMantriota2005}, \citet{YildizEtAl2017}, \citet{DuanEtAl2016} &
    High for metal/chain systems &
    Hydraulic, electronic, or prescribed clamping &
    Often high &
    Dynamic shift and slip behaviour &
    Different architecture, contact mechanics, and actuation \\
    ...
    Launch, clutching, shift, and rotation solved together &
    Lower local contact/loss fidelity, broader mechanical launch closure \\
...
\Sec{sec:axial}, \Sec{sec:no-slip-belt-dynamics},
\Sec{sec:traction-limits}, and \Sec{sec:slip-branch-dynamics}. These sections develop the
