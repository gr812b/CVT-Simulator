...
In particular, its inner width \(b_{\mathrm{in}}\) and radial height \(h_b\)
are treated as constant. Compression between the sheave faces, lateral
bulging, and centrifugal growth of the belt cross-section are neglected.
The belt may still bend along its length to wrap around the pulleys.
\end{assumptionblock}
...
TODO: Update remark box once I have a section on numerical simulation transitions.
\end{remarkbox}
...
The pulley geometry is now closed by the single shift coordinate. The
definition in \Cref{eq:system_shift_coordinate} fixes the primary position,
while \Cref{eq:secondary_coordinate_map} supplies the unique compatible
secondary position. The local pulley maps in
\Cref{eq:local_radius_map,eq:primary_local_radius_map} then give the two pulley
radii. Finally, the rate and acceleration relations in
...
This closure follows from the geometric representation used throughout the
...
spacing. With the two wrapped arcs joined by the straight tangent spans in
\Fig{fig:belt_loop_geometry}, specifying \(s\) leaves no further geometric
freedom in the modeled belt path.
