...
inelastic boundary or topology-capture events. Predicted transmission
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}
    ...
    D02\_P & D02 with the primary installed spring compression restored to the
      reference value. \\
    D02\_M150 & D02 with the flyweight tip mass increased to \(150\%\) of
...
spring preload. D02\_M150 extends the mass correction to test how stronger
...
D02\_M150 carries the mass correction farther, to \(150\%\) of the reference
tip mass, with the associated mass moments changed consistently and D02's
elevated spring compression retained. It also increases centrifugal closing at a
given speed and geometry. That action supplies clamping, but it also opposes
the primary opening needed for backshift. A change that preserves grip can
therefore change how readily the CVT gears down under the hill load. The
three corrections must be understood through both effects.

Both D02\_M and D02\_P reach the same low-ratio seat as D02 and remain there
through the rest of the constant \(38^\circ\) climb. This gives a direct way
to distinguish increased clamping from a change in ratio or engine speed:
their seated states can be compared at a common geometry and primary speed.
\Cref{fig:results_d02_hill_repairs} therefore shows primary normal load and
static use against decreasing primary speed on the constant slope.
...
\begin{figure}[H]
  ...
  \caption{Restored primary clamping in D02\_M and D02\_P. (a) Total primary
  normal load and (b) static utilization during seated sticking on the \(38^\circ\) hold,
  ...
  annotations refer to states near \SI{200}{m}. D02\_M150 is excluded because
  it does not occupy seated sticking states in this speed range.}
...
panel~(a) directly explain the lower fractions used in panel~(b). The repaired
...
The closing force that restores this grip also resists the primary opening
as the load calls for backshift. \Cref{fig:results_d02_hill_backshift} follows
that motion together with primary speed. D02\_M and D02\_P reach the seat
farther along the road than D02, and the stronger mass correction leaves
D02\_M150 more upshifted during hill entry than D02\_M. Near \SI{150}{m}, its
primary speed has fallen to approximately \(2750~\mathrm{rpm}\), compared
with \(3240~\mathrm{rpm}\) in D02\_M. The stronger closing preserves
clamping, but leaves the transmission more upshifted as the engine slows.
Each case also arrives with a different speed and ratio from
the opening flat, so the road-position comparison retains that history.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/results/course/d02_hill_backshift.png}
  \caption{Backshift consequences of the three primary-side corrections.
  (a) Shift position and (b) primary speed, with the entry taper shaded and
  the common low-ratio seat dashed in (a). D02 ends at sustained primary slip
  onset. The inset enlarges the gap above the seat for D02\_M and D02\_M150.
  Following its initial contact with the stop, D02\_M150 remains supported
  over the shaded inset interval, from
  about \SI{171.13}{m} to \SI{173.04}{m}, approximately \SI{0.51}{s}, then
  leaves the seat while both contacts stick. The road remains at \(38^\circ\)
  throughout the inset. Equal road position retains the different states
  inherited from the opening flat.}
  \label{fig:results_d02_hill_backshift}
\end{figure}

D02\_M150 eventually reaches the low-ratio seat, but the inset reveals that
it cannot remain there. The reason is visible in the secondary force balance.
The helix and spring push the secondary closed, while the belt pushes its
sheaves apart; at the seat, the stop carries the excess of closing over belt
opening force (\Cref{eq:low_ratio_stop_reaction}). The two pulley normal loads
are connected through tension in the same belt. Here, stronger primary
clamping is accompanied by a larger secondary normal load and hence a larger
belt-opening force. Just after D02\_M150 has seated and regained sticking,
only about \SI{11}{N} of closing force remains for the stop to carry.
As primary speed and normal load recover slightly, the belt-opening force
increases until that surplus reaches zero. Continuing to hold the secondary
closed would then require the stop to pull, so the secondary leaves the stop
while the road still holds the same grade.

Both contacts continue sticking after release. D02\_M150 approaches an
interior position only about \SI{0.029}{mm} above the seat, using approximately
\(0.20\) of its primary static capacity near \SI{200}{m}. Its climbing speed
there is about \SI{3.763}{m/s}, compared with \SI{3.775}{m/s} for D02\_M,
and it completes the course. The small positional difference therefore marks
a change in how the forces balance, with only a small late climbing-speed
penalty. Ample traction does not ensure that the lowest geometric ratio can
remain supported; the actuator and belt forces must also allow the secondary
to stay against its stop.
...
stronger mass correction then shows why that improvement cannot be judged
from clamping alone: the same closing action changes backshift, engine speed,
and whether the low-ratio stop can remain loaded. These effects follow from
the same hardware change within the coupled motion. The next repeated
