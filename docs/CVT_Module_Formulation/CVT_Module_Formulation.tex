...
but it does not yet determine how the CVT moves within that regime. The continuous
equations must reflect those physical conditions. In the deadzone, primary belt
contact is absent, so its belt force and torque disappear from the primary free
body. At a travel stop, the stopped sheave can no longer accelerate through the
boundary, so the hardware supplies the reaction required to hold it there. The
pulley, belt, and actuator mechanics that remain active continue to follow the
balances already developed.

\subsubsection{Disengaged Primary and Deadzone Motion}
\label{sec:deadzone_continuous_regimes}

Begin at the fully open end of travel, where the primary sheaves do not touch the belt,
and the same remains true while the primary crosses the clearance toward
\(s=s_{\mathrm{dz}}\). With the sheave faces separated, every interaction that
requires primary belt contact is absent, while the primary hardware itself
remains dynamically active. Hence,
...
belt mechanics relied on a taut closed path carrying tension through both
pulley wraps. In the deadzone, the secondary is already fully closed, so it
cannot close farther to take up the remaining belt slack and establish that
taut path. The present belt model also has no longitudinal strain or pretension
state from which a residual tension could be recovered while the geometry
remains slack. Although the belt may still rest on the minimum-radius primary
support, that contact is not represented as a clamped belt--sheave interaction.

Capturing the forces that develop as a slack or pretensioned belt is taken up
would therefore require additional belt elasticity and slack-span geometry
beyond the reduced description used here. The adopted deadzone closure in
\AssRef{assump:deadzone_belt_closure} instead keeps the belt seated on the
fully closed secondary and carries it with the represented secondary contact
surface. Because the secondary has no axial or helix-relative motion in this
regime, \(\dot\theta_s=0\), and
...
These contact-free equations govern the deadzone unless the primary is
supported by its opening stop. At \(s=0\), that support prevents further
opening, so a sustained stop condition requires
...
Once the primary is free of this support, \(0<s<s_{\mathrm{dz}}\), the same
contact-free equations determine
\(\dot\omega_p\), \(\dot\omega_s\), \(\dot v_b\), and \(\ddot s\) through
...
The primary can then cross the clearance until belt contact is recovered.
...
contacts retain the sticking or sliding regimes developed in
\Cref{sec:engaged_dynamic_closure}.
...
The stop reaction is not known from the spring preload alone. In
\Cref{eq:low_ratio_stop_reaction}, the belt term contains the unknown normal
resultant \(N_s\), while the helix term contains the transmitted secondary
torque and \(\dot\omega_s\) through
\Cref{eq:secondary_helix_closing_force}. Those quantities are determined only
together with the rest of the engaged response.

The low-ratio constraint changes how that coupled response is solved. For a
trial traction pair \((\lambda_p,\lambda_s)\), the free engaged mechanical
core in \Cref{eq:engaged_six_by_six_mechanical_core} normally determines
\(\ddot s\) together with
\(\dot\omega_p\), \(\dot\omega_s\), \(\dot v_b\), \(N_p\), and \(N_s\).
At the seat, \(\ddot s=0\) is already prescribed by the stop. The secondary
axial row is therefore replaced by that constraint, while the primary axial
row remains active because the primary itself is not supported by a stop.
Together with the rotational and belt equations, the constrained system
determines the remaining engaged response at the fixed geometry.

Once those quantities are known, the original secondary axial balance has a
new role: substituting the solved \(N_s\), secondary torque, and
\(\dot\omega_s\) into \Cref{eq:low_ratio_stop_reaction} gives
\(F_{\mathrm{stop,LR}}\). For sticking contacts, each trial traction value is
tested with this constrained mechanical solve inside the same root logic
developed in \Cref{sec:engaged_dynamic_closure}; a sliding contact supplies
its kinetic traction utilization directly.
...
Away from the low-ratio seat, the secondary stop supplies no reaction. If
primary belt contact is retained, both sheaves are again free to follow the
shared belt geometry, so all six rows of the engaged mechanical core become
active exactly as derived in \Cref{sec:engaged_dynamic_closure}. The interior
shift therefore needs no additional continuous equation.
...
Here the primary, rather than the secondary, is the stopped member. The
constraint \(\ddot s=0\) therefore replaces the primary axial row of
\Cref{eq:engaged_six_by_six_mechanical_core}, while the secondary axial balance
remains part of the coupled solve. The rotational, secondary axial, and belt
equations determine the fixed-shift engaged response for the chosen
tangential contact regimes. The original primary axial balance is then
evaluated through \Cref{eq:upper_stop_reaction} to recover the force carried
by the upper stop.
...
The three supported shift regimes are summarized in
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}
        ...
        \bottomrule
    \end{tabular}
    \caption{Hardware reactions for the three stop-supported shift
...
The continuous mechanics are now defined for all five shift regimes: the
fully open stop, free deadzone, low-ratio seat, free engaged shift, and upper
primary stop. Within any one of them, the active balances determine the
instantaneous accelerations and any support reaction required by the hardware.
These continuous equations do not, by themselves, determine an instantaneous
change in velocity when a new kinematic constraint is reached.
