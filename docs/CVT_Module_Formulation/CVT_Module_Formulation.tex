...
\subsection{Low-ratio seating at numerical energy resolution}
\label{app:numerical_low_ratio_seating}

When opening motion reaches the low-ratio boundary, a finite secondary-stop
impact can leave the primary moving into the deadzone. Repeated small
captures near this boundary also require a numerical rule for completing
seating. The tagged implementation first evaluates a hypothetical
engaged-to-engaged velocity projection using the finite-event momentum
balance of \Cref{eq:finite_event_kkt_system}. This projection arrests the shared
shift motion while retaining
any existing sticking constraints. Its kinetic-energy loss is compared
with
\[
  8192\,\epsilon_{\mathrm{mach}}\max(\SI{1}{J},T^-),
\]
where $\epsilon_{\mathrm{mach}}$ is floating-point machine precision and
$T^-$ is the incoming retained kinetic energy.

When a nonzero deadzone exists and the projected loss exceeds this
threshold, the ordinary secondary-stop impact is applied: the secondary
axial and helix-relative motion stop, the belt--secondary lock is retained,
and the primary is free to separate into the deadzone. At or below the
threshold, the projected state is tested for seated continuation. The
tangential contact conditions, primary-separation condition and seat
reaction are still evaluated. Primary separation returns the system to the
deadzone; a tensile seat reaction releases the support. Thus the energy
test permits an attempt at seating, rather than prescribing its acceptance.
With a zero-width deadzone, the engaged projection is also used for finite
loss because no separated deadzone topology exists.

This is a completion rule at numerical energy resolution, not a proof of
convergence of the preceding event sequence. It is implemented in
\CINDERcode{cvtModel/src/cinder/execution/hybrid/cvt_regime_switching.py}{cvt_regime_switching.py}.
