...
upshift from the common initial state. Once the belt is engaged and the primary
leaves the low-ratio seat, plotting primary speed against secondary speed gives
a compact view of how the hardware changes the working-speed characteristic.
\Cref{fig:results_shift_curve_shape} shows that response for six primary-side
configurations. The dashed lines mark the no-slip shaft-speed relations at the
low-ratio seat and the high-ratio stop: motion along either line is acceleration
at fixed geometric ratio, while the free-shift trajectories occupy the region
between them. This is the familiar shift-curve view used in practical CVT
tuning \citep{Aaen2007,Skinner2020}, but here no speed curve is prescribed.
Each trajectory follows from the actuator geometry and spring loading acting
through the belt, secondary, and accelerating vehicle.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/results/course/opening_free_shift_characteristics_with_ratio_limits.png}
  \caption{Opening-flat free-shift characteristics for selected primary
  hardware changes. Each solid curve is the solved primary--secondary speed
  trajectory while the CVT is engaged, freely shifting, and in stick--stick
  contact. The dashed lines show the fixed no-slip speed relations at the
  geometric low-ratio seat and high-ratio stop of
  \Cref{sec:low_ratio_supported_regime,sec:upper_primary_stop_regime}. The
  curves themselves are not imposed shift laws.}
...
now obtained from the complete transient balance rather than imposed as a
shift-speed rule. The leading speed-dependent contribution appears explicitly
...
R00 at the low-ratio engagement geometry. Early in the shift, the two cases
...
continue to rise while less ratio change is produced, giving the
primary---and therefore the engine---more opportunity to accelerate relative
to the secondary. The curve therefore peels upward as the shift proceeds,
reaching approximately \(3336~\mathrm{rpm}\) by \(80\%\) travel even though it
began near the R00 operating point. Matching the spring force at one position
has not matched the spring characteristic through the rest of the shift.

The spring and ramp changes are compared directly in
\Cref{fig:results_shift_curve_mechanics}. The spring case changes a force that
varies linearly with displacement. The ramp cases instead change the geometry
through which the roller moves, so the relationship between flyweight motion
and axial closing force can vary nonlinearly as different parts of the profile
are reached.
...
\begin{figure}[H]
  ...
  more rapidly as the primary closes. (b) RC10 and R26B7 preserve the first
  \SI{10}{mm} of the reference ramp, while RC40L preserves the first
  \SI{18}{mm}; the distances are physical ramp coordinates rather than
  movable-sheave travel. (c) The changed ramp geometry gives the leading
  centrifugal closing contribution a different dependence on shift at the
  ...
  isolates one physically important contribution rather than replacing the
  full dynamic force law.}
...
\Cref{tab:results_shift_curve_metrics} collects a few numerical anchors for
these trajectories. The secant slope is useful for distinguishing the broad
trend, but it is only a descriptor of the solved opening-flat path. In
particular, a single average slope does not capture the localized late feature
of RC40L.
...
The changed working-speed characteristics also alter when the fixed high-ratio
geometry is reached. R26B7 and RC40L arrive at the upper stop after about
\SI{39.8}{m} and \SI{29.4}{m}, respectively, while R00 reaches it near
\SI{64.9}{m} and W85 near \SI{84.5}{m}. RC10 remains in free shift throughout
the opening flat; its first upper-stop arrival occurs much later, on the
descent near \SI{652.2}{m}. These distances are not prescribed
``shift-out'' settings. They follow from the different actuator
characteristics acting through the same belt, secondary, engine boundary, and
vehicle.
...
level ground. For that question only, R00 and the more extreme low-mass case
U55 were also run over a separate \SI{800}{m} level road as a supporting
diagnostic; this auxiliary run is not part of the common course. R00 again
reaches the \SI{19.05}{mm} upper stop near \SI{64.9}{m}. U55 instead
...
The same mechanism position need not produce the same clamping force while the
CVT shifts in opposite directions. Acceleration can already contribute to this
distinction in the present model because part of the applied loading
...
The belt couples the two pulleys through its length, motion, and loading. With
...
circular wraps and straight spans of \AssRef{assump:prescribed_belt_path}
remove possible motions before the force balances are solved. The checks in
...
\AssRef{assump:no_parasitic_losses}. Measuring these resistances independently
and applying them to the bodies and relative motions that produce them would
allow their effect on acceleration and efficiency to be examined alongside the
belt losses. The shaft interface in \Cref{eq:shaft_boundary_interface}
identifies which belong inside the CVT and which are supplied by the attached
machine. Each resistance would enter the force or torque balance where it
acts, rather than a single efficiency correction for the complete
transmission.

\subsection*{Experimental Validation and Characterisation}
\label{sec:future_work_validation}

Different operating conditions allow different parts of the model to be
tested. While a travel stop holds the sheaves in place, loading can be varied
...
Belt--sheave friction and parasitic losses also require independent
characterisation over the relevant loads, speeds, and temperatures. These
