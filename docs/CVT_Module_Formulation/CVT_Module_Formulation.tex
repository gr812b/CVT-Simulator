...
spring preload. Together, the set provides several distinct mechanical routes
...
% Revision 4: combined belt discussion and revised recovery argument, 2026-09-21.
...
% Additional bibliography entries: CINDER_Future_Work_References.bib.
...
further kinematic reduction. Relaxing it would allow tangential material
speed to vary around the loop. Such variation need not originate in
longitudinal extension: the inextensible continuum kinematics of
\citet{CarboneMangialardiMantriota2005} already allow local transport to vary
with changing geometry. Following this motion through a transient would
require reconsidering steady-path and constant-mass-flow restrictions such
as those used by \citet{Messick2018}.

That spatial variation also changes how contact can be determined. A single
belt--pulley speed difference would no longer generally describe relative
motion throughout the wrap. The present equations resolve tension and normal
loading, but \AssRef{assump:wrap_contact_resolution} assigns one contact state
and one traction utilisation \(\lambda_j\) to each wrap through
\Cref{eq:lambda_local_definition}. Locally different motions could produce
adhering and sliding regions, or regions with opposing traction, within the
same wrap. A local Coulomb law could still represent friction, with contact
states and traction determined along the wrap rather than assigned to it
as a whole.

The relevant relative velocity is that between the contacting rubber and
the sheave surface. Under \AssRef{assump:fixed-belt-cross-section}, the section
...
moments, and deformed contact geometry to the belt model. If a section
adjusts much faster than the overall belt motion, its deformation may be
treated without a separate inertial
state even when it changes contact. A refined cord and rubber mass
distribution could also improve the inertial description without resolving
every local deformation.

The circumferential-only friction of
\AssRef{assump:gross_coulomb_traction} also omits resistance to radial migration.
...
direction from local relative motion. Coupling these tractions to the belt
and sheave balances would determine how radial resistance, torque
transmission, and axial thrust develop together during shifting. A friction
coefficient that varies with pressure or sliding speed would provide a
further extension. Its influence on available traction could also be
examined within the present reduced kinematics.
...
temperature- and frequency-dependent rubber properties but uses an elastic approximation in
...
the spans and wraps. Extending the energy accounting in
\Cref{eq:verification_energy_balance} to include recoverable strain energy
and material dissipation alongside sliding loss would distinguish a temporary
reduction in output power due to storage from an irreversible loss.

With compliance and inertia retained, changes in tension and deformation
could propagate around the loop after a sudden load change.
\citet{Messick2018} identifies crank-angle-dependent engine torque as a
possible source of vibration outside the scope of his steady model. A
transient belt model could determine whether this excitation materially
changes contact loading and traction. Spatial and temporal refinement would
help distinguish the physical response from numerical oscillation,
particularly given the stiffness difficulties reported by \citet{Ballew2015}.
...
Wherever a reduction is well defined, reinstating the assumptions and closure
relations used here should recover its governing equations. The reduced model
should then reproduce the same response under
the same loading and initial conditions.
...
help establish both where the present formulation remains useful and where
more detail is needed, providing a basis for choosing an appropriate model
for a component or design study.
